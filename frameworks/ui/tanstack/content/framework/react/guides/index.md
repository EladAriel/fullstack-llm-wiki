---
type: "Framework Learn Directory Index"
framework: "tanstack"
directory: "content/framework/react/guides"
source_repo: "https://github.com/tanstack/query"
source_branch: "main"
source_commit: "fd50fa14d283c7d6664a796f758498d1ad5bfce7"
source_commit_short: "fd50fa14"
source_commit_date: "2026-07-24T22:22:47+10:00"
generated_at: "2026-07-25T11:50:41Z"
---
# Guides

This directory contains tanstack learning pages related to guides.

## Breadcrumbs

[Wiki Home](../../../../index.md) -> [Content](../../../index.md) -> [Framework](../../index.md) -> [React](../index.md) -> [Guides](index.md)

## Navigation

- Parent: [Parent Index](../index.md)

## Child Directories

- None

## Pages in This Directory

- [Advanced Ssr](advanced-ssr.md)
- [Background Fetching Indicators](background-fetching-indicators.md)
- [Caching](caching.md)
- [Default Query Function](default-query-function.md)
- [Dependent Queries](dependent-queries.md)
- [Disabling Queries](disabling-queries.md)
- [Does This Replace Client State](does-this-replace-client-state.md)
- [Filters](filters.md)
- [Important Defaults](important-defaults.md)
- [Infinite Queries](infinite-queries.md)
- [Initial Query Data](initial-query-data.md)
- [Invalidations From Mutations](invalidations-from-mutations.md)
- [Migrating To React Query 3](migrating-to-react-query-3.md)
- [Migrating To React Query 4](migrating-to-react-query-4.md)
- [Migrating To V5](migrating-to-v5.md)
- [Mutations](mutations.md)
- [Network Mode](network-mode.md)
- [Optimistic Updates](optimistic-updates.md)
- [Paginated Queries](paginated-queries.md)
- [Parallel Queries](parallel-queries.md)
- [Placeholder Query Data](placeholder-query-data.md)
- [Polling](polling.md)
- [Prefetching](prefetching.md)
- [Queries](queries.md)
- [Query Cancellation](query-cancellation.md)
- [Query Functions](query-functions.md)
- [Query Invalidation](query-invalidation.md)
- [Query Keys](query-keys.md)
- [Query Options](query-options.md)
- [Query Retries](query-retries.md)
- [Render Optimizations](render-optimizations.md)
- [Request Waterfalls](request-waterfalls.md)
- [Scroll Restoration](scroll-restoration.md)
- [Ssr](ssr.md)
- [Suspense](suspense.md)
- [Testing](testing.md)
- [Updates From Mutation Responses](updates-from-mutation-responses.md)
- [Window Focus Refetching](window-focus-refetching.md)

## IDE LLM Guidance

When answering implementation questions related to this directory:

1. Prefer the most specific page listed above.
2. If the topic is broad, read the child directory index first.
3. Use source metadata from each page when giving answers.
