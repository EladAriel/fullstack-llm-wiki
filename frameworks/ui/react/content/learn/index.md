---
type: "Framework Learn Directory Index"
framework: "react"
directory: "content/learn"
source_repo: "https://github.com/reactjs/react.dev"
source_branch: "main"
source_commit: "7b6c3ceb9dd97249e9dce4a8a94e61aed6424698"
source_commit_short: "7b6c3ceb"
source_commit_date: "2026-07-20T15:31:48+02:00"
generated_at: "2026-07-25T11:50:43Z"
---
# Learn

This directory contains react learning pages related to learn.

## Breadcrumbs

[Wiki Home](../../index.md) -> [Content](../index.md) -> [Learn](index.md)

## Navigation

- Parent: [Parent Index](../index.md)

## Child Directories

- [React Compiler](react-compiler/index.md) - pages related to react compiler

## Pages in This Directory

- [Original Source Index](_source_index.md)
- [Add React To An Existing Project](add-react-to-an-existing-project.md)
- [Adding Interactivity](adding-interactivity.md)
- [Build A React App From Scratch](build-a-react-app-from-scratch.md)
- [Choosing The State Structure](choosing-the-state-structure.md)
- [Conditional Rendering](conditional-rendering.md)
- [Creating A React App](creating-a-react-app.md)
- [Describing The Ui](describing-the-ui.md)
- [Editor Setup](editor-setup.md)
- [Escape Hatches](escape-hatches.md)
- [Extracting State Logic Into A Reducer](extracting-state-logic-into-a-reducer.md)
- [Importing And Exporting Components](importing-and-exporting-components.md)
- [Installation](installation.md)
- [Javascript In Jsx With Curly Braces](javascript-in-jsx-with-curly-braces.md)
- [Keeping Components Pure](keeping-components-pure.md)
- [Lifecycle Of Reactive Effects](lifecycle-of-reactive-effects.md)
- [Managing State](managing-state.md)
- [Manipulating The Dom With Refs](manipulating-the-dom-with-refs.md)
- [Passing Data Deeply With Context](passing-data-deeply-with-context.md)
- [Passing Props To A Component](passing-props-to-a-component.md)
- [Preserving And Resetting State](preserving-and-resetting-state.md)
- [Queueing A Series Of State Updates](queueing-a-series-of-state-updates.md)
- [React Developer Tools](react-developer-tools.md)
- [Reacting To Input With State](reacting-to-input-with-state.md)
- [Referencing Values With Refs](referencing-values-with-refs.md)
- [Removing Effect Dependencies](removing-effect-dependencies.md)
- [Render And Commit](render-and-commit.md)
- [Rendering Lists](rendering-lists.md)
- [Responding To Events](responding-to-events.md)
- [Reusing Logic With Custom Hooks](reusing-logic-with-custom-hooks.md)
- [Rsc Sandbox Test](rsc-sandbox-test.md)
- [Scaling Up With Reducer And Context](scaling-up-with-reducer-and-context.md)
- [Separating Events From Effects](separating-events-from-effects.md)
- [Setup](setup.md)
- [Sharing State Between Components](sharing-state-between-components.md)
- [State A Components Memory](state-a-components-memory.md)
- [State As A Snapshot](state-as-a-snapshot.md)
- [Synchronizing With Effects](synchronizing-with-effects.md)
- [Thinking In React](thinking-in-react.md)
- [Tutorial Tic Tac Toe](tutorial-tic-tac-toe.md)
- [Typescript](typescript.md)
- [Understanding Your Ui As A Tree](understanding-your-ui-as-a-tree.md)
- [Updating Arrays In State](updating-arrays-in-state.md)
- [Updating Objects In State](updating-objects-in-state.md)
- [Writing Markup With Jsx](writing-markup-with-jsx.md)
- [You Might Not Need An Effect](you-might-not-need-an-effect.md)
- [Your First Component](your-first-component.md)

## IDE LLM Guidance

When answering implementation questions related to this directory:

1. Prefer the most specific page listed above.
2. If the topic is broad, read the child directory index first.
3. Use source metadata from each page when giving answers.
