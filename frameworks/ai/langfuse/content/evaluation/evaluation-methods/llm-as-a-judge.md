...
source_branch: "main"
source_path: "content/docs/evaluation/evaluation-methods/llm-as-a-judge.mdx"
...
For observation and experiment targets, **Tool Calls** maps the calls recorded on the target observation. Each call contains `id`, `name`, `arguments`, `type`, and `index`. Valid JSON arguments are parsed. Map the full array or use a JSONPath selector such as `$[*].name`.
...
- **JSONPath**: If the data is nested (e.g., within a JSON object), you can use a JSONPath expression (like `$.choices[0].message.content`) to precisely locate it. JSONPath expressions including array slices with negative indices are supported; for example, `$[-1:]` selects the last element of an array, such as the latest message in a conversation history.
...
- **Evaluation rules** define _what_ gets evaluated: the target (live observations or experiments), filters, sampling rate, and the mapping from your data onto the evaluator's variables. Each rule references an evaluator family by `name` and `scope`. The `tool_calls` mapping source is available for both targets.
