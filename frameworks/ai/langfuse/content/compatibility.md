...
source_branch: "main"
source_path: "content/docs/compatibility.mdx"
...
generated_at: "2026-08-29T09:38:37.746256Z"
---
---
...
Langfuse Cloud runs v3 and v4 side by side until the v4 cutover on <V4CutoverDate />. The v3 column is deprecated: everything marked Deprecated below keeps working until the cutover and is then removed. Click a row for details.
...
| Feature                                                                                                 | Langfuse Cloud v3 (Deprecated as of {V4_CUTOVER_DATE_SHORT})    | Langfuse Cloud v4 (GA)                                          |
| ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------- |
...
| Public API & querying (`api.*`) | Supported                                         | Supported until the v4 cutover on <V4CutoverDate />; then the deprecated read APIs are sunset |
...
| Capability                       | Langfuse Cloud v3                                 | Langfuse Cloud v4                                                           |
| -------------------------------- | ------------------------------------------------- | --------------------------------------------------------------------------- |
| Tracing (legacy batch ingestion) | Supported                                         | Removed on <V4CutoverDate />; trace ingestion then stops working            |
| Score ingestion                  | Supported                                         | Supported                                                                   |
| Datasets & experiments           | Datasets only (experiments require Python SDK v4) | Datasets only (experiments require Python SDK v4)                           |
| Public API & querying (`api.*`)  | Supported                                         | Supported until <V4CutoverDate />; then the deprecated read APIs are sunset |
...
| Public API & querying (`api.*`) | Supported                                        | Supported until <V4CutoverDate />; then the deprecated read APIs are removed |
...
Only your SDK version and the API endpoints you call. Langfuse Cloud always runs the latest server version, so self-hosted server minimums never apply to you. Keep your SDKs on the GA major version (Python v4, JS/TS v5) and migrate away from deprecated endpoints before the v4 cutover on <V4CutoverDate />.
...
Python SDK v2 and JS/TS SDK v3 (and older) send traces via the legacy batch ingestion API, which is removed on Langfuse Cloud as of <V4CutoverDate />. Upgrade to the GA SDK majors now; see the [SDK upgrade paths](/docs/observability/sdk/upgrade-path).
