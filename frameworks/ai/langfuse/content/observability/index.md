---
type: "Framework Learn Directory Index"
framework: "Langfuse"
directory: "content/observability"
source_repo: "https://github.com/langfuse/langfuse-docs"
source_branch: "main"
source_commit: "fcd1eca34a924867563c3c4e801254c4e66c0021"
source_commit_short: "fcd1eca3"
source_commit_date: "2026-07-25T00:45:45Z"
generated_at: "2026-07-25T11:51:12Z"
---
# Observability

This directory contains Langfuse learning pages related to observability.

## Breadcrumbs

[Wiki Home](../../index.md) -> [Content](../index.md) -> [Observability](index.md)

## Navigation

- Parent: [Parent Index](../index.md)

## Child Directories

- [Features](features/index.md) - pages related to features
- [Sdk](sdk/index.md) - pages related to sdk

## Pages in This Directory

- [Best Practices](best-practices.md)
- [Data Model](data-model.md)
- [Get Started](get-started.md)
- [Overview](overview.md)
- [Troubleshooting And Faq](troubleshooting-and-faq.md)

## IDE LLM Guidance

When answering implementation questions related to this directory:

1. Prefer the most specific page listed above.
2. If the topic is broad, read the child directory index first.
3. Use source metadata from each page when giving answers.
