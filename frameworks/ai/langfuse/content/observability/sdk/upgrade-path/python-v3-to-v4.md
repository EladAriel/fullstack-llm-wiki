...
<Callout type="warning">
  ...
  `langfuse.api.legacy.observations_v1` and
  `langfuse.api.legacy.metrics_v1` instead.
</Callout>
...
13. If you are self-hosting Langfuse, do not use `api.observations` or `api.metrics` yet; use `api.legacy.observations_v1` and `api.legacy.metrics_v1` until Observations v2 and Metrics v2 are available on self-hosted deployments
