...
source_branch: "main"
source_path: "docs/build/front.rst"
...
While Alembic can be installed system wide, it's more common that it's installed local to a [virtual environment](https://docs.python.org/3/tutorial/venv.html) , as it also uses libraries such as SQLAlchemy and database drivers that are more appropriate for local installations.
...
To build a virtual environment for a specific project, a virtual environment can be created using the [Python venv library](https://docs.python.org/3/library/venv.html):
...
There is now a Python interpreter that you can access in `/path/to/your/project/.venv/bin/python`, as well as the [pip](http://pypi.python.org/pypi/pip) installer tool in `/path/to/your/project/.venv/bin/pip`.
...
Alembic's install process will ensure that SQLAlchemy_ is installed, in addition to other dependencies.  Alembic will work with SQLAlchemy as of version **1.4.0**.
...
Therefore, when [pinning](https://pip.pypa.io/en/stable/topics/repeatable-installs/) Alembic releases, pin to the "major" and "minor" digits to avoid API changes.
...
Alembic is developed by [Mike Bayer](http://techspot.zzzeek.org), and is part of the SQLAlchemy_ project.

User issues, discussion of potential bugs and features are most easily discussed using [GitHub Discussions](https://github.com/sqlalchemy/alembic/discussions/).

# Bugs

Bugs and feature enhancements to Alembic should be reported on the [GitHub issue tracker](https://github.com/sqlalchemy/alembic/issues/).
