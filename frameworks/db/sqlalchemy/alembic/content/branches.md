...
source_branch: "main"
source_path: "docs/build/branches.rst"
...
Relative identifiers, as introduced in `relative_migrations`, work with labels too.  For example, upgrading to `shoppingcart@+2` means to upgrade from current heads on "shoppingcart" upwards two revisions:
...
> **Note:** versioning lineages which **share the same alembic_version table**.  This is
...
alembic_version tables, see the example in `multiple_environments`.
