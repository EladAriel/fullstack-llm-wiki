...
source_branch: "main"
source_path: "docs/build/api/plugins.rst"
...
`.Operations.register_operation` (e.g., `op.create_view()`)

- **Operation implementations** - Provide database-specific implementations
using `.Operations.implementation_for`
...
> **Seealso:**  `replaceable_objects` - Cookbook recipe demonstrating custom
...
See the section `plugins_autogenerate` for background on enabling autogenerate comparison plugins per environment.
...
This plugin depends on the `schemas` plugin in order to iterate through tables.
...
Example from the `replaceable_objects` recipe:
...
For an example of a complete comparison function, see the example at `autogenerate_global_comparison_function`.
...
chain.  These hooks are passed a `.AutogenContext` and an `.UpgradeOps` collection.  Functions that subscribe to the `autogenerate` target should look like:
...
compared, and hooks are passed a `.AutogenContext`, an `.UpgradeOps` collection, and a set of schema names, featuring the value `None` for the "default" schema.   Functions that subscribe to the `"schema"` target should look like:
...
database autoloaded version and the local metadata version. These hooks are passed an `.AutogenContext`, a `.ModifyTableOps` collection, a schema name, table name, a `sqlalchemy.schema.Table` reflected from the database if any or `None`, and a `sqlalchemy.schema.Table` present in the local `sqlalchemy.schema.MetaData`. If the `.ModifyTableOps` collection contains changes after all hooks are run, it is included in the migration script:
...
database autoloaded version and the local metadata version. These hooks are passed an `.AutogenContext`, an `.AlterColumnOp` object, a schema name, table name, column name, a `sqlalchemy.schema.Column` reflected from the database and a `sqlalchemy.schema.Column` present in the local table.  If the `.AlterColumnOp` contains changes after all hooks are run, it is included in the migration script; a "change" is considered to be present if any of the `modify_` attributes are set to a non-default value, or there are any keys in the `.kw` collection with the prefix `"modify_"`:
...
> **Seealso:**  `alembic.autogenerate.toplevel` - Detailed documentation on the
 autogenerate system
 `autogenerate_global_comparison_function` - a companion section
 ...
 comparison function.
 `customizing_revision` - Customizing autogenerate behavior

# Plugin API Reference

> **Seealso:**  `.EnvironmentContext.configure.autogenerate_plugins` -
 Configuration parameter for enabling autogenerate plugins
 `operation_plugins` - Documentation on custom operations
 `replaceable_objects` - Example of custom operations suitable
 for a plugin
 `customizing_revision` - General information on customizing
