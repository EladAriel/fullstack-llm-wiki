...
For backends that support a full FROM expression in the `USING clause, the sql.Delete.using` method may be used to state that expression explicitly, such as a MySQL joined DELETE:
...
.. versionadded:: 2.1 Added `_sql.Delete.using` to support explicit
