...
source_branch: "main"
source_path: "doc/build/changelog/migration_04.rst"
...
> **Admonition:**  This document describes changes between SQLAlchemy version 0.3,
...
The way the `Point object is defined is specific to a custom type; constructor takes a list of arguments, and the _composite_values__()` method produces a sequence of those arguments.  The order will match up to our mapper, as we'll see in a moment.
...
If you'd like to define the way the mapped attributes generate SQL clauses when used in expressions, create your own `sqlalchemy.orm.PropComparator subclass, defining any of the common operators (like _eq__(), _le__()`, etc.), and send it in to `composite()`.  Composite types work as primary keys too, and are usable in `query.get()`:
...
Dictionary-based collections are now much easier to use and fully `dict-like.  Changing _iter__` is no longer needed for `dict`s, and new built-in `dict` types cover many needs:
...
All the behaviors of `SessionContext` and the `query and _init__` methods of `assignmapper` are moved into the new `scoped_session()` function, which is compatible with both `sessionmaker` as well as `create_session()`:
...
There is a [New API](https://www.sqlalchemy.org/docs/04/types .html#types_custom) for subclassing a TypeDecorator. Using the 0.3 API causes compilation errors in some cases.

# SQL Expressions

## All New, Deterministic Label/Alias Generation

All the "anonymous" labels and aliases use a simple <name>_<number> format now.  SQL is much easier to read and is compatible with plan optimizer caches.  Just check out some of the examples in the tutorials: https://www.sqlalchemy.org/docs/04/ormtutorial.html https://www.sqlalchemy.org/docs/04/sqlexpression.html
...
SQL operators and more or less every SQL keyword there is are now abstracted into the compiler layer.  They now act intelligently and are type/backend aware, see: https://www.sqlalchemy.org/docs/04/sqlexpression.html#sql_operators
...
The in\_ function now takes a sequence of values or a selectable as its sole argument. The previous API of passing in values as positional arguments still works, but is now deprecated. This means that
