...
source_branch: "main"
source_path: "doc/build/changelog/migration_11.rst"
...
> **Admonition:**  This document describes changes between SQLAlchemy version 1.0
...
The `.Session` has long supported events that allow some degree of tracking of state changes to objects, including `.SessionEvents.before_attach`, `.SessionEvents.after_attach`, and `.SessionEvents.before_flush`.  The Session documentation also documents major object states at `session_object_states`.  However, there has never been system of tracking objects specifically as they pass through these transitions.  Additionally, the status of "deleted" objects has historically been murky as the objects act somewhere between the "persistent" and "detached" states.
...
In total, there are **ten new events**.  A summary of these events is in a newly written documentation section `session_lifecycle_events`.
...
This deduplication relies upon the hashability of the elements within the row.  With the introduction of PostgreSQL's special types like `_postgresql.ARRAY, postgresql.HSTORE and postgresql.JSON`, the experience of types within rows being unhashable and encountering problems here is more prevalent than it was previously.

In fact, SQLAlchemy has since version 0.8 included a flag on datatypes that are noted as "unhashable", however this flag was not used consistently on built in types.  As described in `change_3499_postgresql`, this flag is now set consistently for all of PostgreSQL's "structural" types.
...
The `indexable_toplevel` extension is an extension to the hybrid attribute feature which allows the construction of attributes which refer to specific elements of an "indexable" data type, such as an array or JSON field:
...
The extension also triggers a change event when the attribute is modified, so that there's no need to use `.mutable.MutableDict` in order to track this change.

> **Seealso:**  `indexable_toplevel`

## New options allowing explicit persistence of NULL over a default

Related to the new JSON-NULL support added to PostgreSQL as part of `change_3514`, the base `.TypeEngine` class now supports a method `.TypeEngine.evaluates_none` which allows a positive set of the `None value on an attribute to be persisted as NULL, rather than omitting the column from the INSERT statement, which has the effect of using the column-level default.  This allows a mapper-level configuration of the existing object-level technique of assigning expression.null` to the attribute.

> **Seealso:**  `session_forcing_null`

:ticket:`3250`

## Further Fixes to single-table inheritance querying

Continuing from 1.0's `migration_3177, the query.Query` should no longer inappropriately add the "single inheritance" criteria when the query is against a subquery expression such as an exists:
...
This issue is a continuation of :ticket:`2696` where we emit a warning so that the original error can be seen when running on Python 2, even though the SAVEPOINT exception takes precedence.  On Python 3, exceptions are chained so both failures are reported individually.
...
A lot of testing went into making sure this wrapper works correctly, including for elaborate schemes like that of the [Custom Value Object](https://techspot.zzzeek.org/2011/10/21/hybrids-and-value-agnostic-types/) recipe, however we'll be looking to see that no other regressions occur for users.
...
Calling `str() on a query.Query` object will consult the `.Session for the correct "bind" to use, in order to render the SQL that would be passed to the database.  In particular this allows a query.Query that refers to dialect-specific SQL constructs to be renderable, assuming the query.Query` is associated with an appropriate `.Session. Previously, this behavior would only take effect if the schema.MetaData to which the mappings were associated were itself bound to the target engine.Engine`.

If neither the underlying `_schema.MetaData` nor the `.Session are associated with any bound engine.Engine`, then the fallback to the "default" dialect is used to generate the SQL string.
...
New helper classes `.MutableList` and `.MutableSet` have been added to the `mutable_toplevel` extension, to complement the existing `.MutableDict` helper.

:ticket:`3297`

## New "raise" / "raise_on_sql" loader strategies

To assist with the use case of preventing unwanted lazy loads from occurring after a series of objects are loaded, the new "lazy='raise'" and "lazy='raise_on_sql'" strategies and corresponding loader option `_orm.raiseload` may be applied to a relationship attribute which will cause it to raise `InvalidRequestError` when a non-eagerly-loaded attribute is accessed for read.  The two variants test for either a lazy load of any variety, including those that would only return None or retrieve from the identity map:
...
This old parameter from the very first versions of SQLAlchemy was part of the original design of the ORM which featured the `_orm.Mapper object as a public-facing query structure.   This role has long since been replaced by the query.Query object, where we use query.Query.order_by to indicate the ordering of results in a way that works consistently for any combination of SELECT statements, entities and SQL expressions.   There are many areas in which orm.Mapper.order_by` doesn't work as expected (or what would be expected is not clear), such as when queries are combined into unions; these cases are not supported.
...
The Python `BaseException` class is below that of `Exception` but is the identifiable base for system-level exceptions such as `KeyboardInterrupt`, `SystemExit`, and notably the `GreenletExit exception that's used by eventlet and gevent. This exception class is now intercepted by the exception- handling routines of engine.Connection, and includes handling by the events.ConnectionEvents.handle_error event.  The engine.Connection` is now **invalidated** by default in the case of a system level exception that is not a subclass of `Exception`, as it is assumed an operation was interrupted and the connection may be in an unusable state.  The MySQL drivers are most targeted by this change however the change is across all DBAPIs.

Note that upon invalidation, the immediate DBAPI connection used by `_engine.Connection is disposed, and the engine.Connection`, if still being used subsequent to the exception raise, will use a new DBAPI connection for subsequent operations upon next use; however, the state of any transaction in progress is lost and the appropriate `.rollback()` method must be called if applicable before this reuse can proceed.
...
New `.expression.over.range_` and `.expression.over.rows` parameters allow RANGE and ROWS expressions for window functions:
...
> **Seealso:**  `tutorial_window_functions`

:ticket:`3049`

## Support for the SQL LATERAL keyword

The LATERAL keyword is currently known to only be supported by PostgreSQL 9.3 and greater, however as it is part of the SQL standard support for this keyword is added to Core.   The implementation of `_expression.Select.lateral` employs special logic beyond just rendering the LATERAL keyword to allow for correlation of tables that are derived from the same FROM clause as the selectable, e.g. lateral correlation:
...
> **Seealso:**  `tutorial_lateral_correlation`
 `_expression.Lateral`
...
The SQL standard TABLESAMPLE can be rendered using the `_expression.FromClause.tablesample method, which returns a expression.TableSample` construct similar to an alias:
...
SQLAlchemy has always had the convenience feature of enabling the backend database's "autoincrement" feature for a single-column integer primary key; by "autoincrement" we mean that the database column will include whatever DDL directives the database provides in order to indicate an auto-incrementing integer identifier, such as the SERIAL keyword on PostgreSQL or AUTO_INCREMENT on MySQL, and additionally that the dialect will receive these generated values from the execution of a `_schema.Table.insert` construct using techniques appropriate to that backend.
...
In order to anticipate some potential backwards-incompatible scenarios, the `_schema.Table.insert` construct will perform more thorough checks for missing primary key values on composite primary key columns that don't have autoincrement set up; given a table such as:
...
> **Note:** this change had an unintended side effect that the interpretation of non-
...
the `.Boolean` datatype is **not supported** and version 1.2 will
...
The `_types.JSON type and its descendant types postgresql.JSON` and `.mysql.JSON` have a flag `.types.JSON.none_as_null` which when set to True indicates that the Python value `None` should translate into a SQL NULL rather than a JSON NULL value.  This flag defaults to False, which means that the Python value `None` should result in a JSON NULL value.
...
a positive value of `None` on the mapped attribute that expects to persist JSON "null" would still result in the column-level default being triggered, replacing the `None` value:
...
value on a JSON column configured with none_as_null=False would still render JSON NULL rather than falling back to not inserting any value, behaving inconsistently vs. all other datatypes:
...
This is a behavioral change that is backwards incompatible for an application that was relying upon this to default a missing value as JSON null.  This essentially establishes that a **missing value is distinguished from a present value of None**.  See `behavior_change_3514` for further detail.

3. When the `.Session.bulk_insert_mappings` method were used, `None`
...
The `_types.JSON` type now implements the `.TypeEngine.should_evaluate_none` flag, indicating that `None` should not be ignored here; it is configured automatically based on the value of `.types.JSON.none_as_null`. Thanks to :ticket:`3061`, we can differentiate when the value `None` is actively set by the user versus when it was never set at all.

The feature applies as well to the new base `_types.JSON` type and its descendant types.

:ticket:`3514`

#### New JSON.NULL Constant Added

To ensure that an application can always have full control at the value level of whether a `_types.JSON, postgresql.JSON`, `.mysql.JSON, or postgresql.JSONB` column should receive a SQL NULL or JSON `"null"` value, the constant `.types.JSON.NULL` has been added, which in conjunction with `.null` can be used to determine fully between SQL NULL and JSON `"null"`, regardless of what `.types.JSON.none_as_null` is set to:
...
Along with the enhancements made to the PostgreSQL `_postgresql.ARRAY` type described in `change_3503, the base class of postgresql.ARRAY itself has been moved to Core in a new class types.ARRAY`.
...
In support of ANY and ALL, the `_types.ARRAY` type retains the same `.types.ARRAY.Comparator.any` and `.types.ARRAY.Comparator.all methods from the PostgreSQL type, but also exports these operations to new standalone operator functions expression.any_ and expression.all_`.  These two functions work in more of the traditional SQL way, allowing a right-side expression form such as:
...
For the PostgreSQL-specific operators "contains", "contained_by", and "overlaps", one should continue to use the `_postgresql.ARRAY type directly, which provides all functionality of the types.ARRAY` type as well.

The `_expression.any_ and expression.all_` operators are open-ended at the Core level, however their interpretation by backend databases is limited.  On the PostgreSQL backend, the two operators **only accept array values**.  Whereas on the MySQL backend, they **only accept subquery values**.  On MySQL, one can use an expression such as:
...
With the new `_types.ARRAY` type we can also implement a pre-typed function for the `array_agg() SQL function that returns an array, which is now available using functions.array_agg`:
...
A PostgreSQL element for an aggregate ORDER BY is also added via `_postgresql.aggregate_order_by`:
...
The PG dialect itself also provides an `_postgresql.array_agg wrapper to ensure the postgresql.ARRAY` type:
...
To support the use case of an application that uses the same set of `_schema.Table` objects in many schemas, such as schema-per-user, a new execution option `.Connection.execution_options.schema_translate_map is added.  Using this mapping, a set of schema.Table objects can be made on a per-connection basis to refer to any set of schemas instead of the schema.Table.schema` to which they were assigned.  The translation works for DDL and SQL generation, as well as with the ORM.
...
The `_expression.type_coerce` function previously would return an object either of type `.BindParameter` or `.Label, depending on the input.  An effect this would have was that in the case where expression transformations were used, such as the conversion of an element from a schema.Column` to a `.BindParameter` that's critical to ORM-level lazy loading, the type coercion information would not be used since it would have been lost already.
...
Above, in the `_orm.relationship.primaryjoin` expression, we are using `.type_coerce` to handle bound parameters passed via lazyloading as integers, since we already know these will come from our `StringAsInt` type which maintains the value as an integer in Python. We are then using `.cast so that as a SQL expression, the VARCHAR "id"  column will be CAST to an integer for a regular non- converted join as with query.Query.join or orm.joinedload`. That is, a joinedload of `.pets` looks like:
...
As detailed in `change_3514, types.JSON` will not render a JSON "null" value if the value is missing entirely.  To prevent SQL NULL, a default should be set up.  Given the following mapping:
...
Note that setting `None` for the default is the same as omitting it entirely; the `.types.JSON.none_as_null` flag does not impact the value of `None passed to schema.Column.default or schema.Column.server_default`:
...
Additionally, when the PostgreSQL DISTINCT ON format is used by passing expressions to `_query.Query.distinct`, the above "column adding" logic is disabled entirely.
...
The `_orm.validates` decorator is only intended to be created once per class for a particular attribute name.   Creating more than one now raises an error, whereas previously it would silently pick only the last defined validator:
...
The new behavior of the `_expression.TextClause.columns` method, which itself was recently added as of the 0.9 series, is that when columns are passed positionally without any additional keyword arguments, they are linked to the ultimate result set columns positionally, and no longer on name.   It is hoped that the impact of this change will be low due to the fact that the method has always been documented illustrating the columns being passed in the same order as that of the textual SQL statement, as would seem intuitive, even though the internals weren't checking for this.

An application that is using this method by passing `_schema.Column objects to it positionally must ensure that the position of those schema.Column` objects matches the position in which these columns are stated in the textual SQL.
...
is now slightly risky, as the "*" specification will generally deliver columns in the order in which they are present in the table itself.  If the structure of the table changes due to schema changes, this ordering may no longer be the same. Therefore when using `_expression.TextClause.columns`, it's advised to list out the desired columns explicitly in the textual SQL, though it's no longer necessary to worry about the names themselves in the textual SQL.

> **Seealso:**  `change_3501`

## String server_default now literal quoted

A server default passed to `_schema.Column.server_default` as a plain Python string that has quotes embedded is now passed through the literal quoting system:
...
The `ON CONFLICT` clause of `INSERT added to PostgreSQL as of version 9.5 is now supported using a PostgreSQL-specific version of the expression.Insert` object, via `sqlalchemy.dialects.postgresql.dml.insert. This expression.Insert subclass adds two new methods expression.Insert.on_conflict_do_update and expression.Insert.on_conflict_do_nothing` which implement the full syntax supported by PostgreSQL 9.5 in this area:
...
> **Seealso:**  `postgresql_insert_on_conflict`

:ticket:`3529`

## ARRAY and JSON types now correctly specify "unhashable"

As described in `change_3499`, the ORM relies upon being able to produce a hash function for column values when a query's selected entities mixes full ORM entities with column expressions.   The `hashable=False flag is now correctly set on all of PG's "data structure" types, including postgresql.ARRAY and postgresql.JSON. The postgresql.JSONB` and `.HSTORE types already included this flag.  For postgresql.ARRAY`, this is conditional based on the `.postgresql.ARRAY.as_tuple` flag, however it should no longer be necessary to set this flag in order to have an array value present in a composed ORM row.
...
For all three of `_postgresql.ARRAY, postgresql.JSON` and `.HSTORE`, the SQL type assigned to the expression returned by indexed access, e.g. `col[someindex]`, should be correct in all cases.

This includes:

- The SQL type assigned to indexed access of an `_postgresql.ARRAY` takes into
account the number of dimensions configured.   An `_postgresql.ARRAY with three dimensions will return a SQL expression with a type of postgresql.ARRAY` of one less dimension.  Given a column with type `ARRAY(Integer, dimensions=3)`, we can now perform this expression:
...
- The `_postgresql.JSON and postgresql.JSONB` types now mirror what PostgreSQL
itself does for indexed access.  This means that all indexed access for a `_postgresql.JSON or postgresql.JSONB type returns an expression that itself is always postgresql.JSON or postgresql.JSONB` itself, unless the `.postgresql.JSON.Comparator.astext modifier is used.   This means that whether the indexed access of the JSON structure ultimately refers to a string, list, number, or other JSON structure, PostgreSQL always considers it to be JSON itself unless it is explicitly cast differently.   Like the postgresql.ARRAY` type, this means that it is now straightforward to produce JSON expressions with multiple levels of indexed access:
...
as well as the "textual" type that is returned by indexed access of `_postgresql.JSON and postgresql.JSONB` in conjunction with the `.postgresql.JSON.Comparator.astext modifier is now configurable; it defaults to expression.TextClause` in both cases but can be set to a user-defined type using the `.postgresql.JSON.astext_type` or `.postgresql.HSTORE.text_type` parameters.

> **Seealso:** `change_3503_cast`

:ticket:`3499` :ticket:`3487`

## The JSON cast() operation now requires `.astext` is called explicitly

As part of the changes in `change_3503, the workings of the expression.ColumnElement.cast operator on postgresql.JSON and postgresql.JSONB` no longer implicitly invoke the `.postgresql.JSON.Comparator.astext` modifier; PostgreSQL's JSON/JSONB types support CAST operations to each other without the "astext" aspect.
...
The PostgreSQL dialect now supports reflection of CHECK constraints both within the method `_reflection.Inspector.get_check_constraints as well as within schema.Table reflection within the schema.Table.constraints` collection.
...
The `.Index` object now accepts the argument `postgresql_tablespace in order to specify TABLESPACE, the same way as accepted by the schema.Table` object.

> **Seealso:**  `postgresql_index_storage`
...
The MySQL dialect now accepts the value "AUTOCOMMIT" for the `_sa.create_engine.isolation_level` and `.Connection.execution_options.isolation_level` parameters:
...
Along with the change `change_3216, composite primary keys with or without auto increment are now easier to specify; schema.Column.autoincrement` now defaults to the value `"auto"` and the `autoincrement=False` directives are no longer needed:
...
The SQLite dialect now supports reflection of CHECK constraints both within the method `_reflection.Inspector.get_check_constraints as well as within schema.Table reflection within the schema.Table.constraints` collection.

## ON DELETE and ON UPDATE foreign key phrases now reflect

The `_reflection.Inspector will now include ON DELETE and ON UPDATE phrases from foreign key constraints on the SQLite dialect, and the schema.ForeignKeyConstraint object as reflected as part of a schema.Table` will also indicate these phrases.

# Dialect Improvements and Changes - SQL Server

## Added transaction isolation level support for SQL Server

All SQL Server dialects support transaction isolation level settings via the `_sa.create_engine.isolation_level` and `.Connection.execution_options.isolation_level` parameters.  The four standard levels are supported as well as `SNAPSHOT`:
...
> **Seealso:**  `mssql_isolation_level`

:ticket:`3534`

## String / varlength types no longer represent "max" explicitly on reflection

When reflecting a type such as `.String, expression.TextClause`, etc. which includes a length, an "un-lengthed" type under SQL Server would copy the "length" parameter as the value `"max"`:
