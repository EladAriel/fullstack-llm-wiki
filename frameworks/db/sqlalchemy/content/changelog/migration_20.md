...
> **Admonition:**  SQLAlchemy 2.0's transition documents are separated into **two**
 ...
 to SQLAlchemy 1.4:
 * `migration_20_toplevel` - this document, 1.x to 2.x API shifts
 * `whatsnew_20_toplevel` - new features and behaviors for SQLAlchemy 2.0
 ...
 `whatsnew_20_toplevel` for an overview of new features and
 capabilities.
...
> **Admonition:**  This document describes changes between SQLAlchemy version 1.4
 ...
 `.orm.query.Query`  and Declarative. It also hopes standardize some
...
- `New ORM statement paradigm <change_5159>`
- `SQL caching throughout Core and ORM <change_4639>`
- `New Declarative features, ORM integration <change_5508>`
- `New Result object <change_result_14_core>`
- `select() / case() Accept Positional Expressions <change_5284>`
- `asyncio support for Core and ORM <change_3414>`
The above bullets link to the description of these new paradigms as introduced in SQLAlchemy 1.4. in the `migration_14_toplevel` document.

For SQLAlchemy 2.0, all API features and behaviors that were marked as `deprecated for 2.0 <deprecation_20_mode>` are now finalized; in particular, major APIs that are **no longer present** include:

- `Bound MetaData and connectionless execution <migration_20_implicit_execution>`
- `Emulated autocommit on Connection <migration_20_autocommit>`
- `The Session.autocommit parameter / mode <migration_20_session_autocommit>`
- `List / keyword arguments to select() <migration_20_5284>`
- Python 2 support
...
The complete steps for this migration path are later in this document at `migration_20_overview`.
...
The first step is getting an existing application onto 1.4, in the case of a typical non trivial application, is to ensure it runs on SQLAlchemy 1.3 with no deprecation warnings.   Release 1.4 does have a few changes linked to conditions that warn in previous version, including some warnings that were introduced in 1.3, in particular some changes to the behavior of the `_orm.relationship.viewonly and orm.relationship.sync_backref` flags.

For best results, the application should be able to run, or pass all of its tests, with the latest SQLAlchemy 1.3 release with no SQLAlchemy deprecation warnings; these are warnings emitted for the `_exc.SADeprecationWarning` class.
...
- `change_5526` - this impacts code that would be manipulating the
`_engine.URL object and may impact code that makes use of the engine.CreateEnginePlugin extension point.   This is an uncommon case but may affect in particular some test suites that are making use of special database provisioning logic.   A github search for code that uses the relatively new and little-known engine.CreateEnginePlugin` class found two projects that were unaffected by the change.

- `change_4617` - this change may impact code that was somehow relying
upon behavior that was mostly unusable in the `_sql.Select` construct, where it would create unnamed subqueries that were usually confusing and non-working.  These subqueries would be rejected by most databases in any case as a name is usually required except on SQLite, however it is possible some applications will need to adjust some queries that are inadvertently relying upon this.

- `change_select_join - somewhat related, the sql.Select` class
featured `.join()` and `.outerjoin() methods that implicitly created a subquery and then returned a sql.Join construct, which again would be mostly useless and produced lots of confusion.  The decision was made to move forward with the vastly more useful 2.0-style join-building approach where these methods now work the same way as the ORM orm.Query.join` method.

- `change_deferred_construction` - some error messages related to
construction of a `_orm.Query or sql.Select` may not be emitted until compilation / execution, rather than at construction time. This might impact some test suites that are testing against failure modes.

For the full overview of SQLAlchemy 1.4 changes, see the `/changelog/migration_14` document.
...
SQLAlchemy 1.4 features a conditional deprecation warning system inspired by the Python "-3" flag that would indicate legacy patterns in a running application.   For SQLAlchemy 1.4, the `_exc.RemovedIn20Warning` deprecation class is emitted only when an environment variable `SQLALCHEMY_WARN_20` is set to either of `true` or `1`.
...
The above program uses several patterns that many users will already identify as "legacy", namely the use of the `_engine.Engine.execute` method that's part of the "connectionless execution" API.  When we run the above program against 1.4, it returns a single line:
...
The goal of "2.0 deprecations mode" is that a program which runs with no `_exc.RemovedIn20Warning` warnings with "2.0 deprecations mode" turned on is then ready to run in SQLAlchemy 2.0.
...
that will select for particular subsets of warnings to either raise an exception, or to be ignored (or logged).   Work with just one subgroup of warnings at a time.  Below, a warnings filter is configured for an application where the change to the Core level `.execute()` calls will be needed in order for all tests to pass, but all other 2.0-style warnings will be suppressed:
...
warnings that are caught by the "always" filter can be added to the list of "errors" to be resolved.

4. Once no more warnings are emitted, the filter can be removed.
### Migration to 2.0 Step Four - Use the `future` flag on Engine

The `_engine.Engine` object features an updated transaction-level API in version 2.0.  In 1.4, this new API is available by passing the flag `future=True to the sa.create_engine` function.

When the `_sa.create_engine.future flag is used, the engine.Engine and engine.Connection objects support the 2.0 API fully and not at all any legacy features, including the new argument format for engine.Connection.execute, the removal of "implicit autocommit", string statements require the sql.text construct unless the engine.Connection.exec_driver_sql method is used, and connectionless execution from the engine.Engine` is removed.

If all `_exc.RemovedIn20Warning warnings have been resolved regarding use of the engine.Engine and engine.Connection, then the sa.create_engine.future` flag may be enabled and there should be no errors raised.

The new engine is described at `_engine.Engine which delivers a new engine.Connection object.    In addition to the above changes, the, engine.Connection object features engine.Connection.commit and engine.Connection.rollback` methods, to support the new "commit-as-you-go" mode of operation:
...
The `_orm.Session object also features an updated transaction/connection level API in version 2.0.  This API is available in 1.4 using the orm.Session.future flag on orm.Session or on orm.sessionmaker`.

The `_orm.Session` object supports "future" mode in place, and involves these changes:

1. The `_orm.Session` no longer supports "bound metadata" when it
resolves the engine to be used for connectivity.   This means that an `_engine.Engine` object **must** be passed to the constructor (this may be either a legacy or future style object).

2. The `_orm.Session.begin.subtransactions` flag is no longer
supported.

3. The `_orm.Session.commit` method always emits a COMMIT to the database,
rather than attempting to reconcile "subtransactions".

4. The `_orm.Session.rollback` method always rolls back the full
stack of transactions at once, rather than attempting to keep "subtransactions" in place.

The `_orm.Session also supports more flexible creational patterns in 1.4 which are now closely matched to the patterns used by the engine.Connection object.   Highlights include that the orm.Session` may be used as a context manager:
...
In addition, the `_orm.sessionmaker object supports a orm.sessionmaker.begin context manager that will create a orm.Session` and begin /commit a transaction in one block:
...
See the section `orm_session_vs_engine for a comparison of orm.Session creational patterns compared to those of engine.Connection`.
...
SQLAlchemy 2.0 has new support for runtime interpretation of `484 typing annotations on ORM models.   A requirement of these annotations is that they must make use of the orm.Mapped generic container.  Annotations which don't use orm.Mapped which link to constructs such as orm.relationship` will raise errors in Python, as they suggest mis-configurations.

SQLAlchemy applications that use the Mypy plugin with explicit annotations that don't use `_orm.Mapped` in their annotations are subject to these errors, as would occur in the example below:
...
Above, the `Foo.bars` and `Bar.foo orm.relationship declarations will raise an error at class construction time because they don't use orm.Mapped (by contrast, the annotations that use schema.Column are ignored by 2.0, as these are able to be recognized as a legacy configuration style). To allow all annotations that don't use orm.Mapped to pass without error, the _allow_unmapped__` attribute may be used on the class or any subclasses, which will cause the annotations in these cases to be ignored completely by the new Declarative system.

> **Note:** runtime behavior of the ORM.  It does not affect the behavior of
...
at `whatsnew_20_orm_typing_migration`.
...
The guide at `whatsnew_20_toplevel` provides an overview of new features and behaviors for SQLAlchemy 2.0 which extend beyond the base set of 1.4->2.0 API changes.
...
The method that is cross-compatible with `1.x style` and `2.0 style execution is to make use of the engine.Connection.begin method, or the engine.Engine.begin` context manager:
...
When using `2.0 style with the sa.create_engine.future flag, "commit as you go" style may also be used, as the engine.Connection features **autobegin** behavior, which takes place when a statement is first invoked in the absence of an explicit call to engine.Connection.begin`:
...
For "commit as you go, or rollback instead" usage, which resembles how the `_orm.Session is normally used today, the "future" version of engine.Connection, which is the one that is returned from an engine.Engine that was created using the sa.create_engine.future flag, includes new engine.Connection.commit and engine.Connection.rollback` methods, which act upon a transaction that is now begun automatically when a statement is first invoked:
...
Above, the `engine.connect() method will return a engine.Connection` that features **autobegin**, meaning the `begin() event is emitted when the execute method is first used (note however that there is no actual "BEGIN" in the Python DBAPI).  "autobegin" is a new pattern in SQLAlchemy 1.4 that is featured both by engine.Connection as well as the ORM orm.Session object; autobegin allows that the engine.Connection.begin` method may be called explicitly when the object is first acquired, for schemes that wish to demarcate the beginning of the transaction, but if the method is not called, then it occurs implicitly when work is first done on the object.

The removal of "autocommit" is closely related to the removal of "connectionless" execution discussed at `migration_20_implicit_execution`. All of these legacy patterns built up from the fact that Python did not have context managers or decorators when SQLAlchemy was first created, so there were no convenient idiomatic patterns for demarcating the use of a resource.

#### Driver-level autocommit remains available

True "autocommit" behavior is now widely available with most DBAPI implementations, and is supported by SQLAlchemy via the `_engine.Connection.execution_options.isolation_level` parameter as discussed at `dbapi_autocommit.  True autocommit is treated as an "isolation level" so that the structure of application code does not change when autocommit is used; the engine.Connection.begin context manager as well as methods like engine.Connection.commit` may still be used, they are simply no-ops at the database driver level when DBAPI-level autocommit is turned on.

### "Implicit" and "Connectionless" execution, "bound metadata" removed

**Synopsis**

The ability to associate an `_engine.Engine with a schema.MetaData` object, which then makes available a range of so-called "connectionless" execution patterns, is removed:
...
For schema level patterns, explicit use of an `_engine.Engine or engine.Connection is required.   The engine.Engine may still be used directly as the source of connectivity for a schema.MetaData.create_all operation or autoload operation. For executing statements, only the engine.Connection object has a engine.Connection.execute method (in addition to the ORM-level orm.Session.execute` method):
...
"Connectionless" execution refers to the still fairly popular pattern of invoking `.execute() from the engine.Engine`:

```
result = engine.execute(some_statement)
```

The above operation implicitly procures a `_engine.Connection` object, and runs the `.execute()` method on it.  While this appears to be a simple convenience feature, it has been shown to give rise to several issues:

- Programs that feature extended strings of `engine.execute()` calls have
become prevalent, overusing a feature that was intended to be seldom used and leading to inefficient non-transactional applications.  New users are confused as to the difference between `engine.execute()` and `connection.execute()` and the nuance between these two approaches is often not understood.

- The feature relies upon the "application level autocommit" feature in order
to make sense, which itself is also being removed as it is also `inefficient and misleading <migration_20_autocommit>`.

- In order to handle result sets, `Engine.execute` returns a result object
with unconsumed cursor results.  This cursor result necessarily still links to the DBAPI connection which remains in an open transaction, all of which is released once the result set has fully consumed the rows waiting within the cursor.   This means that `Engine.execute` does not actually close out the connection resources that it claims to be managing when the call is complete. SQLAlchemy's "autoclose" behavior is well-tuned enough that users don't generally report any negative effects from this system, however it remains an overly implicit and inefficient system left over from SQLAlchemy's earliest releases.
...
The above pattern has all the issues of "connectionless" execution, plus it relies upon the "bound metadata" pattern, which SQLAlchemy has tried to de-emphasize for many years.   This was SQLAlchemy's very first advertised usage model in version 0.1, which became obsolete almost immediately when the `_engine.Connection` object was introduced and later Python context managers provided a better pattern for using resources within a fixed scope.

With implicit execution removed, "bound metadata" itself also no longer has a purpose within this system.   In modern use "bound metadata" tends to still be somewhat convenient for working within `_schema.MetaData.create_all calls as well as with orm.Session objects, however having these functions receive an engine.Engine` explicitly provides for clearer application design.

#### Many Choices becomes One Choice

Overall, the above executional patterns were introduced in SQLAlchemy's very first 0.1 release before the `_engine.Connection` object even existed. After many years of de-emphasizing these patterns, "implicit, connectionless" execution and "bound metadata" are no longer as widely used so in 2.0 we seek to finally reduce the number of choices for how to execute a statement in Core from "many choices":
...
The argument patterns that may be used with the `_engine.Connection execute method in SQLAlchemy 2.0 are highly simplified, removing many previously available argument patterns.  The new API in the 1.4 series is described at engine.Connection`. The examples below illustrate the patterns that require modification:
...
The new `_engine.Connection.execute method now accepts a subset of the argument styles that are accepted by the 1.x engine.Connection.execute` method, so the following code is cross-compatible between 1.x and 2.0:
...
The use of `*args` and `**kwargs has been removed both to remove the complexity of guessing what kind of arguments were passed to the method, as well as to make room for other options, namely the engine.Connection.execute.execution_options dictionary that is now available to provide options on a per statement basis. The method is also modified so that its use pattern matches that of the orm.Session.execute` method, which is a much more prominent API in 2.0 style.

The removal of direct string SQL is to resolve an inconsistency between `_engine.Connection.execute and orm.Session.execute, where in the former case the string is passed to the driver raw, and in the latter case it is first converted to a sql.text construct.  By allowing only sql.text this also limits the accepted parameter format to "named" and not "positional".  Finally, the string SQL use case is becoming more subject to scrutiny from a security perspective, and the sql.text` construct has come to represent an explicit boundary into the textual SQL realm where attention to untrusted user input must be given.

### Result rows act like named tuples

**Synopsis**

Version 1.4 introduces an `all new Result object <change_result_14_core> that in turn returns engine.Row` objects, which behave like named tuples when using "future" mode:
...
Already part of 1.4, the previous `KeyedTuple class that was used when selecting rows from the query.Query` object has been replaced by the `.Row` class, which is the base of the same `.Row that comes back with Core statement results when using the sa.create_engine.future flag with engine.Engine (when the sa.create_engine.future` flag is not set, Core result sets use the `LegacyRow subclass, which maintains backwards-compatible behaviors for the _contains__()` method; ORM exclusively uses the `.Row` class directly).
...
The `_sql.select construct as well as the related method sql.FromClause.select will no longer accept keyword arguments to build up elements such as the WHERE clause, FROM list and ORDER BY.   The list of columns may now be sent positionally, rather than as a list.  Additionally, the sql.case` construct now accepts its WHEN criteria positionally, rather than as a list:
...
Only the "generative" style of `_sql.select will be supported.  The list of columns / tables to SELECT from should be passed positionally.  The sql.select` construct in SQLAlchemy 1.4 accepts both the legacy styles and the new styles using an auto-detection scheme, so the code below is cross-compatible with 1.4 and 2.0:
...
SQLAlchemy has for many years developed a convention for SQL constructs accepting an argument either as a list or as positional arguments.   This convention states that **structural** elements, those that form the structure of a SQL statement, should be passed **positionally**.   Conversely, **data** elements, those that form the parameterized data of a SQL statement, should be passed **as lists**.   For many years, the `_sql.select construct could not participate in this convention smoothly because of the very legacy calling pattern where the "WHERE" clause would be passed positionally. SQLAlchemy 2.0 finally resolves this by changing the sql.select` construct to only accept the "generative" style that has for many years been the only documented style in the Core tutorial.
...
In a similar way as to the previous change to `_sql.select, the constructor arguments to sql.insert, sql.update and sql.delete` other than the table argument are essentially removed:
...
The API and internals is being simplified for the DML constructs in a similar manner as that of the `_sql.select` construct.
...
The `sqlalchemy.ext.declarative` package is mostly, with some exceptions, moved to the `sqlalchemy.orm package.  The orm.declarative_base and orm.declared_attr functions are present without any behavioral changes.  A new super-implementation of orm.declarative_base known as orm.registry` now serves as the top-level ORM configurational construct, which also provides for decorator-based declarative and new support for classical mappings that integrate with the declarative registry.
...
After ten years or so of popularity, the `sqlalchemy.ext.declarative` package is now integrated into the `sqlalchemy.orm` namespace, with the exception of the declarative "extension" classes which remain as Declarative extensions.   The change is detailed further in the 1.4 migration guide at `change_5508`.

> **Seealso:** `orm_mapping_classes_toplevel` - all new unified documentation for
...
The `sqlalchemy.orm.mapper() standalone function moves behind the scenes to be invoked by higher level APIs. The new version of this function is the method orm.registry.map_imperatively taken from a orm.registry` object.
...
To work from a central `_orm.registry` object:
...
The above `_orm.registry is also the source for declarative mappings, and classical mappings now have access to this registry including string-based configuration on orm.relationship`:
...
By popular demand, "classical mapping" is staying around, however the new form of it is based off of the `_orm.registry object and is available as orm.registry.map_imperatively`.

In addition, the primary rationale used for "classical mapping" is that of keeping the `_schema.Table` setup distinct from the class.  Declarative has always allowed this style using so-called `hybrid declarative <orm_imperative_table_configuration>`. However, to remove the base class requirement, a first class `decorator <declarative_config_toplevel>` form has been added.

As yet another separate but related enhancement, support for `Python dataclasses <orm_declarative_dataclasses>` is added as well to both declarative decorator and classical mapping forms.
...
The biggest visible change in SQLAlchemy 2.0 is the use of `_orm.Session.execute in conjunction with sql.select to run ORM queries, instead of using orm.Session.query.  As mentioned elsewhere, there is no plan to actually remove the orm.Session.query` API itself, as it is now implemented by using the new API internally it will remain as a legacy API, and both APIs can be used freely.
...
The `_orm.Query object (as well as the baked.BakedQuery and horizontal.ShardedQuery extensions) become long term legacy objects, replaced by the direct usage of the sql.select construct in conjunction with the orm.Session.execute method.  Results that are returned from orm.Query in the form of lists of objects or tuples, or as scalar ORM objects are returned from orm.Session.execute uniformly as engine.Result` objects, which feature an interface consistent with that of Core execution.
...
Because the vast majority of an ORM application is expected to make use of `_orm.Query objects as well as that the orm.Query interface being available does not impact the new interface, the object will stay around in 2.0 but will no longer be part of documentation nor will it be supported for the most part.  The sql.select construct now suits both the Core and ORM use cases, which when invoked via the orm.Session.execute` method will return ORM-oriented results, that is, ORM objects if that's what was requested.

The `_sql.Select construct **adds many new methods** for compatibility with orm.Query, including sql.Select.filter sql.Select.filter_by, newly reworked sql.Select.join and sql.Select.outerjoin methods, sql.Select.options, etc.    Other more supplemental methods of orm.Query such as orm.Query.populate_existing` are implemented via execution options.

Return results are in terms of a `_result.Result` object, the new version of the SQLAlchemy `ResultProxy object, which also adds many new methods for compatibility with orm.Query, including engine.Result.one, engine.Result.all, engine.Result.first, engine.Result.one_or_none`, etc.

The `_engine.Result object however does require some different calling patterns, in that when first returned it will **always return tuples** and it will **not deduplicate results in memory**.    In order to return single ORM objects the way orm.Query does, the engine.Result.scalars modifier must be called first.  In order to return uniqued objects, as is necessary when using joined eager loading, the engine.Result.unique` modifier must be called first.

Documentation for all new features of `_sql.select` including execution options, etc. are at `/orm/queryguide/index`.

Below are some examples of how to migrate to `_sql.select`:
...
The fact that SQLAlchemy has both a `_expression.select construct as well as a separate orm.Query` object that features an extremely similar, but fundamentally incompatible interface is likely the greatest inconsistency in SQLAlchemy, one that arose as a result of small incremental additions over time that added up to two major APIs that are divergent.

In SQLAlchemy's first releases, the `_orm.Query object didn't exist at all.  The original idea was that the orm.Mapper construct itself would be able to select rows, and that schema.Table objects, not classes, would be used to create the various criteria in a Core-style approach.   The query.Query` came along some months / years into SQLAlchemy's history as a user proposal for a new, "buildable" querying object originally called `SelectResults` was accepted. Concepts like a `.where()` method, which `SelectResults` called `.filter(), were not present in SQLAlchemy previously, and the sql.select` construct used only the "all-at-once" construction style that's now deprecated at `migration_20_5284`.

As the new approach took off, the object evolved into the `_orm.Query object as new features such as being able to select individual columns, being able to select multiple entities at once, being able to build subqueries from a orm.Query object rather than from a sql.select object were added.   The goal became that orm.Query should have the full functionality of sql.select in that it could be composed to build SELECT statements fully with no explicit use of sql.select needed.   At the same time, sql.select had also evolved "generative" methods like sql.Select.where and sql.Select.order_by`.

In modern SQLAlchemy, this goal has been achieved and the two objects are now completely overlapping in functionality.  The major challenge to unifying these objects was that the `_sql.select object needed to remain **completely agnostic of the ORM**.  To achieve this, the vast majority of logic from orm.Query has been moved into the SQL compile phase, where ORM-specific compiler plugins receive the sql.Select` construct and interpret its contents in terms of an ORM-style query, before passing off to the core-level compiler in order to create a SQL string.  With the advent of the new `SQL compilation caching system <change_4639>`, the majority of this ORM logic is also cached.
...
The `_orm.Query.get method remains for legacy purposes, but the primary interface is now the orm.Session.get` method:
...
In 1.4 / 2.0, the `_orm.Session object adds a new orm.Session.get` method:
...
The `_orm.Query object is to be a legacy object in 2.0, as ORM queries are now available using the sql.select object.  As the orm.Query.get method defines a special interaction with the orm.Session and does not necessarily even emit a query, it's more appropriate that it be part of orm.Session, where it is similar to other "identity" methods such as orm.Session.refresh and orm.Session.merge`.

SQLAlchemy originally included "get()" to resemble the Hibernate `Session.load() method.  As is so often the case, we got it slightly wrong as this method is really more about the orm.Session` than with writing a SQL query.

### ORM Query  - Joining / loading on relationships uses attributes, not strings

**Synopsis**

This refers to patterns such as that of `_query.Query.join as well as query options like orm.joinedload` which currently accept a mixture of string attribute names or actual class attributes.   The string forms will all be removed in 2.0:
...
Use individual calls to `_orm.Query.join` for 1.x /2.0 cross compatible use:

```
q = session.query(User).join(User.orders).join(Order.items).join(Item.keywords)
```

For `2.0-style use, sql.Select has the same behavior of sql.Select.join, and also features a new sql.Select.join_from` method that allows an explicit left side:
...
Removing the chaining of attributes is in line with simplifying the calling interface of methods such as `_sql.Select.join`.

### ORM Query - join(..., aliased=True), from_joinpoint removed

**Synopsis**

The `aliased=True option on query.Query.join` is removed, as is the `from_joinpoint` flag:
...
The `aliased=True option on query.Query.join` is another feature that seems to be almost never used, based on extensive code searches to find actual use of this feature.   The internal complexity that the `aliased=True` flag requires is **enormous**, and will be going away in 2.0.
...
The `aliased=True` and `from_joinpoint parameters were developed at a time when the query.Query` object didn't yet have good capabilities regarding joining along relationship attributes, functions like `.PropComparator.of_type` did not exist, and the `.aliased` construct itself didn't exist early on.

### Using DISTINCT with additional columns, but only select the entity

**Synopsis**

`_query.Query` will automatically add columns in the ORDER BY when distinct is used.  The following query will select from all User columns as well as "address.email_address" but only return User objects:
...
In 2.0, the column must be added explicitly.  To resolve the issue of only returning the main entity object, and not the extra column, use the `_result.Result.columns` method:
...
This case is an example of the limited flexibility of `_orm.Query` leading to the case where implicit, "magical" behavior needed to be added; the "email_address" column is implicitly added to the columns clause, then additional internal logic would omit that column from the actual results returned.
...
The `_orm.Query.from_self method will be removed from orm.Query`:
...
The `_query.Query.from_self method is a very complicated method that is rarely used.   The purpose of this method is to convert a query.Query into a subquery, then return a new query.Query` which SELECTs from that subquery. The elaborate aspect of this method is that the returned query applies automatic translation of ORM entities and columns to be stated in the SELECT in terms of the subquery, as well as that it allows the entities and columns to be SELECTed from to be modified.

Because `_query.Query.from_self` packs an intense amount of implicit translation into the SQL it produces, while it does allow a certain kind of pattern to be executed very succinctly, real world use of this method is infrequent as it is not simple to understand.

The new approach makes use of the `_orm.aliased` construct so that the ORM internals don't need to guess which entities and columns should be adapted and in what way; in the example above, the `ua` and `aa objects, both of which are orm.AliasedClass` instances, provide to the internals an unambiguous marker as to where the subquery should be referenced as well as what entity column or relationship is being considered for a given component of the query.
...
The `_orm.Query.select_entity_from` method will be removed in 2.0:
...
As is the case described at `migration_20_query_from_self, the orm.aliased` object provides a single place that operations like "select entity from a subquery" may be achieved.  Using `1.x style`:
...
The situation here is a little bit unusual, in that SQLAlchemy is requiring that a method be invoked that it is in fact entirely capable of doing automatically.   The reason for requiring that the method be called is to ensure the developer is "opting in" to the use of the `_engine.Result.unique` method, such that they will not be confused when a straight count of rows does not conflict with the count of records in the actual result set, which has been a long running source of user confusion and bug reports for many years.    That the uniquifying is not happening in any other case by default will improve performance and also improve clarity in those cases where automatic uniquing was causing confusing results.

To the degree that having to call `_engine.Result.unique when joined eager load collections are used is inconvenient, in modern SQLAlchemy the orm.selectinload strategy presents a collection-oriented eager loader that is superior in most respects to orm.joinedload` and should be preferred.

### "Dynamic" relationship loaders superseded by "Write Only"

**Synopsis**

The `lazy="dynamic"` relationship loader strategy, discussed at `dynamic_relationship, makes use of the query.Query` object which is legacy in 2.0. The "dynamic" relationship is not directly compatible with asyncio without workarounds, and additionally it does not fulfill its original purpose of preventing iteration of large collections as it has several behaviors where this iteration occurs implicitly.

A new loader strategy known as `lazy="write_only" is introduced, which through the orm.WriteOnlyCollection` collection class provides a very strict "no implicit iteration" API and additionally integrates with 2.0 style statement execution, supporting asyncio as well as direct integrations with the new `ORM-enabled Bulk DML <change_8360>` featureset.
...
Therefore the best strategy for migrating from "dynamic" is to **wait until the application is fully running on 2.0**, then migrate directly from `.AppenderQuery`, which is the collection type used by the "dynamic" strategy, to `.WriteOnlyCollection`, which is the collection type used by the "write_only" strategy.
...
- Make use of the `_orm.Query.statement` attribute on an existing
`lazy="dynamic" relationship.   We can use methods like orm.Session.scalars` with the dynamic loader straight away as follows:
...
- Use the `_orm.with_parent function to construct a sql.select`
construct directly:
...
The original idea was that the `_orm.with_parent` function should be sufficient, however continuing to make use of special attributes on the relationship itself remains appealing, and there's no reason a 2.0 style construct can't be made to work here as well.

The new "write_only" loader strategy provides a new kind of collection which does not support implicit iteration or item access.  Instead, reading the contents of the collection is performed by calling upon its `.select()` method to help construct an appropriate SELECT statement.  The collection also includes methods `.insert()`, `.update()`, `.delete()` which may be used to emit bulk DML statements for the items in the collection. In a manner similar to that of the "dynamic" feature, there are also methods `.add()`, `.add_all()` and `.remove()` which queue individual members for addition or removal using the unit of work process.  An introduction to the new feature is as `change_7123`.
...
The `_orm.Session` will no longer support "autocommit" mode, that is, this pattern:
...
The main reason a `_orm.Session is used in "autocommit" mode is so that the orm.Session.begin method is available, so that framework integrations and event hooks can control when this event happens.  In 1.4, the orm.Session` now features `autobegin behavior <change_5074> which resolves this issue; the orm.Session.begin` method may now be called:
...
The "autocommit" mode is another holdover from the first versions of SQLAlchemy.  The flag has stayed around mostly in support of allowing explicit use of `_orm.Session.begin`, which is now solved by 1.4, as well as to allow the use of "subtransactions", which are also removed in 2.0.

### Session "subtransaction" behavior removed

**Synopsis**

The "subtransaction" pattern that was often used with autocommit mode is also deprecated in 1.4.  This pattern allowed the use of the `_orm.Session.begin method when a transaction were already begun, resulting in a construct called a "subtransaction", which was essentially a block that would prevent the orm.Session.commit` method from actually committing.
...
As the `_orm.Query object becomes legacy, these two recipes which previously relied upon subclassing of the orm.Query object now make use of the orm.SessionEvents.do_orm_execute` hook.    See the section `do_orm_execute_re_executing` for an example.
...
See `sql_caching` for full background on the new caching system.

## Asyncio Support

SQLAlchemy 1.4 includes asyncio support for both Core and ORM. The new API exclusively makes use of the "future" patterns noted above. See `change_3414` for background.
