...
source_branch: "main"
source_path: "doc/build/orm/persistence_techniques.rst"
...
SQL expressions and strings can be executed via the `sqlalchemy.orm.session.Session` within its transactional context. This is most easily accomplished using the `.Session.execute` method, which returns a `sqlalchemy.engine.CursorResult` in the same manner as an `sqlalchemy.engine.Engine` or `sqlalchemy.engine.Connection`:
...
The current `sqlalchemy.engine.Connection` held by the `sqlalchemy.orm.session.Session` is accessible using the `.Session.connection` method:

```
connection = session.connection()
```

The examples above deal with a `_orm.Session that's bound to a single engine.Engine or engine.Connection. To execute statements using a orm.Session which is bound either to multiple engines, or none at all (i.e. relies upon bound metadata), both orm.Session.execute and orm.Session.connection accept a dictionary of bind arguments orm.Session.execute.bind_arguments which may include "mapper" which is passed a mapped class or orm.Mapper` instance, which is used to locate the proper context for the desired engine:
...
So what if we want to actually put NULL into this column, even though the column has a default value?  There are two approaches.  One is that on a per-instance level, we assign the attribute using the `_expression.null` SQL construct:
...
The `_expression.null` SQL construct always translates into the SQL NULL value being directly present in the target INSERT statement.
...
In this case, columns should be marked as `.FetchedValue or with an explicit schema.Column.server_default.   The ORM will automatically add these columns to the RETURNING clause when performing INSERT statements, assuming the orm.Mapper.eager_defaults` parameter is set to `True`, or if left at its default setting of `"auto"`, for dialects that support both RETURNING as well as `insertmanyvalues <engine_insertmanyvalues>`:
...
.. versionchanged:: 2.0.0rc1 The `_orm.Mapper.eager_defaults` parameter now defaults

> **Note:** applies to INSERT statements.  UPDATE statements will not use RETURNING,
even if available, unless `_orm.Mapper.eager_defaults` is set to
...
To disable the use of RETURNING for such values, including not just for server generated default values but also to ensure that the ORM will never use RETURNING with a particular table, specify `_schema.Table.implicit_returning` as `False` for the mapped `.Table`.  Using a Declarative mapping this looks like:
...
> **Seealso:**  `mssql_insert_behavior` - background on the SQL Server dialect's
...
This case is the same as case 1 above, except we typically don't want to use `.orm.Mapper.eager_defaults`, as its current implementation in the absence of RETURNING support is to emit a SELECT-per-row, which is not performant.  Therefore the parameter is omitted in the mapping below:
...
A future release of SQLAlchemy may seek to improve the efficiency of eager defaults in the absence of RETURNING to batch many rows within a single SELECT statement.
...
> **Note:** datetime values for MySQL, since the datetime datatypes on this
...
When using the `_types.TIMESTAMP datatype with MySQL, MySQL ordinarily associates a server-side default with this datatype automatically.  However when we use one as a primary key, the Core cannot retrieve the newly generated value unless we execute the function ourselves.  As types.TIMESTAMP` on MySQL actually stores a binary value, we need to add an additional "CAST" to our usage of "NOW()" so that we retrieve a binary value that can be persisted into the column:
...
> **Seealso:**  `metadata_defaults_toplevel`

## Notes on eagerly fetching client invoked SQL expressions used for INSERT or UPDATE

The preceding examples indicate the use of `_schema.Column.server_default` to create tables that include default-generation functions within their DDL.

SQLAlchemy also supports non-DDL server side defaults, as documented at `defaults_client_invoked_sql; these "client invoked SQL expressions" are set up using the schema.Column.default and schema.Column.onupdate` parameters.

These SQL expressions currently are subject to the same limitations within the ORM as occurs for true server-side defaults; they won't be eagerly fetched with RETURNING when `_orm.Mapper.eager_defaults` is set to `"auto"` or `True` unless the `.FetchedValue directive is associated with the schema.Column`, even though these expressions are not DDL server defaults and are actively rendered by SQLAlchemy itself. This limitation may be addressed in future SQLAlchemy releases.

The `.FetchedValue construct can be applied to schema.Column.server_default or schema.Column.server_onupdate at the same time that a SQL expression is used with schema.Column.default and schema.Column.onupdate`, such as in the example below where the `func.now() construct is used as a client-invoked SQL expression for schema.Column.default and schema.Column.onupdate.  In order for the behavior of orm.Mapper.eager_defaults to include that it fetches these values using RETURNING when available, schema.Column.server_default and schema.Column.server_onupdate` are used with `.FetchedValue` to ensure that the fetch occurs:
...
Above, SQL operations against either class will make usage of the `_engine.Engine linked to that class.     The functionality is comprehensive across both read and write operations; a query.Query` that is against entities mapped to `engine1` (determined by looking at the first entity in the list of items requested) will make use of `engine1` to run the query.   A flush operation will make use of **both** engines on a per-class basis as it flushes objects of type `User` and `Account`.

In the more common case, there are typically base or mixin classes that  can be used to distinguish between operations that are destined for different database connections.  The `.Session.binds argument can accommodate any arbitrary Python class as a key, which will be used if it is found to be in the _mro__` (Python method resolution order) for a particular  mapped class. Supposing two declarative bases are representing two different database connections:
...
Due to limited support within DBAPIs,  SQLAlchemy has limited support for two- phase transactions across backends.  Most typically, it is known to work well with the PostgreSQL backend and to  a lesser extent with the MySQL backend. However, the `.Session` is fully capable of taking advantage of the two phase transaction feature when the backend supports it, by setting the `.Session.use_twophase` flag within `.sessionmaker` or `.Session`.  See `session_twophase` for an example.
...
This approach can be combined with multiple `_schema.MetaData objects, using an approach such as that of using the declarative _abstract__` keyword, described at `declarative_abstract`.

> **Note:** to a so-called "leader" or "follower" database based on whether or not the
...
to construct the `_orm.Session` up front as a "reader" or "writer"
...
`session_transaction_isolation_enginewide` for a recipe that sets up
one `_orm.sessionmaker` for "read only" operations using autocommit
connections, and another for "write" operations which will include DML /
COMMIT.

> **Seealso:**  [Django-style Database Routers in SQLAlchemy](https://techspot.zzzeek.org/2012/01/11/django-style-database-routers-in-sqlalchemy/)  - blog post on a more comprehensive example of `.Session.get_bind`

## Horizontal Partitioning

Horizontal partitioning partitions the rows of a single table (or a set of tables) across multiple databases.    The SQLAlchemy `.Session` contains support for this concept, however to use it fully requires that `.Session and query.Query` subclasses are used.  A basic version of these subclasses are available in the `horizontal_sharding_toplevel` ORM extension.   An example of use is at: `examples_sharding`.
