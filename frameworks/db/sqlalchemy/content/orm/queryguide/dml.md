...
source_branch: "main"
source_path: "doc/build/orm/queryguide/dml.rst"
...
> **Admonition:**  This section makes use of ORM mappings first illustrated in the
 `unified_tutorial`, shown in the section
 `tutorial_declaring_mapped_classes`, as well as inheritance
 mappings shown in the section `inheritance_toplevel`.
 `View the ORM setup for this page <_dml_setup>`.

The `_orm.Session.execute method, in addition to handling ORM-enabled sql.Select objects, can also accommodate ORM-enabled sql.Insert, sql.Update and sql.Delete` objects, in various ways which are each used to INSERT, UPDATE, or DELETE many database rows at once.  There is also dialect-specific support for ORM-enabled "upserts", which are INSERT statements that automatically make use of UPDATE for rows that already exist.

The following table summarizes the calling forms that are discussed in this document:

=====================================================   ==========================================   ========================================================================     ========================================================= ============================================================================ ORM Use Case                                            DML Construct Used                           Data is passed using ...                                                     Supports RETURNING?                                       Supports Multi-Table Mappings? =====================================================   ==========================================   ========================================================================     ========================================================= ============================================================================ `orm_queryguide_bulk_insert                       dml.insert                          List of dictionaries to orm.Session.execute.params`              `yes <orm_queryguide_bulk_insert_returning>`         `yes <orm_queryguide_insert_joined_table_inheritance>` `orm_queryguide_bulk_insert_w_sql                 dml.insert                          orm.Session.execute.params with dml.Insert.values`      `yes <orm_queryguide_bulk_insert_w_sql>`             `yes <orm_queryguide_insert_joined_table_inheritance>` `orm_queryguide_insert_values                     dml.insert                          List of dictionaries to dml.Insert.values`                           `yes <orm_queryguide_insert_values>`                 no `orm_queryguide_upsert                            dml.insert                          List of dictionaries to dml.Insert.values`                           `yes <orm_queryguide_upsert_returning>`              no `orm_queryguide_bulk_update                       dml.update                          List of dictionaries to orm.Session.execute.params`              no                                                        `yes <orm_queryguide_bulk_update_joined_inh>` `orm_queryguide_update_delete_where               dml.update, dml.delete     keywords to dml.Update.values`                                       `yes <orm_queryguide_update_delete_where_returning>` `partial, with manual steps <orm_queryguide_update_delete_joined_inh>` =====================================================   ==========================================   ========================================================================     ========================================================= ============================================================================

## ORM Bulk INSERT Statements

A `_dml.insert construct can be constructed in terms of an ORM class and passed to the orm.Session.execute method.   A list of parameter dictionaries sent to the orm.Session.execute.params parameter, separate from the dml.Insert` object itself, will invoke **bulk INSERT mode** for the statement, which essentially means the operation will optimize as much as possible for many rows:
...
The parameter dictionaries contain key/value pairs which may correspond to ORM mapped attributes that line up with mapped `._schema.Column or orm.mapped_column` declarations, as well as with `composite <mapper_composite>` declarations.   The keys should match the **ORM mapped attribute name** and **not** the actual database column name, if these two names happen to be different.

> **Tip:** The operation will emit multiple INSERT statements with different VALUES
clauses for each set of keys. This is distinctly different from a Core
`_sql.Insert` operation, which as introduced at
`tutorial_core_insert_values_clause` only uses the **first** dictionary
in the list to determine a single VALUES clause for all parameter sets.

.. versionchanged:: 2.0  Passing an `_dml.Insert` construct to the

### Getting new objects with RETURNING

>>> session.rollback() ROLLBACK... >>> session.connection() BEGIN (implicit)...

The bulk ORM insert feature supports INSERT..RETURNING for selected backends, which can return a `.Result` object that may yield individual columns back as well as fully constructed ORM objects corresponding to the newly generated records.    INSERT..RETURNING requires the use of a backend that supports SQL RETURNING syntax as well as support for `executemany` with RETURNING; this feature is available with all `SQLAlchemy-included <included_dialects>` backends with the exception of MySQL (MariaDB is included).

As an example, we can run the same statement as before, adding use of the `.UpdateBase.returning` method, passing the full `User entity as what we'd like to return.  orm.Session.scalars` is used to allow iteration of `User` objects:
...
.. versionadded:: 2.0.10 Added `_dml.Insert.returning.sort_by_parameter_order`

> **Seealso:**  `engine_insertmanyvalues_returning_order` - background on approaches
...
Above, the bulk INSERT of four rows is broken into three separate statements, the second statement reformatted to not refer to the NULL column for the single parameter dictionary that contains a `None` value.    This default behavior may be undesirable when many rows in the dataset contain random NULL values, as it causes the "executemany" operation to be broken into a larger number of smaller operations; particularly when relying upon `insertmanyvalues <engine_insertmanyvalues>` to reduce the overall number of statements, this can have a bigger performance impact.
...
> **Tip:** make use of the `_dml.Insert.returning.sort_by_parameter_order`
...
The ORM bulk insert feature supports the addition of a fixed set of parameters which may include SQL expressions to be applied to every target row. To achieve this, combine the use of the `_dml.Insert.values method, passing a dictionary of parameters that will be applied to all rows, with the usual bulk calling form by including a list of parameter dictionaries that contain individual row values when invoking orm.Session.execute`.
...
If we wanted to INSERT a series of `LogRecord` elements, each with a unique `message` field, however we would like to apply the SQL function `now()` to all rows, we can pass `timestamp within dml.Insert.values` and then pass the additional records using "bulk" mode:
...
>>> session.rollback() ROLLBACK >>> session.execute( ...     insert(User), ...     [ ...         { ...             "name": "spongebob", ...             "fullname": "Spongebob Squarepants", ...             "species": "Sea Sponge", ...         }, ...         {"name": "sandy", "fullname": "Sandy Cheeks", "species": "Squirrel"}, ...         {"name": "patrick", "species": "Starfish"}, ...         { ...             "name": "squidward", ...             "fullname": "Squidward Tentacles", ...             "species": "Squid", ...         }, ...         {"name": "ehkrabs", "fullname": "Eugene H. Krabs", "species": "Crab"}, ...     ], ... ) BEGIN...

The `_dml.Insert.values method itself accommodates a list of parameter dictionaries directly. When using the dml.Insert construct in this way, without passing any list of parameter dictionaries to the orm.Session.execute.params` parameter, bulk ORM insert mode is not used, and instead the INSERT statement is rendered exactly as given and invoked exactly once. This mode of operation may be useful both for the case of passing SQL expressions on a per-row basis, and is also used when using "upsert" statements with the ORM, documented later in this chapter at `orm_queryguide_upsert`.

A contrived example of an INSERT that embeds per-row SQL expressions, and also demonstrates `_dml.Insert.returning` in this form, is below:
...
- `Joined table inheritance <orm_queryguide_insert_joined_table_inheritance>`
or other multi-table mappings are not supported, since that would require multiple INSERT statements.

- `Heterogeneous parameter sets <orm_queryguide_insert_heterogeneous_params>`
...
`insertmanyvalues <engine_insertmanyvalues>` are not available; statements will need to ensure the total number of parameters does not exceed limits imposed by the backing database.

For the above reasons, it is generally not recommended to use multiple parameter sets with `_dml.Insert.values` with ORM INSERT statements unless there is a clear rationale, which is either that "upsert" is being used or there is a need to embed per-row SQL expressions in each parameter set.

> **Seealso:**  `orm_queryguide_upsert`

### Legacy Session Bulk INSERT Methods

The `_orm.Session` includes legacy methods for performing "bulk" INSERT and UPDATE statements.  These methods share implementations with the SQLAlchemy 2.0 versions of these features, described at `orm_queryguide_bulk_insert` and `orm_queryguide_bulk_update`, however lack many features, namely RETURNING support as well as support for session-synchronization.
...
> **Seealso:**  `orm_queryguide_legacy_bulk_update`

### ORM "upsert" Statements

Selected backends with SQLAlchemy may include dialect-specific `_dml.Insert` constructs which additionally have the ability to perform "upserts", or INSERTs where an existing row in the parameter set is turned into an approximation of an UPDATE statement instead. By "existing row" , this may mean rows which share the same primary key value, or may refer to other indexed columns within the row that are considered to be unique; this is dependent on the capabilities of the backend in use.

The dialects included with SQLAlchemy that include dialect-specific "upsert" API features are:

- SQLite - using `_sqlite.Insert` documented at `sqlite_on_conflict_insert`
- PostgreSQL - using `_postgresql.Insert` documented at `postgresql_insert_on_conflict`
- MySQL/MariaDB - using `_mysql.Insert` documented at `mysql_insert_on_duplicate_key_update`
...
Third party backends such as those mentioned at `external_toplevel` may also feature similar constructs.

While SQLAlchemy does not yet have a backend-agnostic upsert construct, the above `_dml.Insert variants are nonetheless ORM compatible in that they may be used in the same way as the dml.Insert` construct itself as documented at `orm_queryguide_insert_values, that is, by embedding the desired rows to INSERT within the dml.Insert.values method.   In the example below, the SQLite sqlite.insert function is used to generate an sqlite.Insert construct that includes "ON CONFLICT DO UPDATE" support.   The statement is then passed to orm.Session.execute where it proceeds normally, with the additional characteristic that the parameter dictionaries passed to dml.Insert.values` are interpreted as ORM mapped attribute keys, rather than column names:

>>> session.rollback() ROLLBACK >>> session.execute( ...     insert(User).values( ...         [ ...             dict(name="sandy"), ...             dict(name="spongebob", fullname="Spongebob Squarepants"), ...         ] ...     ) ... ) BEGIN...
...
From the SQLAlchemy ORM's point of view, upsert statements look like regular `_dml.Insert constructs, which includes that dml.Insert.returning` works with upsert statements in the same way as was demonstrated at `orm_queryguide_insert_values`, so that any column expression or relevant ORM entity class may be passed.  Continuing from the example in the previous section:
...
The example above uses RETURNING to return ORM objects for each row inserted or upserted by the statement. The example also adds use of the `orm_queryguide_populate_existing` execution option. This option indicates that `User objects which are already present in the orm.Session for rows that already exist should be **refreshed** with the data from the new row. For a pure dml.Insert statement, this option is not significant, because every row produced is a brand new primary key identity. However when the dml.Insert also includes "upsert" options, it may also be yielding results from rows that already exist and therefore may already have a primary key identity represented in the orm.Session` object's `identity map`.

> **Seealso:**  `orm_queryguide_populate_existing`

## ORM Bulk UPDATE by Primary Key

>>> session.rollback() ROLLBACK >>> session.execute( ...     insert(User), ...     [ ...         {"name": "spongebob", "fullname": "Spongebob Squarepants"}, ...         {"name": "sandy", "fullname": "Sandy Cheeks"}, ...         {"name": "patrick", "fullname": "Patrick Star"}, ...         {"name": "squidward", "fullname": "Squidward Tentacles"}, ...         {"name": "ehkrabs", "fullname": "Eugene H. Krabs"}, ...     ], ... ) BEGIN ... >>> session.commit() COMMIT... >>> session.connection() BEGIN ...

The `_dml.Update construct may be used with orm.Session.execute in a similar way as the dml.Insert` statement is used as described at `orm_queryguide_bulk_insert, passing a list of many parameter dictionaries, each dictionary representing an individual row that corresponds to a single primary key value. This use should not be confused with a more common way to use dml.Update` statements with the ORM, using an explicit WHERE clause, which is documented at `orm_queryguide_update_delete_where`.

For the "bulk" version of UPDATE, a `_dml.update construct is made in terms of an ORM class and passed to the orm.Session.execute method; the resulting dml.Update object should have **no values and typically no WHERE criteria**, that is, the dml.Update.values method is not used, and the dml.Update.where` is **usually** not used, but may be used in the unusual case that additional filtering criteria would be added.

Passing the `_dml.Update` construct along with a list of parameter dictionaries which each include a full primary key value will invoke **bulk UPDATE by primary key mode** for the statement, generating the appropriate WHERE criteria to match each row by primary key, and using `executemany` to run each parameter set against the UPDATE statement:
...
.. versionchanged:: 2.0  Passing an `_dml.Update` construct to the
...
2. the `_orm.Session` is used to execute the statement, and not a
Core `_engine.Connection`

3. The parameters passed are a **list of dictionaries**.
In order to invoke an UPDATE statement without using "ORM Bulk Update by Primary Key", invoke the statement against the `_engine.Connection directly using the orm.Session.connection method to acquire the current engine.Connection` for the transaction:
...
>>> session.execute( ...     insert(Manager).returning(Manager), ...     [ ...         {"name": "sandy", "manager_name": "Sandy Cheeks"}, ...         {"name": "ehkrabs", "manager_name": "Eugene H. Krabs"}, ...     ], ... ) INSERT... >>> session.commit() COMMIT... >>> session.connection() BEGIN (implicit)...

ORM bulk update has similar behavior to ORM bulk insert when using mappings with joined table inheritance; as described at `orm_queryguide_insert_joined_table_inheritance`, the bulk UPDATE operation will emit an UPDATE statement for each table represented in the mapping, for which the given parameters include values to be updated (non-affected tables are skipped).
...
As discussed at `orm_queryguide_legacy_bulk_insert, the orm.Session.bulk_update_mappings method of orm.Session is the legacy form of bulk update, which the ORM makes use of internally when interpreting a sql.update` statement with primary key parameters given; however, when using the legacy version, features such as support for session-synchronization are not included.
...
> **Seealso:**  `orm_queryguide_legacy_bulk_insert`

## ORM UPDATE and DELETE with Custom WHERE Criteria

>>> session.rollback() ROLLBACK... >>> session.connection() BEGIN (implicit)...

The `_dml.Update and dml.Delete constructs, when constructed with custom WHERE criteria (that is, using the dml.Update.where and dml.Delete.where methods), may be invoked in an ORM context by passing them to orm.Session.execute, without using the orm.Session.execute.params parameter. For dml.Update, the values to be updated should be passed using dml.Update.values`.

This mode of use differs from the feature described previously at `orm_queryguide_bulk_update` in that the ORM uses the given WHERE clause as is, rather than fixing the WHERE clause to be by primary key.   This means that the single UPDATE or DELETE statement can affect many rows at once.
...
> **Warning:** for important notes regarding how the functionality of ORM-Enabled UPDATE and DELETE
diverges from that of ORM `unit of work` features, such
as using the `_orm.Session.delete` method to delete individual objects.
...
assumed that ON UPDATE CASCADE and/or ON DELETE CASCADE is configured for any foreign key references which require it, otherwise the database may emit an integrity violation if foreign key references are being enforced. See the notes at `passive_deletes` for some examples.

- After the UPDATE or DELETE, dependent objects in the `.Session` which
were impacted by an ON UPDATE CASCADE or ON DELETE CASCADE on related tables, particularly objects that refer to rows that have now been deleted, may still reference those objects.  This issue is resolved once the `.Session` is expired, which normally occurs upon `.Session.commit` or can be forced by using `.Session.expire_all`.

- ORM-enabled UPDATEs and DELETEs do not handle joined table inheritance
automatically.   See the section `orm_queryguide_update_delete_joined_inh` for notes on how to work with joined-inheritance mappings.

- The WHERE criteria needed in order to limit the polymorphic identity to
specific subclasses for single-table-inheritance mappings **is included automatically** .   This only applies to a subclass mapper that has no table of its own.

- The `_orm.with_loader_criteria` option **is supported** by ORM
update and delete operations; criteria here will be added to that of the UPDATE or DELETE statement being emitted, as well as taken into account during the "synchronize" process.

- In order to intercept ORM-enabled UPDATE and DELETE operations with event
handlers, use the `_orm.SessionEvents.do_orm_execute` event.

### Selecting a Synchronization Strategy

When making use of `_dml.update or dml.delete in conjunction with ORM-enabled execution using orm.Session.execute`, additional ORM-specific functionality is present which will **synchronize** the state being changed by the statement with that of the objects that are currently present within the `identity map of the orm.Session`. By "synchronize" we mean that UPDATEd attributes will be refreshed with the new value, or at the very least `expired` so that they will re-populate with their new value on next access, and DELETEd objects will be moved into the `deleted` state.

This synchronization is controllable as the "synchronization strategy", which is passed as an string ORM execution option, typically by using the `_orm.Session.execute.execution_options` dictionary:
...
The execution option may also be bundled with the statement itself using the `_sql.Executable.execution_options` method:
...
backends that support RETURNING, which includes all SQLAlchemy-native drivers except for MySQL.   If RETURNING is not supported, the `'evaluate'` strategy will be used instead.

- `'fetch'` - Retrieves the primary key identity of affected rows by either
performing a SELECT before the UPDATE or DELETE, or by using RETURNING if the database supports it, so that in-memory objects which are affected by the operation can be refreshed with new values (updates) or expunged from the `_orm.Session (deletes). This synchronization strategy may be used even if the given dml.update or dml.delete construct explicitly specifies entities or columns using dml.UpdateBase.returning`.

.. versionchanged:: 2.0 Explicit `_dml.UpdateBase.returning` may be

- `'evaluate'` - This indicates to evaluate the WHERE
criteria given in the UPDATE or DELETE statement in Python, to locate matching objects within the `_orm.Session`. This approach does not add any SQL round trips to the operation, and in the absence of RETURNING support, may be more efficient. For UPDATE or DELETE statements with complex criteria, the `'evaluate'` strategy may not be able to evaluate the expression in Python and will raise an error. If this occurs, use the `'fetch'` strategy for the operation instead.

> **Tip:**  If a SQL expression makes use of custom operators using the
 `_sql.Operators.op or sql.custom_op` feature, the
 `_sql.Operators.op.python_impl` parameter may be used to indicate
...
.. warning::
 ...
 to run on a `_orm.Session` that has many objects which have
...
for backends that don't support RETURNING where the `"evaluate" strategy is not able to be used.  In this case, the state of objects in the orm.Session` is unchanged and will not automatically correspond to the UPDATE or DELETE statement that was emitted, if such objects that would normally correspond to the rows matched are present.
...
.. versionadded:: 2.0  `.UpdateBase.returning` may be used for

### UPDATE/DELETE with Custom WHERE Criteria for Joined Table Inheritance

>>> session.rollback() ROLLBACK... >>> session.connection() BEGIN (implicit)...

The UPDATE/DELETE with WHERE criteria feature, unlike the `orm_queryguide_bulk_update, only emits a single UPDATE or DELETE statement per call to orm.Session.execute. This means that when running an dml.update or dml.delete` statement against a multi-table mapping, such as a subclass in a joined-table inheritance mapping, the statement must conform to the backend's current capabilities, which may include that the backend does not support an UPDATE or DELETE statement that refers to multiple tables, or may have only limited support for this. This means that for mappings such as joined inheritance subclasses, the ORM version of the UPDATE/DELETE with WHERE criteria feature can only be used to a limited extent or not at all, depending on specifics.

The most straightforward way to emit a multi-row UPDATE statement for a joined-table subclass is to refer to the sub-table alone. This means the `_dml.Update` construct should only refer to attributes that are local to the subclass table, as in the example below:
...
The ORM enabled UPDATE/DELETE with WHERE feature was originally part of the now-legacy `.Query object, in the orm.Query.update and orm.Query.delete` methods.  These methods remain available and provide a subset of the same functionality as that described at `orm_queryguide_update_delete_where`.  The primary difference is that the legacy methods don't provide for explicit RETURNING support.

> **Seealso:**  `_orm.Query.update`
 `_orm.Query.delete`

>>> session.close() ROLLBACK... >>> conn.close()
