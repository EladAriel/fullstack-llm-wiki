...
source_branch: "main"
source_path: "doc/build/orm/inheritance.rst"
...
> **Seealso:**  `loading_joined_inheritance` - in the `queryguide_toplevel`
 `examples_inheritance` - complete examples of joined, single and
...
In the above example, the discriminator is the `type column, whichever is configured using the orm.Mapper.polymorphic_on parameter. This parameter accepts a column-oriented expression, specified either as a string name of the mapped attribute to use or as a column expression object such as schema.Column or orm.mapped_column` construct.

The discriminator column will store a value which indicates the type of object represented within the row. The column may be of any datatype, though string and integer are the most common.  The actual data value to be applied to this column for a particular row in the database is specified using the `_orm.Mapper.polymorphic_identity` parameter, described below.
...
> **Note:** Currently, **only one discriminator column or SQL expression may be
...
See the section `inheritance_loading_toplevel` for background on inheritance loading techniques, including configuration of tables to be queried both at mapper configuration time as well as query time.
...
Even though subclasses share the base table for all of their attributes, when using Declarative, `_orm.mapped_column objects may still be specified on subclasses, indicating that the column is to be mapped only to that subclass; the orm.mapped_column will be applied to the same base schema.Table` object:
...
Above, when `Manager` is mapped, the `start_date` column is already present on the `Employee` class, having been provided by the `Engineer mapping already.   The orm.mapped_column.use_existing_column parameter indicates to orm.mapped_column that it should look for the requested schema.Column` on the mapped `.Table` for `Employee first, and if present, maintain that existing mapping.  If not present, orm.mapped_column` will map the column normally, adding it as one of the columns in the `.Table` referenced by the `Employee` superclass.

.. versionadded:: 2.0.0b4 - Added `_orm.mapped_column.use_existing_column`,

A similar concept can be used with mixin classes (see `orm_mixins_toplevel`) to define a particular series of columns and/or other mapped attributes from a reusable mixin class:
...
When building any kind of inheritance hierarchy, a mapped class may include the `_orm.Mapper.polymorphic_abstract` parameter set to `True, which indicates that the class should be mapped normally, however would not expect to be instantiated directly and would not include a orm.Mapper.polymorphic_identity. Subclasses may then be declared as subclasses of this mapped class, which themselves can include a orm.Mapper.polymorphic_identity and therefore be used normally. This allows a series of subclasses to be referenced at once by a common base class which is considered to be "abstract" within the hierarchy, both in queries as well as in orm.relationship` declarations. This use differs from the use of the `declarative_abstract attribute with Declarative, which leaves the target class entirely unmapped and thus not usable as a mapped class by itself. orm.Mapper.polymorphic_abstract` may be applied to any class or classes at any level in the hierarchy, including on multiple levels at once.

As an example, suppose `Manager` and `Principal` were both to be classified against a superclass `Executive`, and `Engineer` and `Sysadmin` were classified against a superclass `Technologist`. Neither `Executive` or `Technologist is ever instantiated, therefore have no orm.Mapper.polymorphic_identity. These classes can be configured using orm.Mapper.polymorphic_abstract` as follows:
...
In the above example, the new classes `Technologist` and `Executive` are ordinary mapped classes, and also indicate new columns to be added to the superclass called `executive_background` and `competencies.   However, they both lack a setting for orm.Mapper.polymorphic_identity`; this is because it's not expected that `Technologist` or `Executive` would ever be instantiated directly; we'd always have one of `Manager`, `Principal`, `Engineer` or `SysAdmin`.   We can however query for `Principal` and `Technologist roles, as well as have them be targets of orm.relationship`.  The example below demonstrates a SELECT statement for `Technologist` objects:
...
The `Technologist` and `Executive abstract mapped classes may also be made the targets of orm.relationship` mappings, like any other mapped class.  We can extend the above example to include `Company`, with separate collections `Company.technologists` and `Company.principals`:
...
> **Seealso:**  `declarative_abstract` - Declarative parameter which allows a
...
> **Warning:**  Concrete table inheritance is **much more complicated** than joined
...
To establish a class as using concrete inheritance, add the `_orm.Mapper.concrete parameter within the _mapper_args__`. This indicates to Declarative as well as the mapping that the superclass table should not be considered as part of the mapping:
...
the same name.  A column such as `Employee.name` here is **not** copied out to the tables mapped by `Manager` or `Engineer` for us.

- while the `Engineer` and `Manager` classes are
mapped in an inheritance relationship with `Employee`, they still **do not include polymorphic loading**.  Meaning, if we query for `Employee` objects, the `manager` and `engineer` tables are not queried at all.
...
As discussed in `inheritance_loading_toplevel, mapper inheritance configurations of any type can be configured to load from a special selectable by default using the orm.Mapper.with_polymorphic argument.  Current public API requires that this argument is set on a orm.Mapper` when it is first constructed.

However, in the case of Declarative, both the mapper and the `_schema.Table that is mapped are created at once, the moment the mapped class is defined. This means that the orm.Mapper.with_polymorphic argument cannot be provided yet, since the schema.Table` objects that correspond to the subclasses haven't yet been defined.
...
With the above `_schema.Table objects, the mappings can be produced using "semi-classical" style, where we use Declarative in conjunction with the _table__ argument; our polymorphic union above is passed via _mapper_args__ to the orm.Mapper.with_polymorphic` parameter:
...
Alternatively, the same `_schema.Table` objects can be used in fully "classical" style, without using Declarative at all. A constructor similar to that supplied by Declarative is illustrated:
...
The "abstract" example can also be mapped using "semi-classical" or "classical" style.  The difference is that instead of applying the "polymorphic union" to the `_orm.Mapper.with_polymorphic` parameter, we apply it directly as the mapped selectable on our basemost mapper.  The semi-classical mapping is illustrated below:
...
> **Seealso:**  `orm_imperative_mapping` - background information on imperative, or "classical" mappings
...
The next complexity with concrete inheritance and relationships involves when we'd like one or all of `Employee`, `Manager` and `Engineer` to themselves refer back to `Company.   For this case, SQLAlchemy has special behavior in that a orm.relationship` placed on `Employee` which links to `Company` **does not work** against the `Manager` and `Engineer classes, when exercised at the instance level.  Instead, a distinct orm.relationship` must be applied to each class.   In order to achieve bi-directional behavior in terms of three separate relationships which serve as the opposite of `Company.employees, the orm.relationship.back_populates` parameter is used between each of the relationships:
