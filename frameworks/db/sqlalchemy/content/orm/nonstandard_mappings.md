...
source_branch: "main"
source_path: "doc/build/orm/nonstandard_mappings.rst"
...
Mappers can be constructed against arbitrary relational units (called selectables) in addition to plain tables. For example, the `_expression.join function creates a selectable unit comprised of multiple tables, complete with its own composite primary key, which can be mapped in the same way as a schema.Table`:
...
> **Note:**  A mapping against multiple tables as illustrated above supports
 ...
 INSERT construct with `_schema.Table.insert`, then executing it  using the
 given `_engine.Connection` which is the same one being used to emit other
...
Similar to mapping against a join, a plain `_expression.select` object can be used with a mapper as well.  The example fragment below illustrates mapping a class called `Customer to a expression.select` which includes a join to a subquery:
...
> **Note:**  The practice of mapping to arbitrary SELECT statements, especially
 ...
 based on the very early history of SQLAlchemy where the `_orm.Mapper`
 construct was meant to represent the primary querying interface;
 in modern usage, the `_query.Query` object can be used to construct
