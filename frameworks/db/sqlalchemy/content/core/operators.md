...
source_branch: "main"
source_path: "doc/build/core/operators.rst"
...
Finally, the `_sql.ColumnOperators.in_ and sql.ColumnOperators.not_in operators work with subqueries.   The form provides that a sql.Select` construct is passed in directly, without any explicit conversion to a named subquery:
...
* `_sql.ColumnOperators.__add__, sql.ColumnOperators.__radd__` (Python "`+`" operator):
...
* `_sql.ColumnOperators.__sub__, sql.ColumnOperators.__rsub__` (Python "`-`" operator):
...
```
 ...
 `_sql.ColumnOperators.__mul__, sql.ColumnOperators.__rmul__` (Python "``" operator):
...
- `_sql.ColumnOperators.__truediv__, sql.ColumnOperators.__rtruediv__` (Python "`/`" operator).
...
- `_sql.ColumnOperators.__floordiv__, sql.ColumnOperators.__rfloordiv__` (Python "`//`" operator).
This is the Python `floordiv` operator, which will ensure floor division occurs. For the default backend as well as backends such as PostgreSQL, the SQL `/` operator normally behaves this way for integer values:
...
* `_sql.ColumnOperators.__mod__, sql.ColumnOperators.__rmod__` (Python "`%`" operator):
...
Bitwise operator functions provide uniform access to bitwise operators across different backends, which are expected to operate on compatible values such as integers and bit-strings (e.g. PostgreSQL `_postgresql.BIT` and similar). Note that these are **not** general boolean operators.

.. versionadded:: 2.0.2 Added dedicated operators for bitwise operations.

- `_sql.ColumnOperators.bitwise_not, sql.bitwise_not`.
Available as a column-level method, producing a bitwise NOT clause against a parent object:
...
* `_sql.ColumnOperators.bitwise_and` produces bitwise AND:
...
* `_sql.ColumnOperators.bitwise_or` produces bitwise OR:
...
* `_sql.ColumnOperators.bitwise_xor` produces bitwise XOR:
...
- `_sql.ColumnOperators.bitwise_rshift, sql.ColumnOperators.bitwise_lshift`
...
The most common conjunction, "AND", is automatically applied if we make repeated use of the `_sql.Select.where method, as well as similar methods such as sql.Update.where and sql.Delete.where`:
...
`_sql.Select.where, sql.Update.where and sql.Delete.where` also accept multiple expressions with the same effect:
...
The "AND" conjunction, as well as its partner "OR", are both available directly using the `_sql.and_ and sql.or_` functions:
...
The above conjunction functions `_sql.and_, sql.or_, sql.not_` are also available as overloaded Python operators:

> **Note:** in the language; as a result, parenthesis must usually be applied
for operands that themselves contain expressions, as indicated in the
examples below.

- `_sql.Operators.__and__` (Python "`&`" operator):
The Python binary `& operator is overloaded to behave the same as sql.and_` (note parenthesis around the two operands):
...
The Python binary `| operator is overloaded to behave the same as sql.or_` (note parenthesis around the two operands):
...
The Python binary `~ operator is overloaded to behave the same as sql.not_`, either inverting the existing operator, or applying the `NOT` keyword to the expression as a whole:
...
More background on this is in the FAQ at `faq_sql_expression_paren_rules`.
