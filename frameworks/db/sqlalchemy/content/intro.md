...
source_branch: "main"
source_path: "doc/build/intro.rst"
...
- `unified_tutorial` - this all-new tutorial for the 1.4/2.0/2.1 series of
SQLAlchemy introduces the entire library holistically, starting from a description of Core and working more and more towards ORM-specific concepts. New users, as well as users coming from the 1.x series of SQLAlchemy, should start here.

- `orm_toplevel` - In this section, reference documentation for the ORM is
presented.

- `core_toplevel` - Here, reference documentation for
everything else within Core is presented. SQLAlchemy engine, connection, and pooling services are also described here.

- `dialect_toplevel` - Provides reference documentation
for all `dialect` implementations, including `DBAPI` specifics.
...
There is also a wide variety of examples involving both core SQLAlchemy constructs as well as the ORM on the wiki.  See [Theatrum Chemicum](https://www.sqlalchemy.org/trac/wiki/UsageRecipes).
...
This command will download the latest **released** version of SQLAlchemy from the [Python Cheese Shop](https://pypi.org/project/SQLAlchemy) and install it to your system. For most common platforms, a Python Wheel file will be downloaded which provides native Cython / C extensions prebuilt.
...
SQLAlchemy's `asyncio` support depends upon the [greenlet](https://pypi.org/project/greenlet/) project.    This dependency is not included by default.   To install with asyncio support, run this command:

```text
 pip install sqlalchemy[asyncio]
```

This installation will include the greenlet dependency in the installation. See the section `asyncio_install` for additional details on ensuring asyncio support is present.
...
The source install is platform agnostic and will install on any platform regardless of whether or not Cython / C build tools are installed. As the next section `c_extensions` details, `setup.py` will attempt to build using Cython / C if possible but will fall back to a pure Python installation otherwise.
...
Source builds may also be performed using `517` techniques, such as using build_:
...
SQLAlchemy is designed to operate with a `DBAPI` implementation built for a particular database, and includes support for the most popular databases. The individual database sections in `/dialects/index` enumerate the available DBAPIs for each database, including external links.

## Checking the Installed SQLAlchemy Version

This documentation covers SQLAlchemy version 2.1. If you're working on a system that already has SQLAlchemy installed, check the version from your Python prompt like this:
...
With SQLAlchemy installed, new and old users alike can `Proceed to the SQLAlchemy Tutorial <unified_tutorial>`.

# 2.0 to 2.1 Migration

Users coming SQLAlchemy version 2.0 will want to read:

- `What's New in SQLAlchemy 2.1? <changelog/migration_21>` - New features and behaviors in version 2.1
...
- `Migrating to SQLAlchemy 2.0 <changelog/migration_20>` - Complete background on migrating from 1.3 or 1.4 to 2.0
- `What's New in SQLAlchemy 2.0? <changelog/whatsnew_20>` - New 2.0 features and behaviors beyond the 1.x migration
An index of all changelogs and migration documentation is at:

- `Changelog catalog <changelog/index>` - Detailed changelogs for all SQLAlchemy Versions
