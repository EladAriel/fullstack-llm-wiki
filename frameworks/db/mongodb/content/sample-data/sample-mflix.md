...
source_branch: "main"
source_path: "content/manual/manual/source/sample-data/sample-mflix.txt"
...
created from the `plot` field using |voyage|'s `voyage-3-large` embedding model and converted to `binData` for efficient storage and retrieval.

- `plot_embedding` - Contains 1536d embeddings created from the
`plot` field using OpenAI's `text-embedding-ada-002` embedding model and converted to `binData` for efficient storage and retrieval.
...
> **Note:** The number of dimensions in the sample have been truncated for
readability.
...
This collection contains metadata about `users <mflix-users>`. Each document contains a user and their corresponding [JSON Web Token](https://en.wikipedia.org/wiki/JSON_Web_Token)_
...
This collection contains movie theater locations. Each document contains a single movie theater and its location in both string and :manual:`GeoJSON </reference/geojson/>` forms.
