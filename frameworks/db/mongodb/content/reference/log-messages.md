...
generated_at: "2026-08-29T09:39:19.678070Z"
---
.. _log-messages-ref:
...
**meta:** :description: MongoDB maintains a log of events such as incoming connections, commands run, and issues encountered. Log messages help diagnose issues and tune performance.
...
encountered. Log messages help diagnose issues, monitor your deployment,
...
**example:** The following is an example log message in JSON format as it would
...
.. list-table::
   ...
   * - ``attr``
     ...
     - | One or more key-value pairs for additional log attributes. If a
       ...
       | ``msg`` message body, depending on the message. When required,
...
The **message** and **attributes** fields escape control characters
according to the Relaxed Extended JSON v2.0 specification:
...
the ``size`` field in the ``truncated`` object.
...
.. list-table::
   ...
   * - ``E``
     - Error
       ...
       For more information on error logging, see :ref:`server-error-codes`.
...
Each component is individually configurable through its own
...
**data:** ASSERT
   ...
   An assertion is triggered when an operation returns an error. The default 
   verbosity level is ``0``. However, the verbosity setting must be at least 
   ``1`` in order for operations that return errors to be included in the
   system logs.  To specify the log level for :data:`ACCESS` components, use the 
   :setting:`assert <systemLog.component.assert.verbosity>` setting. 
...
**data:** REJECTED
   ...
   Messages related to :ref:`rejected query operations
   <operation-rejection-filters>`.

   To specify the log level for ``REJECTED`` component messages, set
   the :setting:`systemLog.component.query.rejected.verbosity`
   parameter.

   MongoDB only logs the ``REJECTED`` component messages if the
   verbosity level is set to at least ``2``.

   The parent component for ``REJECTED`` is :data:`QUERY`.
...
:binary:`~bin.mongosh`) can send identifying information at the
...
when the :ref:`log verbosity level <log-message-verbosity-levels>` is
at least 1. [#slow-oplogs]_ These log entries include the full command
...
**important:** A single operation may log more than one entry. For example, if more 
   than one write in a :ref:`bulk write operation <bulk-write-operations>` exceeds 
...
.. list-table::
   ...
   * - 6.1
     - Slow operation log messages include :ref:`cache refresh time fields 
   ...
   * - 6.3
     - Slow operation log messages and :ref:`database profiler <profiler>` entries 
       include a ``cpuNanos`` field that specifies the total CPU time spent by a 
       query operation in nanoseconds. The ``cpuNanos`` field is only available on 
       Linux systems. 
   ...
   * - 8.0 
     - The slow query output includes a ``queues`` document that contains 
       information about the operation's :ref:`server-status-queues`. Each 
       queue in the ``queues`` field contains a ``totalTimeQueuedMicros``
       field that contains the total cumulative time in microseconds that
       the operation spent in the corresponding queue.
       
       The ``queryShapeHash`` field for a :ref:`query shape <query-shapes>` 
       is also included in the slow query log when available.

       If a command with a specific query shape is :ref:`rejected
       <operation-rejection-filters>`, MongoDB logs a message
       that states the query command was rejected. The message contains
       the query namespace, ``queryShapeHash``, and the command with the
       rejected query. MongoDB only logs the message if the log verbosity
       level is set to at least ``2``.

   * - 8.1
     - .. |disk-spilling-intro| replace:: Slow query log messages

       .. include:: /includes/disk-spilling-metrics-overview.rst

       - ``<executionPart>Spills`` indicates the number of times the
         corresponding query execution stage wrote temporary files to
         disk.

       - ``<executionPart>SpilledBytes`` indicates the size, in bytes,
         of the memory released by writing temporary files to disk. 

       - ``<executionPart>SpilledDataStorageSize`` indicates the size,
         in bytes, of disk space used for temporary files.

       - ``<executionPart>SpilledRecords`` indicates the number of
         records written to temporary files on disk.

       For more information on writing temporary files to disk, see
       :method:`~cursor.allowDiskUse()`.

   * - 8.3
     - Slow query log entries have an optional ``originalQueryShapeHash`` field
       that contains that query shape of the following operations that originate
       ...
       The original query shape of the ``mongos`` operation may differ after
       the ``mongos`` sends the operation to the :program:`mongod` . 

       MongoDB 8.3 also introduces slow in-progress query log entries. Queries
       trigger a slow-in progress log during execution. These logs contain less
       information than standard slow query logs. MongoDB logs queries at most
       once if the query's duration surpasses the
       :setting:`operationProfiling.slowOpInProgressThresholdMs`. 

       When an operation specifies a :ref:`time limit for a query <manual-query-timeout>`,
       slow query logs contain a ``deadline`` field. ``deadline`` indicates the 
       time the operation must complete by, which is equal to the time at the
       start of the operation plus the value of the query timeout. 
...
query, compare the ``workingMillis`` and ``remoteOpWaitMillis`` time
fields in the log. ``workingMillis`` is the total time the query took
to complete. Specifically:

- If ``workingMillis`` is slightly longer than ``remoteOpWaitMillis``,
  then waiting for a shard response took the most time. For example,
  ``workingMillis`` of 17 and ``remoteOpWaitMillis`` of 15.

- If ``workingMillis`` is significantly longer than
  ``remoteOpWaitMillis``, then performing the merge took the most time.
  For example, ``workingMillis`` of 100 and ``remoteOpWaitMillis`` of
  15.
...
You could then filter your log output to show only these
...
- ``workingMillis``
  ...
  The amount of time that MongoDB spends working on
  the operation.
...
.. code-block:: javascript
   :emphasize-lines: 59-61,72,89,92,93
   :copyable: false
...
.. list-table::
   ...
   * - ``catalogCacheDatabaseLookupDurationMillis``
     - Time in milliseconds to retrieve metadata from the catalog cache
   ...
   * - ``databaseVersionRefreshDurationMillis``
     - Time in milliseconds to refresh database metadata for
       operations like :dbcommand:`movePrimary`, ``createDatabase``, and
       :dbcommand:`dropDatabase`.

   * - ``shardVersionRefreshMillis``
     - Time in milliseconds that specifies how often to refresh the
       cache for shard version operations.
...
field to ``placementVersionRefreshDurationMillis``.

``placementVersionRefreshDurationMillis`` is the time for refreshing the
cache for operations like:
