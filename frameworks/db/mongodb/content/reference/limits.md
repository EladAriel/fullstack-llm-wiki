...
generated_at: "2026-08-29T09:39:19.702144Z"
---
...
**meta:** :keywords: case sensitive
   :description: Explore the hard and soft limitations of MongoDB, including maximum document size, collection limits, index restrictions, and namespace constraints across Atlas, Enterprise, and Community deployments.
...
.. composable-tutorial::
   :options: atlas-provider, atlas-class
   :defaults: aws, general

   .. selected-content::
      :selections: aws, general
   ...
   .. selected-content::
      ...
      .. [#f1] The ``M140`` cluster tier is not available for {+azure+} clusters.
...
.. _limits-atlas-collection-and-index:
...
.. list-table::
   ...
   * - ``Free`` clusters per {+atlas+} project
     - 1

   * - Number of :ref:`alert configurations <configure-alerts>` for
       each {+atlas+} project.
     - 500
...
Additional limitations apply to {+atlas+} free clusters, and {+flex-clusters+}. 
...
strategies such as :ref:`sharding <sharding-sharded-cluster>`,
...
migrating to a clustered setup such {+atlas+}, which
...
**limit:** Restriction on Collection Names
   ...
   :binary:`~bin.mongosh` or a :driver:`similar method for your driver <>`.
...
**limit:** Write Command Batch Limit Size
   ...
   perform, it allows a maximum of 100,000 :ref:`writes 
   <query-and-write-commands>` in a single batch operation. A single batch 
