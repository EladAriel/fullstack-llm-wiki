...
source_branch: "main"
source_path: "content/manual/manual/source/reference/glossary.txt"
source_commit: "b9f2bc487a2878b65e3c1f80024bebab76954f27"
source_commit_short: "b9f2bc48"
source_commit_date: "2026-08-28T17:09:45-05:00"
generated_at: "2026-08-29T09:39:19.673121Z"
---
.. _glossary:
...
**meta:** :description: Browse the MongoDB glossary for definitions of key terms, including database operations, deployment types, replication concepts, and storage engine features.
...
**glossary:** :sorted:
   ...
   absolute system CPU utilization
      ...
      to cloud instances that share CPU. In |service| metrics, this value
      appears as :guilabel:`System CPU`.
   ...
   {+avs+}
      Feature that allows you to perform semantic search
   ...
   baseline CPU utilization
      ...
      the full CPU available to shared-cloud instances. This value is not
      directly visible in |service| metrics. To look up the baseline for
      AWS instance types, see the
      :guilabel:`Baseline CPU utilization per vCPU` column in the
      |aws-burstable-instances| documentation. See also
      :term:`relative system CPU utilization` and
      :term:`burstable instances`.
   ...
   chunk
      ...
      data needs to be balanced inside the cluster. While the default
      chunk size is 128 megabytes, it is possible for chunks to grow
      larger than the default size without triggering an automatic
      split. Chunk splitting occurs as part of the balancing process to
      distribute data evenly across the cluster. For more details, see
      :ref:`sharding-data-partitioning`, :ref:`sharding-balancing`, and
   ...
   cluster class     
      *Configurable for M40+ clusters hosted on AWS.*
      ...
      Storage class of your cluster. AWS refers to these cluster classes as
      **instance types**. Your selected class affects cluster
      ...
      Cluster class is a separate concept from :term:`cluster generation`.
   ...
   cluster generation
      *Configurable for M30+ dedicated clusters hosted on AWS.*
      
      The generation of your cluster, such as Gen1 or Gen2. The cluster
      generation you select affects the cluster's avaliable |iops| and storage
      options you can choose. This, in turn, affects your cluster costs.

      Cluster generation is a separate concept from :term:`cluster class`.
   ...
   {+enc-schema+}
      In :ref:`{+qe+} <qe-manual-feature-qe>`, the :ref:`encryptedFields document <qe-encryption-schema>` 
   ...
   free tier
      ...
      .. seealso::
         ...
         - :ref:`Get Started With Deployments <unified-get-started>`
         - :ref:`atlas-free-tier`
   ...
   journal
      ...
      data files. Journal files are pre-allocated and exist as files
      in the :term:`data directory`. See :doc:`/core/journaling/`.
   ...
   JSON pointer
      A string prefixed with a ``/`` character that specifies a 
      particular field value in a :term:`JSON document`.
   ...
   kyber scheduler
      An I/O scheduler designed to improve performance in environments
      with high resource contention. It improves I/O interpolation and
      reduces the latency impact of noisy neighbors.
   ...
   none scheduler
      The ``none`` scheduler option in the multi-queue block layer queueing
      mechanism (``blk-mq``) serves the same purpose as the legacy
      No Operation (``noop``) scheduler. It performs no request scheduling
      and passes I/O operations as received to the block device driver.

   noop
      The No Operation (``noop``) scheduler allocates I/O bandwidth
      for incoming processes based on a first in, first out (FIFO)
      queue. Starting in Linux kernel 5.3, this scheduler is replaced
      by the functionally equivalent ``none`` scheduler.

   normalized string
      The `Unicode Normalized Form <https://unicode.org/reports/tr15/>`__ of a
      string applies Unicode code points in a standardized manner. Two strings
      may appear identical to a user, but have differences such as the order of
      combining marks. Normalizing the strings ensures they have the same
   ...
   project ID
      ...
      <listgroups>` |api| endpoint returns the ID of all 
   ...
   relative system CPU utilization
      ...
      relative to the :term:`baseline CPU utilization` assigned to
      the instance. Calculated by dividing the
      :guilabel:`Normalized System CPU` value by the baseline CPU
      percentage. This value is not directly visible in |service|
      metrics, and the baseline CPU percentage is not exposed in
      the metrics panel. To estimate it, divide your
      :guilabel:`Normalized System CPU` reading by a baseline
      value in the 20%–50% range typical of burstable instance
      types.

      |service| caps this value at 100%. See also
      :term:`absolute system CPU utilization`,
      :term:`baseline CPU utilization`, and
      :term:`burstable instances`.
   ...
   replication lag
      ...
      Lag <manual-troubleshooting-replication-lag>`.
   ...
   client session
      A client session groups related database operations in a
      shared application context. Drivers expose client sessions
      through a ``ClientSession`` object. Client sessions enable
      :ref:`causal consistency <causal-consistency>`,
      :ref:`retryable writes <retryable-writes>`,
      :ref:`multi-document transactions <transactions>`, and
      :ref:`snapshot reads <read-concern-snapshot>`. They use
      :ref:`server sessions <server-sessions>` as their underlying
      framework.
   ...
   wall clock time
      The time that elapses between the start and completion of a computer 
