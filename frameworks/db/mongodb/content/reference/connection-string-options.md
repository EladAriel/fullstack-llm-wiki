...
generated_at: "2026-08-29T09:39:19.689422Z"
---
...
**meta:** :description: Use connection string options to establish connections between MongoDB instances, tools, and applications that use drivers. 
...
This page lists all connection options to connect to your database using
SRV connection strings and standard connection strings.
...
**note:** Semi-colon separator for connection string arguments
   ...
   To provide backwards compatibility, drivers accept
...
.. list-table::
   ...
   * - .. urioption:: replicaSet
     ...
     - Specifies the name of the :term:`replica set`, if the
       ...
       When connecting to a replica set, provide a :term:`seed list`
       of the replica set members in the ``host[:port]`` component.
       See your :driver:`driver </>` documentation for details.
   ...
   * - .. urioption:: directConnection
     ...
     - Specifies whether the client connects directly to the
       ...
       - ``true``: The client sends operations only to the specified host and
         does not attempt to discover other replica set members.
       ...
       :gold:`IMPORTANT:` When a replica set runs in Docker, it might expose only one 
...
.. list-table::
   ...
   * - .. urioption:: tls
     ...
     - Enables or disables TLS/SSL for the connection:
       ...
       - ``true``: Initiates the connection with TLS/SSL. Default for
         :ref:`connections-dns-seedlist`.

       - ``false``: Initiates the connection without TLS/SSL. Default
         for :ref:`connections-standard-connection-string-format`.

       The :urioption:`tls` option is equivalent to
       :urioption:`ssl`.

       If the :binary:`mongosh` shell specifies additional
       :ref:`tls/ssl <mongosh-ssl>` options from the
   ...
   * - .. urioption:: tlsCertificateKeyFilePassword
     ...
     - Specifies the password to decrypt the :urioption:`tlsCertificateKeyFile`.
   ...
   * - .. urioption:: tlsCAFile
     ...
     - Specifies the path to a local :file:`.pem` file that contains 
       the root certificate chain from the Certificate Authority. The 
       client uses this file to validate the certificate presented by
...
.. list-table::
   ...
   * - .. urioption:: timeoutMS
     ...
     - The time in milliseconds to attempt an operation before timing
       out. The timeout applies to all steps in an operation, including
       server selection, connection checkout, and server-side execution.
       
       ``timeoutMS`` has no default value. If you do not specify a
       value, the driver applies individual timeout options, such as
       :urioption:`socketTimeoutMS`.

       If you set ``timeoutMS`` in the connection string, the driver
       ignores other timeout options, and the timeout applies to every
       operation in the client. You can override ``timeoutMS`` at a more
       specific level, but you cannot unset the value. To learn more
       about timeout inheritence, see :ref:`node-csot`.
       
       :gold:`IMPORTANT:` ``timeoutMS`` is not yet supported by all
       official drivers. For availability, see the :driver:`driver </>`
       documentation.
   ...
   * - .. urioption:: socketTimeoutMS
     ...
     - The time in milliseconds to attempt a send or receive on a
       socket before the attempt times out. The default is no
...
.. list-table::
   ...
   * - .. urioption:: compressors
     ...
     - Comma-delimited string of compressors to enable
       ...
       Messages are compressed only when both parties enable network
       compression and share a common compressor. Otherwise, messages
...
.. list-table::
   ...
   * - .. urioption:: maxConnecting
     ...
     - Maximum number of connections a pool can establish
   ...
   * - .. urioption:: waitQueueMultiple
     ...
     - A number that the driver multiplies the :urioption:`maxPoolSize`
       value by, to provide the maximum number of threads allowed to
...
acknowledgment requested from MongoDB. These options are supported by:
...
You can specify write concern both in the connection string and as a
parameter to methods like ``insert`` or ``update``. If specified in
both places, the method parameter overrides the connection string.
...
.. list-table::
   ...
   * - .. urioption:: wtimeoutMS
     ...
     - :gold:`IMPORTANT:` The :urioption:`wtimeoutMS` option is
       deprecated. Set :urioption:`timeoutMS` instead.
       :urioption:`timeoutMS` overrides :urioption:`wtimeoutMS`.
       ...
       When ``wtimeoutMS`` is ``0``, write operations never time
...
.. list-table::
   ...
   * - .. urioption:: readConcernLevel
     ...
     - The level of isolation. Accepts one of the following values:
...
- The following connection string to a sharded cluster specifies
  :readmode:`secondary` read preference mode and three
...
.. list-table::
   ...
   * - .. urioption:: maxStalenessSeconds
     ...
     - Specifies, in seconds, how stale a secondary can be before the client
       ...
       By default, there is no maximum staleness and clients do not consider a
       secondary's lag when choosing where to direct a read operation.

       The minimum :urioption:`maxStalenessSeconds` value is 90
       seconds. Specifying a value between 0 and 90 seconds produces
...
.. list-table::
   ...
   * - .. urioption:: authSource
     ...
     - Database name associated with the user's credentials. Defaults
       to ``defaultauthdb`` if specified, otherwise defaults to
       ``admin``.

       The ``PLAIN``, ``GSSAPI``, and ``MONGODB-AWS`` mechanisms
       require :urioption:`authSource` to be set to ``$external``,
       as they delegate credential storage to external services.

       MongoDB ignores :urioption:`authSource` if no username is
       provided in the connection string or via ``--username``.

   * - .. urioption:: authMechanism

       Default: SCRAM-SHA-256

     - Authentication mechanism for the connection. Without an
       explicit ``authMechanism``, MongoDB tries SCRAM-SHA-256 and
       falls back to SCRAM-SHA-1 if that fails.
       ...
       To use ``MONGODB-AWS``, you must connect to a
       `MongoDB Atlas
       <https://www.mongodb.com/atlas/database>`__ cluster
       configured to support authentication via
       `AWS IAM credentials
       ...
       If the AWS access key ID, secret access key, or session token are defined 
       on your platform by using their respective
       `AWS IAM environment variables
       <https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-envvars.html#envvars-list>`__,
       :binary:`~bin.mongosh` uses these environment variable values to 
       authenticate automatically. You do not need to specify them in the connection string.
       There are other methods to provide AWS credentials depending on your deployment 
       environment and security requirements.

       For example usage of the ``MONGODB-AWS`` authentication
       mechanism, see :ref:`find-connection-string`.
   ...
   * - .. urioption:: authMechanismProperties
     ...
     - Specify properties for the specified :urioption:`authMechanism`
       ...
       ``AWS_SESSION_TOKEN:<security_token>``
         ...
         and a secret access key as well. For example usage, see
...
.. list-table::
   ...
   * - .. urioption:: serverSelectionTryOnce
     ...
     - **Single-threaded drivers only**. When ``true``, the
       driver scans the MongoDB deployment exactly once after server
       selection fails and then either selects a server or raises an
...
.. list-table::
   ...
   * - .. urioption:: uuidRepresentation
     ...
     - Possible values are:
       ...
       ``csharpLegacy``
         The default representation for the .NET/C# driver.
