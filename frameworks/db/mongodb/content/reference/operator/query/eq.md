...
source_branch: "main"
source_path: "content/manual/manual/source/reference/operator/query/eq.txt"
...
The `$eq` operator is equivalent to the `{ field: <value> }` form, except when `<value>` is a regular expression. See below for `examples <eq-usage-examples>`.
...
If `<value>` is an array, MongoDB matches documents where `<field>` matches the array exactly or `<field>` contains an element that matches the array exactly. The order of the elements matters. For an example, see `eq-match-array-value`.
...
For examples, see `eq-regex-matching`.
...
The following example selects all documents in the `inventory` collection where the `item.name` field value equals `"ab"`. To specify a condition on a field in an embedded document, use the `dot notation`.
...
> **Seealso:** `Query Embedded Documents <read-operations-embedded-documents>`

### Array Element Equals a Value

The following example selects all documents in the `inventory` collection where the `tags` array contains an element with the value `"B"` [#match-string-value]_:
...
> **Seealso:** - :query:`$elemMatch`
- `Query Arrays <read-operations-arrays>`
...
$eq Match on a String Strings return the same values whether you use an implicit match or an explicit use of `$eq`. The following queries return the same result:
...
$eq Match on a Regular Expression An explicit query that uses `$eq` and a regular expression only matches an object that is also a regular expression. The example query returns no results because values in the `company` field are strings.

```javascript
   db.companies.find( { company: { $eq: /MongoDB/ } }, {_id: 0 } )
```

Regular Expression Matches A query with an implicit match against a regular expression is equivalent to a query that uses the `$regex` operator. The following queries return the same result:
