...
generated_at: "2026-08-29T09:39:20.187525Z"
---
# $setWindowFields (aggregation stage)

**meta:** :description: Use the `$setWindowFields` aggregation stage to perform operations on a specified span of documents in a collection to analyze data and return computed results without modifying the source documents.
...
**seealso:** :ref:`setWindowFields-examples`
...
If you run :dbcommand:`explain` on a ``$setWindowsFields`` operation, the output
contains a ``peakTrackedMemBytes`` field that contains the maximum number of
bytes of tracked memory in use. For more information, see
...
### Comparison with Previous Values Example
      ...
      The following example uses ``$setWindowFields`` with the :group:`$shift` operator
      and the :pipeline:`$set` stage to compare fields in a collection.
      ...
      - ``partitionBy: "$type"`` :ref:`partitions
        <setWindowFields-partitionBy>` the documents in the collection by
        ``type`` so each type's calculations are kept separate.

      - ``sortBy: { orderDate: 1 }`` :ref:`sorts
        <setWindowFields-sortBy>` the documents in each partition by
        ``orderDate`` in ascending order (``1``).

      - ``output.previousPrice`` uses the :group:`$shift` operator to capture
        the ``price`` from the immediately preceding order within the same year.

      - :pipeline:`$set` adds a new field called ``priceComparison``,
        and sets that field's value to ``higher``, ``lower``, or ``same``
        based on the comparison to ``previousPrice``.

      In this example output, the comparison result is shown in the
      ``priceComparison`` field:
   ...
   .. tab::
      ...
      The following ``WeatherMeasurement`` class represents documents in a
      collection of weather measurements:

      .. literalinclude:: /code-examples/tested/csharp/driver/Aggregation/Builders/SetWindowFields.snippet.weather-measurement-class.cs
         :language: csharp
      ...
      .. sharedinclude:: dbx/csharp/aggregation/rst-files/method-intro.rst
         ...
         .. replacement:: method-description
            ...
            uses the ``Rainfall`` and ``Temperature`` fields to compute
            the accumulated rainfall, moving average temperature, median
            temperature, and 90th percentile rainfall over the past
      ...
      .. io-code-block::
         ...
         .. input:: /code-examples/tested/csharp/driver/Aggregation/Builders/SetWindowFields.snippet.set-window-fields.cs
            :language: csharp
            :category: usage example

         .. output:: /code-examples/tested/csharp/driver/Aggregation/Builders/OutputFiles/SetWindowFieldsOutput.txt
            :language: json
            :visible: false
   ...
   .. tab::
      ...
      .. include:: /includes/driver-examples/node/aggregation/stage-intro.rst
         ...
         .. replacement:: method-description
            ...
            calculates the average ``airTemperature.value`` and the total
            ``waveMeasurement.waves.height`` for each unique value of
            ``callLetters`` over the past month
