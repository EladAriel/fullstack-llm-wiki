...
source_branch: "main"
source_path: "content/manual/manual/source/reference/operator/aggregation/locf.txt"
...
If a field being filled contains both `null` and non-null values, :group:`$locf` sets the `null` and missing values to the field's last known non-null value according to the sort order specified in :pipeline:`$setWindowFields`.

`null` and missing field values that appear before non-null values in the sort order remain `null`.

If a field being filled contains only `null` or missing values in a `partition <setWindowFields-partitionBy>`, :group:`$locf` sets the field value to `null` for that partition.
...
When you use the :pipeline:`$fill` stage, the field you specify in the output is the same field used as the source data. See `fill-example-locf`.

- The :group:`$locf` operator inside of a
:pipeline:`$setWindowFields` stage.

When you use the :group:`$locf` operator, you can set values for a different field than the field used as the source data. See `locf-example-multiple-methods`.
...
The following example uses the :group:`$locf` operator to set missing fields to the value from the last-observed non-`null` value:
...
<setWindowFields-sortBy>` the documents in each partition by `time` in ascending order (`1`), so the earliest `time` is first.

- For documents where the `price` field is missing, the :group:`$locf`
operator sets the `price` to the last-observed value in the sequence.
