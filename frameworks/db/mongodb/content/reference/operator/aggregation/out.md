...
generated_at: "2026-08-29T09:39:20.121117Z"
---
.. _agg-out:

# $out (aggregation stage)

**meta:** :description: Use the `$out` stage in MongoDB's aggregation pipeline to write results to a collection, supporting output to different databases and time series collections.
...
If the output collection already exists, the ``$out`` stage
atomically replaces it upon completion of the aggregation.
...
If the ``$out`` operation modifies a collection with a
:atlas:`{+fts+} </atlas-search>` index, you must delete and
...
### Output to Same Database
      ...
      The following aggregation operation filters movies with a
      runtime greater than 1000 minutes, groups them by release
      year, and collects their title into an array. The pipeline
      writes the results to the ``movies_by_year`` collection in
      the ``sample_mflix`` database.

      .. literalinclude:: /code-examples/tested/command-line/mongosh/aggregation/stages/out/out-same-db.snippet.out-same-db.js
         :language: javascript
         :category: usage example

      First Stage (``$match``):
         The :pipeline:`$match` stage filters for movies with a
         ``runtime`` greater than ``1000``.

      Second Stage (``$group``):
         The :pipeline:`$group` stage groups by the ``year`` field
         and uses :group:`$push` to add the titles to a ``movies``
         array field.

      Third Stage (``$out``):
         The ``$out`` stage outputs the documents to the
         ``movies_by_year`` collection in the ``sample_mflix``
         database.

      To view the documents in the output collection, run the
      following operation:

      .. io-code-block::
         :copyable: true

         .. input:: /code-examples/tested/command-line/mongosh/aggregation/stages/out/out-same-db-find.snippet.out-same-db-find.js
            :language: javascript
            :category: usage example

         .. output:: /code-examples/tested/command-line/mongosh/aggregation/stages/out/out-same-db-find-output.sh
            :language: javascript
...
### Output to a Different Database
      ...
      ``$out`` can output to a collection in a database different from
      where you run the aggregation.
      ...
      .. literalinclude:: /code-examples/tested/command-line/mongosh/aggregation/stages/out/out-different-db.snippet.out-different-db.js
         :language: javascript
         :category: usage example
      ...
      Third Stage (``$out``):
         The ``$out`` stage outputs the documents to the
         ``movies_by_year`` collection in the ``reporting`` database.
      ...
      .. io-code-block::
         ...
         .. input:: /code-examples/tested/command-line/mongosh/aggregation/stages/out/out-different-db-find.snippet.out-different-db-find.js
            :language: javascript
            :category: usage example

         .. output:: /code-examples/tested/command-line/mongosh/aggregation/stages/out/out-different-db-find-output.sh
            :language: javascript
   ...
   .. tab::
      ...
      .. sharedinclude:: dbx/csharp/aggregation/rst-files/method-intro.rst
         ...
         .. replacement:: method-description
            ...
            finds movies where the value of the ``metacritic`` field is
            ``100``, sorts
            them by title, and writes the results to the ``top_movies``
            collection:
      ...
      .. io-code-block::
         ...
         .. input:: /code-examples/tested/csharp/driver/Aggregation/Builders/Out.snippet.out.cs
            :language: csharp
            :category: usage example

         .. output:: /code-examples/tested/csharp/driver/Aggregation/Builders/OutputFiles/OutOutput.txt
            :language: json
            :visible: false
