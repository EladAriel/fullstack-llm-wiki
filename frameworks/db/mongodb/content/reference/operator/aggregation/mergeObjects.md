...
generated_at: "2026-08-29T09:39:20.127602Z"
---
# $mergeObjects (accumulator operator)

**meta:** :description: Combine multiple documents into a single document using the `$mergeObjects` operator in various aggregation stages like `$group` and `$replaceRoot`.
...
When used as a :pipeline:`$bucket`, :pipeline:`$bucketAuto`, or
:pipeline:`$group` stage accumulator, :expression:`$mergeObjects` has
this syntax:
