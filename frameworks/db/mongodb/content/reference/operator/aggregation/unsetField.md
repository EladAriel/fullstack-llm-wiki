...
source_branch: "main"
source_path: "content/manual/manual/source/reference/operator/aggregation/unsetField.txt"
...
# $unsetField  (expression operator)
...
:expression:`$unsetField` returns `null` and does not update `input`.

- If `input` evaluates to anything other than an object, `missing`,
`undefined`, or `null`, :expression:`$unsetField` returns an error.
...
arrays. For example, :expression:`$unsetField` evaluates a `field` value of `"a.b.c"` as a top-level field `"a.b.c"` instead of as a nested field, `{ "a": { "b": { "c": } } }`.
...
Use the :pipeline:`$replaceWith` pipeline stage and the :expression:`$unsetField` operator to remove the `"price.usd"` field from each document:
...
Use the :pipeline:`$replaceWith` pipeline stage with the :expression:`$unsetField` and :expression:`$literal` operators to remove the `"$price"` field from each document:
...
Use the :pipeline:`$replaceWith` pipeline stage with :expression:`$setField` and a nested :expression:`$unsetField` operation to remove the `"euro"` field:
