...
source_branch: "main"
source_path: "content/manual/manual/source/reference/operator/aggregation/firstN.txt"
...
constant or depends on the `_id` value for :pipeline:`$group`. For details see `group key example<first-n-with-group-key>`.
...
`$firstN` is supported as an `aggregation expression <aggregation-expressions>`.

`$firstN` is supported as a :pipeline:`window operator <$setWindowFields>`.
...
The following example uses the :expression:`$firstN` operator to retrieve the first three scores for each player. The scores are returned in the new field `firstScores` created by :pipeline:`$addFields`.
