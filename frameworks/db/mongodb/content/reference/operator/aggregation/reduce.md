...
#. Use `$group` to group by the `experimentId` and use `$push` to create an array with the probability of each event.

#. Use `$reduce` with `$multiply` to multiply and combine the elements of `probabilityArr` into a single value and project it.
...
The following example returns the same documents as the example in the previous section `reduce-index-example`, but uses `$$IDX` instead of `arrayIndexAs`:
