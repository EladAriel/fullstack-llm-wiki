...
generated_at: "2026-08-29T09:39:20.237209Z"
---
# $geoNear (aggregation stage)

**meta:** :description: Use `$geoNear` in MongoDB to sort documents by proximity to a point, with options for distance calculations and geospatial index specification.
...
**pipeline:** $geoNear
   ...
   The ``$geoNear`` operator accepts a :term:`document` that
...
``$geoNear`` calculates distance based on the nearest point of
the input document's perimeter.

For example, if the input document is a shape, ``$geoNear``
...
- The ``near`` parameter supports the :ref:`let option 
  <geoNear_let_example>` and :ref:`bound let option 
  <geoNear_bounded_let_example>`.

- You can use the ``$geoNear`` pipeline operator on any field 
  in a :ref:`time series collection <manual-timeseries-collection>`.
  However, you cannot use the ``query`` field for ``$geoNear`` on a
  time series collection.

- You can create :ref:`partial <index-type-partial>` and :ref:`2dsphere 
  <2dsphere-index>` indexes on any field in a :ref:`time series 
...
### Specify Which Geospatial Index to Use
   ...
   .. tab::
      ...
      The following ``Theater``, ``TheaterLocation``, and
      ``TheaterAddress`` classes model the documents in the
      ``sample_mflix.theaters`` collection:

      .. literalinclude:: /code-examples/tested/csharp/driver/Aggregation/Builders/GeoNear.snippet.theater-class.cs
         :language: csharp
...
### Maximum Distance
      ...
      .. io-code-block::
         ...
         .. input:: /code-examples/tested/csharp/driver/Aggregation/Builders/GeoNear.snippet.geo-near.cs
            :language: csharp
            :category: usage example

         .. output:: /code-examples/tested/csharp/driver/Aggregation/Builders/OutputFiles/GeoNearOutput.txt
            :visible: false
            :language: json
...
### Minimum Distance
      ...
      .. io-code-block::
         ...
         .. input:: /code-examples/tested/csharp/driver/Aggregation/Builders/GeoNear.snippet.geo-near-min.cs
            :language: csharp
            :category: usage example

         .. output:: /code-examples/tested/csharp/driver/Aggregation/Builders/OutputFiles/GeoNearMinOutput.txt
            :visible: false
            :language: json
...
### Maximum Distance
         ...
         .. replacement:: method-description
            ...
            returns documents in an ``8000`` meter radius of the specified
            point, in order of ascending distance. The code includes a ``query``
            field that only matches documents where the value of the
            ``location.address.state`` field is ``"NJ"``. The code also stores
            the calculated distance in the ``distance`` field of the output
            documents
...
### Minimum Distance
      ...
      The following example returns the first ``4`` matching documents outside
      of an ``8000`` meter radius of the specified point, in order of ascending
      distance. The code includes a ``query`` field that only matches
      documents where the value of the ``location.address.state`` field is
      ``"NJ"``. The code also stores the calculated distance in the ``distance``
      field of the output documents:
