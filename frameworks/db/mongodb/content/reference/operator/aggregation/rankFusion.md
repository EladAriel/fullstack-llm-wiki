...
generated_at: "2026-08-29T09:39:20.232823Z"
---
...
**meta:** :description: Learn how to use an aggregation stage to combine the results of multiple ranked aggregation pipelines.
...
**important:** ``$rankFusion`` is only available for deployments that use MongoDB
   ...
   You must open a :ref:`support case <request-support>` to use
   ``$rankFusion`` on 8.0.X versions. When you upgrade from 8.0, you
   might have to pause executing ``$rankFusion`` queries.
...
.. list-table::
   ...
   * - ``rank``
     ...
     - The rank of this document in the input pipeline. Rank is ``N/A``
       in a pipeline stage output if a document that is returned in
       other pipeline stage output is not present in this pipeline
       stage's output.
   ...
   .. tab::
      ...
      .. include:: /includes/driver-examples/csharp/aggregation/method-intro.rst
         ...
         .. replacement:: method-name-and-link 
            ...
            `RankFusion() <{+csharp-api-docs+}/MongoDB.Driver/MongoDB.Driver.PipelineStageDefinitionBuilder.RankFusion.html>`__
         ...
         .. replacement:: stage-specific-info
            ...
            .. literalinclude:: /includes/driver-examples/csharp/aggregation/BuildersExamples.cs
               :start-after: // start rankFusionIndex
               :end-before: // end rankFusionIndex
               :language: csharp
               :dedent:

         .. replacement:: method-description

            executes two pipelines, ``searchPlot`` and ``searchGenre``,
            that perform :pipeline:`$search` operations by using the ``default`` search index. The
