---
type: "Framework Learn Page"
framework: "MongoDB"
source_repo: "https://github.com/mongodb/docs.git"
source_branch: "main"
source_path: "content/manual/manual/source/reference/operator/aggregation/listSearchIndexes.txt"
source_commit: "b9f2bc487a2878b65e3c1f80024bebab76954f27"
source_commit_short: "b9f2bc48"
source_commit_date: "2026-08-28T17:09:45-05:00"
generated_at: "2026-08-29T09:39:20.141362Z"
---
# $listSearchIndexes (aggregation stage)

**meta:** :description: Retrieve information about existing {+fts+} indexes on a specified collection using the `$listSearchIndexes` aggregation command.

.. default-domain:: mongodb

**contents:** On this page
   :local:
   :backlinks: none
   :depth: 2
   :class: singlecol

## Definition

**pipeline:** $listSearchIndexes

**versionadded:** 7.0 (*Also available starting in 6.0.7*)

.. |fts-indexes| replace:: :atlas:`{+fts+} indexes </atlas-search/atlas-search-overview/#fts-indexes>`

**include:** /includes/atlas-search-commands/command-descriptions/getSearchIndexes-description.rst

## Syntax

Command syntax:

.. code-block:: javascript

   db.<collection>.aggregate(
      [
         {
            $listSearchIndexes:
               {
                  id: <indexId>,
                  name: <indexName>
               }
         }
      ]
   )

## Command Fields

``$listSearchIndexes`` takes either of the following fields:

.. list-table::
  :header-rows: 1
  :widths: 20 20 20 80

  * - Field
    - Type
    - Necessity
    - Description

  * - ``id``
    - string
    - Optional
    - The id of the index to return information about.

  * - ``name``
    - string
    - Optional
    - The name of the index to return information about.
    

You cannot specify both ``id`` and ``name``. If you omit both the ``id``
and ``name`` fields, ``$listSearchIndexes`` returns information about
all {+fts+} indexes on the collection.

## Access Control

.. |method-name| replace:: ``$listSearchIndexes``

**include:** /includes/atlas-search-commands/access-control/list-access-control.rst

## Output

.. |status-link| replace:: :ref:`search-index-status-aggregation`

.. |index-details-link| replace:: :ref:`search-index-details-aggregation`

.. |synonym-detail-link| replace:: :ref:`search-mapping-details-aggregation`

**include:** /includes/atlas-search-commands/command-output/listSearchIndex-output.rst

.. _search-index-details-aggregation:

### Index Status Details

**include:** /includes/atlas-search-commands/command-output/search-index-details.rst

.. _search-mapping-details-aggregation:

### Synonym Mapping Details

**include:** /includes/atlas-search-commands/command-output/search-index-synonym-details.rst

.. _search-index-status-aggregation:

### {+fts+} Index Statuses

**include:** /includes/atlas-search-commands/command-output/search-index-statuses.rst

### Errors

**versionchanged:** 7.1

   This command throws an error when not executed on Atlas.

   .. io-code-block::

      .. input::
         :language: javascript

         db.names.aggregate( [
            { $listSearchIndexes: { } }
         ] )

      .. output::

         MongoServerError: PlanExecutor error during aggregation :: caused by :: Search index commands are only supported with Atlas.

   In previous releases, this command returns an empty result when not executed 
   on Atlas.


## Examples

.. tabs-drivers::

   .. tab::
      :tabid: shell

      These examples demonstrate how to perform the following actions:

      - :ref:`listSearchIndexes-examples-return-all`

      - :ref:`listSearchIndexes-examples-return-by-name`

      - :ref:`listSearchIndexes-examples-return-by-id`

      .. _listSearchIndexes-examples-return-all:

### Return All Search Indexes

      The following example returns all {+fts+} indexes on the ``movies``
      collection:

      .. code-block:: javascript

         db.movies.aggregate(
            [
               {
                  $listSearchIndexes: { }
               }
            ]
         )

      Sample output:

      .. include:: /includes/atlas-search-commands/command-output/examples/multi-doc-example-output.rst

      .. _listSearchIndexes-examples-return-by-name:

### Return a Single Search Index by Name

      The following example returns the index named ``synonym-mappings`` on
      the ``movies`` collection:

      .. code-block:: javascript

         db.movies.aggregate(
            [
               {
                  $listSearchIndexes:
                     {
                        name: "synonym-mappings"
                     }
               }
            ]
         )

      Sample output:

      .. include:: /includes/atlas-search-commands/command-output/examples/single-doc-synonyms-example-output.rst

      .. _listSearchIndexes-examples-return-by-id:

### Return a Single Search Index by id

      The following example returns the search index with the provided ``id`` value:

      .. code-block:: javascript

         db.movies.aggregate(
            [
               {
                  $listSearchIndexes:
                     {
                        id: "6524096020da840844a4c4a7"
                     }
               }
            ]
         )

      Sample output:

      .. include:: /includes/atlas-search-commands/command-output/examples/single-doc-default-example-output.rst

   .. tab::
      :tabid: nodejs

      To use the {+node-driver-full+} to add a ``$listSearchIndexes`` stage to
      an aggregation pipeline, use the ``$listSearchIndexes`` operator in a
      pipeline object.
      
      .. note::

         The Node.js driver provides a helper method, `listSearchIndexes(),
         <{+node-api-docs+}/classes/Collection.html#listSearchIndexes>`__ that
         performs the same operation as the ``$listSearchIndexes`` pipeline
         stage. To learn more, see the :driver:`Indexes
         </node/current/indexes/#list-search-indexes/>` page in the
         {+node-driver-full+} documentation.

### Return All Search Indexes

      The following example returns all {+fts+} indexes on a collection:

      .. literalinclude:: /includes/driver-examples/node/aggregation/examples.js
         :start-after: //start listSearchIndexes
         :end-before: //end listSearchIndexes
         :language: javascript
         :dedent: 2

### Return a Single Search Index by Name

      The following example returns the index named ``synonym-mappings``:

      .. literalinclude:: /includes/driver-examples/node/aggregation/examples.js
         :start-after: //start listSearchIndexesName
         :end-before: //end listSearchIndexesName
         :language: javascript
         :dedent: 2

### Return a Single Search Index by id

      The following example returns the search index with the provided ``id`` value:

      .. literalinclude:: /includes/driver-examples/node/aggregation/examples.js
         :start-after: //start listSearchIndexesID
         :end-before: //end listSearchIndexesID
         :language: javascript
         :dedent: 2

## Learn More

To use a ``mongosh`` method to view {+fts+} indexes, see
:method:`db.collection.getSearchIndexes()`.

To create {+fts+} indexes, see:

- The :method:`db.collection.createSearchIndex()` ``mongosh`` method

- The :dbcommand:`createSearchIndexes` database command