...
source_branch: "main"
source_path: "content/manual/manual/source/reference/operator/aggregation/bsonSize.txt"
...
If the argument resolves to a data type other than an object or `null`, :expression:`$bsonSize` errors.

## Examples

### Return Sizes of Documents

In :binary:`~bin.mongosh`, create a sample collection named `employees` with the following documents:
...
to return the size of the document in bytes. The :variable:`$$ROOT <ROOT>` variable references the document currently being processed by the pipeline. To learn more about variables in the aggregation pipeline, see `/reference/aggregation-variables`.
...
When you specify an :pipeline:`$group` _id value of `null`, or any other constant value, the :pipeline:`$group` stage calculates accumulated values for all the input documents as a whole.

The operation uses the :group:`$sum` operator to calculate the combined :expression:`$bsonSize` of each document in the collection. The :variable:`$$ROOT <ROOT>` variable references the document currently being processed by the pipeline. To learn more about variables in the aggregation pipeline, see `/reference/aggregation-variables`.
...
to return the size of the document's `current_task` field in bytes.
...
Second Stage The second stage :pipeline:`sorts <$sort>` the documents by `task_object_size` in descending order.
