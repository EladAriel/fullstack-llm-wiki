...
generated_at: "2026-08-29T09:39:20.199001Z"
---
# $replaceOne  (expression operator)

**meta:** :description: Replace the first occurrence of a search string in an input string using the `$replaceOne` aggregation operator. Case-sensitive and diacritic-sensitive.
...
**expression:** $replaceOne
   ...
   Replaces the first instance of a search string or regex pattern in
   an input string with a replacement string.
...
.. list-table::
   ...
   * - :ref:`find <replaceOne-find>`
     ...
     - .. _replaceOne-find:
       ...
       :ref:`expression<aggregation-expressions>` that resolves to a
       string, a regex, or a ``null``. If ``find`` refers to a missing field, 
       ``$replaceOne`` returns ``null``.
...
- A string 
- ``null``
- A regex for :ref:`find <replaceOne-find>` operations
...
The following example replaces the first instance of "blue" as a whole
word in the ``item`` field with "navy":
