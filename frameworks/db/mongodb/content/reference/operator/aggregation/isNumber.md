...
source_branch: "main"
source_path: "content/manual/manual/source/reference/operator/aggregation/isNumber.txt"
...
# $isNumber (expression operator)

## Definition

> **Seealso:** - :expression:`$type (Aggregation) <$type>` - returns the BSON type of the argument.
...
The following aggregation uses the :pipeline:`$addFields` aggregation stage to add the following fields to each document:
...
The following aggregation uses the :pipeline:`$addFields` stage to add a `points` field containing the numeric grade value for that course. The stage uses the :expression:`$cond` operator to set the value of `points` based on the output of `$isNumber`:
...
:expression:`$switch` to convert the letter grade to its equivalent point value and assign to `points`.

The aggregation pipeline then uses the :pipeline:`$group` stage to group on the `student_id` and calculate the student's :group:`average <$avg>` GPA.
