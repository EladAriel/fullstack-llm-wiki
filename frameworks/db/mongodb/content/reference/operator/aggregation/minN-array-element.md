...
source_branch: "main"
source_path: "content/manual/manual/source/reference/operator/aggregation/minN-array-element.txt"
...
in the `input` array, :expression:`$minN` returns all elements in the `input` array.
...
are sorted before string elements according to the `BSON comparison order <bson-types-comparison-order>`.
...
The following example uses the :expression:`$minN` operator to retrieve the two lowest scores for each player. The lowest scores are returned in the new field `minScores` created by :pipeline:`$addFields`.
