...
generated_at: "2026-08-29T09:39:20.158456Z"
---
# $meta (expression operator)

**meta:** :description: Explore the `$meta` operator in MongoDB for accessing document metadata like `textScore` and `indexKey` during aggregation and find operations.
...
**expression:** $meta
   ...
   .. important::
      ...
      You can use ``searchScore`` only in :pipeline:`$search` queries
      and ``vectorSearchScore`` only in :pipeline:`$vectorSearch`
      queries. If you use ``searchScore`` and ``vectorSearchScore`` 
      in any other query, that query will fail. 

      You can use ``searchRootDocumentId`` only in :pipeline:`$search`
      queries with ``returnScope``. 
