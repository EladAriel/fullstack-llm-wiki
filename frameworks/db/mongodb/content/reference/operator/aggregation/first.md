...
source_branch: "main"
source_path: "content/manual/manual/source/reference/operator/aggregation/first.txt"
...
Returns the result of an `expression <aggregation-expressions>` for the first document in a group of documents. Only meaningful when documents are in a defined order.
...
`$setWindowFields`, set the `sortBy <setWindowFields-sortBy>` field.
...
from the first document. It does not traverse array elements. This is the case with stages like :pipeline:`$group` and :pipeline:`$setWindowFields`.
...
`null` for empty windows. For example, when you have a `{ documents:[ -1, -1] }` `documents` window on the first document of a partition.

For more information, see the `Missing Data <first-missing-values-example>` example later in this topic.
...
This example uses `$first` in the :pipeline:`$setWindowFields` stage to output the first cake sales order `type` for each `state`:
