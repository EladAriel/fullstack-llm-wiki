...
generated_at: "2026-08-29T09:39:20.163444Z"
---
# $densify (aggregation stage)

**meta:** :description: Use the `$densify` aggregation stage to fill gaps in data sequences by creating new documents for missing values, supporting time series and grouped data.
...
- .. versionadded:: 8.1
     ...
     ``field`` shares its prefix with any field in the
     ``partitionByFields`` array. For example, the following combinations
     of ``field`` and ``partitionByFields`` result in an error:
...
**note:** Starting in MongoDB 8.0, :pipeline:`$densify` treats bounds with an equal
   ...
   For example, a :ref:`range.bounds <densify-bounds>` of ``[10, 10]`` generates an extra document
   with field value ``10`` in versions prior to 8.0, but does not generate such a
   document in 8.0 and later.

.. _densify-document-limit:

### Document Generation Limit

:pipeline:`$densify` returns an error if it generates more documents
than the limit set by the ``internalQueryMaxAllowedDensifyDocs``
parameter. By default, this limit is 500,000 documents.

To allow more generated documents, increase the value of the
``internalQueryMaxAllowedDensifyDocs`` parameter.
...
### Densify Values within Each Partition
   ...
   .. tab::
      ...
      .. sharedinclude:: dbx/csharp/aggregation/rst-files/sample-data-weather-intro.rst

      The following ``Weather`` and ``Point`` classes model the documents in
      the ``sample_weatherdata.data`` collection:

      .. literalinclude:: /code-examples/tested/csharp/driver/Aggregation/Builders/Densify.snippet.weather-class.cs
         :language: csharp
      ...
      .. sharedinclude:: dbx/csharp/aggregation/rst-files/method-intro.rst
         ...
         .. replacement:: method-description
            ...
            filters to documents at coordinates ``[-47.9, 47.6]``, and then adds
            a document at every 15-minute interval between the two source
            documents. The code partitions the documents by the values of
            their ``Position.Coordinates`` field.

         .. replacement:: more-method-description

      .. literalinclude:: /code-examples/tested/csharp/driver/Aggregation/Builders/Densify.snippet.densify.cs
         :language: csharp
         :copyable: true
   ...
   .. tab::
      ...
      .. include:: /includes/driver-examples/node/aggregation/stage-intro.rst
         ...
         .. replacement:: method-description
            ...
            adds a document at every 15-minute interval between the previous two
            documents. The code then groups these documents by the values of
            their ``position.coordinates`` field
      ...
      The previous aggregation stage generates the following highlighted
