...
generated_at: "2026-08-29T09:39:20.181689Z"
---
# $graphLookup (aggregation stage)

**meta:** :description: Perform recursive searches in MongoDB using `$graphLookup` to explore connections within collections, with configurable options for depth and query filters.
   ...
   .. list-table::
      ...
      * - ``from``
        ...
        - Target collection for the :pipeline:`$graphLookup`
          ...
          For sharding support, see
          :ref:`graphLookup-sharded-collections`.
...
See :ref:`aggregation pipeline limits <agg-pipeline-limits>`
...
### With a Query Filter
   ...
   .. tab::
      ...
      .. literalinclude:: /code-examples/tested/csharp/driver/Aggregation/Builders/GraphLookup.snippet.employee-class.cs
         :language: csharp
      ...
      .. sharedinclude:: dbx/csharp/aggregation/rst-files/method-intro.rst
         ...
         .. replacement:: method-description
            ...
            recursively matches on the ``ReportsTo`` and ``Name`` fields
            in the ``employees`` collection, returning the reporting
            hierarchy for each person:
      ...
      .. io-code-block::
         ...
         .. input:: /code-examples/tested/csharp/driver/Aggregation/Builders/GraphLookup.snippet.graph-lookup-basic.cs
            :language: csharp
            :category: usage example

         .. output:: /code-examples/tested/csharp/driver/Aggregation/Builders/OutputFiles/GraphLookupBasicOutput.txt
            :visible: false
            :language: json

      You can use an `AggregateGraphLookupOptions
      <{+csharp-api-docs+}/MongoDB.Driver/MongoDB.Driver.AggregateGraphLookupOptions-3.html>`__
      object to specify the depth to recurse and the name of the depth
      field. The following code example performs the same
      ``$graphLookup`` operation as the previous example, but specifies
      a maximum recursion depth of ``1``:

      .. io-code-block::
         :copyable: true

         .. input:: /code-examples/tested/csharp/driver/Aggregation/Builders/GraphLookup.snippet.graph-lookup-depth.cs
            :language: csharp
            :category: usage example

         .. output:: /code-examples/tested/csharp/driver/Aggregation/Builders/OutputFiles/GraphLookupDepthOutput.txt
            :visible: false
            :language: json

      You can also use an ``AggregateGraphLookupOptions`` object to
      specify a filter that documents must match in order for MongoDB to
      include them in your search. The following code example performs
      the same ``$graphLookup`` operation as the previous examples, but
      includes only ``Employee`` documents where the ``Hobbies`` field
      contains ``"golf"``:

      .. io-code-block::
         :copyable: true

         .. input:: /code-examples/tested/csharp/driver/Aggregation/Builders/GraphLookup.snippet.graph-lookup-match.cs
            :language: csharp
            :category: usage example

         .. output:: /code-examples/tested/csharp/driver/Aggregation/Builders/OutputFiles/GraphLookupMatchOutput.txt
            :visible: false
            :language: json
   ...
   .. tab::
      ...
      .. include:: /includes/driver-examples/node/aggregation/stage-intro.rst
         ...
         .. replacement:: method-description
            ...
            recursively matches the ``reportsTo`` fields to the ``name`` fields
            in the ``employees`` collection, returning the reporting hierarchy
            for each person in a new field named ``reportingHierarchy``

         .. replacement:: more-method-description

      .. literalinclude:: /includes/driver-examples/node/aggregation/examples.js
         :start-after: //start graphLookup
         :end-before: //end graphLookup
         :language: javascript
         :dedent: 2

      To specify the depth of the recursion, use the ``maxDepth``
      field. The following code example performs the same ``$graphLookup``
      operation as the previous example, but specifies a maximum recursion depth
      of ``1``:

      .. literalinclude:: /includes/driver-examples/node/aggregation/examples.js
         :start-after: //start graphLookup maxDepth
         :end-before: //end graphLookup maxDepth
         :language: javascript
         :dedent: 2

      To specify a filter that documents must match for the operation
      to include them in the search results, use the ``restrictSearchWithMatch``
      field. The following code example performs the same ``$graphLookup``
      operation as the previous examples, but includes only ``employee`` documents
      where the ``hobbies`` field contains ``"golf"``:

      .. literalinclude:: /includes/driver-examples/node/aggregation/examples.js
         :start-after: //start graphLookup restricted
         :end-before: //end graphLookup restricted
         :language: javascript
         :dedent: 2

## Learn More

To learn more about how to use :pipeline:`$graphLookup`, see `Working
with Graph Data in MongoDB.
<https://www.mongodb.com/resources/basics/databases/mongodb-graph-database/>`__