...
generated_at: "2026-08-29T09:39:20.124394Z"
---
...
**meta:** :description: Learn how to use an aggregation stage to combine the results of multiple scored aggregation pipelines.
...
.. list-table::
   ...
   * - Search Stages
     - - :pipeline:`$match`, including ``$match`` with :ref:`legacy text search
       ...
       ``includeLocs`` or ``distanceField`` because those fields modify
       documents. 
...
- Contain an explicit :pipeline:`$score` stage if the preceding pipeline
  doesn't inherently return a score. For example, :pipeline:`$geoNear`
  returns a distance rather than a score, so it requires an explicit
  ``$score`` stage.
...
**example:** The following code blocks show the ``scoreDetails`` field for a
