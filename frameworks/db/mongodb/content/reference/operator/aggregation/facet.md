...
generated_at: "2026-08-29T09:39:20.133479Z"
---
.. _agg-facet:
...
**meta:** :description: Learn how to use the $facet aggregation stage to process multiple aggregation pipelines in a single stage.
...
**pipeline:** $facet
   ...
   enabling various aggregations on the same set of input documents.
...
If a ``$facet`` stage constructs a document where the size exceeds the
100 megabyte limit, MongoDB returns an :error:`ExceededMemoryLimit
<146>` error.
...
**seealso:** :ref:`agg-pipeline-limits`
...
Other :ref:`aggregation stages
<aggregation-pipeline-operator-reference>` can also be used with
``$facet`` with the following exceptions:
   ...
   .. tab::
      ...
      .. sharedinclude:: dbx/csharp/aggregation/rst-files/method-intro.rst
         ...
         .. replacement:: method-description
            ...
            executes two parallel aggregations. The first aggregation
            distributes incoming documents into five groups by the
            value of their ``Runtime`` field. The second aggregation
            counts each value in the ``Rated`` field and returns the
      ...
      .. io-code-block::
         ...
         .. input:: /code-examples/tested/csharp/driver/Aggregation/Builders/Facet.snippet.facet.cs
            :language: csharp
            :category: usage example

         .. output:: /code-examples/tested/csharp/driver/Aggregation/Builders/OutputFiles/FacetOutput.txt
            :language: json
            :visible: false
