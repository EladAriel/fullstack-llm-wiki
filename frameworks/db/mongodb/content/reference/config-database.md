...
generated_at: "2026-08-29T09:39:19.696732Z"
---
.. _config-database:

# Config Database

**meta:** :description: Explore the collections in the `config` database that support sharded cluster operations and sessions, and understand their roles and restrictions.
...
Starting in MongoDB 8.0, you must connect to ``mongos`` to read some
``config`` collections. For the list of these collections, see
:ref:`node-direct-single-shard-reads`.
...
support sharding operations, connect :binary:`~bin.mongosh` to a
...
**data:** config.collections
   ...
   Starting in MongoDB 8.0, ``config.collections`` stores metadata for
   all sharded collections and unsharded collections moved with
   :dbcommand:`moveCollection`.

   In MongoDB versions earlier than 8.0, ``config.collections`` only
   stores metadata for sharded collections.

   ``config.collections`` only stores metadata about collections, not
   the documents stored in the collections.

   For a collection named ``pets``
...
**data:** config.databases
   ...
   database's :ref:`primary shard <primary-shard>` , and a version.
...
**data:** config.placementHistory

   .. include:: /includes/admonition-config-db-is-internal.rst

   The :data:`~config.placementHistory` collection exists on the config
   server and records which shards contained data for a database or
   collection at specific cluster timestamps. MongoDB uses this
   information to reconstruct historical data placement.

   For a collection named ``myCollection`` in the ``test`` database, a
   document in the :data:`~config.placementHistory` collection would
   resemble the following:

   .. code-block:: javascript

      {
        _id: ObjectId("61e0a1f4e5b8c9d0a1b2c3d4"),
        nss: "test.myCollection",
        collUuid: UUID("07c63242-51b3-460c-865f-a67b3372d792"),
        timestamp: Timestamp(1626895826, 1),
        shards: [ "shardA", "shardB" ]
      }

   The ``nss`` field can contain either a namespace or a database name.
   The ``collUuid`` field is present only for collections.
...
**data:** config.settings
   ...
   - Range size. To change range size,
     see :ref:`Modify Range Size in a Sharded Cluster 
     <tutorial-modifying-range-size>`. The specified ``chunksize`` value 
     is in megabytes.
...
:binary:`~bin.mongos` instance, connect :binary:`~bin.mongosh` to the
instance.
