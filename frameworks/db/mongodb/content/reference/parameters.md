...
generated_at: "2026-08-29T09:39:19.687411Z"
---
.. _server-parameters:
...
**meta:** :keywords: on-prem
   :description: Explore various MongoDB server parameters for self-managed deployments, including configuration options for authentication, logging, replication, and storage.
...
MongoDB provides several configuration options that you can set
using:
...
:ref:`configuration-options`, :binary:`~bin.mongod` and
:binary:`~bin.mongos`.
...
**parameter:** allowRolesFromX509Certificates
   ...
   *Default*: true
   ...
   Allows or disallows the retrieval of authorization roles from
...
**parameter:** authenticationMechanisms
   ...
   *Default*: ``MONGODB-X509,SCRAM-SHA-1,SCRAM-SHA-256``
   ...
   Starting in MongoDB 8.3, if :option:`mongod --tlsFIPSMode` or
   :option:`mongos --tlsFIPSMode` is enabled, ``SCRAM-SHA-1`` is
   excluded from the effective default.
...
**parameter:** JWKSMinimumQuiescePeriodSecs
   ...
   *Default*: 60 seconds (1 minute)
   ...
   a given |idp-poss| ``JWKSetURI`` endpoint. If an RSA public key is changed multiple 
...
**parameter:** ldapForceMultiThreadMode
   ...
   *Default*: false
   ...
   .. tip::
      ...
      If you have any concerns regarding your MongoDB version, OS version,
      or libldap version, contact MongoDB Support.
...
**parameter:** ocspStaplingRefreshPeriodSecs
   ...
   You can only set :parameter:`ocspStaplingRefreshPeriodSecs` during
...
**parameter:** oidcIdentityProviders
   ...
   Use this parameter to specify |idp-abbr| configurations when 
   using :ref:`OpenID Connect Authentication <authentication-oidc>`. 

   ``oidcIdentityProviders`` accepts an array of zero or more |idp|
   configurations. An empty array (default) indicates no OpenID Connect 
   support is enabled. 
   
   When more than one |idp| is defined, ``oidcIdentityProviders`` uses the 
   ``matchPattern`` field to select an |idp|. Array order determines the priority 
   and the first |idp| is always selected.
   
   .. include:: /includes/fact-oidc-multiple-config-for-same-issuer.rst
    
   
   .. _oidcidentityproviders-fields:
...
**parameter:** opensslCipherConfig
   ...
   *Available on Linux only*
   ...
   The use of ``TLS`` options is preferred over ``SSL`` options. The TLS options 
   have the same functionality as the ``SSL`` options. The following example 
   configures a :binary:`~bin.mongod` with a :parameter:`opensslCipherConfig` 
...
**parameter:** opensslDiffieHellmanParameters
   ...
   *Available on Linux only*
   ...
   .. note::
      ...
      If :parameter:`opensslDiffieHellmanParameters` is unset but
...
**parameter:** pessimisticConnectivityCheckForAcceptedConnections
   ...
   *Default:* false
   ...
   When set to ``true``, instructs the server to check the connectivity of accepted connections 
...
**parameter:** sslMode
   ...
   Set the ``net.ssl.mode`` to either ``preferSSL`` or
...
**parameter:** tlsMode
   ...
   Use the :parameter:`tlsMode` parameter during :doc:`rolling
...
**parameter:** capMemoryConsumptionForPreAuthBuffers
   ...
   .. versionadded:: 8.3 *(also available starting in 8.2.4, 8.0.18, and 7.0.29)*
   ...
   *Default*: 20
   ...
   Specifies the maximum percentage of total memory to allocate to pre-auth 
   buffers for connections. Setting the parameter to ``100`` disables this 
   limitation.

   .. include:: /includes/fact-startup-parameter

   The following operation sets ``capMemoryConsumptionForPreAuthBuffers``
   to ``30``:
...
**parameter:** cursorTimeoutMillis
   ...
   *Default*: 600000 (10 minutes)
   ...
   .. warning::
      ...
      MongoDB cleans up :term:`orphaned cursors <orphaned cursor>` after the
      ``cursorTimeoutMillis`` threshold only if they are not tied to sessions.

      MongoDB cleans up cursors linked to sessions with the
      :parameter:`localLogicalSessionTimeoutMinutes` lifecycle, regardless of
      the ``cursorTimeoutMillis`` value. To handle long idle periods,
      use :method:`Mongo.startSession()` and refresh the session using the
      :dbcommand:`refreshSessions` command. For details, see
...
**parameter:** defaultClientBaseBackoffMillis
   ...
   Base delay in milliseconds for the default retry strategy
   used in internal server-side connections, such as
   :program:`mongos` and :program:`mongod`. The server uses this
   value as the initial delay for exponential backoff between
   retries on operations that fail due to overload.
...
**parameter:** defaultClientMaxBackoffMillis
   ...
   Maximum delay in milliseconds for the default retry strategy
   used by internal server-side client connections, such as
   :program:`mongos` and :program:`mongod`.  The server uses
   this value as the maximum wait time between retries of
   operations that fail due to overload.
...
**parameter:** defaultClientRetryAttempts
   ...
   Maximum number of times the server retries an operation for
   internal server-side client connections, such as
   :program:`mongos` and :program:`mongod`.
...
**parameter:** ingressAdmissionControllerTicketPoolSize
   ...
   *Default*: ``1000000``
   ...
   to the ingress queue. The default value of ``1000000`` represents the 
   ...
   .. warning::
      ...
      Avoid modifying :parameter:`ingressAdmissionControllerTicketPoolSize`
...
**parameter:** ingressConnectionEstablishmentRateLimiterEnabled
   ...
   .. versionadded:: 8.2 *(also available in 8.1.1 and 8.0.12)*
   ...
   *Default*: ``false``
   ...
   Determines whether rate limiting for new connection establishment is enabled. 
   When enabled, MongoDB applies rate limiting to control the 
   number of new connections that can be established per second.
...
**parameter:** ingressConnectionEstablishmentRatePerSec
   ...
   *Minimum*: 1
   ...
   Specifies the maximum number of new connections that can be established per second 
...
**parameter:** ingressConnectionEstablishmentBurstCapacitySecs
   ...
   *Minimum*: 1
   ...
   Describes how many seconds worth of connection establishments that the server can admit
   before rate limiting begins. This allows the server to handle temporary bursts of 
   connection requests that exceed the rate limit specified by 
   :parameter:`ingressConnectionEstablishmentRatePerSec`.
...
**parameter:** ingressConnectionEstablishmentMaxQueueDepth
   ...
   *Default*: ``0`` (disabled)
   ...
   Specifies the maximum number of connection attempts in the connection 
   establishment queue. After the queue reaches this number of connection 
   attempts, the server rejects new connection attempts.
   
   The default value of ``0`` means that the server rejects all 
   connections that will queue for establishment.
...
**parameter:** ingressConnectionEstablishmentRateLimiterBypass
   ...
   *Default*: ``[]``
   ...
   Provides a list of IP addresses and CIDR ranges that the server must exempt from the 
   connection establishment rate limits. This allows specific trusted clients to 
   bypass rate limiting restrictions.
   
   .. include:: /includes/fact-runtime-startup-parameter

**parameter:** ingressRequestRateLimiterApplicationExemptions
   ...
   *Default*: ``{ appnames: [] }``
   ...
   The ``ingressRequestRateLimiterApplicationExemptions`` parameter lets you 
   specify applications to exempt from ingress request rate limiting.
   ``ingressRequestRateLimiterApplicationExemptions`` accepts a document 
   containing an array of application names to exempt from ingress request rate 
   limiting as input.

   .. include:: /includes/fact-runtime-startup-parameter

   The following command exempts the connections with the `application name 
   <https://github.com/mongodb/specifications/blob/master/source/mongodb-handshake/handshake.md#clientapplicationname>`__
   "MongoDB Internal Client" at startup:
   ...
   The following command exempts connections with the application name "MongoDB
   Internal Client" during run time:
...
**parameter:** overloadAwareServerSelectionEnabled
   ...
   *Default*: ``false``
   ...
   This parameter controls whether overload-induced retries against
   other servers in the cluster avoid servers that recently returned an
   error labeled ``SystemOverloadedError``. It affects operations that
   perform remote targeting, for example when ``mongos`` retries
   requests against shard members or when ``mongod`` performs remote
   reads during aggregation.

   When you set this parameter to ``true``, retry logic for remote
   targeting prefers non-overloaded members for operations that fail
   with ``SystemOverloadedError``. For example, when ``mongos`` retries
   reads:

   - With the ``primaryPreferred`` read preference, ``mongos`` can
     route a retried read to a secondary if the primary indicates
     overload, even if the primary remains reachable.

   - With the ``secondaryPreferred`` read preference, ``mongos`` can
     route a retried read to the primary if secondaries indicate
     overload.

   When you set this parameter to ``false``, retries do not avoid
   members that signal overload. If a member continues to return
   ``SystemOverloadedError``, retries may continue targeting that member.

   This parameter affects only operations retried after
   ``SystemOverloadedError`` errors that can occur when you enable ingress
   request rate limiting or other overload mechanisms on shard nodes.
...
**parameter:** maxIndexBuildMemoryUsageMegabytes
   ...
   Limits the amount of memory that simultaneous :ref:`index builds <index-build-process>` 
   ...
   :method:`db.collection.createIndexes()`. Increasing the limit may avoid negative
   performance impacts in some cases, such as when building numerous indexes 
   simultaneously with a single :dbcommand:`createIndexes` command and/or when 
   working with index key data set sizes that exceed 500GB. The memory consumed 
   by an index build is separate from the WiredTiger cache memory 
   (see :setting:`~storage.wiredTiger.engineConfig.cacheSizeGB`).
   ...
   If this setting allocates less than 50 MB, ``mongod`` uses a minimum of 50
   MB. If the percentage setting is greater than 0.8, ``mongod`` uses a maximum
...
**parameter:** maxNumActiveUserIndexBuilds
   ...
   *Default*: 3
   ...
   Sets the maximum number of index build requests that MongoDB can
   perform concurrently. ``maxNumActiveUserIndexBuilds`` is a global
   limit that applies for all collections.

   .. include:: /includes/maxNumActiveUserIndexBuilds-atlas-tier-limits-note.rst

   The :dbcommand:`createIndexes` command builds indexes. Even if the
   ``createIndexes`` command builds multiple indexes at once, the
   command execution only adds ``1`` to ``maxNumActiveUserIndexBuilds``.
...
**parameter:** messageSizeErrorRateSec
   ...
   *Default*: 5
   ...
   Specifies the rate, in seconds, at which the server logs message size errors
   at the Info level. 

   .. include:: /includes/fact-startup-parameter

   The following operation sets ``messageSizeErrorRateSec``
   to ``6``:
...
**parameter:** notablescan
   ...
   Set :parameter:`notablescan` to ``true`` to test application queries
   and identify queries that scan an entire collection and cannot use
   an index.

   To detect unindexed queries without ``notablescan``, consider reading
   the :doc:`/tutorial/evaluate-operation-performance` section and using
   the :parameter:`logLevel` parameter, :binary:`~bin.mongostat` and
   :ref:`profiling <database-profiling>`.
...
**parameter:** preAuthMaximumMessageSizeBytes
   ...
   *Default*: 16384 (16 KiB)
   ...
   The maximum size of a message in bytes that you can send before the session
   authenticates.

   .. include:: /includes/fact-runtime-startup-parameter

   For example, the following sets the maximum size of the message to 
   1700 bytes:
...
**parameter:** tcmallocEnableBackgroundThread
   ...
   *Type*: boolean
   ...
   If set to ``true``, ``tcmallocEnableBackgroundThread`` creates a background 
   thread that periodically releases memory back to the operating system. The 
   value of :parameter:`tcmallocReleaseRate` determines the rate, in bytes per 
   second, at which the background thread releases memory. 

   .. include:: /includes/fact-background-thread-release-rate.rst

   For improved memory usage, we recommend using the default value 
   of ``true``. To learn more about improvements to performance and memory 
   management, see :ref:`8.0-tcmalloc-upgrade`.
   
   .. include:: /includes/fact-startup-parameter.rst

   The following operation sets ``tcmallocEnableBackgroundThread``
   to ``false``:
...
**parameter:** tcmallocReleaseRate
   ...
   Specifies the `TCMalloc release rate <https://github.com/google/tcmalloc/blob/master/docs/tuning.md>`__
   in bytes per second. Release rate refers to the rate at which MongoDB 
   releases unused memory to the system. If ``tcmallocReleaseRate`` is set to 
   ``0`` MongoDB doesn't release memory back to the system. Increase 
   this value to return memory faster; decrease it to return memory slower.

   If ``tcmallocReleaseRate`` is a small non-zero value, the actual
   memory release rate can exceed the configured value. TCMalloc
   releases memory at page-span granularity, and a page span can
   consist of multiple TCMalloc pages.

   .. include:: /includes/fact-background-thread-release-rate.rst

   Starting in MongoDB 8.0, the default value of ``tcmallocReleaseRate`` is 
   reduced to ``0`` due to a :ref:`tcmalloc upgrade <8.0-tcmalloc-upgrade>` 
   ...
   .. important:: 
      ...
      If you run MongoDB on a platform that uses ``tcmalloc-gperf`` such as 
      Windows, PPC, or s390x, ``tcmallocReleaseRate`` has the same behavior 
...
**parameter:** enableDetailedConnectionHealthMetricLogLines
   ...
   *Default*: true 
   ...
   :parameter:`enableDetailedConnectionHealthMetricLogLines` is set to 
...
**parameter:** suppressNoTLSPeerCertificateWarning
   ...
   *Default*: false
   ...
   :ref:`TLS/SSL enabled <configure-mongod-mongos-for-tls-ssl>` and
   ``net.ssl.allowConnectionsWithoutCertificates`` : ``true``
...
**parameter:** diagnosticDataCollectionDirectorySizeMB
   ...
   *Default*: ``500``
   ...
   The minimum value for ``diagnosticDataCollectionDirectorySizeMB`` is 
   ``10`` megabytes. ``diagnosticDataCollectionDirectorySizeMB`` must
...
**parameter:** changeSyncSourceThresholdMillis
   ...
   *Default*: 5
   ...
   The minimum latency improvement, in milliseconds, that a candidate
   :ref:`sync source <replica-set-replication-sync-source-selection>`
   must have over the current sync source before a secondary considers
   switching to it. A lower value makes secondaries more sensitive to
   small latency differences, which increases the number of sync
   source changes. A higher value reduces switching between candidates
   with similar latency.

   .. include:: /includes/fact-runtime-startup-parameter

   For example, to set :parameter:`changeSyncSourceThresholdMillis` for
   a :binary:`~bin.mongod` instance to ``200`` using the
   :dbcommand:`setParameter` command within :binary:`~bin.mongosh`:
...
**parameter:** connectTimeoutMs
   ...
   *Default*: 10000
   ...
   in milliseconds, for the :ref:`replica set 
   monitor <replica-set-monitoring>`.

   This parameter is only available at startup. If you set this parameter,
   it must be greater than or equal to 500.

   The following example sets ``connectTimeoutMs`` for a ``mongod`` 
...
**parameter:** heartBeatFrequencyMs
   ...
   *Default*: 10000
   ...
   When :parameter:`replicaSetMonitorProtocol` is set to ``'sdam'``, 
   ``heartBeatFrequencyMs`` determines how long, 
   in milliseconds, to wait between :dbcommand:`hello` requests.

   When :parameter:`replicaSetMonitorProtocol` is set to ``'streamable'``, 
   ``heartBeatFrequencyMs`` determines how long, 
   in milliseconds, to wait between :dbcommand:`hello` round trip time
   (RTT) measurements. RTT measurements are used in 
   :ref:`server selection <replica-set-read-preference-behavior>`.

   This parameter is only available at startup. If you set this
   parameter, it must be greater than or equal to 500.

   The following example sets ``heartBeatFrequencyMs`` 
   in a ``mongod`` instance to 700 milliseconds:

   .. code-block:: bash

      mongod --setParameter heartBeatFrequencyMs=700

**parameter:** initialSyncIndexBuildMemoryPercentage

   .. versionadded:: 8.2 (and 8.0.13, 7.0.26)
   ...
   *Default*: 10.0
   ...
   during :ref:`initial sync <replica-set-initial-sync>`. The amount of system 
   memory used is bound by the values of 
   :parameter:`initialSyncIndexBuildMemoryMinMB` and 
   ...
   For example, to set the ``initialSyncIndexBuildMemoryPercentage``
   for a :binary:`~bin.mongod` instance to 40%:
...
**parameter:** initialSyncIndexBuildMemoryMaxMB
   ...
   *Default*: 16384
   ...
   index builds during :ref:`initial sync <replica-set-initial-sync>`.

   You can specify a value between ``50`` and ``10000000``. 

   .. include:: /includes/fact-runtime-startup-parameter

   For example, to set the ``initialSyncIndexBuildMemoryMaxMB``
   for a :binary:`~bin.mongod` instance to 20000 MB:
...
**parameter:** initialSyncIndexBuildMemoryMinMB
   ...
   *Default*: 200
   ...
   For example, to set the ``initialSyncIndexBuildMemoryMinMB``
   for a :binary:`~bin.mongod` instance to 60 MB:
...
**parameter:** localThresholdMs
   ...
   *Default*: 15
   ...
   :ref:`server selection <replica-set-read-preference-behavior>` in
   milliseconds.

   This parameter is only available at startup. If you set this parameter,
   it must be greater than or equal to 0.

   The following example sets ``localThresholdMs``
   for a ``mongod`` instance to 20 milliseconds:
...
**parameter:** maxSyncSourceLagSecs
   ...
   *Default*: 30
   ...
   The maximum lag, in seconds, that a candidate
   :ref:`sync source <replica-set-replication-sync-source-selection>`
   can have relative to the newest oplog entry on the primary and
   still qualify as an eligible sync source. If a candidate's most
   recent oplog entry is more than :parameter:`maxSyncSourceLagSecs`
   seconds behind the primary, MongoDB rejects it as a sync source.

   .. include:: /includes/fact-startup-parameter

   For example, to set :parameter:`maxSyncSourceLagSecs` for a
   :binary:`~bin.mongod` instance to ``20``:
   ...
   .. list-table::
      ...
      * - ``samplingRate``
        ...
        - The sampling rate used to mirror a subset of :ref:`operations
          ...
          ``1.0``
             The primary mirrors all :ref:`operations that
             support mirroring
             <mirrored-reads-supported-operations>` to each
             electable secondary.

          Number between ``0.0`` and ``1.0`` (exclusive)
             The primary randomly samples each electable
             secondary at the specified rate to be sent mirrored
             reads.
      ...
      * - ``targetedMirroring``
        ...
        - Contains fields to configure how you target specific nodes for 
          ...
          mirroring, see :ref:`targeted-mirrored-reads`.
           
          Includes the following fields: 

          ``tag``
             Defaults to an empty ``BSONObj``. The :ref:`replica
             set tag <replica-set-configuration-tag-sets>` that
             you can use to target nodes for mirroring. You can
             configure a node for targeted mirroring using the
             following syntax: 
          ...
          ``samplingRate``
             ...
             *Range*: ``0.0`` to ``1.0`` (inclusive)
             ...
             The rate at which targeted reads are mirrored to
             the host or hosts. A rate of ``0.0`` means no reads
             are mirrored, and a rate of ``1.0`` means all reads
             are mirrored. Although ``samplingRate`` defaults to
             ``0.01``, the ``targetedMirroring`` feature is off
             by default, since the ``tag`` field defaults to
             empty.

          ``maxTimeMS``
             *Type*: int

             The maximum time in milliseconds before the
             mirrored read times out. The minimum value for
             ``maxTimeMS`` is ``0``. Defaults to ``1000``.

   .. include:: /includes/fact-runtime-startup-parameter

   If you specify from the configuration file or on the command
   line, **enclose the** ``mirrorReads`` **document in quotes**.
...
**parameter:** mirrorReadsMaxConnPoolSize
   ...
   *Default*: 4
   ...
   affects both :ref:`general <mirrored-reads>` and 
   :ref:`targeted <targeted-mirrored-reads>` mirrored reads. 
   ``mirrorReadsMaxConnPoolSize`` has a minimum value of 1.
...
**parameter:** replicaSetMonitorProtocol
   ...
   *Default*: "streamable"
   ...
   parameter to ``"streamable"``, which is compliant with `Server Discovery and
   Monitoring (SDAM) <https://github.com/mongodb/specifications/blob/master/source/server-discovery-and-monitoring/server-discovery-and-monitoring.md>`__ 
   specification and allows for constant :dbcommand:`hello`
   requests. You can also set this parameter to ``"sdam"``, which is compliant
   with SDAM specification.

   This parameter is only available at startup. 
   
   The following example sets
   ``replicaSetMonitorProtocol`` to ``"sdam"`` on a ``mongod``
   instance:
...
**parameter:** replWriterMinThreadCount
   ...
   *Default*: 0
   ...
   parallel. Values can range from 0 to 256 inclusive. 
   ...
   :parameter:`replWriterMinThreadCount` must be less than or equal to
   :parameter:`replWriterThreadCount`. MongoDB rejects the update if
   you set :parameter:`replWriterMinThreadCount` to a value greater
   than :parameter:`replWriterThreadCount`.
...
**parameter:** replWriterThreadCount
   ...
   *Default*: 16
   ...
   :parameter:`replWriterThreadCount` must be greater than or equal
   to :parameter:`replWriterMinThreadCount`. MongoDB rejects the
   update if you set :parameter:`replWriterThreadCount` to a value
   less than :parameter:`replWriterMinThreadCount`.
...
**parameter:** analyzeShardKeyNumSamplesPerRange
   ...
   *Default*: 10
   ...
   The number of documents to sample per shard key :ref:`range <shard-key-range>`. 
   Must be a value greater than zero and less than or equal to ``10000``.
...
**parameter:** catalogCacheCollectionMaxEntries
   ...
   .. versionadded:: 8.1 (*also available starting in 8.0.10, 7.0.22*)
...
**parameter:** directConnectionChecksWithSingleShard
   ...
   *Default*: true starting in MongoDB 8.0.10
   ...
   warnings. If a user connects directly to the single-shard cluster, ``mongod``
   logs a warning every hour and
   updates :serverstatus:`shardingStatstics.unauthorizedDirectShardOps` with
   unauthorized direct shard operations.

   To avoid this, users should connect to single-shard clusters through a
   router, like ``mongos``. 
...
**parameter:** disableResumableRangeDeleter
   ...
   *Default*: false
   ...
   If set on a shard's primary, specifies if MongoDB pauses range deletion on
   the shard. If set to ``true``, MongoDB pauses cleanup of :term:`ranges
   <range>` containing :term:`orphaned documents <orphaned document>`. The shard
   can continue to donate chunks to other shards, but the shard does not remove
   donated documents until you set this parameter to ``false``. This shard can
   continue to receive chunks from other shards as long as it does not have a
   pending range deletion task in the :data:`config.rangeDeletions` collection
   that overlaps with the incoming chunk's range.
   ...
   Starting in MongoDB 8.0, you can set ``disableResumableRangeDeleter`` during
...
**parameter:** enableFinerGrainedCatalogCacheRefresh
   ...
   .. note:: Deprecated in 8.0 
      ...
      Starting in MongoDB 8.0, the parameter is deprecated and doesn't 
...
**parameter:** findShardsOnConfigTimeoutMS
   ...
   *Default*: 20000 milliseconds
   ...
   The timeout in milliseconds for ``find`` operations against the 
   ``config.shards`` namespace.

   .. include:: /includes/fact-runtime-startup-parameter

   The following example sets ``findShardsOnConfigTimeoutMS`` to 
   ``30000`` milliseconds during runtime:
...
**parameter:** maxTimeMSForHedgedReads
   ...
   .. include:: /includes/hedged-reads-removed-parameter.rst
...
**parameter:** metadataRefreshInTransactionMaxWaitMS
   ...
   *Default*: 500
   ...
   .. warning::
      ...
      If :parameter:`metadataRefreshInTransactionMaxWaitMS`
      is too low, :binary:`mongos` could use all of its retry attempts
      and return an error.

   .. include:: /includes/fact-runtime-startup-parameter

   For example, to set :parameter:`metadataRefreshInTransactionMaxWaitMS`
   to 400 milliseconds:
...
**parameter:** orphanCleanupDelaySecs
   ...
   and then waits an additional :parameter:`orphanCleanupDelaySecs` seconds.

   Starting in 8.2, the behavior of in-progress queries on shard secondaries 
   is determined by :parameter:`terminateSecondaryReadsOnOrphanCleanup`.
...
**parameter:** persistedChunkCacheUpdateMaxBatchSize
   ...
   information propagates from a shard's primary node to its
...
**parameter:** queryAnalysisSamplerConfigurationRefreshSecs
   ...
   *Default*: 10
   ...
   ``queryAnalysisSamplerConfigurationRefreshSecs`` during run time. 
...
**parameter:** queryAnalysisWriterIntervalSecs
   ...
   *Default*: 90
   ...
   Starting in MongoDB 7.0.1, you can set
   ``queryAnalysisWriterIntervalSecs`` during run time. 
...
**parameter:** ShardingTaskExecutorPoolMaxSize
   ...
   - On-prem deployments: ``32767`` for both :binary:`~bin.mongos` and
     :binary:`~bin.mongod`
   - |service-fullname|:

     - ``250`` for :binary:`~bin.mongos` routers
     - ``32767`` for :binary:`~bin.mongod` instances

   .. note::

      While the maximum configurable value is 2\ :sup:`64` - 1, the
      defaults for both on-premises and |service-fullname| deployments
      are significantly lower to optimize performance and resource
      usage.
...
**parameter:** ShardingTaskExecutorPoolMaxQueueDepth
   ...
   The maximum number of connection requests that an executor 
   queues before it begins rejecting additional requests. 
    
   A value of ``0`` means there is no limit to the size of the queue.

   .. include:: /includes/fact-runtime-startup-parameter

   The following example sets ``ShardingTaskExecutorPoolMaxQueueDepth`` 
   to ``10`` during startup:
   ...
   .. list-table::
      ...
      * - ``"matchPrimaryNode"``
        ...
        - When set for a :binary:`~bin.mongos`, the minimum size limit
          ...
          :red:`WARNING:` If multiple shard servers in your topology can experience a
...
**parameter:** shardRetryTokenBucketCapacity
   ...
   *Default*: 1000
   ...
   Maximum number of overload retry tokens allocated for each
   shard. Each retry attempt in response to an overload error
   uses a token and each attempt that succeeds frees a token. If
   there are no tokens available, the operation fails. 

   For example, if ``mongos`` retries an operation that was
   previously load shed by the shard, it uses a token in the
   retry. If the attempt succeeds, it returns the token to the
   shard plus the :parameter:`shardRetryTokenReturnRate`. If the
   attempt fails, it tries again with a new token. If it finds
   the bucket empty, it doesn't retry the operation.

   .. include:: /includes/fact-runtime-startup-parameter

   The following example sets the
   ``shardRetryTokenBucketCapacity`` to 5000 during startup:
...
**parameter:** shardRetryTokenReturnRate
   ...
   *Default*: 0.1
   ...
   MongoDB allows the number of available retry tokens to
   increase over time as it processes requests. 

   .. include:: /includes/fact-runtime-startup-parameter

   The following example sets the
   ``shardRetryTokenBucketReturnRate`` to 0.25 during startup:
...
**parameter:** terminateSecondaryReadsOnOrphanCleanup
   ...
   *Default*: true
   ...
   automatically terminate before :term:`orphaned document` 
   deletion following a :term:`chunk` migration.
   ...
   period controlled by :parameter:`orphanCleanupDelaySecs` (default: 1 hour). 
   ...
   ``terminateSecondaryReadsOnOrphanCleanup`` is set to ``true``.
   
   When ``terminateSecondaryReadsOnOrphanCleanup`` is set to ``true``, read 
   ...
   When set to ``false``, read operations on secondary nodes continue executing 
...
### Error Response
   ...
   When a read operation terminates due to ``terminateSecondaryReadsOnOrphanCleanup``, 
   MongoDB returns the following error:

   .. literalinclude:: /code-examples/reference/parameters/terminateSecondaryReadsOnOrphanCleanup-error-message.sh
      :language: shell
      :copyable: false
   
   This error is not retryable by design. To learn more about handling these 
   errors, see :ref:`long-running-secondary-reads`.
...
**parameter:** spillPath
   ...
   *Default*: The value of the :setting:`~storage.dbPath` setting.

   *Type*: string

   Path used by the spill engine when it writes to disk. You may
   find this useful when there is a risk of disk spill operations
   reducing the IOPS for concurrent queries.
...
**parameter:** storageEngineConcurrentReadTransactions
   ...
   *Type*: integer
   ...
   .. include::  /includes/fact-concurrent-read-write-dynamic-behavior
...
**parameter:** temporarilyUnavailableBackoffBaseMs
   ...
   *Default*: 1000
   ...
   .. include:: /includes/transactions/cache-pressure-support.rst
...
**parameter:** upsertMaxRetryAttemptsOnDuplicateKeyError
   ...
   Maximum number of retry attempts when an ``upsert`` operation
   ...
   *Default*: 100
   ...
   retries an ``upsert`` operation when encountering a duplicate key 
   error to ``50``.
...
**parameter:** wiredTigerFileHandleCloseIdleTime
   ...
   *Default*: 600
   ...
   .. tip::
      ...
      This parameter is case sensitive. If you capitalize the ``w`` in 
      ``wiredTigerFileHandleCloseIdleTime`` and run the parameter, the operation
...
**parameter:** auditEncryptKeyWithKMIPGet
   ...
   *Default*: false
   ...
   The following example sets
   :parameter:`auditEncryptKeyWithKMIPGet` to ``true``:
...
**parameter:** AbortExpiredTransactionsSessionCheckoutTimeout
   ...
   *Default*: 100 milliseconds
   ...
   The following example sets
   ``AbortExpiredTransactionsSessionCheckoutTimeout`` to ``120``
   milliseconds:
...
**parameter:** cachePressureQueryPeriodMilliseconds
   ...
   *Default*: 1000 milliseconds
   ...
   The time interval MongoDB waits between checks for storage engine cache pressure. 
   If the storage engine is under cache pressure, MongoDB aborts the oldest transaction.
   ...
   The following example sets
   ``cachePressureQueryPeriodMilliseconds`` to ``3000`` milliseconds at runtime:
...
**parameter:** coordinateCommitReturnImmediatelyAfterPersistingDecision
   ...
   *Default*: false
   ...
   - When set to ``false``, the :ref:`shard <shards-concepts>`
     transaction coordinator waits for all participating shards
     to acknowledge the decision to either commit or cancel a
     :ref:`multi-document transaction <transactions-atomicity>`
     before returning the result to the client.

   - When set to ``true``, the shard transaction coordinator
     returns a :ref:`multi-document transaction
     <transactions-atomicity>` commit decision to the client as
     soon as the decision is made :term:`durable` with the
     requested transaction :ref:`write concern <write-concern>`.
...
**parameter:** internalQueryGlobalProfilingLockDeadlineMs
   ...
   *Default*: 1
   ...
   The maximum number of milliseconds to wait for a lock acquisition before
   performing writes to :data:`~<database>.system.profile` collections.

   .. include:: /includes/fact-runtime-startup-parameter

   The following command sets ``internalQueryGlobalProfilingLockDeadlineMs`` 
   to ``100`` milliseconds at runtime:
   ...
   The following command sets ``internalQueryGlobalProfilingLockDeadlineMs`` 
   to ``100`` milliseconds at startup:
...
**parameter:** internalProfilingMaxAbandonedWritesPerSecondPerDb
   ...
   *Default*: 1000
   ...
   Specifies the maximum number of lock timeouts per second when trying to write
   to :data:`~<database>.system.profile` collections. Once this threshold is 
   reached, profiling is disabled for this database. This setting lasts until 
   you manually re-enable profiling. 
   
   The value can be ``0``, which means a single lock timeout disables profiling
   for the database.

   .. include:: /includes/fact-runtime-startup-parameter

   The following command sets ``internalProfilingMaxAbandonedWritesPerSecondPerDb`` 
   to ``2000`` at runtime:
