...
generated_at: "2026-08-29T09:39:19.869908Z"
---
.. _mongod:
...
**meta:** :description: Explore mongod command-line options for MongoDB's primary daemon, ``mongod``, that handles data requests, access, and background management operations.
   :keywords: on-prem
...
``mongod`` command-line options should primarily be used for testing.
...
**option:** --auth
   ...
   Enables authorization to control user access to database resources
   and operations. When authorization is enabled, MongoDB requires all
   clients to authenticate before it determines their access.
...
**option:** --bind_ip <hostnames|ipaddresses|Unix domain socket paths>
   ...
   *Default*: localhost
   ...
   which ``mongod`` listens for client connections. You can attach
   ``mongod`` to any interface. To bind to multiple addresses, enter a
...
**option:** --quiet
   ...
   Runs ``mongod`` in a quiet mode that limits output.
...
**option:** --redactClientLogData
   ...
   *Available in MongoDB Enterprise only.*
   ...
   Information (PII) in one or more collections. The ``mongod`` instance
   logs events such as CRUD operations and sharding metadata, and might
   expose PII as part of these logging operations. A ``mongod`` running
...
**option:** --transitionToAuth
   ...
   not match, the ``mongod`` uses a non-authenticated connection instead.
...
**option:** --ldapUserToDNMapping <string>
   ...
   *Available in MongoDB Enterprise only.*
   ...
   .. example::
     ...
     :gold:`IMPORTANT` You must pass the array to
     :option:`--ldapUserToDNMapping` as a string.
...
**option:** --wiredTigerCacheSizeGB <float>
   ...
   .. include:: /includes/extracts/wt-cache-default-values-mongod-cmdline-option.rst

   To learn more, see :ref:`wiredtiger-RAM`.
   ...
   .. note::
      ...
      To view the :data:`memory limit <hostInfo.system.memLimitMB>`, the value
      that WiredTiger utilizes as the maximum amount of RAM available use the
      :dbcommand:`hostInfo` command. 


**option:** --wiredTigerCacheSizePct <float>
   ...
   system with 2GB of RAM the ``--wiredTigerCacheSizePct`` cannot be set to 10 
   because 10% of 2GB is 0.2GB, which is less than 0.256GB. 
   
   To learn more about memory limits, see :ref:`wiredtiger-RAM`.
   ...
   .. include:: /includes/extracts/wt-cache-pct-additional-constraints-mongod-cmdline-option.rst
...
**option:** --replicaSetConfigShardMaintenanceMode
   ...
   Configures the ``mongod`` instance to start in maintenance
...
**option:** --defaultSlowInProgMS <integer>
