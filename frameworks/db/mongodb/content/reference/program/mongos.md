...
generated_at: "2026-08-29T09:39:19.874976Z"
---
.. _mongos:
...
**meta:** :keywords: on-prem
   :description: Explore the `mongos` options for configuring sharded clusters, including TLS settings, logging, and network parameters.
...
For a :ref:`sharded cluster <sharding-sharded-cluster>`, the ``mongos``
...
.. |newversion| replace:: {+latest-lts-version+}

.. |oldversion| replace:: {+last-supported-version+}
...
**option:** --configExpand <none|rest|exec>
   ...
   *Default*: none
   ...
   expansion directives not specified to :option:`--configExpand`, the ``mongos``
...
**option:** --quiet
   ...
   Runs ``mongos`` in a quiet mode that limits output.
...
**option:** --bind_ip <hostnames|ipaddresses|Unix domain socket paths>
   ...
   *Default*: localhost
   ...
   The hostnames, IP addresses, and full Unix domain socket paths on
   which ``mongos`` listens for client connections. You can attach
   ``mongos`` to any interface. To bind to multiple addresses, enter a
   comma-separated list of values.

   For example: ``localhost,/tmp/mongod.sock``

   You can specify both IPv4 and IPv6 addresses, or hostnames that
   resolve to an IPv4 or IPv6 address.

   For example: ``localhost,2001:0DB8:e132:ba26:0d5c:2774:e7f9:d513``
   
   .. note::
   
      If specifying an IPv6 address *or* a hostname that resolves to an
      IPv6 address to :option:`--bind_ip`, you must start ``mongos`` with 
   ...
   to that address (that is, ``fe80::<address>%<adapter-name>``).

   For example: ``localhost,fe80::a00:27ff:fee0:1fcf%enp0s3``
   ...
   expansion). Alternatively, use the :setting:`net.bindIpAll` setting.
...
**option:** --bind_ip_all
   ...
   ``mongos`` only supports IPv6 if started with :option:`--ipv6`. Specifying
...
**option:** --logappend
   ...
   instance restarts. Without this option, ``mongos`` backs up the
   existing log and creates a new file.
...
**option:** --redactClientLogData
   ...
   *Available in MongoDB Enterprise only.*
   ...
   A ``mongos`` running with :option:`--redactClientLogData` redacts any message accompanying a given
   ...
   Information (PII) in one or more collections. The ``mongos`` logs
   events such as CRUD operations and sharding metadata, and might
   expose PII as part of these logging operations. A ``mongos`` running
   with :option:`--redactClientLogData` removes any message accompanying
   these events before writing them to the log, which removes the PII.
   
   Diagnostics on a ``mongos`` running with :option:`--redactClientLogData` may be more difficult
   ...
   On a running ``mongos``, use :dbcommand:`setParameter` with the
...
**option:** --pidfilepath <path>
   ...
   specified, the process does not create a PID file. Use this option
   mainly with the :option:`--fork` option.
...
**option:** --nounixsocket
   ...
   .. |mongodb-package| replace:: ``mongos``
...
**option:** --fork
   ...
   Enables a :term:`daemon` mode that runs the ``mongos`` process in the
...
**option:** --transitionToAuth
   ...
   :ref:`internal authentication <inter-process-auth>`, the ``mongos`` creates
   an authenticated connection with any :binary:`~bin.mongod` or ``mongos``
   in the deployment using a matching keyfile. If the security mechanisms do
   not match, the ``mongos`` uses a non-authenticated connection instead.
   
   A ``mongos`` running with :option:`--transitionToAuth` does not enforce :ref:`user access
   ...
   .. note::
      ...
      A ``mongos`` running with :ref:`internal authentication
      ...
      connect to the ``mongos`` using the appropriate :ref:`user <users>`
      prior to restarting ``mongos`` without :option:`--transitionToAuth`.
...
**option:** --networkMessageCompressors <string>
   ...
   *Default*: snappy,zstd,zlib
   ...
   Both :binary:`~bin.mongod` and ``mongos`` instances 
...
**seealso:** :doc:`/tutorial/configure-ssl` for full documentation of MongoDB's
   support.
...
**option:** --tlsClusterPassword <value>
   ...
   - On Linux/BSD, if the private key in the X.509 file is encrypted and
     you do not specify the :option:`--tlsClusterPassword` option, MongoDB prompts for a
     passphrase. See :ref:`ssl-certificate-password`.
...
**option:** --slowms <integer>
   ...
   *Default*: 100
   ...
   At higher :parameter:`logLevel` settings, all operations appear
...
**option:** --slowOpSampleRate <double>
   ...
   *Default*: 1.0
   ...
   For ``mongos`` instances, :option:`--slowOpSampleRate` affects the diagnostic log
...
**option:** --ldapServers <host1>:<port>,<host2>:<port>,...,<hostN>:<port>
   ...
   *Available in MongoDB Enterprise only.*
   ...
   If unset, ``mongos`` cannot use :doc:`LDAP authentication or authorization
   </core/security-ldap>`.
...
**option:** --ldapBindMethod <string>
   ...
   * ``sasl`` - ``mongos`` uses SASL protocol for authentication
   ...
   using :option:`--ldapBindSaslMechanisms`. ``mongos`` defaults to
   ...
   *Available in MongoDB Enterprise only.*
   ...
   ``Linux``
     ...
     - The client keytab includes a
       :ref:`kerberos-user-principal` for the ``mongos`` to use when
...
**option:** --ldapTransportSecurity <string>
   ...
   *Available in MongoDB Enterprise only.*
   ...
   Set :option:`--ldapTransportSecurity` to ``none`` to disable TLS/SSL between ``mongos`` and the LDAP
   server.
...
**option:** --ldapUserToDNMapping <string>
   ...
   *Available in MongoDB Enterprise only.*
   ...
   If :option:`--ldapUserToDNMapping` is unset, ``mongos`` applies no transformations to the username
...
**option:** --ipv6
   ...
   Setting :option:`--ipv6` does *not* direct the ``mongos`` to listen on any
