...
source_branch: "main"
source_path: "content/manual/manual/source/reference/installation-ubuntu-community-troubleshooting.txt"
...
Occurs during the :guilabel:`Import the public key used by the package management system` step of `install-community-ubuntu-pkg`.
...
Occurs after running `sudo apt update` as part of the `install-community-ubuntu-pkg` procedure. Often caused by a missing MongoDB Public GPG key.
...
Repeat the :guilabel:`Import the public key used by the package management system` step in `install-community-ubuntu-pkg`. Copy the command and key exactly as documented.
...
Occurs after running `sudo apt install -y {+package-name-org+}` in step :guilabel:`Create a list file for MongoDB` of the `install-community-ubuntu-pkg` procedure. Often caused by a missing or misconfigured `/etc/apt/sources.list.d/mongodb-org-{+pgp-version+}.list` file.
...
Occurs during `sudo apt install -y {+package-name-org+}` step of the `install-community-ubuntu-pkg` procedure. Often caused by a conflicting |distro-name| `mongodb` package already installed.
...
> **Note:** `sudo apt purge mongodb` removes default configuration files. To preserve
modified configuration files, copy them to another directory (e.g.,
`/home/your-user-name`) before purging.

For mixed packages:

If output shows both `{+package-name-org+}` and `mongodb` packages, remove `{+package-name-org+}` first, then remove `mongodb` packages. After clearing all MongoDB-related packages, retry `install-community-ubuntu-pkg`.

## `mongod` Socket Errors

Occurs when starting a :binary:`mongod <bin.mongod>`. Often caused by another process (typically another :binary:`mongod <bin.mongod>`) using the configured port.
...
1. Modify :setting:`net.port` in the `configuration file <configuration-file>`
2. Use :option:`--port <mongod --port>` on the command line
## Data Directory Errors

Occurs in the :binary:`mongod <bin.mongod>` `process log <monitoring-standard-loggging>`. Often caused by a data directory that doesn't exist or isn't accessible to the :binary:`mongod <bin.mongod>`.
...
Specify the data directory with :setting:`storage.dbPath` in the `configuration file <configuration-file>` or :option:`--dbpath <mongod --dbpath>` on the command line.

- Set default paths:
- `apt` package manager installation: The default `/etc/mongod.conf`
`configuration file <configuration-file>` sets :setting:`storage.dbPath` to `/var/lib/mongodb`

- :binary:`mongod <bin.mongod>` on the command line without
:option:`--dbpath <mongod --dbpath>`: `/data/db`

Ensure that the data directory exists or create the directory  using `mkdir` before starting :binary:`mongod <bin.mongod>`.
