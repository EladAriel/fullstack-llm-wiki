...
generated_at: "2026-08-29T09:39:19.712182Z"
---
...
**meta:** :description: Specify configuration file options for self-managed MongoDB deployments. Configure mongod and mongos instances at startup using YAML-format configuration files.
   :keywords: on-prem, code example
...
The configuration file options on this page apply to MongoDB
{+latest-lts-version+}. For options in other versions, see the
...
``mongos`` command-line options. See :ref:`conf-file-command-line-mapping`.

You can also add comments to the configuration file to
...
The following sample configuration file includes several
:binary:`~bin.mongod` settings:
...
**setting:** systemLog.component.assert.verbosity
   ...
   *Default*: 0
   ...
   error. See :data:`ASSERT` components.
...
**setting:** systemLog.component.query.rejected.verbosity
   ...
   *Default*: 0
   ...
   The log message verbosity level for components related to
   :ref:`rejected query operations <operation-rejection-filters>`. For
   details, see the :data:`REJECTED` component.
...
**setting:** net.port
   ...
   *Default*:
   ...
   - 27018 if ``mongod`` is a :setting:`shard member
...
**setting:** net.maxIncomingConnections
   ...
   *Type*: integer
   ...
   .. versionchanged:: 8.1 (and 8.0.16, 7.0.31)

   | *Default (Windows):* 1,000,000
   | *Default (Linux):* (`RLIMIT_NOFILE
     <https://man7.org/linux/man-pages/man2/getrlimit.2.html>`__ / 2) * 0.8
...
**setting:** net.unixDomainSocket.enabled
   ...
   *Default*: true
   ...
   When ``net.unixDomainSocket.enabled`` is ``true``, :binary:`~bin.mongos` or :binary:`~bin.mongod` listens on the UNIX socket.
...
**setting:** net.tls.certificateKeyFilePassword
   ...
   *Type*: string
   ...
   :binary:`~bin.mongos` or ``mongod`` redacts the
...
**setting:** net.tls.clusterCertificateSelector
   ...
   *Type*: string
   ...
   .. warning:: 
      ...
      ``net.tls.CAFile`` or :setting:`net.tls.clusterCAFile` to specify 
...
**setting:** net.tls.clusterPassword
   ...
   *Type*: string
   ...
   For more information, see ssl-certificate-password.
...
**setting:** net.tls.clusterAuthX509.extensionValue
   ...
   *Type*: string
   ...
   membership extension :abbr:`OID (OpenID)`, ``1.3.6.1.4.1.34601.2.1.2``. The 
...
**setting:** net.tls.CAFile
   ...
   *Type*: string
   ...
   Windows/macOS Only
     If using ``net.tls.certificateSelector`` and/or
...
**setting:** net.tls.clusterCAFile
   ...
   *Type*: string
   ...
   If ``net.tls.clusterCAFile`` does not specify the :file:`.pem`
...
**setting:** net.tls.allowInvalidHostnames
   ...
   *Default*: false
   ...
   For more information about TLS and MongoDB, see 
   configure-mongod-mongos-for-tls-ssl.
...
**setting:** security.keyFile
   ...
   *Type*: string
   ...
   :term:`sharded cluster` or :term:`replica set`. ``keyFile`` implies
...
**setting:** security.authorization
   ...
   *Default*: disabled
   ...
   See :ref:`authorization` for more information.
   

   The ``security.authorization`` setting is available only for :binary:`~bin.mongod`.
...
**setting:** security.transitionToAuth
   ...
   *Default*: false
   ...
   Allows the ``mongod`` or :binary:`~bin.mongos` to accept and 
   ...
   .. note::
      ...
      A ``mongod`` or ``mongos`` running with internal authentication
      and *without* ``security.transitionToAuth`` requires clients to
      connect
      using user access controls. Update clients to
      connect to the ``mongod`` or ``mongos`` using the appropriate :ref:`user <users>`
...
**setting:** security.javascriptEnabled
   ...
   *Default*: true
   ...
   Enables or disables :ref:`server-side JavaScript execution
   <server-side-javascript>`. When disabled, you cannot use
   ...
   ``security.javascriptEnabled`` is available for both
...
**setting:** security.redactClientLogData
   ...
   *Available in MongoDB Enterprise only.*
   ...
   :ref:`security-encryption-at-rest` and
   :ref:`transport-encryption` to assist compliance with
   ...
   On a running ``mongod`` or ``mongos``, use :dbcommand:`setParameter` with the
...
**setting:** security.ldap.servers
   ...
   *Available in MongoDB Enterprise only.*
   ...
   4.1.10 <https://www.rfc-editor.org/rfc/rfc4511.txt>`_. Do not use ``security.ldap.servers``
   ...
   If unset, ``mongod`` or ``mongos`` cannot use :ref:`LDAP authentication or authorization
   <security-ldap>`.
...
**setting:** security.ldap.bind.method
   ...
   * ``sasl`` - ``mongod`` or ``mongos`` uses SASL protocol for authentication
   ...
   using :setting:`security.ldap.bind.saslMechanisms`. ``mongod`` or ``mongos`` defaults to
   ...
   *Available in MongoDB Enterprise only.*
   ...
   ``Linux``
     ...
     - The client keytab includes a
       :ref:`kerberos-user-principal` for the ``mongod`` or ``mongos`` to use when
...
.. _security.ldap.userToDNMapping:
...
.. _set-parameter-search-options:

### ``setParameter`` MongoDB Search Options

**setting:** setParameter.searchIndexManagementHostAndPort
   ...
   *Default*: ""
   ...
   .. note::
      ...
      This parameter must have the same value as :setting:`setParameter.mongotHost`.
...
**setting:** setParameter.skipAuthenticationToSearchIndexManagementServer
...
**setting:** setParameter.mongotHost
   ...
   *Default*: ""
   ...
   .. note::
      ...
      This parameter must have the same value as :setting:`setParameter.searchIndexManagementHostAndPort`.
...
**setting:** setParameter.skipAuthenticationToMongot
...
**setting:** setParameter.useGrpcForSearch
...
**setting:** setParameter.searchTLSMode
   ...
   *Default*: ``globalTLS``
   ...
   the setting that you specified in :setting:`net.tls.mode`, 
...
**setting:** storage.dbPath
   ...
   *Default*:
   ...
   .. note:: Configuration Files
      ...
      package manager installations uses these platform-specific
...
**setting:** storage.syncPeriodSecs
   ...
   **Do not set this value on
   ...
   :ref:`journaling-internals`, but if ``storage.syncPeriodSecs`` is 
...
**setting:** storage.engine
   ...
   *Default*: ``wiredTiger``
   ...
   .. list-table::
      ...
      * - ``wiredTiger``
        - To specify the :ref:`storage-wiredtiger`.
   
      * - ``inMemory``
   
        - To specify the :ref:`storage-inmemory`.
...
**setting:** storage.oplogMinRetentionHours
   ...
   *Type*: double
   ...
   - The ``mongod`` compares the system wall clock to an 
     oplog entry creation :term:`wall clock time` when enforcing oplog entry
...
**setting:** storage.wiredTiger.engineConfig.cacheSizeGB
   ...
   *Type*: float
   ...
   .. include:: /includes/extracts/wt-cache-default-values-mongod-config.rst
...
**setting:** storage.wiredTiger.engineConfig.cacheSizePct
   ...
   *Type*: float
   ...
   .. include:: /includes/extracts/wt-cache-pct-additional-constraints-mongod-config.rst
...
**setting:** storage.wiredTiger.engineConfig.directoryForIndexes
   ...
   *Default*: false
   ...
   :binary:`~bin.mongod` stores indexes and collections in separate
...
**setting:** storage.wiredTiger.engineConfig.zstdCompressionLevel
   ...
   *Changed in version 8.2*
   ...
   or :setting:`~storage.wiredTiger.engineConfig.journalCompressor` (or both) are
   set to ``zstd``.
...
**setting:** storage.inMemory.engineConfig.inMemorySizeGB
   ...
   *Default*: 50% of physical RAM less 1 GB
   ...
   Maximum amount of memory to allocate for :ref:`in-memory storage
   engine <storage-inmemory>` data, including indexes, oplog if the
...
**setting:** operationProfiling.mode
   ...
   *Default*: ``off``
   ...
   Specifies which operations should be :ref:`profiled
   <database-profiler>`.
    ...
    * - ``off``
      - The profiler is off and does not collect any data.
        ...
        to profiler :ref:`level 0 <database-profiling-level>`.
...
.. _configuration-profiling-slowOpInProgressThresholdMs:
**setting:** operationProfiling.slowOpInProgressThresholdMs
   ...
   *Default*: 5000
   ...
   .. include:: /includes/slowOpInProgressThreshold-defintion.rst
...
**setting:** replication.oplogSizeMB
   ...
   *Type*: integer
   ...
   :dbcommand:`replSetResizeOplog`. ``replSetResizeOplog``
...
**setting:** auditLog.destination
   ...
   *Type*: string
   ...
   When set, ``auditLog.destination`` enables :ref:`auditing <auditing>` and
...
**setting:** auditLog.filter
   ...
   *Type*: string representation of a document
   ...
   <audit-action-details-results>` the audit system records. The
   option takes a string representation
   ...
   The ``<field>`` can be :ref:`any field in the audit message
...
**setting:** auditLog.format
   ...
   *Type*: string
   ...
   The format of the output file for auditing if ``destination`` is
   ``file``. The
...
.. _mongot-configuration-options:

## ``mongot``\ Options

To view the options to use for configuring ``mongot`` with ``mongod``, 
see :ref:`Configure mongot <mongot-config-settings>`.
...
**setting:** replication.localPingThresholdMs
   ...
   *Default*: 15
   ...
   :term:`secondary` members, the ``mongos``:
