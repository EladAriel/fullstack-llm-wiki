...
generated_at: "2026-08-29T09:39:19.694655Z"
---
.. _explain-results:

# Explain Results

**meta:** :description: Understand how to interpret MongoDB's explain results for query plans and execution statistics, including key fields and stages.
...
**note:** .. include:: /includes/cbr-default-plan.rst
   ...
   For more information, see :ref:`query-plan-options`.
...
**tabs:** tabs:
      ...
      - id: cbr
        name: "Cost-Based Ranker"
        content: |
            If the query optimizer uses the cost-based ranker, the output
            contains several additional fields. 
...

               {
                  queryPlanner: {
                     ...
                     winningPlan: { 
                        ...
                        stage: <STAGE1>,
                        costEstimate: <float>,
                        cardinalityEstimate: <float>,
                        estimatesMetadata: { ceSource: '<string>' },
                        inputStage: {
                          stage: <STAGE1>,
                          costEstimate: <float>,
                          cardinalityEstimate: <float>,
                          filterNumKeysEstimate: <float>,
                          numKeysEstimate: <float>,
                          estimatesMetadata: { ceSource: '<string>'} ,
                          ...
                        }
                     }
...
**data:** explain.queryPlanner
   ...
   .. data:: explain.queryPlanner.winningPlan.costEstimate

      Float that represents the estimated abstract cost units used to execute
      the cost-based ranker per stage. The cost-based ranker chooses the plan
      with the lowest top level ``costEstimate``.

      .. versionadded:: 8.3.3

   .. data:: explain.queryPlanner.winningPlan.cardinalityEstimate

      Float that represents the estimated number of documents or keys produced
      by the stage of the cost-based ranker.

      .. versionadded:: 8.3.3

   .. data:: explain.queryPlanner.winningPlan.numKeysEstimate

      Float that represents the estimated number of index keys that the query
      planner predicts the plan to process.

      A significant difference between the ``numKeysEstimate`` and the
      ``costEstimate`` can indicate issues with the ``winningPlan``.

      .. versionadded:: 8.3.3

   .. data:: explain.queryPlanner.winningPlan.numDocsEstimate

      Float that represents the estimated number of documents scanned.

      .. versionadded:: 8.3.3

   .. data:: explain.queryPlanner.winningPlan.estimatesMetadata

      Contains metadata about how the estimate was derived.

      .. versionadded:: 8.3.3

   .. data:: explain.queryPlanner.winningPlan.estimatesMetadata.ceSource

      The cardinality estimation method (CE) used for this stage. Possible 
      float values include:

      - ``sampling``: MongoDB calculates the estimate over a sample of the
        base collection.
      - ``heuristics``: Uses a set of rules to determine selectivity.
        Only used when sampling fails. 
      - ``mixed``: The estimate is a result of some computation that mixes
        estimates of different types.
      - ``metadata``: The estimate comes from database metadata like collection
        cardinality. 
      - ``code``: C++ computes the estimate directly, for instance a
...
**data:** explain.executionStats
   ...
   *not* apply the modifications to the database.
   ...
   .. data:: explain.executionStats.peakTrackedMemBytes

      .. include:: /includes/peak-memory.rst

      The explain execution statistics provide the peak memory for the
      entire query and also for each blocking stage. If there is only
      one blocking stage, the query and stage ``peakTrackedMemBytes``
      values are the same.
   ...
   .. data:: explain.executionStats.score

      Double that represents the score of the winning plan chosen by the 
      multi-planner. 
   ...
   .. data:: explain.executionStats.executionStages
      ...
      .. data:: explain.executionStats.executionStages.inputStage
         ...
         .. list-table::                                    
            ...
            * - ``spilledBytes``
              - .. _explain-results-spilledBytes-table:
            ...
            * - ``spilledRecords``
              - .. _explain-results-spilledRecords-table:
            ...
            * - ``spilledDataStorageSize``
              - .. _explain-results-spilledDataStorageSize-table:
   ...
   .. data:: explain.executionStats.operationMetrics
      ...
      .. warning::
         ...
         Therefore, code depending on :ref:`operationMetrics <executionStats>`
         does not work. 
...
.. _explain-results-search:

## Execution Plan Statistics for Search Queries

**versionadded:** 8.1

The explain results includes :ref:`execution statistics <executionStats>` for 
queries that use a :pipeline:`$search`, :pipeline:`$searchMeta`, or 
:pipeline:`$vectorSearch` pipeline stage. To include execution statistics for 
...
stage that is **not** a descendant of a ``FETCH`` stage.
...
When ``explain`` is run in either :ref:`executionStats
<explain-method-executionStats>` or :ref:`allPlansExecution
<explain-method-allPlansExecution>` verbosity mode, certain aggregation
stages that may spill to disk have additional output. These stages
include :pipeline:`$sort`, :pipeline:`$group`,
:pipeline:`$setWindowFields`, :pipeline:`$bucketAuto`,
:pipeline:`$graphLookup`, and :pipeline:`$lookup` (when using the
...
.. list-table::
   ...
   * - :pipeline:`$sort`, :pipeline:`$group`, :pipeline:`$setWindowFields`,
       :pipeline:`$bucketAuto`, :pipeline:`$graphLookup`,
       :pipeline:`$lookup` (SBE only)
   ...
   * - :pipeline:`$sort`, :pipeline:`$group`, :pipeline:`$setWindowFields`,
     ...
     - An estimate of the number of bytes written to disk before
       compression.
