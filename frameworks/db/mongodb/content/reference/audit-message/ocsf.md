...
generated_at: "2026-08-29T09:39:20.005895Z"
---
...
.. list-table::
   ...
   * - ``src_endpoint``
     ...
     - .. include:: /includes/client-id-audit-logs.rst
       ...
       The ``direct_endpoint`` field records the host that opens the
   ...
   * - ``direct_endpoint``
     - Document
     - IP address and port of the host that connects directly to the
       MongoDB instance. If the client connects through a Unix domain
       socket, ``direct_endpoint`` stores an ``interface`` field set to
       ``unix`` and omits the ``port`` field.
