...
generated_at: "2026-08-29T09:39:20.005166Z"
---
.. _event-audit-messages-mongo:
...
.. code-block:: none

   {
     ...
     directRemote: { ip: <string>, port: <int> || unix: <string> },   // Added in MongoDB 8.3.9
...
.. list-table::
   ...
   * - ``intermediates``
     ...
     - .. include:: /includes/client-id-audit-logs.rst
       ...
       .. include:: /includes/client-id-audit-log-fields.rst
     
       Each array element in ``intermediates`` is a document with ``ip``
       and ``port`` fields, which are either the IP address and port of
       a load balancer or ``mongos`` that the origin client computer
       request passes through.

       If the origin client computer request passes through a load
       balancer:

       - Starting in MongoDB 8.1, the ``remote`` field stores the client
         computer IP address and port that are read from the
         proxy protocol header. The ``intermediates`` document contains
         the load balancer IP address and port.
       - In MongoDB versions earlier than 8.1, the ``remote`` field
         stores the load balancer IP address and port. The
         ``intermediates`` document isn't in the audit log.

       If the audit event occurs on a shard:
       
       - Starting in MongoDB 8.1, the ``remote`` field stores the client
         computer IP address and port. The address and port are read
         from the proxy protocol header or, if the client computer
         connects to ``mongos``, the address and port are read from the
         origin client computer connection. The ``intermediates``
         document stores the ``mongos`` IP address and port. If a load
         balancer is used, the ``intermediates`` document also stores
         the load balancer IP address and port.
       - In MongoDB versions earlier than 8.1, the ``remote`` field
         stores the ``mongos`` IP address and port. The
         ``intermediates`` document isn't in the audit log.
   ...
   * - ``remote``
     - document
     - A document that stores the ``ip`` address and the ``port``
       ...
       The ``directRemote`` field records the host that opens the
       connection when a load balancer or reverse proxy asserts the
       client address.
   ...
   * - ``directRemote``
     - document
     - A document that stores the ``ip`` address and the ``port``
       number of the host that connects directly to the instance. If the
       client connects through a Unix domain socket, the document stores
       a ``unix`` field instead of the ``ip`` and ``port`` fields.

       .. include:: /includes/audit-direct-remote.rst
...
.. list-table::
   ...
   * - .. _audit-message-authCheck:
     ...
     - .. code-block:: none
       ...
       Logs authorization checks for user attempts to read from or
