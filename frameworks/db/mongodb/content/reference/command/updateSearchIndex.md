...
generated_at: "2026-08-29T09:39:20.078125Z"
---
# updateSearchIndex (database command)

**meta:** :description: Update an existing {+fts+} index using the `updateSearchIndex` command, specifying the index's name or ID and new definition.
...
.. list-table::
   ...
   * - ``type``
     - string
     - Optional
     - Type of the index. Value must be one of the following options: 
   
       - ``search`` - for |fts| index. 
       - ``vectorSearch`` - for {+avs+} index.

       The value that you specify must match the type of index that you
       want to update. You can't change the index type.

       This field is not supported for updating an index on a collection
