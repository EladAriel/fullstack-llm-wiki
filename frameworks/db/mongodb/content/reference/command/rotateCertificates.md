...
source_branch: "main"
source_path: "content/manual/manual/source/reference/command/rotateCertificates.txt"
...
The :dbcommand:`rotateCertificates` command returns a document with the following field:
...
#. Run the :dbcommand:`rotateCertificates` command to rotate the certificates used by the :binary:`~bin.mongod` or :binary:`~bin.mongos` instance.
...
If you have configured :parameter:`OCSP <ocspEnabled>` for your deployment, the :dbcommand:`rotateCertificates` command will also fetch stapled OCSP responses during rotation.

The :dbcommand:`rotateCertificates` command may be run on a running :binary:`~bin.mongod` or :binary:`~bin.mongos` regardless of replication status.
...
If the :binary:`~bin.mongod` or :binary:`~bin.mongos` is running with :option:`--tlsCertificateSelector <mongod --tlsCertificateSelector>` set to `thumbprint`, :dbcommand:`rotateCertificates` will fail and write a warning message to the log file.
