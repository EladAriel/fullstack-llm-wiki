...
If you terminate `compact` with the :method:`db.killOp() <db.killOp()>` method or restart the server before the operation finishes, `compact` ends. The operation may not release disk space back to the operating system.
...
run `compact` on secondaries, see the `force <compact-force-option>` option.
