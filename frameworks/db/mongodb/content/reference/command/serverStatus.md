...
generated_at: "2026-08-29T09:39:20.040675Z"
---
# serverStatus (database command)

**meta:** :description: Retrieve an overview of the database state, query, index, and operation statistics, and the health of the MongoDB instance by running the serverStatus command.
...
The value (that is, ``1`` above) does not affect the operation of
the command. The ``db.serverStatus()`` command returns a large amount of
...
This command always returns a value, even on a fresh database. The
...
**serverstatus:** asserts.user
   ...
   or deployment. Server logs may have limited information about "user asserts."
   To learn more information about the source of "user asserts," check the 
...
A document that reports metrics related to the :ref:`internal storage of
time series collections <timeseries-bucket-catalog>`. 
...
.. list-table::
  ...
  * - ``numOpenBuckets``
    ...
    - The number of tracked buckets with a full representation stored 
      in-cache, ready to receive new documents.

  * - ``numIdleBuckets``

    - The number of buckets that are open and currently without an uncommitted 
      document insertion pending. A subset of numOpenBuckets.

  * - ``numArchivedBuckets`` 

    - The number of tracked buckets with a minimal representation stored 
  ...
  * - ``numBucketsClosedDueToSchemaChange``
    ...
    - The number of buckets closed because the schema of an incoming document
      was incompatible with that of the documents in the open bucket.
  
  * - ``numBucketsClosedDueToSize``
  
    - The number of buckets closed because an incoming document would make the
  ...
  * - ``numBucketsClosedDueToMemoryThreshold``
    ...
    - The number of buckets closed because the set of active buckets didn't
  ...
  * - ``numMeasurementsGroupCommitted``
    ...
    - The number of commits that included measurements from concurrent insert 
      commands.

  * - ``numWaits``

    - The number of times an operation waited on another thread to either 
  ...
  * - ``numBucketsClosedDueToReopening``
    ...
    - The number of buckets closed because a suitable bucket was re-opened 
      instead.

  * - ``numBucketsArchivedDueToMemoryThreshold``
    
    - The number of buckets archived because the set of active buckets didn't
      fit within the allowed bucket catalog cache size.
  ...
  * - ``numBucketsReopened``
    ...
    - The number of buckets re-opened because a document arrived that didn't
      match any open buckets, but did match an existing non-full bucket.

  * - ``numBucketsKeptOpenDueToLargeMeasurements``

    - The number of buckets that would have been closed due size, but were kept
  ...
  * - ``numBucketsClosedDueToCachePressure``
    ...
    - The number of buckets closed because their size exceeds the bucket
      catalog's dynamic :ref:`bucket size limit <timeseries-bucket-closure>`
      derived from available storage engine cache size and ``numBuckets``. This
      limit is distinct from the maximum bucket size limit.

  * - ``numBucketsFrozen``

    - The number of frozen buckets. Buckets are frozen if attempting to 
  ...
  * - ``numBucketsFetched``
    ...
    - The number of archived buckets fetched to check if they were suitable
      for re-opening.
  
  * - ``numBucketsQueried``

    - The total number of buckets queried to see if they could hold an  
      incoming document.
  
  * - ``numBucketFetchesFailed``

    - The number of archived buckets fetched that were not suitable for 
      re-opening.
  
  * - ``numBucketQueriesFailed``

    - The number of queries for a suitable open bucket that failed due to
      lack of candidate availability.
  
  * - ``numBucketReopeningsFailed``

    - The number of attempted bucket reopenings that failed due to reasons 
      including conflicts with concurrent operations, malformed buckets, and 
      more.
  
  * - ``numDuplicateBucketsReopened``

    -  The number of re-opened buckets that are duplicates of currently open
       buckets.
  ...
  * - ``stateManagement.bucketsManaged``
    ...
    - The total number of buckets that are being tracked for conflict
      management. This includes open buckets in the bucket catalog and
      any buckets that are being directly written to, including by
      update and delete commands.
  
  * - ``stateManagement.currentEra``

    - The current era of the bucket catalog. The bucket catalog starts
      at era 0 and increments when a bucket is cleared. Attempting to insert
      into a bucket either causes it to be removed if it was cleared, or
  ...
  * - ``stateManagement.trackedClearOperations``
    ...
    - The number of times the a set of buckets has been cleared, but the 
...
You can also use the :pipeline:`$collStats` aggregation pipeline stage to find time series
metrics. To learn more, see :ref:`storage-stats-time-series`. 
...
**serverstatus:** catalogStats.systemProfile

   The total number of :data:`~<database>.system.profile` collections on all databases.
...
**serverstatus:** changeStreamPreImages.purgingJob
   ...
   .. list-table::
      ...
      * - ``maxTimestampEligibleForTruncate``
        - Most recent timestamp up to which old pre-images can be
          truncated to reduce storage space. Pre-images older than
          ``maxTimestampEligibleForTruncate`` can be truncated.
   
          .. versionadded:: 8.1

      * - ``maxStartWallTimeMillis``
        - Maximum :term:`wall clock time` in milliseconds from the first document of each 
...
**serverstatus:** connections.current
   ...
   server. This includes connections from the current shell session
   and from other servers, such as :term:`replica set` members and
   :binary:`~bin.mongos` instances. Consider the value of
   :serverstatus:`connections.available` for additional context.
...
**serverstatus:** connections.available
   ...
   load on the database, and the :ref:`ulimit` document for
...
**serverstatus:** connections.queuedForEstablishment
   ...
   .. versionadded:: 8.2 *(also available in 8.1.1, 8.0.12, and 7.0.23)*

   The number of incoming connections currently queued and waiting for
   establishment. This metric is relevant when connection establishment
   rate limiting is enabled using the :parameter:`ingressConnectionEstablishmentRateLimiterEnabled`
   parameter.
...
**serverstatus:** connections.establishmentRateLimit
   ...
   A document that contains metrics related to the ingress connection establishment 
   rate limiter. These metrics provide insights into how the rate limiter handles
   connection requests when :parameter:`ingressConnectionEstablishmentRateLimiterEnabled` 
   is set to ``true``. For more information on rate limiting, see :ref:`configure-rate-limiter`.

**serverstatus:** connections.establishmentRateLimit.rejected

   .. versionadded:: 8.2 *(also available in 8.1.1, 8.0.12, and 7.0.23)*

   The number of incoming connections the server rejects due to connection establishment
   rate limiting. This metric shows how many connection attempts the server rejected rejected
   because they exceeded the rate limits set by the ingress connection
   establishment rate limiter parameters.

**serverstatus:** connections.establishmentRateLimit.exempted

   .. versionadded:: 8.2 *(also available in 8.1.1, 8.0.12, and 7.0.23)*
   
   The number of incoming connections that bypassed the rate limiter because they 
   originated from IP addresses or CIDR ranges specified in the 
   :parameter:`ingressConnectionEstablishmentRateLimiterBypass` parameter. The server 
   does note rate limit these connections and establishes them immediately,
   regardless of current queue size or rate limits.

**serverstatus:** connections.establishmentRateLimit.interruptedDueToClientDisconnect

   .. versionadded:: 8.2 *(also available in 8.1.1, 8.0.12, and 7.0.23)*
   
   The number of incoming connections that were interrupted while waiting in the 
   establishment queue because the client disconnected before establishment could 
   complete. A high value for this metric sometimes indicates that the client's 
   ``connectTimeoutMS`` setting is too short relative to the queue wait time, which 
   is affected by :parameter:`ingressConnectionEstablishmentMaxQueueDepth` and 
   :parameter:`ingressConnectionEstablishmentRatePerSec`. If this value is high, 
   consider adjusting these parameters using the following formula: 
   ``maxQueueDepth < (establishmentRatePerSec / 1000) * (connectTimeoutMs - avgEstablishmentTimeMs)``.
...
**serverstatus:** defaultRWConcern.updateWallClockTime
   ...
   The :term:`wall clock time` when the instance last updated its copy of any
...
**serverstatus:** defaultRWConcern.localUpdateWallClockTime
   ...
   The local system :term:`wall clock time` when the instance last updated its
...
**serverstatus:** globalLock.currentQueue.total
   ...
   The total number of operations queued waiting for the lock (that is,
...
**serverstatus:** globalLock.activeClients.total
   ...
   This metric is higher than the total of ``activeClients.readers``
...
.. _server-status-ftdcCollectionMetrics:
...
**serverstatus:** indexStats

   A document that reports statistics on all indexes on databases and collections 
   in non-system namespaces only. ``indexStats`` does not report statistics 
   on indexes in the ``admin``, ``local``, and ``config`` databases.

   .. versionadded:: 6.0

**serverstatus:** indexStats.count

   The total number of indexes.

   .. versionadded:: 6.0

**serverstatus:** indexStats.features

   A document that provides counters for each index type and the number of 
   accesses on each index. Each index type under ``indexStats.features`` 
   has a ``count`` field that counts the total number of indexes for that 
   type, and an ``accesses`` field that counts the number of accesses on that 
   index.
...
**serverstatus:** uptimeEstimate
   ...
   coarse-grained time keeping system.
...
.. _server-status-lockContentionMetrics:
...
**serverstatus:** lockContentionMetrics

   A document that shows how often threads use certain internal
   server locks and how often these locks cause threads to wait.
   This document focuses on a small set of important, long-lived locks that
   coordinate core server activities, such as global server state,
   replication, and connection management. Contention on these
   locks has a noticeable impact on performance. Use these metrics
   to identify when coordination points become bottlenecks, such as
   when contention and wait time increase alongside slower
   operations or replication lag.

   By default, ``serverStatus`` does not include ``lockContentionMetrics`` in its output. Use
   the following syntax to include ``lockContentionMetrics`` in your output:
...
**serverstatus:** lockContentionMetrics.<tag>.exclusive

   Aggregated statistics for exclusive or write acquisitions of all mutex instances 
   that use ``<tag>``.

**serverstatus:** lockContentionMetrics.<tag>.exclusive.total

   Counts how many times this lock runs in write mode, blocking other threads until the write finishes.

**serverstatus:** lockContentionMetrics.<tag>.exclusive.contentions

   Counts how many write‑mode lock requests wait because another operation holds the lock.
   
**serverstatus:** lockContentionMetrics.<tag>.exclusive.waitCycles

   Total CPU cycles spent waiting for contended write‑mode lock requests on ``<tag>``.

**serverstatus:** lockContentionMetrics.<tag>.shared 

   Aggregated statistics for shared or read acquisitions of all mutex instances 
   that use ``<tag>``.

**serverstatus:** lockContentionMetrics.<tag>.shared.total

   Total number of read‑mode lock acquisitions on ``<tag>``.

**serverstatus:** lockContentionMetrics.<tag>.shared.contentions

   Number of read‑mode lock requests that encounter contention on ``<tag>``.

**serverstatus:** lockContentionMetrics.<tag>.shared.waitCycles

   Total CPU cycles spent waiting for contended read‑mode lock requests on ``<tag>``.

**serverstatus:** lockContentionMetrics.<tag>.mutexes
...
**serverstatus:** logicalSessionRecordCache
   ...
   Provides metrics around the caching of
...
**serverstatus:** logicalSessionRecordCache.sessionsCollectionJobCount
   ...
   The number of times the refresh process has run on the
   :data:`config.system.sessions` collection.
...
**serverstatus:** metrics.abortExpiredTransactions.successfulKills
   ...
   .. include:: /includes/session-pool.rst

   .. versionadded:: 8.1 *(also available in 8.0.13)*

**serverstatus:** metrics.abortExpiredTransactions.timedOutKills

   Number of expired transactions unsuccessfully ended by MongoDB
   because it timed out when attempting to check out a session.
...
**serverstatus:** metrics.apiVersions
   ...
   - API version metrics are retained for 24 hours. If
     ...
     are removed from the metrics. This also applies to the
   ...
   - If no ``appname`` is configured, a default value is
...
**serverstatus:** metrics.commands
   ...
   in :serverstatus:`metrics.commands` are the names of
   :ref:`database commands <database-commands>`. For each command, the
   :dbcommand:`serverStatus` reports the total number of executions and
   the number of failed executions.

   :serverstatus:`metrics.commands` includes
   ``replSetStepDownWithForce`` (that is, the :dbcommand:`replSetStepDown`
...
**serverstatus:** metrics.extension
   ...
   A document that reports information on the MongoDB Extension API for
   aggregation stages.
...
**serverstatus:** metrics.getLastError.wtime.num
   ...
   (that is, ``w``) that wait for one or more members of a replica set
   to acknowledge the write operation (that is, a ``w`` value greater
   than ``1``.)
...
**serverstatus:** metrics.getLastError.wtime.totalMillis
   ...
   <write-concern>` operations with a write concern (that is, ``w``)
   that waits for one or more members of a replica set to acknowledge
   the write operation (that is, a ``w`` value greater than ``1``.)
...
**serverstatus:** metrics.network.averageTimeToCompletedTLSHandshakeMicros
   ...
   .. versionadded:: 8.2 *(also available in 8.1.1)*

   The average time in microseconds that it takes to complete a TLS handshake
...
**serverstatus:** metrics.network.averageTimeToCompletedHelloMicros
   ...
   The time in microseconds between the beginning of connection 
   establishment and the completion of the ``hello`` command. You can use
   this metric to tune the :parameter:`ingressConnectionEstablishmentMaxQueueDepth`
   and :parameter:`ingressConnectionEstablishmentRatePerSec` to ensure that there is proper 
   time allotted to complete the connection establishment after exiting the queue. 

**serverstatus:** metrics.network.averageTimeToCompletedAuthMicros

   .. versionadded:: 8.2 *(also available in 8.1.1)*

   The time in microseconds the SASL auth exchange takes to be completed after 
   the beginning of connection establishment.
...
**serverstatus:** metrics.operation.interrupt

   A document that holds counters for server interrupt checks.

   .. versionadded:: 8.3 Also available in 8.2.2 and 8.0.20.

**serverstatus:** metrics.operation.interrupt.totalChecks

   Total number of interrupt checks performed since the
   server started.
...
**serverstatus:** metrics.operation.killedDueToRangeDeletion
   ...
   Total number of operations terminated because of orphan range cleanup. 
   To learn more, see :parameter:`terminateSecondaryReadsOnOrphanCleanup`.
...
**serverstatus:** metrics.query.bucketAuto.spilledBytes
   ...
   The number of in-memory bytes spilled to disk by the :pipeline:`$bucketAuto`
   stage.
...
**serverstatus:** metrics.query.bucketAuto.spilledDataStorageSize
   ...
   The total disk space, in bytes, used by the spilled data from the 
   :pipeline:`$bucketAuto` stage.
 
   .. versionadded:: 8.2

**serverstatus:** metrics.query.bucketAuto.spilledRecords

   The number of records spilled to disk by the :pipeline:`$bucketAuto` stage.

   .. versionadded:: 8.2

**serverstatus:** metrics.query.bucketAuto.spills

   The number of times the :pipeline:`$bucketAuto` stage spilled to disk.

   .. versionadded:: 8.2

**serverstatus:** metrics.query.cbr

   A document that holds metrics on the cost-based ranking
   (CBR) of query plans.
...
**serverstatus:** metrics.query.cbr.histograms.micros
   ...
   A histogram measuring the amount of time in microseconds
   spent on cost-based ranking query plans, which includes both
   sampling and ranking time.

   .. versionadded:: 8.3.3

**serverstatus:** metrics.query.cbr.histograms.numPlans

   A histogram measuring the number of plans in the candidate
   set used by the cost-based ranker.

   .. versionadded:: 8.3.3

**serverstatus:** metrics.query.cbr.histograms.samplingMicros

   A histogram measuring the amount of time in microseconds the
   server spent on sampling during cost-based ranking of query
   plans.
...
**serverstatus:** metrics.query.cbr.micros
   ...
   Amount of time in microseconds spent on cost-based ranking
   query plans, which includes both sampling and ranking time.
...
**serverstatus:** metrics.query.cbr.numPlansFailedCostEstimation
   ...
   Number of candidate plans for which the cost-based ranker
   could not estimate a cost.
...
**serverstatus:** metrics.query.cbr.samplingMicros
   ...
   Amount of time in microseconds the server spent on sampling
   during cost-based ranking of query plans.
...
**serverstatus:** query.planning.fastPath.express 
   ...
   The number of queries that use an optimized index scan plan consisting of 
   one of the following plan stages: 
...
**serverstatus:** query.planning.fastPath.idHack 
   ...
   The number of queries that contain the ``_id`` field. For these queries, 
   MongoDB uses the default index on the ``_id`` field and skips all query plan 
   analysis. 
...
**serverstatus:** metrics.queryExecutor.collectionScans.total
   ...
   consists of queries that did and did not use a
   :ref:`tailable cursor <tailable-cursors-landing-page>`.

**serverstatus:** metrics.queryExecutor.profiler.collectionScans.nonTailable

   The number of queries that performed a collection scan on a
   :data:`~<database>.system.profile` collection that did not use a
   :ref:`tailable cursor <tailable-cursors-landing-page>`.

**serverstatus:** metrics.queryExecutor.profiler.collectionScans.tailable

   The number of queries that performed a collection scan on a
   :data:`~<database>.system.profile` collection that used a
   :ref:`tailable cursor <tailable-cursors-landing-page>`.

**serverstatus:** metrics.queryExecutor.profiler.collectionScans.total

   The total number of queries that performed a collection scan on a 
   :data:`~<database>.system.profile` collection. This includes queries that
   used both tailable and non-tailable cursors.
...
**serverstatus:** metrics.repl.apply.batchSize
   ...
   The total number of oplog entries applied. The
   :serverstatus:`metrics.repl.apply.batchSize` is incremented with the
   number of oplog entries in a batch at the batch boundaries instead of
   being incremented by one after each entry.
...
**serverstatus:** metrics.repl.apply.ops
   ...
   operation, including operations that derive from oplog entries during
   application, such as updates to internal metadata collections like 
   ``config.transactions``.
...
**serverstatus:** metrics.repl.network.oplogFetcherHighestFetchedOptime

   .. versionadded: 8.3

   The highest optime that the oplog fetcher has fetched from its sync source in
...
**serverstatus:** metrics.repl.network.oplogFetcherLagSeconds
   ...
   The replication fetch lag, in seconds, between the sync source’s last applied
   optime and the oplog fetcher’s highest fetched optime on this node.
...
**serverstatus:** metrics.repl.network.readersCreated
   ...
   The total number of oplog query processes created. MongoDB
   creates a new oplog query any time an error occurs in the
   connection, including a timeout, or a network operation. This
   value also increments every time MongoDB selects a new source
   for replication.
...
**serverstatus:** metrics.repl.timestamps.oldestTimestamp
   ...
   The timestamp for the oldest :term:`snapshot`. A snapshot is a
   copy of the data in a :binary:`mongod` instance at a specific point
   in time.
...
**serverstatus:** metrics.repl.waiters.replCoordMutexTotalWaitTimeInOplogServerStatusMillis

   The average wait time in milliseconds to acquire the replication coordinator mutex.
   MongoDB measures this time when it generates the server status oplog section. This 
   metric helps you identify potential replication performance issues related to mutex 
   contention.
...
**serverstatus:** metrics.ttl.durationMicros
   ...
   The cumulative amount of time, in microseconds, that the TTL 
   monitor has spent deleting expired documents and index keys. 
...
**serverstatus:** metrics.ttl.invalidTTLIndexSkips
   ...
   Number of TTL deletes skipped due to a TTL :term:`secondary index 
   <secondary index>` being present, but not valid for TTL deletion.
   ...
   If there is an invalid secondary TTL index, you must manually modify 
   the secondary index to use automatic TTL deletion. 
...
**serverstatus:** mirroredReads
   ...
   .. serverstatus:: mirroredReads.processedAsSecondary
      ...
      The number of mirrored reads processed by this member
      while a secondary. For more information, see the
   ...
   .. serverstatus:: mirroredReads.seen
      ...
      <mirrored-reads-supported-operations>` received by this
      member. For more information, see the
      :parameter:`mirrorReads` parameter.
...
**serverstatus:** network
   ...
   statistics measure ingress and egress connections, specifically the
...
**serverstatus:** network.egress

   Reports data on the traffic from egress connections initiated by this 
   ``mongod`` or ``mongos`` instance. In most cases, the egress connections 
   are ``mongos`` communicating with ``mongod`` for sharding, or ``mongod`` 
   communicating with ``mongod`` for replication. It is also possible that 
   ``mongod`` or ``mongos`` are communicating with external services, 
   such as ``mongot``.

**serverstatus:** network.egress.bytesIn

   The total number of logical bytes that ``mongod`` 
   or ``mongos`` instances have *received* over network connections that they have 
   initiated to other nodes/services. Logical bytes are the exact number 
   of bytes that a given file contains.

**serverstatus:** network.egress.bytesOut

   The total number of logical bytes that ``mongod`` or
   ``mongos`` instances have *sent* over network connections that they have 
   initiated to other nodes/services. Logical bytes correspond to the number 
   of bytes that a given file contains.

**serverstatus:** network.egress.physicalBytesIn

   The total number of physical bytes that ``mongod`` or
   ``mongos`` instances has *received* over network connections that they have 
   initiated to other nodes/services. Physical bytes are the number
   of bytes that actually reside on disk.

**serverstatus:** network.egress.physicalBytesOut

   The total number of physical bytes that ``mongod`` or
   ``mongos`` instances have *sent* over network connections that they have 
   initiated to other nodes/services. Physical bytes are the number of bytes that actually reside on disk.

**serverstatus:** network.egress.numRequests

   The total number of distinct requests that ``mongod`` or ``mongos`` have sent and
   received responses to. Use this value to provide context for the
   :serverstatus:`network.egress.bytesIn` and :serverstatus:`network.egress.bytesOut`
...
**serverstatus:** network.bytesIn
   ...
   The total number of logical bytes that the server has *received* over 
   network connections initiated by clients or other ``mongod`` or
   ``mongos`` instances. Logical bytes are the exact number of bytes
   that a given file contains.

**serverstatus:** network.bytesOut

   The total number of logical bytes that the server has *sent* over 
   network connections initiated by clients or other ``mongod`` or
   ``mongos`` instances. Logical bytes correspond to the number of 
   bytes that a given file contains.

**serverstatus:** network.physicalBytesIn

   The total number of physical bytes that the server has *received* 
   over network connections initiated by clients or other ``mongod`` or
   ``mongos`` instances. Physical bytes are the number of bytes that 
   actually reside on disk.

**serverstatus:** network.physicalBytesOut

   The total number of physical bytes that the server has *sent* over 
   network connections initiated by clients or other ``mongod`` or
   ``mongos`` instances. Physical bytes are the number of bytes that 
   actually reside on disk.
...
**serverstatus:** opLatencies
   ...
   See :ref:`latency-stats-document` for a description of this document.
   ...
   To include the histogram in the ``opLatencies`` output, run the following 
   command: 
...
**serverstatus:** opWorkingTime
   ...
   Document that includes information on operation execution for the instance. 
   See :ref:`latency-stats-document` for a description of this document.

   The fields under ``opWorkingTime`` are measured in ``workingMillis``, which 
   is the amount of time that MongoDB spends working on that operation. This 
   means that factors such as waiting for locks and flow control don't affect 
   ``opWorkingTime``. 

   To include the histogram in the ``opWorkingTime`` output, run the following 
   command: 
...
**serverstatus:** opWriteConcernCounters
   ...
   A document that reports on the :ref:`write concerns <write-concern>`
   specified by write operations to the
   :binary:`~bin.mongod` instance since it last started.
...
**serverstatus:** opcountersRepl
   ...
   These values differ from the :serverstatus:`opcounters` values
   because of how MongoDB serializes operations during replication.
   See :ref:`replication` for more information on replication.

   These numbers grow over time in response to database use until
...
**serverstatus:** oplogTruncation
   ...
   replica set and uses either the :ref:`storage-wiredtiger` or
   :ref:`storage-inmemory`.

   Available in the :ref:`storage-wiredtiger`.
...
**serverstatus:** oplogTruncation.totalTimeProcessingMicros
   ...
   data files (that is, not meaningful for :ref:`storage-inmemory`).
...
**serverstatus:** oplogTruncation.processingMethod
   ...
   (that is, not meaningful for :ref:`storage-inmemory`).
...
**serverstatus:** planCache.totalQueryShapes
   ...
   Approximate number of :term:`plan cache query shapes <plan cache query shape>`

   Prior to version 7.2, information on the number of plan cache query shapes was stored in 
...
**serverstatus:** planCache.classic.replanned
   ...
   Number of classic execution engine query plans that were discarded
   and re-optimized.
...
**serverstatus:** profiler.totalWrites

   Total number of writes to :data:`~<database>.system.profile`
   collections on all databases.

**serverstatus:** profiler.activeWriters
...
**serverstatus:** profiler.totalAbandonedWrites
   ...
   Total number of abandoned writes to :data:`~<database>.system.profile` 
   collections to preserve availability.
   
   You can specify the amount of time to wait before abandoning writes to
   :data:`~<database>.system.profile` collections by setting the 
   :parameter:`internalQueryGlobalProfilingLockDeadlineMs` parameter.

**serverstatus:** profiler.dbsPastThreshold

   .. versionadded:: 8.3

   Total number of databases where the :ref:`database-profiler` is disabled 
   after reaching the lock timeout threshold. 
   
   You can specify the maximum number of lock timeouts per second that disable 
   the profiler by setting the :parameter:`internalProfilingMaxAbandonedWritesPerSecondPerDb`
   parameter.
...
.. _server-status-queues:

### queues

As an operation proceeds through its stages, it may 
enter a queue if the number of concurrent operations at the current
stage exceeds a maximum threshold. This prevents excessive resource 
contention and provides observability into the state of the database.
...
**serverstatus:** queues.execution
   ...
   A document that returns monitoring and 
   :ref:`queue<server-status-queue-information>` information 
   for operations waiting to be scheduled for execution within the 
   storage layer (concurrent transactions).
   ...
   A histogram that tracks the distribution of completed operations by their
   total number of ticket acquisitions.
   ...
   If ``true`` and :serverstatus:`~queues.execution.deprioritizationGate` is ``true``,
   background tasks such as index builds and TTL deletions
   initiate as low priority.
   ...
   If ``true`` and :serverstatus:`~queues.execution.deprioritizationGate` is ``true``,
   operations that re-acquire tickets frequently are automatically 
   deprioritized to the low-priority pool.

   .. serverstatus:: queues.execution.heuristicDeprioritizationThreshold

   .. versionadded:: 8.3

   The number of ticket re-acquisitions required
   before an operation triggers heuristic deprioritization.

   .. serverstatus:: queues.execution.totalDeprioritizations

   .. versionadded:: 8.3

   The cumulative count of operations that are heuristically deprioritized
   ...
   Indicates whether operations can be deprioritized to a separate
   low-priority ticket pool.

   .. serverstatus:: queues.execution.usesThroughputProbing

   .. versionadded:: 8.3

   Indicates whether the system dynamically adjusts concurrency using the
   throughput probing algorithm.

   .. serverstatus:: queues.execution.nonDeprioritizable

   .. versionadded:: 8.3

   A document containing operation metrics for non-deprioritizable operations in 
   this queue. For fields, see :ref:`server-status-query-operation-execution`.

   .. serverstatus:: queues.execution.deprioritizable

   .. versionadded:: 8.3

   A document containing operation metrics for deprioritizable operations in 
   this queue. For fields, see :ref:`server-status-query-operation-execution`.

   .. serverstatus:: queues.execution.write

   Returns a document containing :ref:`server-status-queue-information` 
   for concurrent write transactions (write tickets) allowed into 
   the WiredTiger storage engine. 

   .. serverstatus:: queues.execution.read

   Returns a document containing :ref:`server-status-queue-information` 
   for concurrent read transactions (read tickets) allowed into 
   the WiredTiger storage engine.
   
   .. serverstatus:: queues.execution.monitor

   A document that returns monitoring metrics for adjustments that 
   the system has made to the number of allowed concurrent 
   transactions (tickets).

   .. serverstatus:: queues.execution.monitor.timesDecreased

   The number of times the queue size was decreased.

   .. serverstatus:: queues.execution.monitor.timesIncreased

   The number of times the queue size was increased.

   .. serverstatus:: queues.execution.monitor.totalAmountDecreased

   The total amount of operations the queue decreased by.

   .. serverstatus:: queues.execution.monitor.totalAmountIncreased

   The total number of operations the queue increased by.

   .. serverstatus:: queues.execution.monitor.resizeDurationMicros

   The cumulative time in milliseconds that the system spent resizing 
   the queue.

   .. serverstatus:: queues.execution.monitor.timesProbedStable

   .. versionadded:: 8.3

   The number of probing iterations in a stable state.

   .. serverstatus:: queues.execution.monitor.timesProbedUp

   .. versionadded:: 8.3

   The number of probing iterations while testing increased concurrency.

   .. serverstatus:: queues.execution.monitor.timesProbedDown

   .. versionadded:: 8.3

   The number of probing iterations while testing decreased concurrency.

**serverstatus:** queues.ingress

   .. versionadded:: 8.0

   A document that returns :ref:`server-status-queue-information` for 
   ingress admission control. Use these values to protect and mitigate 
   against resource overload by limiting the number of operations 
   waiting for entry to the database from the network.

   The maximum number of allowed concurrent operations is constrained by   
   :parameter:`ingressAdmissionControllerTicketPoolSize`.

**serverstatus:** queues.ingressSessionEstablishment

   .. versionadded:: 8.2 *(also available in 8.1.1, 8.0.12, and 7.0.23)*

   A document that contains information about the ingress session establishment
   queue. This includes metrics related to connections established
   and processed through the connection establishment rate limiter.

**serverstatus:** queues.ingressSessionEstablishment.addedToQueue

   .. versionadded:: 8.2 *(also available in 8.1.1, 8.0.12, and 7.0.23)*

   The number of incoming connections that the server adds to the ingress
   session establishment queue. This metric tracks connections that are
   processed through the rate limiter queue when the rate limiter is enabled.

**serverstatus:** queues.ingressSessionEstablishment.removedFromQueue

   .. versionadded:: 8.2 *(also available in 8.1.1, 8.0.12, and 7.0.23)*

   The number of incoming connections that the server removes from the ingress 
   session establishment queue after acquiring a connection establishment token. This 
   metric tracks connections that have completed their wait in the rate limiter queue.

**serverstatus:** queues.ingressSessionEstablishment.interruptedInQueue

   .. versionadded:: 8.2 *(also available in 8.1.1, 8.0.12, and 7.0.23)*
   
   The number of incoming connections that halt while waiting in the queue, 
   typically due to client disconnects or server shutdown.

**serverstatus:** queues.ingressSessionEstablishment.rejectedAdmissions

   .. versionadded:: 8.2 *(also available in 8.1.1, 8.0.12, and 7.0.23)*
   
   The number of incoming connection attempts that the server rejects because the queue 
   depth exceeded the :parameter:`ingressConnectionEstablishmentMaxQueueDepth` limit. 
   When this happens, the server immediately closes the connection rather than 
   queuing it.
...
**serverstatus:** queues.ingressSessionEstablishment.exemptedAdmissions
   ...
   The number of incoming connection attempts that bypass the rate limiter due to 
   being on the :parameter:`ingressConnectionEstablishmentRateLimiterBypass` list. 
   Connections from IP addresses or CIDR ranges specified in 
   :parameter:`ingressConnectionEstablishmentRateLimiterBypass` 
...
**serverstatus:** queues.ingressSessionEstablishment.successfulAdmissions
   ...
   The total number of incoming connection attempts that the rate limiter
   successfully processes, either immediately or after waiting in the queue.
...
**serverstatus:** queues.ingressSessionEstablishment.averageTimeQueuedMicros
   ...
   The average time in microseconds that connections spend waiting in the queue before the server 
   processes them. This metric uses an exponentially-weighted moving average formula and can be 
   used to tune the :parameter:`ingressConnectionEstablishmentMaxQueueDepth`. The value 
   roughly equals ``(maxQueueDepth / establishRatePerSec) * 1e6``.

**serverstatus:** queues.ingressSessionEstablishment.totalAvailableTokens

   .. versionadded:: 8.2 *(also available in 8.1.1, 8.0.12, and 7.0.23)*
   
   The current number of available tokens in the token bucket. This represents the capacity 
   for immediately processing new connections without queuing. When this value is ``0``, 
   new connections must wait in the queue, or are rejected if the queue is full.

.. _server-status-queue-information:
...
**serverstatus:** out
   ...
   The total number of currently held tickets.
   When :serverstatus:`~queues.execution.usesPrioritization` is ``false``, this value
   only includes tickets in the normal-priority queue.
...
**serverstatus:** totalTickets

   The ticket capacity across all pools.
   When :serverstatus:`~queues.execution.usesPrioritization` is ``false``, this value
   only includes tickets in the normal-priority queue.

**serverstatus:** nonDeprioritizable
...
**serverstatus:** exempt
   ...
   A document that returns metrics for operations exempt from the 
   queue.

**serverstatus:** exempt.startedProcessing

   The total number of operations that acquired an admission ticket.

**serverstatus:** exempt.processing

   The total number of operations that are currently being processed.

**serverstatus:** exempt.finishedProcessing

   The total number of operations that released their admission ticket.

**serverstatus:** exempt.totalTimeProcessingMicros

   The total time operations held their admission tickets.

**serverstatus:** exempt.canceled

   The total number of operations that timed out in the queue.

**serverstatus:** exempt.newAdmissions

   The total number of new admissions to the queue.

**serverstatus:** exempt.totalTimeQueuedMicros

   The total time operations spent waiting on the queue.

**serverstatus:** normalPriority

   A document that returns metrics for operations subject to 
   the queue.

**serverstatus:** normalPriority.out

   The number of tickets currently held by active operations in the queue.

**serverstatus:** normalPriority.available

   The number of tickets remaining for acquisition in the queue.

**serverstatus:** normalPriority.totalTickets

   The total capacity of the ticket pool.

**serverstatus:** normalPriority.addedToQueue

   The total number of operations added to the queue.

**serverstatus:** normalPriority.removedFromQueue

   The total number of operations removed from the queue.

**serverstatus:** normalPriority.queueLength

   The total number of operations in the queue.

**serverstatus:** normalPriority.startedProcessing

   The total number of operations that acquired an admission ticket.

**serverstatus:** normalPriority.processing

   The total number of operations that are currently being processed.

**serverstatus:** normalPriority.finishedProcessing

   The total number of operations that released their admission ticket.

**serverstatus:** normalPriority.totalTimeProcessingMicros

   The total time operations held their admission tickets.

**serverstatus:** normalPriority.canceled

   The total number of operations that timed out in the queue.

**serverstatus:** normalPriority.newAdmissions

   The total number of new admissions to the queue.

**serverstatus:** normalPriority.totalTimeQueuedMicros

   The total time that operations spend waiting on the queue.

**serverstatus:** normalPriority.totalDelinquentAcquisitions

   .. versionadded:: 8.3

   The amount of tickets that are held longer than the configured 
   delinquency threshold.

**serverstatus:** normalPriority.totalAcquisitionDelinquencyMillis

   .. versionadded:: 8.3

   The cumulative excess time in milliseconds beyond the threshold for all 
   delinquent holds in the queue.

**serverstatus:** normalPriority.maxAcquisitionDelinquencyMillis

   .. versionadded:: 8.3

   The maximum excess time in milliseconds observed for any single delinquent 
   hold in the queue.

**serverstatus:** lowPriority

   .. versionadded:: 8.3

   A document that returns metrics for operations that were deprioritized to a separate
   low-priority ticket pool.

**serverstatus:** lowPriority.out

   The number of tickets currently held by active operations in the low-priority queue.

**serverstatus:** lowPriority.available

   The number of tickets remaining for acquisition in the low-priority queue.

**serverstatus:** lowPriority.totalTickets

   The total capacity of the low-priority ticket pool.

**serverstatus:** lowPriority.addedToQueue

   The total number of operations added to the low-priority queue.

**serverstatus:** lowPriority.removedFromQueue 
   
   The total number of operations removed from the low-priority queue.

**serverstatus:** lowPriority.queueLength
   
   The total number of operations in the low-priority queue.

**serverstatus:** lowPriority.startedProcessing

   The total number of operations that acquired a low-priority admission ticket.

**serverstatus:** lowPriority.processing

   The total number of operations that are currently being processed with a low-priority admission ticket.

**serverstatus:** lowPriority.finishedProcessing

   The total number of operations that released their low-priority admission ticket.

**serverstatus:** lowPriority.totalTimeProcessingMicros

   The total time operations held their low-priority admission tickets.

**serverstatus:** lowPriority.canceled

   The total number of operations that timed out in the low-priority queue.

**serverstatus:** lowPriority.newAdmissions

   The total number of new admissions to the low-priority queue.

**serverstatus:** lowPriority.totalTimeQueuedMicros

   The total time operations spent waiting on the low-priority queue.

**serverstatus:** lowPriority.totalDelinquentAcquisitions

   The number of ticket holds that exceed the delinquency threshold in the 
   low-priority queue.

**serverstatus:** lowPriority.totalAcquisitionDelinquencyMillis

   The cumulative excess time in milliseconds beyond the threshold for all 
   delinquent holds in the low-priority queue.

**serverstatus:** lowPriority.maxAcquisitionDelinquencyMillis

   The maximum excess time in milliseconds for any single delinquent 
   hold in the low-priority queue.

.. _server-status-query-operation-execution:
...
**serverstatus:** querySettings.count

   Total number of query settings.

**serverstatus:** querySettings.rejectCount

   Total number of query settings that have the ``reject`` field set to
   ``true``. To set the ``reject`` field, use ``setQuerySettings``.

**serverstatus:** querySettings.size
...
**serverstatus:** readPreferenceCounters
   ...
   User-initiated operations that run through :program:`mongos`
   increment ``internal`` counters and not ``external``
   counters.
...
**serverstatus:** repl
   ...
   set. See :ref:`replication` for more information on replication.
...
**serverstatus:** repl.userWriteBlockReason

   A numeric value that represents the reason why user writes are 
   blocked. This field is relevant only when you set ``userWriteBlockMode`` 
   to ``2`` to enable write-blocking.
   ...
   This field corresponds to the ``reason`` parameter specified in the 
   :dbcommand:`setUserWriteBlockMode` command when write-blocking is enabled.

**serverstatus:** repl.userWriteBlockModeCounters

   A document that contains counters tracking the number of times write-blocking 
   is enabled with different reasons since the server started.

**serverstatus:** repl.userWriteBlockModeCounters.Unspecified

   The number of times write-blocking is enabled with the reason 
   ``Unspecified`` since the server started.

**serverstatus:** repl.userWriteBlockModeCounters.ClusterToClusterMigrationInProgress

   The number of times write-blocking is enabled with the reason 
   ``ClusterToClusterMigrationInProgress`` since the server started.

**serverstatus:** repl.userWriteBlockModeCounters.DiskUseThresholdExceeded

   The number of times write-blocking is enabled with the 
   reason ``DiskUseThresholdExceeded`` since the server started.
...
**serverstatus:** security
   ...
   .. serverstatus:: security.authentication.mechanisms
      ...
      .. note::
         ...
         document for a deployment that only supports
         :ref:`X.509 <security-auth-x509>` authentication.
...
**serverstatus:** shardingStatstics.unauthorizedDirectShardOps

   The number of direct shard operations attempted without sufficient 
   authorization.  If the operation generates a warning rather than an 
   error, ``unauthorizedDirectShardOps`` might count it more than once.

   On a sharded cluster with a single shard, you can control this counter 
   by setting the ``directConnectionChecksWithSingleShard`` parameter to
   ``true``.

**serverstatus:** shardingStatistics.databaseCriticalSectionStatistics

   A document with statistics about the critical sections for
   databases, including counts of how many operations hold the critical section, 
   how many are waiting, and how long they have waited.
...
**serverstatus:** shardingStatistics.collectionCriticalSectionStatistics
   ...
   A document with statistics about the critical sections for
   collections, including counts of how many operations hold the critical section, 
   how many are waiting, and how long they have waited.
...
**serverstatus:** shardingStatistics.databaseVersionUpdateCounters
   ...
   A document with counters that track updates to the sharding database
   version, including information on how many sets, clears, access
   changes, and durable changes have been made since the server started.
...
**serverstatus:** shardingStatistics.countTransitionToDedicatedConfigServerStarted 
   ...
   Number of times the :dbcommand:`transitionToDedicatedConfigServer` command 
   has started. 
...
**serverstatus:** shardingStatistics.countTransitionFromDedicatedConfigServerCompleted
   ...
   Number of times the :dbcommand:`transitionFromDedicatedConfigServer` command 
   has finished. 
...
**serverstatus:** shardingStatistics.configServerInShardCache
   ...
   A boolean that indicates whether the config server is a :term:`config shard`. 
   This value periodically refreshes, so the value of 
   ``configServerInShardCache`` might be stale for up to approximately one 
   minute in a healthy cluster. If the node can't communicate with the config 
   server, ``configServerInShardCache`` may remain stale for a longer period.
...
**serverstatus:** shardingStatistics.resharding.active.documentsCopied
...
**serverstatus:** shardingStatistics.numHostsTargeted
   ...
   command is run, the following metrics increment.
   ...
   .. note:: 
      ...
      provides insight into the CRUD and aggregation operations that
...
**serverstatus:** shardedIndexConsistency
   ...
   *Available only on config server instances.*
   ...
   the :ref:`config server replica set <sharding-config-server>` for a 
   sharded cluster.
...
**serverstatus:** shardedIndexConsistency.numShardedCollectionsWithInconsistentIndexes
   ...
   *Available only on config server instances.*
   ...
   <sharding-config-server>` for a sharded cluster.
...
.. _server-status-spill-wiredTiger:
...
**serverstatus:** spillWiredTiger
   ...
   A document that contains metrics on the WiredTiger **spill instance**.
   When MongoDB writes to disk to fulfill certain operations, it
   utilizes a separate WiredTiger instance, which contains its own
   in-memory cache. This separate cache isolates operations from the
   main WiredTiger cache.
   
   The ``spillWiredTiger`` document contains a subset of the fields
   reported in the :ref:`wiredTiger <server-status-wiredTiger>`
   document. The ``spillWiredTiger`` document only appears when using
   the WiredTiger storage engine. For details on the ``spillWiredTiger``
   metrics, see the corresponding ``wiredTiger`` metric description.
...
**serverstatus:** storageEngine.supportsCommittedReads
   ...
   :readconcern:`"majority"` :ref:`read concern <read-concern>`.

**serverstatus:** storageEngine.persistent

   A boolean that indicates whether the
   :ref:`storage engine <storage-engines>` does or does not persist
   data to disk.

.. _server-status-tcmalloc:
...
**serverstatus:** tcmalloc 
   ...
   A document that contains information on memory allocation for the server.
   By default, ``tcmalloc`` metrics are included in the ``serverStatus`` output. 
   To change the verbosity of the ``tcmalloc`` section, specify an integer 
   between ``0`` and ``3`` (inclusive):

   - If you set verbosity to ``0``, ``tcmalloc`` metrics aren't included in the 
     ``serverStatus`` output.
   - If you set verbosity to ``1``, the ``serverStatus`` output includes the 
     default ``tcmalloc`` metrics.
   - If you set verbosity to ``2``, the ``serverStatus`` output includes default 
     ``tcmalloc`` metrics and the :serverstatus:`tcmalloc.tcmalloc.cpuCache` 
     section.
   - If you set verbosity to ``3``, the ``serverStatus`` output includes all 
     ``tcmalloc`` metrics.
   
   If you specify a value higher than ``3``, MongoDB sets the ``verbosity`` 
   to ``3``.
   
   For example, to call ``serverStatus`` with ``verbosity`` set to ``2``, run 
   ...
   .. seealso:: 
      ...
      `TCMalloc Stats <https://google.github.io/tcmalloc/stats.html>`__

**serverstatus:** tcmalloc.usingPerCPUCaches

   A boolean that indicates whether TCMalloc is running with per-CPU caches. 
   If ``tcmalloc.usingPerCPUCaches`` is ``false``, ensure that: 

   - You :ref:`disabled glibc rseq <tcmalloc-disable-glibc-rseq>`. 
   - You're using Linux :ref:`kernel version <tcmalloc-kernel-version>` 4.18 or 
     later.

   .. versionadded:: 8.0

**serverstatus:** tcmalloc.maxPerCPUCacheSizeBytes
...
**serverstatus:** tcmalloc.generic.peak_memory_usage 
   ...
   Total amount of memory, in bytes, allocated by MongoDB and sampled by 
   TCMalloc.
...
**serverstatus:** tcmalloc.generic.current_allocated_bytes 
   ...
   Total number of bytes that are currently allocated to memory and actively 
   used by MongoDB.

**serverstatus:** tcmalloc.generic.heap_size

   Amount of memory, in bytes, allocated from the operating system. This value 
   includes memory that's currently in use and memory that's been
   allocated but isn't in use.

**serverstatus:** tcmalloc.tcmalloc.central_cache_free

   Amount of memory, in bytes, held in the central free list. The central free 
   list is a structure that manages free memory for reuse.
...
**serverstatus:** tcmalloc.tcmalloc.release_rate
   ...
   Rate, in bytes per second, at which unused memory is released to the 
   operating system. The :parameter:`tcmallocReleaseRate` parameter determines 
   the value of ``tcmalloc.tcmalloc.release_rate``.

**serverstatus:** tcmalloc.tcmalloc.cpuCache 

   A document that provides data on each CPU cache. 
   
   ``cpuCache`` metrics are excluded at the default verbosity level. To view 
   ``cpuCache`` metrics, you must set the ``tcmalloc`` verbosity to at least 
   ``2``.
...
**serverstatus:** tcmalloc.tcmalloc.cpuCache.N.overflows 
   ...
   Number of overflows that the CPU cache experienced. Overflows occur when a 
   user deallocates memory and the cache is full.

   .. versionadded:: 8.0

**serverstatus:** tcmalloc.tcmalloc.cpuCache.N.underflows 

   Number of underflows that the CPU cache experienced. Underflows occur when a 
   user allocates memory and the cache is empty.

   .. versionadded:: 8.0

**serverstatus:** tcmalloc.tcmalloc_derived.total_free_bytes 

   Amount of memory remaining before ``tcmalloc`` has to request for more memory 
...
``wiredTiger`` information only appears if using the
:ref:`WiredTiger <storage-wiredtiger>` storage engine. Some of the statistics roll up for the server.
