...
generated_at: "2026-08-29T09:39:20.073953Z"
---
.. _collstats-database-command:

# collStats (database command)

**meta:** :description: Access storage statistics for a collection using the `collStats` command, with options for scaling and compatibility across different MongoDB environments.
...
**dbcommand:** collStats
   ...
   .. include:: /includes/fact-coll-stats-deprecated.rst
...
**data:** collStats.timeseries
   ...
   ``collStats`` command on a :ref:`time series collection
...
**data:** collStats.nindexes
   ...
   :data:`~collStats.nindexes` includes indexes currently being built in
   its count.
...
**data:** collStats.indexDetails
   ...
   :data:`~collStats.indexDetails` contains
...
**data:** collStats.totalIndexSize
   ...
   Sum of the disk space used by all indexes. The ``scale`` argument 
   affects this value.
   ...
   :data:`~collStats.totalIndexSize` includes indexes currently being
...
**data:** collStats.indexSizes
   ...
   :data:`~collStats.indexSizes` includes the sizes of indexes currently being built.
...
**data:** collStats.wiredTiger
   ...
   WiredTiger storage engine. When using :ref:`Queryable Encryption
   <qe-manual-feature-qe>`, WiredTiger data is redacted to only the ``url`` field.
