...
generated_at: "2026-08-29T09:39:20.071968Z"
---
# replSetGetStatus (database command)

**meta:** :description: Retrieve the status of a replica set using the `replSetGetStatus` command, which provides detailed information about the replica set's state and members.
...
**dbcommand:** replSetGetStatus
   ...
   set as seen by the server that processed the command. Run
   ``replSetGetStatus`` on the :term:`admin database`.

   The :binary:`~bin.mongod` instance must be a replica set member for
   :dbcommand:`replSetGetStatus` to return successfully.

   This command derives its data from heartbeats sent by other replica
   set members. Because of heartbeat frequency, this data can be
...
If you run :dbcommand:`replSetGetStatus` or the :binary:`~bin.mongosh`
helper :method:`rs.status()` on a member during its :ref:`initial sync
<replica-set-initial-sync>` in the :replstate:`STARTUP2` state,
the command returns :data:`replSetGetStatus.initialSyncStatus` metrics.
...
**data:** replSetGetStatus.writeMajorityCount
   ...
   The number of data-bearing voting members needed to fulfill write
   concern :writeconcern:`"majority"`. Writes
   can only be applied to data-bearing members.
...
**data:** replSetGetStatus.optimes
   ...
   .. data:: replSetGetStatus.optimes.readConcernMajorityOpTime
      ...
      :readconcern:`"majority"` queries. That is, the most recent
...
**data:** replSetGetStatus.electionCandidateMetrics
   ...
   .. data:: replSetGetStatus.electionCandidateMetrics.electionTerm
      ...
      The member's election count (that is,
      :data:`~replSetGetStatus.term`) at the time it called the new
      election.
...
**data:** replSetGetStatus.members
   ...
   .. data:: replSetGetStatus.members[n].lastAppliedWallTime
      ...
      An :term:`ISODate` formatted date string reflecting the :term:`wall clock time`
      at which the last operation applied by this member of the 
      replica set was applied on the primary node.

   .. data:: replSetGetStatus.members[n].lastDurableWallTime

      An :term:`ISODate` formatted date string reflecting the :term:`wall clock time`
      at which the last operation written to this member's journal 
