...
generated_at: "2026-08-29T09:39:20.022698Z"
---
.. _find-and-modify:

# findAndModify (database command)

**meta:** :description: Update and return a single document using the `findAndModify` database command, with options for sorting, upserting, and returning modified documents.
...
**dbcommand:** findAndModify
   ...
   document does not include the modifications made by the update.
   To return the modified document, use the ``new`` option.
...
.. list-table::
   ...
   * - ``query``
     ...
     - Optional. The selection criteria for the modification. The ``query`` field
       ...
       **selects only one document to update**.
...
- You :red:`must` run either in a :ref:`transaction <transactions>`
  or as a :ref:`retryable write <retryable-writes>`.
...
- You :red:`must` run either in a :ref:`transaction <transactions>`
  or as a :ref:`retryable write <retryable-writes>` if the new
  shard key value is not ``null``.
...
If a ``findAndModify`` operation fails because the write concern wasn't
fulfilled, the operation returns a :data:`writeConcernError` document.
...
#. The ``sort`` orders the results of the query in ascending order.
   If multiple documents match the ``query`` condition, the command
   modifies the first document in sort order.
...
The following command uses ``upsert: true`` to update a matching
document or insert a new one if no match exists:
...
The following command uses both ``upsert: true`` and ``new: true``
to update a matching document or insert a new one, and returns the
new or updated document in the ``value`` field.
...
field values or updating one field using the value of another field.
