...
generated_at: "2026-08-29T09:39:20.069005Z"
---
# create (database command)

**meta:** :description: Create collections or views in MongoDB using the `create` command, specifying options like capped collections, time series, clustered indexes, and more.
...
.. list-table::
   ...
   * - ``autoIndexId``
     - boolean
     - Optional. Specify ``false`` to disable the automatic creation
       of an index on the ``_id`` field.

       :gold:`IMPORTANT:` Starting in MongoDB 4.0, you cannot set
       ``autoIndexId`` to ``false`` when creating collections in
   ...
   * - ``viewOn``
     ...
     - The name of the source collection or view from which to create the
       ...
       view. It does not include the database name. You must create
       views in the same database as the source collection.
   ...
   * - ``collation``
     - Specifies the default :ref:`collation <collation>` for the
       ...
       collation is the "basic" binary comparison collator. For a view on
...
When using :ref:`Stable API <stable-api>` V1, all ``create``
command fields are supported except the following fields, which
are not available in Stable API V1:
...
returns documents in the following order:
