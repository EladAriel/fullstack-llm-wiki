...
generated_at: "2026-08-29T09:39:20.093986Z"
---
.. _find-database-command:

# find (database command)

**meta:** :description: Execute queries using the find command to retrieve documents with options such as sort order, limits, projections, index hints, collation, and read concern.
...
.. list-table::
   ...
   * - ``filter``
     - document
     - Optional. The query predicate. If unspecified, all documents in
       the collection match the predicate.
   ...
   * - ``hint``
     ...
     - Optional. Index specification. Specify either the index name as a
       string or the index key pattern. If specified, the query system
       only considers plans using the hinted index.

       With the following exception, ``hint`` is required if the command
       includes the ``min`` or ``max`` fields. ``hint`` is not required
       with ``min`` or ``max`` if the ``filter`` is an equality condition
   ...
   * - ``returnKey``
     - boolean
     - Optional. If true, returns only the index keys in the resulting
       documents. Defaults to false. If returnKey is true and the
       :dbcommand:`find` command does not use an index, the returned
       documents are empty.
   ...
   * - ``awaitData``
     - boolean
     - Optional. Use with the tailable option to block a
       :dbcommand:`getMore` command on the cursor temporarily at the
       end of data rather than returning no data. After a timeout
       period, :dbcommand:`find` returns as normal.
   ...
   * - ``noCursorTimeout``
     ...
     - Optional. Prevents the server from timing out non-session idle cursors 
       ...
       part of a session. For more information, see
   ...
   * - :ref:`allowPartialResults <cmd-find-allowPartialResults>`
     ...
     - .. _cmd-find-allowPartialResults:
       ...
       partial results instead of an error if one or more queried
...
The command returns a document with cursor information, including
the :ref:`cursor ID <cursor-id>` and the first batch of documents.
The following example shows the output when run against a sharded
collection:
...
In addition to the preceding :dbcommand:`find`-specific fields,
:method:`db.runCommand()` includes the following information for
...
.. _find-order-of-operations:

### Order of Operations

A ``find()`` operation generally uses the following order of
operations:
...
MongoDB may execute components in a different order to optimize query
performance while maintaining the above order of operations.
...
:dbcommand:`find` command fields are supported except the
following fields, which are not available in Stable API V1:
...
The following command queries the ``movies`` collection for movies
with an IMDB rating of at least 9 that are in the Drama genre. The
command includes a ``projection`` to return only the ``title``,
``imdb.rating``, and ``year`` fields.

The command sorts results by ``title`` and limits the output to 5
documents.

**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/find/specify-sort-and-limit.snippet.specify-sort-and-limit.js
   :language: javascript
...
The following code example performs these operations on the
``movies`` collection on a replica set:

- finds movies with an IMDB rating below 5
- limits the results to 5 documents
- specifies a :term:`read concern` of :readconcern:`"majority"` to read the most
  recent copy of the data that MongoDB wrote to a majority of the
  nodes.

**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/find/override-read-concern.snippet.override-read-concern.js
   :language: javascript
   :category: usage example
...
**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/find/read-concern-find.snippet.read-concern-find.js
   :language: javascript
   :category: usage example
...
**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/find/specify-collation.snippet.specify-collation.js
   :language: javascript
   :category: usage example
...
**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/find/use-let.snippet.use-let.js
   :language: javascript
   :category: usage example