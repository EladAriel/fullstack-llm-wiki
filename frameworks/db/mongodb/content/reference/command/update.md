...
generated_at: "2026-08-29T09:39:20.087396Z"
---
# update (database command)

**meta:** :description: Modify documents in a collection using the update command, which supports multiple update statements and options such as upsert, multi, collation, and aggregation pipelines.
...
.. list-table::
   ...
   * - :ref:`c <update-command-c>`
     ...
     - .. _update-command-c:
       ...
       :gold:`IMPORTANT:` Starting in MongoDB 8.2, if you specify ``c`` within an
       ``update`` command that does not contain a pipeline, ``update`` returns
       an error.
...
The update statement :ref:`u <update-command-u>` field accepts a
replacement document that contains *only* ``field:value``
expressions. For example:
...
replace a *single* matching document. The ``multi`` field cannot be
``true``. The :dbcommand:`update` command *does not* replace the
``_id`` value.
...
``imdb.rating`` fields. Create a :ref:`document validator
<schema-validation-overview>` on the ``movies`` collection with a rule
that the ``imdb.rating`` value must be less than or equal to ``10``: 

**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/validator.js
   :language: javascript
...
The aggregation pipeline lets you write expressive update statements,
such as conditional updates based on current field values or updating
one field using the value of another field.
...
and update sizes (that is, ``q`` and ``u``) must be less than or equal
...
If an ``update`` operation on a sharded cluster exceeds the :ref:`slow
operation threshold <slowms-threshold-option>`, MongoDB adds a :ref:`log message
<log-messages-ref>` that contains a ``routerQueryShapeHash`` field. This field
contains the query shape of the operation on the :program:`mongos`  before the
``mongos`` sends the operation to the :program:`mongod`. 
...
**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/update-1.js
   :language: javascript
   :category: usage example

Because the ``<update>`` document does not specify the optional
``multi`` field, this operation modifies only one document, even if
more than one document matches the ``q`` query.
...
The following command uses the :update:`$inc` update operator to increment
the ``num_mflix_comments`` field for all movies released in 1924:

**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/update-specific-fields.js
   :language: javascript
   :category: usage example
...
**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/update-specific-fields-output.sh
   :language: javascript
...
The following example uses the aggregation pipeline to modify a field
...
The following update operation uses an aggregation pipeline to add new fields
to a specific user's document:

**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/update-with-aggregation-1.js
   :language: javascript
   :category: usage example
...
The aggregation pipeline lets the update perform conditional updates
based on current field values and use those values to calculate a
separate field value.

Documents in the ``movies`` collection from the ``sample_mflix`` database 
have a ``year`` field.

The following example uses an aggregation pipeline to calculate the age of
"The Great Train Robbery" and assign an era classification based on when
it was released.

**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/update-with-aggregation-2.js
   :language: javascript
   :category: usage example
...
First Stage
   The :pipeline:`$set` stage calculates a new field ``age`` based
   on the difference between 2026 and the movie's release year. See :expression:`$subtract`
   for more information.

Second Stage
   The :pipeline:`$set` stage calculates a new field ``era`` based on
   the ``year`` field using conditional logic. See
   :expression:`$switch` for more information on the ``$switch``
   aggregation operator.

### Bulk Update

The following example performs multiple update operations in a single
command to both update existing documents and insert new documents. The operation: 
...
**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/bulk-update.js
   :language: javascript
   :category: usage example

The returned document shows that the command modified existing documents
and inserted a new document through an upsert. See
:ref:`update-command-output` for details.

**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/bulk-update-output.sh
   :language: javascript
   :category: example return object
...
**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/specify-collation.js
   :language: javascript
   :category: usage example
...
The following example updates all movies that have ``"English"`` in
their ``languages`` array. The operation replaces ``"English"`` with
``"EN"``:

**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/array-filters.js
   :language: javascript
   :category: usage example
...
The following example finds a movie with the title ``"The Godfather"`` and 
replaces ``"Al Pacino"`` with ``"REDACTED"`` in its ``cast`` array. The 
``arrayFilters`` option specifies which array elements to update:

**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/update-specific-elements.js
   :language: javascript
   :category: usage example
...
**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/create-index.js
   :language: javascript
   :category: usage example
...
**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/update-with-hint.js
   :language: javascript
   :category: usage example


To see the index used, you can run :dbcommand:`explain` on an update operation.
...
**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/explain.js
   :language: javascript
   :category: usage example
...
The following example uses the ``let`` option to define variables for
finding and adding a new field to a movie:

**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/let-option.js
   :language: javascript
   :category: usage example

The next example defines ``movieTitle`` and ``franchiseName`` variables
in ``c`` and uses the variables to add a ``franchise`` field:

**literalinclude:** /code-examples/tested/command-line/mongosh/database-commands/update/let-option-2.js
   :language: javascript
   :category: usage example
...
The following example finds all movies from 1972 and updates the one
