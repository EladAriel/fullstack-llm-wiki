...
generated_at: "2026-08-29T09:39:19.935419Z"
---
# db.collection.findOne() (mongosh method)

.. default-domain:: mongodb

**meta:** :description: Find a single document in a collection or view.
...
``db.collection.findOne()`` has the following
form:
...
``db.collection.findOne()`` takes the following
parameters:
...
**literalinclude:** /code-examples/tested/command-line/mongosh/mongosh-commands/db.collection.findOne/empty-specification.snippet.empty-specification.js
   :language: javascript
...
``movies`` collection where either the ``title`` field starts with the
letter "T" **or** the ``year`` field is less than 1950:

**literalinclude:** /code-examples/tested/command-line/mongosh/mongosh-commands/db.collection.findOne/query-specification.snippet.query-specification.js
   :language: javascript
   :category: usage example
...
The following operation finds a document in the ``movies`` collection and
returns only the ``title``, ``genres``, and ``imdb`` fields:

**literalinclude:** /code-examples/tested/command-line/mongosh/mongosh-commands/db.collection.findOne/specify-return-fields.snippet.specify-return-fields.js
   :language: javascript
   :category: usage example

### Return All but the Excluded Fields

The following operation returns a document in the ``movies`` collection
where the ``cast`` array contains ``"Al Pacino"``. The projection returns all
fields *except* the ``_id`` field, the ``votes`` field in the ``imdb``
embedded document, and the ``fullplot`` field:

**literalinclude:** /code-examples/tested/command-line/mongosh/mongosh-commands/db.collection.findOne/exclude-return-fields.snippet.exclude-return-fields.js
   :language: javascript
   :category: usage example
...
matching document from the *sorted* ``movies`` collection. In this
example, the collection is sorted by ``year`` in ascending order.

**literalinclude:** /code-examples/tested/command-line/mongosh/mongosh-commands/db.collection.findOne/use-option.snippet.use-option.js
   :language: javascript
   :category: usage example
...
``findOne()`` because a single document is
returned. You have access to the document directly:

**literalinclude:** /code-examples/tested/command-line/mongosh/mongosh-commands/db.collection.findOne/result-document.snippet.result-document.js
   :language: javascript
   :category: usage example
...
The following example defines a ``targetTitle`` variable in ``let`` and
uses the variable to retrieve the movie titled ``"The Godfather"``:


.. io-code-block::
    :copyable: true

   .. input::  /code-examples/tested/command-line/mongosh/mongosh-commands/db.collection.findOne/use-let.snippet.use-let.js
      :language: javascript
      :category: usage example

   .. output:: /code-examples/tested/command-line/mongosh/mongosh-commands/db.collection.findOne/use-let-output.sh
      :visible: false
      :language: javascript