...
generated_at: "2026-08-29T09:39:19.897627Z"
---
# db.createCollection() (mongosh method)

.. default-domain:: mongodb

**meta:** :description: Use the db.createCollection() method to create a new collection or view, including capped collections, clustered collections, and collections with schema validation.
...
**method:** db.createCollection(name, options)
   ...
   MongoDB creates a collection implicitly when you first reference it
   in a command. Use ``db.createCollection()`` primarily to create
   collections that use specific options, including:
...
.. list-table::
   ...
   * - ``timeseries.metaField``
     ...
     - Optional. The name of the field which contains metadata in
       ...
       labels a unique series of documents. The metadata should
       rarely, if ever, change.

       The name of the specified field cannot be ``_id`` or the same
   ...
   * - ``storageEngine``
     ...
     - Optional. Available for the WiredTiger storage engine only.
       ...
       Specifies storage engine configuration on a per-collection
       basis. The value of the ``storageEngine`` option takes the
       following form:
   ...
   * - ``validator``
     ...
     - Optional. Specifies :ref:`validation rules or expressions
       <schema-validation-overview>` for the collection.
       ...
       <query-selectors>` except for :query:`$near`,
   ...
   * - ``indexOptionDefaults``
     - document
     - Optional. Specifies a default configuration for indexes in the
       collection.
   ...
   * - ``encryptedFields``    
     ...
     - Optional. A document that configures :ref:`{+qe+}
       ...
       For a helper that creates an encrypted collection, see
...
Capped collections have a maximum size and, optionally, a maximum
document count. When a capped collection reaches its maximum size,
MongoDB removes older documents to make room for new ones. The
following example creates a capped collection named ``log``:
...
This collation is used by indexes and operations that support
...
The same operation on a collection that uses binary collation
(that is, no specific collation set) returns documents in the following order:
...
specific configuration string that MongoDB passes to the
