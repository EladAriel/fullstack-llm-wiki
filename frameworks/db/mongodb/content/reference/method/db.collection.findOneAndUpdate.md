...
generated_at: "2026-08-29T09:39:19.980751Z"
---
# db.collection.findOneAndUpdate() (mongosh method)

.. default-domain:: mongodb

**meta:** :description: Update a single document.
...
**method:** db.collection.findOneAndUpdate( filter, update, options )
   ...
   .. tip::
      ...
      .. versionadded:: 8.0
         ...
         The :method:`~db.collection.updateOne` method includes a ``sort`` option
         to update the first document in a user specified sort order.
...
``db.collection.findOneAndUpdate()`` has the following
form:
...
``db.collection.findOneAndUpdate()`` takes the following
parameters:
...
.. |findMethod| replace:: ``findOneAndUpdate()``
**include:** /includes/fact-findAndUpdate


### Document Match

``findOneAndUpdate()`` updates the first matching
...
``findOneAndUpdate()`` to set the document's
**missing** shard key, 
...
.. |operation| replace:: ``findOneAndUpdate()``

### Oplog Entries

If a ``findOneAndUpdate()`` operation successfully updates
