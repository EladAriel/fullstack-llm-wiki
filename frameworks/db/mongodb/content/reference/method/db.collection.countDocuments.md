...
generated_at: "2026-08-29T09:39:19.959953Z"
---
# db.collection.countDocuments() (mongosh method)

.. default-domain:: mongodb

**meta:** :description: Return the number of documents in a collection or view.
...
The ``countDocuments()`` method takes the following
parameters:
...
``countDocuments()`` does not use the metadata to
...
``countDocuments()`` wraps the following
...
``countDocuments()`` returns ``0`` on an empty or
...
.. |countMethod| replace:: ``countDocuments()``

**include:** /includes/extracts/transactions-usage.rst

.. |operation| replace:: ``countDocuments()``
...
To count the number of documents in the ``movies`` collection, use the
following operation:

**literalinclude:** /code-examples/tested/command-line/mongosh/mongosh-commands/db.collection.countDocuments/count-all/count-all.snippet.count-all-docs.js
   :language: javascript
...
This example counts the number of documents in the ``movies``
collection where the ``directors`` field contains ``"David Lynch"``:

**literalinclude:** /code-examples/tested/command-line/mongosh/mongosh-commands/db.collection.countDocuments/count-match/count-match.snippet.count-matched-docs.js
   :language: javascript
   :category: usage example
...
- :dbcommand:`count`
- :ref:`collStats pipeline stage with the count option <collstat-count>` 