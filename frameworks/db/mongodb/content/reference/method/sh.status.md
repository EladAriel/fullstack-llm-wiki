...
generated_at: "2026-08-29T09:39:19.912261Z"
---
# sh.status() (mongosh method)

**meta:** :description: Display a formatted report of sharding configuration and chunk information in a sharded cluster, including data distribution metrics, using `sh.status()`.
...
**method:** sh.status()
   ...
   The output also includes a ``shardedDataDistribution`` section,
   which provides data distribution metrics for each sharded
   collection. This data is the same as the output of the
...
The :ref:`sharding-status-sharded-data-distribution` section displays
data distribution information for each sharded collection:
...
**data:** sh.status.automerge

   Indicates whether the :ref:`{+auto-merge-upper+}
   <automerger-concept>` is enabled.

   .. note::

      The ``automerge`` section only appears if you explicitly enable or
      disable the {+auto-merge-upper+}. By default, the
      {+auto-merge-upper+} is enabled.
...
**data:** sh.status.databases.<collection>.allowMigrations

   Displays whether migrations are allowed for the collection. When ``true``,
   the collection allows migrations. When ``false``, the 
   collection does not allow migrations, preventing chunk migrations for this
   collection. Use :method:`sh.enableMigrations()` and 
   :method:`sh.disableMigrations()` to control this setting.
...
.. _sharding-status-sharded-data-distribution:

### Sharded Data Distribution

**data:** sh.status.shardedDataDistribution

   An array of documents that provides data distribution metrics for
   each sharded collection in the cluster. The data is the same as
   the output of the :pipeline:`$shardedDataDistribution` aggregation
   stage.

**data:** sh.status.shardedDataDistribution.ns

   The namespace of the sharded collection in
   ``<database>.<collection>`` format.

**data:** sh.status.shardedDataDistribution.shards

   An array of documents that reports data distribution metrics for
   each shard.

**data:** sh.status.shardedDataDistribution.shards.shardName

   The name of the shard.

**data:** sh.status.shardedDataDistribution.shards.numOrphanedDocs

   The number of orphaned documents in the shard.

**data:** sh.status.shardedDataDistribution.shards.numOwnedDocuments

   The number of documents owned by the shard.

**data:** sh.status.shardedDataDistribution.shards.ownedSizeBytes

   The estimated size in bytes of documents owned by the shard when
   the documents are uncompressed.

**data:** sh.status.shardedDataDistribution.shards.orphanedSizeBytes

   The estimated size in bytes of orphaned documents in the shard
   when the documents are uncompressed.
