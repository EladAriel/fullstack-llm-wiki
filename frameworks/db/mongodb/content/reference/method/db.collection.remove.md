...
generated_at: "2026-08-29T09:39:19.951821Z"
---
...
**meta:** :keywords: deprecated
   :description: The mongosh db.collection.remove() method is deprecated. Use db.collection.deleteOne() or db.collection.deleteMany() to remove documents from a collection.
...
``db.collection.remove()`` can have one of two
syntaxes. ``remove()`` can take a
...
``db.collection.remove()`` takes the following
parameters:
...
By default, ``remove()`` removes all documents
...
with other read or write operations to the collection.

### Time Series Collections

You cannot use ``remove()`` on a
:term:`time series collection`.

### Sharded Collections

.. |single-modification-operation-names| replace:: ``remove()``
...
``remove()`` returns a :method:`WriteResult`
...
**seealso:** - :data:`WriteResult.writeConcernError`

### Errors Unrelated to Write Concern

If ``remove()`` encounters a non-write
