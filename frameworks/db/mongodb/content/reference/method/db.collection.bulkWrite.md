...
generated_at: "2026-08-29T09:39:19.919971Z"
---
# db.collection.bulkWrite() (mongosh method)

.. default-domain:: mongodb

**meta:** :description: Perform a series of ordered or unordered write operations.
...
**note:** You can't perform :ref:`bulk write <bulk-write-operations>` 
...
.. list-table::
   ...
   * - ``ordered``
     - boolean
     - Optional. A boolean specifying whether operations execute serially (``true``,
       the default) or may execute in any order (``false``). When ``true``, stops
       on the first error; when ``false``, continues processing remaining operations.
...
With ``ordered : true`` (default), operations execute serially. If an error occurs,
subsequent operations are not executed.

With ``ordered : false``, operations may execute in parallel. All operations without
errors are completed even if some operations fail.
...
``bulkWrite()`` has restrictions on :term:`capped collections <capped collection>`:
...
.. |operation| replace:: ``bulkWrite()``

.. _bulkWrite-error-handling:

### Error Handling

``bulkWrite()`` throws a ``BulkWriteError``
