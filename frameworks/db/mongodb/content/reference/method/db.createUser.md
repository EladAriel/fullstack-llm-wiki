...
generated_at: "2026-08-29T09:39:19.988585Z"
---
# db.createUser() (mongosh method)

**meta:** :description: Create a new user in MongoDB with specified roles, authentication restrictions, and password options using the db.createUser() mongosh method in the current database.
...
**method:** db.createUser(user, writeConcern)
   ...
   .. |local-cmd-name| replace:: ``db.createUser()``
   ...
   .. list-table::
      ...
      * - ``customData``
        ...
        - Optional. Any arbitrary information you want to associate
          with the user, such as the user's full name or employee ID.
...
The following operation creates the ``accountAdmin01`` user on the
``products`` database.
...
**note:** To use :ref:`SCRAM-SHA-256 <authentication-scram-sha-256>`, the
