...
source_branch: "main"
source_path: "content/manual/manual/source/reference/method/db.collection.stats.txt"
...
> **Note:** :method:`db.collection.stats()` internally calls the
:pipeline:`$collStats` aggregation stage by default.
...
Depending on the storage engine, the data returned may differ. For details on the fields, see `output details <collStats-output>`.
...
Where `<string>>` is the field that is indexed and `<value>` is either the direction of the index, or the special index type such as `text` or `2dsphere`. See `index types <index-types>` for the full list of index types.

.. include:: /includes/persistent-query-settings-avoid-index-filters-intro.rst

### Unexpected Shutdown and Count

For MongoDB instances using the `WiredTiger <storage-wiredtiger>` storage engine, after an unclean shutdown, statistics on size and count may off by up to 1000 documents as reported by :dbcommand:`collStats`, :dbcommand:`dbStats`, :dbcommand:`count`. To restore the correct statistics for the collection, run :dbcommand:`validate` on the collection.

### In-Progress Indexes

The :method:`db.collection.stats` includes information on indexes currently being built. For details, see:
...
> **Note:** You can find the collection data used for these examples in our
`primer-dataset.json
<https://raw.githubusercontent.com/mongodb/docs-assets/primer-dataset/primer-dataset.json>`_

### Basic Stats Lookup

The following operation returns stats on the `restaurants` collection in the `test` database:
...
The following operation changes the scale of data from `bytes` to `kilobytes` by specifying a `scale` of `1024`:
...
The following operation creates an `indexDetails` document that contains information related to each of the indexes within the collection:
...
To filter the indexes in the  `collStats.indexDetails` field, you can either specify the index keys using the `indexDetailsKey` option or specify the index name using the `indexDetailsName`.  To discover index keys and names for the collection, use :method:`db.collection.getIndexes()`.
...
For explanation of the output, see `output details<collStats-output>`.
