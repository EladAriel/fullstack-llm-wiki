...
generated_at: "2026-08-29T09:39:19.998808Z"
---
.. _db-aggregate-method:

# db.aggregate() (mongosh method)

**meta:** :description: Explore how to use `db.aggregate()` for running admin or diagnostic pipelines without requiring an underlying collection, including syntax and options.
...
not require an underlying collection, such as :pipeline:`$currentOp` 
...
.. list-table::
   ...
   * - ``explain``
     - boolean
     - Optional. Specifies whether the method returns
       information on the pipeline processing. 
       
       See :ref:`example-aggregate-method-explain-option` for an example.
       
       :gold:`IMPORTANT:` This option is not available in :ref:`multi-document transactions <transactions>`.
       

   * - ``allowDiskUse``
     - boolean
     - Optional. Enables writing to temporary files. When set to ``true``, aggregation
       operations write data to the :file:`_tmp` subdirectory in the
   ...
   * - ``cursor``
     ...
     - Optional. Specifies the *initial* batch size for the cursor. The value of the ``cursor``
       ...
       :ref:`example-aggregate-method-initial-batch-size` for syntax and an example.
   ...
   * - ``maxTimeMS``
     ...
     - Optional. Specifies a time limit in milliseconds for processing
       ...
       operations do not time out. A value of ``0`` explicitly
   ...
   * - ``hint``
     ...
     - Optional. The index to use for the aggregation. The index is on the initial
       ...
       :gold:`IMPORTANT:` The ``hint`` does not apply to :pipeline:`$lookup` and
   ...
   * - ``writeConcern``
     ...
     - Optional. A document that expresses the :ref:`write concern <write-concern>`
       ...
       Omit this parameter to use the default write concern with the :pipeline:`$out` or
