...
MongoDB 7.0 adds the :parameter:`enableDetailedConnectionHealthMetricLogLines` parameter which lets you specify whether or not a set of `log messages <log-messages-ref>` related to cluster connection health metrics appears in the log.
...
Starting in MongoDB 7.0, the :dbcommand:`configureCollectionBalancing` command accepts the `enableAutoMerger` parameter. Use `enableAutoMerger` to set whether or not the `{+auto-merge-upper+} <automerger-concept>` takes this collection into account.
