...
source_branch: "main"
source_path: "content/manual/manual/source/includes/crud/python-delete-documents.rst"
...
This page uses the following [PyMongo](https://pymongo.readthedocs.io/en/stable/index.html)_ Python driver methods:
...
To delete all documents from a collection, pass an empty `filter<document-query-filter>` document `{}` to the `pymongo.collection.Collection.delete_many` method.

.. include:: /includes/fact-delete-all-inventory.rst

The `pymongo.collection.Collection.delete_many` method returns an instance of `pymongo.results.DeleteResult` with the status of the operation.
...
To delete all documents that match a deletion criteria, pass a `filter <document-query-filter>` parameter to the `pymongo.collection.Collection.delete_many` method.
...
> **Seealso:** - `pymongo.collection.Collection.delete_many`
- `pymongo.collection.Collection.delete_one`
- `additional-deletes`
