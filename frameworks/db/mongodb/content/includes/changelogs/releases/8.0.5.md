...
source_branch: "main"
source_path: "content/manual/manual/source/includes/changelogs/releases/8.0.5.rst"
...
:issue:`SERVER-91180` dbCheck oplog entries contain duplicate field names in batchStart/batchEnd
...
collection_catalog_two_phase_drops.js assumes write with journaling will advance the latest timestamp
...
inProgressLookup entry are always called with the same cached value in the ReadThroughCache
...
timestamp robust against extended range events and changing granularity
...
shouldRecoverFromOplogAsStandalone and add checks for magic restore as needed
...
sub-stages that makes cursors on another shard can return NotARetryableWriteCommand error
...
ShardVersion check before inspecting the existence of the buckets collection
...
validate_timeseries_bucketing_parameters_change.js have hard-coded timestamps
...
"previousTime" parameter set cannot be reissued whenever the previous invocation sets the parameter and then fails
...
positives due to looking at sharding metadata while critical section is active
...
temporarily hang shard1 instead of rejecting cursor establishing request from mongos
...
CommandOnShardedViewNotSupportedOnMongod into CommandNotSupportedOnView on all code paths
...
reshardingDelayBeforeRemainingOperationTimeQueryMillis when launching patch versions where this parameter doesn't exist
...
assertion that a compressed bucket will be reopened fails on 7.0 in multiversion suites

- :issue:`SERVER-99483` Upgrade mongo-task-generator to 0.7.20
- :issue:`SERVER-99547` Replace invalid db in
query_settings_index_hints_tests when run with the no_passthrough suite
...
manually constructed bucket with mismatch in control ids and observed IDs
...
namespaces should be ignored when cross-checking their routing tables on test teardown
