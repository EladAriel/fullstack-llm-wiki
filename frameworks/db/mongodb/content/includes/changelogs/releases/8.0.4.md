...
source_branch: "main"
source_path: "content/manual/manual/source/includes/changelogs/releases/8.0.4.rst"
...
:issue:`SERVER-88046` Support $vectorSearch execution within unionWith subpipeline
...
internal_transactions_sharded_from_mongod.js's handling of causal consistency across sessions
...
jstests/concurrency/fsm_workloads/timeseries_mixed_operations.js fails in case of concurrent moveCollection
...
interleaved with no scalars if we transition from arrays to a new empty object
...
transaction_too_large_for_cache/temporarily_unavailable_on_secondary_transaction_application.js and replace with unit test
...
all_shard_and_config_hosts_brought_down_one_by_one from timing out on slow variants
...
ttl_deletes_not_targeting_orphaned_documents.js and make it deterministic
...
getRecipientHighEstimateRemainingTimeMillis can incorrectly cast < 1s elapsed durations to 0.
...
on enterprise-rhel8-debug-tsan-all-feature-flags [mongodb-mongo-master @ 26e083fa]
