...
source_branch: "main"
source_path: "content/manual/manual/source/includes/changelogs/releases/7.3.2.rst"
...
enabled by default under different FCV values across different branches
...
:issue:`SERVER-88949` Explain for classic multi planner displays incorrect skip amount
...
NamespaceNotFound if the database has been concurrently dropped (sharding-only)
...
ShardServerProcessInterface should use the operation context to configure their write concern
...
checkHistoricalPlacementMetadataConsistency to tolerate the existence of resharding temp collections created through shardCollection
...
timeField though it's renamed by the $addFields and excluded by a project
...
nodes_eventually_sync_from_closer_data_center.js more robust to transient slow heartbeat issues
...
jstests/core/command_let_variables_expressions.js so it can run in multiversion tests prior to 7,3
