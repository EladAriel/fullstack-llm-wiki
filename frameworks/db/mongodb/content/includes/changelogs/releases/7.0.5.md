...
source_branch: "main"
source_path: "content/manual/manual/source/includes/changelogs/releases/7.0.5.rst"
...
config.tag docs, leading to config server primary fassert loop from duplicate key error

- :issue:`SERVER-82838` ReshardingOplogApplier uses {w: "majority",
wtimeout: 60000} write concern when persisting resharding oplog application progress

- :issue:`SERVER-82883` Recovering TransactionCoordinator on stepup may
block acquiring read/write tickets while participants are in the prepared state
...
:issue:`SERVER-83470` Introduce internalQueryFrameworkControl setting for 6.0-style engine selection logic
...
is overwriting the binaries of "Enterprise RHEL 8.3 s390x" (non-Shared)
...
ActiveIndexBuilds::registerIndexBuild() during index build setup doesn't unregister itself

- :issue:`SERVER-82984` FLE2 with upsert: true can have
upsertedDocuments.size() == 1 and updateReply.getNModified() == 1 at the same time
