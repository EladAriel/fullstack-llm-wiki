...
source_branch: "main"
source_path: "content/manual/manual/source/includes/changelogs/releases/6.0.3.rst"
...
succeed causing resharding_coordinator_recovers_abort_decision.js to fail
...
:issue:`SERVER-69861` Uninterruptible lock guard in election causes FCBIS to hang
...
assert.adminCommandWorkedAllowingNetworkError in fcbis_fails_if_backup_cursor_is_already_open_on_sync_source.js
...
AuditEncCompManagerTest.EncryptAndEncodeLargePayloadSucceeds When GCM not supported
...
swap with a $project if an inclusion projection has already been absorbed

- :issue:`SERVER-69389` Command checkAuthorization may throw
ErrorCodes::NamespaceNotFound for existing collection while trying to resolve UUID to namespace when the node is  shutting down.
