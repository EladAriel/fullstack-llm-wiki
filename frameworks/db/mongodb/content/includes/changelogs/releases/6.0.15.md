...
source_branch: "main"
source_path: "content/manual/manual/source/includes/changelogs/releases/6.0.15.rst"
...
:issue:`SERVER-84368` CreateIndex fails with StaleConfig if run from a stale mongos against a sharded non-empty collection

### Query

:issue:`SERVER-83602` $or -> $in MatchExpression rewrite should not generate $or directly nested in another $or
...
:issue:`WT-12587` Re-enable compile-clang tasks for older versions of clang
...
tries to strip non-existent index assignment from $_internalSchemaAllElemMatchFromIndex
...
setCompatibilityFeatureFeature to crash the config server while downgrading the cluster
...
branches in evergreen to generate and add SBE $group/$lookup tests with $skip/$limit prefixes
