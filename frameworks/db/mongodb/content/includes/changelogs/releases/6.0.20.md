...
source_branch: "main"
source_path: "content/manual/manual/source/includes/changelogs/releases/6.0.20.rst"
...
:issue:`SERVER-94622` [v6.0] Skip logging oplog constraint violation if coll is image.collections
...
:issue:`SERVER-91436` Remove unused _internalCollation parameter to $lookup aggregation stage
...
may survive the shutdown of the mongod (and raise false memory leak notifications).
...
featureFlagCollModIndexUnique is enabled when the indexOptions contain prepareUnique.
...
for the range deleter if refineCollectionShardKey is run concurrently in random_moveChunk_refine_collection_shard_key.js
