...
source_branch: "main"
source_path: "content/manual/manual/source/includes/changelogs/releases/6.0.7.rst"
...
:issue:`SERVER-75922` Partial unique indexes created on MongoDB 4.0 can be missing index keys after upgrade to 4.2 and later, leading to uniqueness violations
...
search query requests to local mongot, but not search index management requests to the local Envoy
...
TaskExecutorCursorTest Should Allow getMores on cursors to occur in either order

- :issue:`SERVER-77154` Change server's internal client implementations
to always use "hello" rather than "isMaster" for the connection handshake
