---
type: "Framework Learn Page"
framework: "mongodb"
source_repo: "https://github.com/mongodb/docs.git"
source_branch: "main"
source_path: "content/manual/manual/source/includes/enable-KMIP-on-windows.rst"
source_commit: "96788e8ed140cbdde184ff82e1066dff4996bde4"
source_commit_short: "96788e8e"
source_commit_date: "2026-06-19T21:35:03-06:00"
generated_at: "2026-06-21T07:41:52Z"
---

> **Important:** Enabling encryption using a KMIP server on Windows fails when using
|kmip-client-cert-file| and the KMIP server enforces TLS 1.2.
To enable encryption at rest with KMIP on Windows, you must:
- Import the client certificate into the Windows Certificate Store.
- Use the |kmip-client-cert-selector| configuration option.
