...
source_branch: "main"
source_path: "content/manual/manual/source/includes/time-series/timeseries-secondary-index-nodejs.rst"
...
The aggregation pipeline uses the :pipeline:`$match` and :pipeline:`$sort` stages to determine which indexes a time series collection can use. An index can be used in the following scenarios:
...
If the `queryPlanner.winningPlan.inputStage.stage` is either `CLUSTERED_IXSCAN` or `IXSCAN`, the index was used. For more information on the explain plan output, see `explain-results`.
...
On a time series collection, you can specify hints using either the index name or the index key pattern. To get the names of the indexes on a collection, use the :method:`db.collection.getIndexes()` method.
