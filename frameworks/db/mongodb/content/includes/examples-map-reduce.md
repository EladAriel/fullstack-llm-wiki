...
source_branch: "main"
source_path: "content/manual/manual/source/includes/examples-map-reduce.rst"
...
The examples in this section include aggregation pipeline alternatives without custom aggregation expressions. For alternatives that use custom expressions, see `Map-Reduce to Aggregation Pipeline Translation Examples <mr-to-agg-examples>`.
...
#. The :pipeline:`$group` stage groups by the `cust_id` and calculates the `value` field (See also :group:`$sum`). The `value` field contains the total `price` for each `cust_id`.
...
#. Then, the :pipeline:`$out` writes the output to the collection `agg_alternative_1`. Alternatively, you could use :pipeline:`$merge` instead of :pipeline:`$out`.
...
> **Seealso:** For an alternative that uses custom aggregation expressions, see
...
object `value` that contains the `count` of `1` and the item `qty` for the order and emits the `sku` (stored in the `key`) and the `value`.
...
mapped to the grouped `keySKU` values passed by map function to the reducer function.

- The function reduces the `countObjVals` array to a single
object `reducedValue` that contains the `count` and the `qty` fields.

- In `reducedVal`, the `count` field contains the sum of the
`count` fields from the individual array elements, and the `qty` field contains the sum of the `qty` fields from the individual array elements.
...
Aggregation Alternative ``````````````````````` Using the available aggregation pipeline operators, you can rewrite the map-reduce operation without defining custom functions:
...
array of distinct order `_id`'s for the `items.sku` (See :group:`$addToSet`).
...
#. The :pipeline:`$project stage reshapes the output document to mirror the map-reduce's output to have two fields id` and `value`. The :pipeline:`$project` sets:
...
- The `qty` field. The `qty` field contains the
total `qty` ordered per each `items.sku` using :group:`$sum`.

- The `orders_ids` array. The `orders_ids` field contains an
array of distinct order `_id`'s for the `items.sku` using :group:`$addToSet`.
...
#. Finally, the :pipeline:`$merge` writes the output to the collection `agg_alternative_3. If an existing document has the same key id` as the new result, the operation overwrites the existing document. If there is no existing document with the same key, the operation inserts the document.
