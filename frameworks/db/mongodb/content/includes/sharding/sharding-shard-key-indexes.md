...
source_branch: "main"
source_path: "content/manual/manual/source/includes/sharding/sharding-shard-key-indexes.rst"
...
Sharded collections typically require an index that supports the `shard key`. The index can be an index on the shard key or a `compound index` where the shard key is a `prefix <compound-index-prefix>` of the index.
