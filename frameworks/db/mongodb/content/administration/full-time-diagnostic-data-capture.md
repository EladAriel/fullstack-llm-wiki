...
generated_at: "2026-08-29T09:39:19.763292Z"
---
.. _self-managed-ftdc:
.. _ftdc-stub:
...
**meta:** :description: Learn how Full Time Diagnostic Data Capture works in MongoDB. Understand data collection, storage defaults, privacy protections, and on-prem retention guidance.
   :keywords: on-prem
...
**note:** If the :binary:`~bin.mongod` process runs in a :term:`container`, FTDC 
...
regulatory bodies. For clusters where these fields are
configured with protected or confidential data, please notify
...
- 500 MB maximum ``diagnostic.data`` folder size.
  ...
  .. versionchanged:: 8.3
     ...
     Default increased from 250 MB (500 MB for sharded clusters).

These defaults provide useful data to MongoDB engineers with minimal
impact on performance or storage size. Modify these values only if
requested by MongoDB engineers for specific diagnostic purposes.
...
MongoDB stores FTDC data only on the local host. You cannot use
FTDC data forwarded by OTel or other log shipping pipelines for
diagnostic purposes.

In on-premises environments that use ephemeral storage or
automation tools such as Terraform, FTDC data is lost when a host
is terminated or rebuilt. To improve the availability of FTDC data
for diagnostic investigations, consider one or both of the
following actions:
...
For information on MongoDB Support, visit `Get Started With MongoDB Support <https://www.mongodb.com/support/get-started>`_.
