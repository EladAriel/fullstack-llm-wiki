...
generated_at: "2026-08-29T09:39:19.769009Z"
---
...
- Use a :ref:`supported architecture <tcmalloc-new-platform-support>`.
...
Support for the updated TCMalloc depends on CPU architecture. Linux
systems that support MongoDB 8.0 support the updated TCMalloc on the
:ref:`x86_64 <prod-notes-supported-platforms-x86_64>` and
:ref:`ARM64 <prod-notes-supported-platforms-ARM64>` architectures
only.

MongoDB uses the legacy TCMalloc version on the
:ref:`PPC64LE <prod-notes-supported-platforms-PPC64LE>` and
:ref:`s390x <prod-notes-supported-platforms-s390x>` architectures,
regardless of operating system version. If you use one of these
architectures, :ref:`disable THP <disable-thp>`.
...
To enable per-CPU caches, disable glibc’s rseq registration by setting
...
If per-CPU caches are still not enabled after disabling glibc rseq,
verify that you're using Linux kernel 4.18 or later:
