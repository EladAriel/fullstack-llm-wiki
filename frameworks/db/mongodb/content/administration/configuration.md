...
generated_at: "2026-08-29T09:39:19.766819Z"
---
.. _configuration-file:
...
**meta:** :keywords: on-prem
   :description: Configure MongoDB using command line or configuration files for self-managed deployments, including security, replication, and sharding settings.
...
**seealso:** The :ref:`Replica Set Security <replica-set-security>` section for
...
-  :setting:`operationProfiling.slowOpInProgressThresholdMs` configures the
   threshold that determines whether an in-progress query is "slow" for the
   purpose of the logging system. The default value is 5000 milliseconds. Set to
   a lower value if the logging system and the database profiler do not return
   useful results or set to a higher value to only log the longest running
   queries.

   .. versionadded:: 8.3

- :setting:`operationProfiling.slowOpSampleRate` configures the fraction of slow
  operation that MongoDB logs or profiles. This setting must be a value between
  0 and 1, inclusive.
