...
generated_at: "2026-08-29T09:39:19.761942Z"
---
.. _change-streams-prod-recommendations:

# Change Streams Production Recommendations

**meta:** :description: Optimize change streams in production by managing BSON size limits, handling replica set and sharded cluster configurations, and performance impacts.
...
updateLookup`` option may return ``null`` for the lookup document. When using a
:pipeline:`$match` filter, this prevents the change stream from finding the
resume token.
