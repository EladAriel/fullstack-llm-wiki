...
source_branch: "main"
source_path: "content/manual/manual/source/appendix/security/appendixC-openssl-client.txt"
...
> **Warning:** This page is provided for :red:`testing purposes` only and the certificates
...
The procedure outlined on this page uses the :red:`test` intermediate authority certificate and key :file:`mongodb-test-ia.crt` and :file:`mongodb-test-ia.key` created in `appendix-ca-certificate`.

## Procedure

The following procedure outlines the steps to create :red:`test` certificates for MongoDB clients. For steps to create :red:`test` certificates for MongoDB servers, see `appendix-server-certificate`.

### A. Create the OpenSSL Configuration File

#. Create a :red:`test` configuration file `openssl-test-client.cnf` for your client with the following content:
...
.. note::
   ...
   Subject Alternative Name (SAN) is the preferred modern standard
   for defining valid hostnames, IP addresses, and other certificate
   subjects. Although a Common Name (CN) value is specified, CN is
   secondary to SAN in modern TLS standards. The MongoDB server only
   uses the CN for hostname matching if SAN is omitted entirely.
```

#. Optional. You can update the default Distinguished Name (DN) values.

> **Important:**    Ensure that client certificates differ from server certificates
...
#. Create the :red:`test` certificate signing request :file:`mongodb-test-client.csr`. When asked for Distinguished Name values, enter the appropriate values for your :red:`test` certificate:

> **Important:**    The client certificate subject must differ to a server certificate
