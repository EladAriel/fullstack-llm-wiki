...
generated_at: "2026-08-29T09:39:19.590114Z"
---
.. _ttl-collections:
...
**meta:** :description: Automatically remove MongoDB data after a set time with TTL or time to live collections.
...
**procedure:** :style: normal
   ...
   .. step:: Add a document that contains the ``expiresAfter`` field to the collection
      ...
      #. Click :guilabel:`Insert`.
         ...
         The document expires automatically one second after the
         ``expiresAfter`` field's value.

         The TTL index may take one to two seconds to expire the document.
...
value greater than or equal to zero in the ``expireAfterSeconds``
field. A document expires when the number of seconds in the
``expireAfterSeconds`` field has passed since the time specified in
its indexed field.
...
MongoDB automatically deletes documents from the ``log_events``
...
To expire documents at a specific clock time, create a TTL index on a
field that holds values of BSON date type or an array of BSON
date-typed objects, and specify an ``expireAfterSeconds`` value of
``0``. For each document in the collection, set the indexed date
...
number of seconds specified in ``expireAfterSeconds``, that is, ``0``
seconds older in this case.
