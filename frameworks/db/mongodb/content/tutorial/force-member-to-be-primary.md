...
generated_at: "2026-08-29T09:39:19.607117Z"
---
# Force a Self-Managed Replica Set Member to Become Primary

**meta:** :keywords: on-prem
   :description: Force a replica set member to become primary by adjusting its priority value higher than other members in the set.
...
1. In a :binary:`~bin.mongosh` session that is connected to the primary,
   use the following sequence of operations to make ``m3.example.net``
   the primary:
...
#. In a :binary:`~bin.mongosh` session that is connected to the
   :binary:`~bin.mongod` instance running on ``mdb2.example.net``,
   freeze ``mdb2.example.net`` so that it does not attempt to become
...
#. In a :binary:`~bin.mongosh` session that is connected to the
   :binary:`~bin.mongod` running on ``mdb0.example.net``, step down this
   instance that the :binary:`~bin.mongod` is not eligible to become
   primary for 120 seconds:
