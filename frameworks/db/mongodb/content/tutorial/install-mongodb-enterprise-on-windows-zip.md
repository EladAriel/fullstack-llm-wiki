...
generated_at: "2026-08-29T09:39:19.653700Z"
---
...
This tutorial installs MongoDB on Windows by extracting from a zip
file. You can also install MongoDB on Windows using these methods:
...
#. Open a Windows Command prompt as an :guilabel:`Administrator`,
   change to your MongoDB install directory, and run ``mongod`` with
   the ``--dbpath``, ``--logpath`` and ``--install`` parameters. For
   example, the following command installs MongoDB as a service that
   uses ``C:\data\db`` for its data location and ``C:\data\log.txt``
   for the log file:
