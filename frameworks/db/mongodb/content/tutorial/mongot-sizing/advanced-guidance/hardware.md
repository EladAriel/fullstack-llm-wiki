...
source_branch: "main"
source_path: "content/manual/manual/source/tutorial/mongot-sizing/advanced-guidance/hardware.txt"
...
> **Note:** Horizontal scaling adds additional load to a replica set because
...
> **Note:** Increasing the JVM Heap size beyond 50% of available memory may
result in insufficient memory for filesystem cache usage.

Vector Search Guidance ```````````````````````

For vector search, "Search Process Memory" is used for efficient storage of data structures like the HNSW graph. If the Vector Index Size exceeds 3GB, use `vector quantization <avs-quantization>`. When you quantize your vectors, only 4% of the index needs to be stored in memory, rather than the full index.
...
a. Review your index definition and reduce storedSource fields and remove all non-essential fields from the index to reduce heap pressure.

b. Use static mapping. A dynamic mapping will create an index field for every unique field in a collection's documents. Being more selective and only indexing essential fields will reduce heap consumption.
...
See `Disk Sizing Guideline <mongot-indexes-data-characteristics>`.
...
To track current index consumption, monitor :guilabel:`Search Disk Space Used`. Sustained IOPS usage over 1K warrants investigation.

> **Note:** When the `mongot` host's storage utilization reaches 90%,
