...
generated_at: "2026-08-29T09:39:19.629569Z"
---
.. _backup-sharded-lvm:
...
**meta:** :keywords: on-prem
   :description: Learn how to back up a self-managed sharded cluster using file system snapshots, including stopping writes and locking the cluster.
...
components of a self-managed :term:`sharded cluster`. This procedure uses file 
system snapshots to capture a copy of the :binary:`~bin.mongod` instance.
...
**procedure:** :style: normal
   ...
   .. step:: Lock the cluster
      ...
      To lock a sharded cluster, use the :method:`db.fsyncLock` method on 
      :program:`mongos` and the primary :program:`mongod` of the config servers:

      .. code-block:: javascript

         db.getSiblingDB("admin").fsyncLock()

      To confirm the lock, run the following aggregation pipeline on both 
      :program:`mongos` and the primary :program:`mongod` of the config servers. 
      Ensure that the ``fysncLocked`` field returns ``true`` and
