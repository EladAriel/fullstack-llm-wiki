...
`Replication lag <replication lag>` makes "lagged" members ineligible to quickly become primary and increases the possibility that distributed read operations will be inconsistent.
...
call the :method:`rs.printSecondaryReplicationInfo()` method to display the current lag on each secondary relative to the primary host.
...
or increasing oplog time values in the :guilabel:`Replication Lag` graph available in [Cloud Manager](https://docs.cloudmanager.mongodb.com/reference/alerts/replication-lag/) and in [Ops Manager](https://docs.opsmanager.mongodb.com/current/reference/alerts/replication-lag/).

Additionally, you can monitor replication lag in Atlas by checking the :guilabel:`Replication Lag`, :guilabel:`Oplog GB/Hour`, and :guilabel:`Replication Oplog Window` in the metrics tab of your cluster. For more information, see [Review Available Metrics](https://www.mongodb.com/docs/atlas/review-available-metrics/).
...
Use tools including :dbcommand:`ping` to test latency between set members and `traceroute` to expose the routing of packets between network endpoints.

Alternatively, run :dbcommand:`replSetGetStatus` and examine the `pingMs` field. This returns the current network latency in milliseconds between the primary and secondary nodes.
...
Incoming queries must evict cached data before they execute, which can increase replication lag.

Atlas users can also monitor the :guilabel:`WiredTiger Cache Activity` and :guilabel:`Page Faults` metrics to investigate cache-related issues.

For potential strategies to fix this issue, see `Scale your resources <sharding-scaling-strategies>` .
...
Atlas users can access [Atlas metrics](https://www.mongodb.com/docs/atlas/review-available-metrics/) such as :guilabel:`Disk IOPS` and :guilabel:`Disk Space Used` to investigate disk issues.
...
- Increasing provisioned `IOPS <disk-io-util>`
- Upgrading to `NVMe Storage <create-cluster-storage>`
...
Start by deleting small date ranges. If those operations complete in seconds, increase the batch size. Monitor replication lag with :method:`rs.printSecondaryReplicationInfo()`. Increase the batch size until you reach a balance between throughput and replication lag. Continue to monitor system load, the impact on other users and applications, and how far the secondaries lag.
...
reduce contention caused by bulk deletes when other write operations occur simultaneously.

> **Note:**   Take caution when modifying :parameter:`storageEngineConcurrentWriteTransactions`,
...
execution time to prevent long-running operations that affect server performance. Some examples of complex operations include matching many documents or querying by non-indexed fields.
...
To check the size of the oplog and the date ranges of its operations for a given replica set member, connect to the member in :binary:`~bin.mongosh` and run the :method:`rs.printReplicationInfo()` method. The oplog should be long enough to hold all transactions for the longest downtime you expect on a secondary. [#oplog]_ At a minimum, an oplog should be able to hold minimum 24 hours of operations; however, many users prefer to have 72 hours or even a week's work of operations.
...
To confirm that the issue is resolved, call the :method:`rs.printSecondaryReplicationInfo()` method and check that there are no longer any lagging members.

## Diagnostics to Collect for More Support

If none of the above solutions reduce your lag, `contact support <request-support>`. Support may ask for diagnostics to further diagnose your problem.
...
- [Troubleshoot Replica Sets](https://www.mongodb.com/docs/manual/tutorial/troubleshoot-replica-sets/#replication-lag-causes)_
