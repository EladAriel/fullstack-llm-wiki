...
generated_at: "2026-08-29T09:39:19.523955Z"
---
**facet:** :name: programming_language
   :values: c, cpp, csharp, go, java, javascript/typescript, php, python, ruby, rust, scala
...
**meta:** :keywords: motor, java sync, code example, node.js, compass
   :description: Use distributed transactions across multiple collections, databases, and documents for situations requiring atomicity of reads/writes to multiple documents.
...
documents and collections, multi-document transactions are not necessary
for many practical use cases.

For situations that require atomicity of operations on multiple
documents, MongoDB supports transactions. You can use transactions across
multiple operations, collections, databases, documents, and shards.

## Transactions API

Use the language selector to set the language of the
following example.
...
.. composable-tutorial::
   ...
   .. selected-content::
      ...
      .. literalinclude:: /code-examples/tested/java/driver-sync/crud/transactions/WithTransaction.snippet.with-transaction.java
         :language: java
         :copyable: true
   ...
   .. selected-content::
      ...
      .. note:: 
         ...
         When possible, use the :ref:`callback API <txn-callback-api>`, which 
         incorporates automatic retry logic. 
...
Transactions can be used across multiple operations,
...
**seealso:** :ref:`Transactions and Operations Reference <transactions-operations-ref>`

.. _transactions-create-collections-indexes:

### Create Collections and Indexes in a Transaction

You can perform the following operations in a :ref:`transaction <transactions>` 
if it is not a cross-shard write transaction:
...
**seealso:** :ref:`transactions-ops-restricted`
...
:ref:`read concern <read-concern>`. This means a read concern set at
the collection and database level is ignored inside the transaction.

You can set the transaction-level :ref:`read concern <read-concern>`
...
- If transaction-level and the session-level read concern are unset,
  ...
  - :ref:`MongoDB Defaults <default-mongodb-read-write-concerns>`
...
Transactions use the transaction-level :ref:`write concern
<write-concern>` to commit the write operations. Write
...
time, the writes are committed using the transaction-level write
concern.
...
You can set the transaction-level :ref:`write concern <write-concern>`
at the transaction start:
...
**seealso:** :ref:`MongoDB Defaults <default-mongodb-read-write-concerns>`
...
The following sections describe transaction requirements and
considerations that vary by deployment type, storage engine, and
MongoDB version.
...
- :ref:`Drivers API <transactions-drivers>`
- :ref:`Production Considerations <production-considerations>`
- :ref:`Sharded Clusters <production-considerations-sharded>`
- :ref:`Transactions and Operations <transactions-operations-ref>`
