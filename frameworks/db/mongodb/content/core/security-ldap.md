...
generated_at: "2026-08-29T09:39:19.513325Z"
---
.. _security-ldap:
...
**meta:** :keywords: on-prem
   :description: Configure LDAP authentication for MongoDB Enterprise using operating system libraries or `saslauthd` for secure user authentication.
...
.. list-table::
   ...
   * - Operating system libraries 
     ...
     - MongoDB supports binding to an LDAP server via operating system 
       libraries. 
...
When using LDAP authentication **without** :ref:`LDAP authorization
<ldap-authorization>`, user management requires managing users both on
the LDAP server and the MongoDB server. For each user authenticating via
LDAP, MongoDB requires a user on the ``$external`` database whose name
...
**example:** A user authenticates as ``sam@dba.example.com``. The MongoDB server
