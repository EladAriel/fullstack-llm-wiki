...
source_branch: "main"
source_path: "content/manual/manual/source/core/schema-validation/use-json-schema-query-conditions.txt"
...
To find documents that either match or don't match a specified schema, use :query:`$jsonSchema` with query operators. Similarly, you can update or delete documents based on a schema by using `$jsonSchema` in query conditions for write operations.
...
To find documents in a collection that don't match the schema validation rules, use :query:`$jsonSchema` with the :query:`$nor` operator. For example:
...
- `query-projection-operators-top`
