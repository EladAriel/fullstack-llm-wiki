...
generated_at: "2026-08-29T09:39:19.547293Z"
---
.. _gridfs:
...
**meta:** :description: GridFS is a specification for storing and retrieving files that exceed the BSON-document size limit of 16 MiB.
   :keywords: on-prem
...
<limit-bson-document-size>` of 16 MiB.

**note:** GridFS does not support :ref:`multi-document transactions <transactions>`.
...
a separate document. By default, GridFS uses a chunk size of 255 KiB;
that is, GridFS divides a file into chunks of 255 KiB with the exception
...
When you query GridFS for a file, the driver reassembles the chunks
...
GridFS is useful not only for storing files that exceed 16 MiB but also
...
Use :term:`GridFS` for storing files larger than 16 MiB.
...
- When you want to keep your files and metadata synced and deployed
  across a number of systems and facilities, use GridFS. With
  :ref:`geographically distributed replica sets
  <replica-set-geographical-distribution>`, MongoDB distributes
  files and their metadata to :binary:`~bin.mongod` instances
...
If your files are all smaller than the 16 MiB :limit:`BSON
Document Size` limit, consider storing each file in a single document
instead of using GridFS. Use the BinData data type to store the binary
data. See your :driver:`drivers </>` documentation for details on
using BinData.
...
**data:** files.chunkSize
   ...
   large as needed. The default size is 255 kibibytes (KiB).
...
**data:** files.md5
   ...
   **Deprecated**
   ...
   An MD5 hash of the complete file returned by the
   :dbcommand:`filemd5` command. This value has the ``String`` type.
...
**data:** files.aliases
   ...
   **Deprecated**
   ...
   Use :data:`files.metadata` for storing alias information.

**data:** files.metadata

   Optional. The metadata field may be of any data type and can hold
   any additional information you want to store. To add arbitrary
   fields to documents in the ``files`` collection, add them to an
...
</gridfs/gridfs-spec.rst>` automatically ensure that this index
...
specification </gridfs/gridfs-spec.rst>` automatically ensure that
this index exists before read and write operations. See the relevant
driver documentation for the specific behavior of your GridFS
application.
...
:term:`GridFS` has two collections to consider: ``files`` and
``chunks``.
...
successful upload, you can use :ref:`hashed sharding <sharding-hashed-sharding>` for the
``chunks`` collection.

If the MongoDB driver runs :dbcommand:`filemd5`, you cannot use
:ref:`hashed sharding <sharding-hashed-sharding>`. For details, see :issue:`SERVER-9888`.
