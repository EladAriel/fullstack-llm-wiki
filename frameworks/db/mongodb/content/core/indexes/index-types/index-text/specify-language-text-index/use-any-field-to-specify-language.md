...
generated_at: "2026-08-29T09:39:19.842972Z"
---
...
**note:** :atlas:`{+fts+} </atlas-search/>` offers advanced full-text search 
   capabilities, including :ref:`custom analyzers <custom-analyzers>` with 
   :ref:`token filters <token-filters-ref>`. We recommend using 
...
and ignore stop words when you run queries with the ``$text`` operator.
...
The index supports ``$text`` queries on the ``quote`` field and uses
