...
generated_at: "2026-08-29T09:39:19.834205Z"
---
...
**meta:** :keywords: on-prem
   :description: Assign weights to $text query results in MongoDB to prioritize certain fields, enhancing relevance in self-managed deployments.
...
**note:** :atlas:`{+fts+} </atlas-search/>` offers advanced full-text 
   ...
   <fts-customize-score>` and :ref:`score boosting <scoring-boost>`. We 
...
When MongoDB returns results for ``$text`` queries, it assigns a **score** to 
each returned document. The score indicates the relevance of the document to a 
given search query. You can sort returned documents by score to have the most 
relevant documents appear first in the result set.
...
The default weight for indexed fields is 1. To adjust
...
You want to create a text index so users can perform ``$text`` queries on 
blog posts. Your application supports searches on content, topics, and keywords.
