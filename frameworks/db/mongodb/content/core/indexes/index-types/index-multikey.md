...
generated_at: "2026-08-29T09:39:19.831266Z"
---
...
**meta:** :description: Details on when and how to use multikey indexes in a MongoDB database.
...
values. These indexes improve performance for queries on array fields.

You do not need to explicitly specify an index as multikey. If you
create an index on a field that contains an array value, MongoDB automatically creates the index as a multikey index.

For each distinct value in the array, MongoDB creates a separate entry
in the index, and each entry points back to the same document. As a
result, a single document can have multiple entries in a multikey index.
...
For example, documents in a ``movies`` collection contain a
``genres`` field: an array of genres associated with each movie.
You regularly query movies that have specific genres, such as
finding all movies that are both ``Drama`` and ``Action``.

You can create an index on the ``genres`` field to improve
performance for this query. Because ``genres`` contains an array
...
}`` on the ``movies`` collection because for each document, only one field
indexed by the compound multikey index is an array. No document contains array
values for both ``genres`` and ``year`` fields.

However, after you create the compound multikey index, if you attempt
to insert a document where both ``genres`` and ``year``
...
For example, the following operation creates a compound multikey index on the
``movies`` collection on the ``genres`` and ``title`` fields:

**literalinclude:** /code-examples/tested/command-line/mongosh/indexes/multikey/compound-multikey.js
   :language: javascript
   :category: usage example

The preceding index is multikey because the ``genres`` field contains
array values.

The index covers these queries:

**literalinclude:** /code-examples/tested/command-line/mongosh/indexes/multikey/query-1.js
   :language: javascript
   :category: usage example

**literalinclude:** /code-examples/tested/command-line/mongosh/indexes/multikey/query-2.js
   :language: javascript
   :category: usage example

The index does not cover the following query because the projection
contains the ``genres`` array field:

**literalinclude:** /code-examples/tested/command-line/mongosh/indexes/multikey/query-3.js
   :language: javascript
   :category: usage example

### Query on an Array Field as a Whole

When a query specifies an :ref:`exact match for an array as a whole
<array-match-exact>`, MongoDB uses the multikey index to locate
documents that contain the first element of that array. However, it cannot
match the entire array using the index alone. MongoDB then fetches the
candidate documents and filters them to return only those whose array
exactly matches the query array.

For example, the following operation creates a multikey index on the ``movies``
collection on the ``genres`` field:

**literalinclude:** /code-examples/tested/command-line/mongosh/indexes/multikey/array-field.js
   :language: javascript
   :category: usage example

The following query looks for documents where the ``genres`` field is
the array ``[ "Drama" ]``:

**literalinclude:** /code-examples/tested/command-line/mongosh/indexes/multikey/query-4.js
   :language: javascript
   :category: usage example

MongoDB can use the multikey index to find documents that have ``"Drama"`` at
any position in the ``genres`` array. Then, MongoDB retrieves these
documents and filters for documents whose ``genres`` array equals the
query array ``[ "Drama" ]``.
