...
generated_at: "2026-08-29T09:39:19.849590Z"
---
.. _wildcard-index-restrictions:

# Wildcard Index Restrictions

**meta:** :description: Understand Wildcard index limitations, like compound index restrictions, incompatible properties, unsupported query patterns, and shard key constraints.
...
In the case that a single wildcard index could support multiple query fields, 
MongoDB can only use the wildcard index to support one of the query fields.
...
Instead, MongoDB uses the wildcard index to support only one of the query 
predicates. MongoDB automatically chooses which predicate to support based 
on relevant wildcard index paths. The unsupported query predicates are
shown in the :data:`~explain.queryPlanner.rejectedPlans` of the
:ref:`explain results <explain-results>`.

This is also true for compound wildcard indexes. The wildcard term of a compound
index can only support one query predicate, although the non-wildcard terms
can support the remaining predicates.
