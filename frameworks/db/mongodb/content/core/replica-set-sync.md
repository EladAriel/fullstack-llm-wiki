...
generated_at: "2026-08-29T09:39:19.559216Z"
---
.. _replica-set-sync:

# Replica Set Data Synchronization

**meta:** :description: Understand how secondary members in a replica set synchronize data from a source member using initial sync and ongoing replication processes.
...
**note:** During the initial sync, MongoDB truncates the oplog on the
   |destin|. This oplog truncation can impact processes such as
   :ref:`change streams <changeStreams>` and :ref:`Atlas Stream
   Processing checkpoints <atlas-sp-checkpoints>` that depend on
   oplog data.
...
**note:** Cloud-based Initial Sync Disambiguation
   ...
   For self-managed deployments, initial sync is the process that
   MongoDB uses to add a new node to a replica set. This is different
   from :ref:`cloud-based initial syncs <cloud-based-initial-sync>`
...
#. In parallel with the data clone, ``mongod`` builds all collection indexes as 
   it copies all the documents for each collection.
...
**tabs:** .. tab:: First Pass
      ...
      - The |source| *must* be within
        :parameter:`maxSyncSourceLagSecs` (default ``30``) seconds of
        the newest oplog entry on the primary.
...
The :term:`oplog window` must be long enough so that a |destin| can
fetch any new :term:`oplog` entries that occur between the start and
end of the :ref:`replica-set-initial-sync-logical`.
If the window is too short, some entries may fall off the ``oplog``
before the |destin| can apply them.

Size the ``oplog`` with additional time to accommodate changes that
may occur during initial syncs.
