...
generated_at: "2026-08-29T09:39:19.802034Z"
---
.. _manual-views-create:

# Create and Query a View

**meta:** :description: Create and query views in MongoDB using `db.createCollection()` or `db.createView()` with examples and restrictions.
...
To create a view in the {+atlas+} UI, use the
:ref:`Aggregation Pipeline Builder <atlas-ui-agg-builder>`.
To learn more, see :ref:`atlas-ui-views`.
...
The ``{ _id: 0 }`` :ref:`projection
<method-find-projection>` suppresses the ``_id`` field in the
output.
...
- The aggregation pipeline optimizer reshapes the view aggregation
  pipeline stages to improve performance without changing the
  query results.
