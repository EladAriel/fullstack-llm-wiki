...
generated_at: "2026-08-29T09:39:19.545199Z"
---
...
**meta:** :keywords: on-prem
   :description: Configure LDAP authorization for MongoDB deployments, mapping LDAP group DNs to roles for user access control.
...
user based on the mapped roles and their associated privileges.
...
<https://www.mongodb.com/products/consulting>`_ for optimal
...
**example:** The following query template returns any groups listed in the
...
**blockquote:** The ``dn`` is an LDAP Distinguished Name using the string format described
...
**note:** :ref:`LDAP <security-ldap>` and :ref:`kerberos <security-kerberos>`
...
**example:** A database has the following roles configured on the ``admin`` database:
...
When LDAP authorization is enabled, MongoDB can
have different authorization behavior based on your authentication mechanism.
...
The following mechanisms use internal authorization, whether LDAP authorization
is enabled or not:
