...
generated_at: "2026-08-29T09:39:19.528041Z"
---
...
**meta:** :description: Definition, structure, and limitations of documents in MongoDB.
...
MongoDB's object-oriented data model lets you structure data to mirror 
the object models in your codebase, which eliminates the object-relational 
mapping required by relational databases.

**note:** MongoDB stores data records as BSON documents. BSON is a binary
...
**note:** Positional Operators
   ...
   - The :update:`$[]` all positional operator updates all elements in the specified array field during an update operation. 

   - The :update:`$[\<identifier\>]` filtered positional operator updates all array elements that match the conditions specified in arrayFilters. 

   - The :update:`$` positional operator updates the first array element that matches the query condition, without requiring you to specify its index. 

   - The :projection:`$` projection operator returns the first array element that matches the query condition in a read operation. 

   - For dot notation examples with arrays, including queries by zero-based array index position, see :ref:`read-operations-arrays`.
...
- Use your driver's BSON UUID facility to generate UUIDs. Be aware
  ...
  with other drivers. See your :driver:`driver documentation <>` for
...
**toctree:** :titlesonly:
   ...
   Other Uses of the Document Structure </core/documents-other-uses>