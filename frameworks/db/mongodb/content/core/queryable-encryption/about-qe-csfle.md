...
generated_at: "2026-08-29T09:39:19.776678Z"
---
**facet:** :name: genre
   :values: reference

**meta:** :keywords: queryable encryption, in-use encryption, client-side field level encryption
...
Support for prefix, suffix, and substring queries on encrypted string fields is
in Public Preview. You can configure these queries for diacritic folding or
case insensitivity to ensure different representations of the same string
match.
...
Both {+qe+} and {+csfle+} use the `AEAD
<https://en.wikipedia.org/wiki/Authenticated_encryption#Authenticated_encryption_with_associated_data>`__
AES-256-CBC encryption algorithm in authenticated mode to perform encryption.
See :ref:`qe-cryptographic-primitives` for more information.

The encryption algorithm for {+qe+} uses randomized encryption based on
