...
generated_at: "2026-08-29T09:39:19.824244Z"
---
...
**meta:** :keywords: QE, read operations, write operations
   :description: Explore supported operations for Queryable Encryption, including data types, commands, and query operators compatible with automatic encryption.
...
{+qe+} supports :ref:`configuring <qe-create-encryption-schema>` the
equality query type ``queryType: "equality"`` for all :ref:`BSON types
<bson-types>` **except** the following:
...
{+qe+} supports :ref:`configuring <qe-create-encryption-schema>` the
range query type ``queryType: "range"`` for the following :ref:`BSON
types <bson-types>`:
...
- {+qe+} bulk write operations can target only one namespace. If a
  :dbcommand:`bulkWrite` command specifies more than one namespace in
  ``nsInfo``, the operation returns an error.
...
If the query type is unspecified, it defaults to ``none``. If the query type is
``none``, MongoDB encrypts the field, and clients can't query it.
...
Range queries implicitly convert equality queries to :query:`$lte` and
:query:`$gte`. Thus, fields configured for ``queryType: "range"`` support all 
expressions above, as well as the following expressions:
...
- :pipeline:`$graphLookup` (For usage requirements, see 
  :ref:`qe-lookup-graphLookup-behavior`)
...
.. _qe-lookup-graphLookup-behavior:
...
Automatic encryption supports :pipeline:`$graphLookup` *only if* the ``from`` 
collection matches the collection the aggregation runs against. ``$graphLookup`` 
stages that reference a different ``from`` collection return an error.
...
Compatible drivers configured for automatic encryption support the following
expressions against encrypted fields configured for equality queries:
...
Compatible drivers configured for automatic encryption support the following
expressions against encrypted fields configured for ``prefix``, ``suffix``, or
``substring`` queries:
...
types, because {+qe+} doesn't adequately hide the type information for these values:
...
{+qe+} supports automatic and explicit encryption of array fields if the query
type is ``none``. You can encrypt an array, but not the fields within it, and
you can't query an encrypted array.
