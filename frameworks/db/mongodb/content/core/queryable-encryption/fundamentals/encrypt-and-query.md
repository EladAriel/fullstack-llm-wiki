...
generated_at: "2026-08-29T09:39:19.819680Z"
---
**facet:** :name: genre
   :values: tutorial

**meta:** :keywords: queryable encryption, contention
   :description: Define encrypted fields at the collection level using an encryption schema to enable queries, impacting storage and performance.
...
{+qe+} requires a server-side {+enc-schema+} to enforce encryption of specific
fields in a collection. Clients using automatic {+qe+} behave differently
depending on the database connection configuration:
...
Decide which fields should be encrypted and/or queryable prior to creating your
collection. Changing which fields are encrypted or queryable requires rebuilding the collection's {+enc-schema+} and re-creating the
collection.
...
**parameter:** strMaxLength
   ...
   *Type*: Integer from 1-60, inclusive.
   ...
   The maximum allowed length for a substring-indexed field. Attempting to
   insert a longer string returns an error. 
   
   .. important::
   
      You can override the character limit by setting 
      :parameter:`fleDisableSubstringPreviewParameterLimits`, but running
      substring queries against longer strings has a significant impact on 
      performance.
...
**parameter:** strMinQueryLength 
   ...
   *Type*: Positive integer.
   ...
   The minimum allowed prefix/suffix/substring length to query. Attempting to
   query a shorter string returns an error.
...
**parameter:** strMaxQueryLength 
   ...
   *Type*: Positive integer
   - Must be >=1 for ``prefixPreview`` or ``suffixPreview`` queries.
   - Must 2-10 inclusive for ``substringPreview`` queries.

   The maximum allowed prefix/suffix/substring length to query. Attempting to
   query a longer string returns an error.
   
   .. important::

      This setting strongly impacts query performance. Limit the maximum query
...
**parameter:** caseSensitive
   ...
   *Type*: Boolean
   ...
   Whether prefix/suffix/substring queries are case-sensitive. Set to ``false``
...
**parameter:** diacriticSensitive
   ...
   *Type*: Boolean
   ...
   Whether prefix/suffix/substring queries must match diacritical marks. Set to
   ``false`` for diacritic-insensitive matching.
...
**parameter:** precision
   ...
   *Type*: Integer.
   ...
   Limits how many digits after the decimal point are taken into account
   when querying a ``double`` or ``decimal`` field. Additional digits are
   dropped, not rounded. For example, a ``precision`` of 1 treats ``10.18``
   as ``10.1`` for queries. The encrypted value is still stored as ``10.18``.
   
   Specify ``precision`` and limit it when possible. Every digit increases 
   storage overhead and has a high impact on searchable range and index 
   generation. 
