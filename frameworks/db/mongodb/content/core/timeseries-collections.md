...
generated_at: "2026-08-29T09:39:19.512265Z"
---
...
Time series collections efficiently store time series data. Writes are
organized so that data from the same source is stored alongside other
data points from a similar point in time.
...
Time series documents can contain a ``metaField``. MongoDB uses the
``metaField`` to group documents for storage optimization and query
efficiency. For more information about the ``metaField``, see
:ref:`metaField Considerations <timeseries-metafield-considerations>`.
