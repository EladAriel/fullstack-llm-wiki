...
generated_at: "2026-08-29T09:39:19.524274Z"
---
.. _text-search-operators-onprem:
.. _text-search-operators-on-premises:

# $text Query Operators
**meta:** :description: Run $text queries and sort results by relevance with the $meta operator.
...
:pipeline:`$match` with a ``$text`` expression. To sort the results in order of 
relevance score, use the :expression:`$meta` aggregation operator in the 
:pipeline:`$sort` stage.
