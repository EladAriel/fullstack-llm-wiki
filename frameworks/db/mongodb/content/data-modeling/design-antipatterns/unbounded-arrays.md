...
source_branch: "main"
source_path: "content/manual/manual/source/data-modeling/design-antipatterns/unbounded-arrays.txt"
...
In this example, the bookstore application only needs to show three book reviews per book. To avoid unbounded arrays, you can use the subset design pattern or `document references <data-modeling-referencing>`, depending on your use case.
...
If your `books` and `reviews` information is stored in separate collections, the application needs to perform a :pipeline:`$lookup` operation to join the data.
