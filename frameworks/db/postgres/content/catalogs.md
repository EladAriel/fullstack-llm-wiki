...
generated_at: "2026-08-29T09:39:24.498622Z"
---
# System Catalogs
...
SCRAM-SHA-256$<iteration count>:<salt>$<StoredKey>:<ServerKey>
  ...
  pg_class
   ...
   pg_class Columns
       ...
       231−1 are stored as negative values.
       ...
       also be used along with relallvisible for
       scheduling manual vacuums and tuning vacuum's freezing
       behavior.
  ...
  pg_collation
   ...
   pg_collation Columns
       ...
       Provider of the collation: d = database default,
       b = builtin, c = libc,
       i = icu 
       ...
       LC_COLLATE for this collation object. If the provider is
       not libc, collcollate is
       NULL and colllocale is
       used instead.
       ...
       LC_CTYPE for this collation object. If the provider is
       not libc, collctype is
       NULL and colllocale is
       used instead.
  ...
  pg_constraint
   ...
   pg_constraint Columns
       ...
       This constraint is defined with WITHOUT OVERLAPS
       (for primary keys and unique constraints) or PERIOD
       (for foreign keys).
  ...
  pg_database
   ...
   pg_database Columns
       ...
       Locale provider for this database: b = builtin,
       c = libc, i = icu 
  ...
  pg_propgraph_element
   ...
   pg_propgraph_element Columns
       ...
       pgesrcvertexid oid
       (references pg_propgraph_element.oid)
       ...
       pgerelid that defines the key to use for this
       ...
       pgerelid that defines the source key to use
       for this element table.  (Null for a vertex.)  The combination of
       pgesrckey and
       pgesrcref creates the link between the edge
       ...
       pgesrcvertexid.  (Null for a vertex.)  The
       combination of pgesrckey and
       pgesrcref creates the link between the edge
       and the source vertex.
       ...
       pgesrcref =
       pgesrckey comparison. (Null for a vertex.)
       ...
       For an edge, an array of column numbers in the table referenced by
       pgerelid that defines the destination key to use
       for this element table.  (Null for a vertex.)  The combination of
       pgedestkey and
       pgedestref creates the link between the edge
       ...
       For an edge, an array of column numbers in the table reached via
       pgedestvertexid.  (Null for a vertex.)  The
       combination of pgedestkey and
       pgedestref creates the link between the edge
       and the destination vertex.
       ...
       For an edge, an array of equality operators for
       pgedestref =
       pgedestkey comparison. (Null for a vertex.)
  ...
  pg_propgraph_element_label
   ...
   The catalog pg_propgraph_element_label stores
   ...
   pg_propgraph_element_label Columns
       ...
       pgellabelid oid
       (references pg_propgraph_label.oid)
  ...
  pg_propgraph_label
   ...
   The catalog pg_propgraph_label stores
  ...
  pg_propgraph_label_property
   ...
   The catalog pg_propgraph_label_property stores
   ...
   pg_propgraph_label_property Columns
       ...
       plppropid oid
       (references pg_propgraph_property.oid)
       ...
       plpellabelid oid
       (references pg_propgraph_element_label.oid)
      
      
       Reference to the label (indirectly via
       pg_propgraph_element_label, which then links
       to pg_propgraph_label)
       ...
       reached via pg_propgraph_element_label and
       pg_propgraph_element.
  ...
  pg_propgraph_property
   ...
   The catalog pg_propgraph_property stores
   ...
   actual expressions that define the properties are in the catalog pg_propgraph_label_property.
   ...
   pg_propgraph_property Columns
      ...
      typmod to be applied to the data type of this property.
