...
source_path: "doc/src/sgml/cube.sgml"
...
`cube-repr-table` shows the valid external representations for the `cube` type. `x`, `y`, etc. denote floating-point numbers.
...
`cube-operators-table` shows the specialized operators provided for type `cube`.
...
The `cube` module also provides a GiST index operator class for `cube` values. A `cube` GiST index can be used to search for values using the `=`, `&&`, `@>`, and `` operator can also be used in this way to efficiently retrieve the first few values sorted by a selected coordinate. For example, to get the first few cubes ordered by the first coordinate (lower left corner) ascending one could use the following query:
