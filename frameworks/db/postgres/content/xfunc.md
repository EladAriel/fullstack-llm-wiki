...
generated_at: "2026-08-29T09:39:24.371683Z"
---
...
...
     ...
     SET_VARSIZE function, not by simple assignment.
...
my_shmem_init(void *arg)
{
    ...
    ... initialize the rest of MyShmem fields ...
      ...
      complex initialization, set the init_fn() callback,
      ...
      On Windows, the attach_fn callback, if any, is
      ...
      area that is inherited via fork() on other systems.
     ...
     Requesting Shared Memory after Startup with ShmemRequestStruct
      ...
      The ShmemRequestStruct() can also be called after
      ...
      By default, RegisterShmemCallbacks() fails with an
      error if called after system startup. To use it after startup, you must
      set the SHMEM_CALLBACKS_ALLOW_AFTER_STARTUP flag in
      the argument ShmemCallbacks struct to
...
void *GetNamedDSMSegment(const char *name, size_t size,
      ...
      backend.  In the former case, GetNamedDSMSegment
      passes the void *arg argument to the
      init_callback.  This is particularly useful for
      reusing an initialization callback function for multiple DSM segments.
     

     
      GetNamedDSMSegment ensures that only
...
extern PgStat_Kind pgstat_register_kind(PgStat_Kind kind,
     ...
     Custom Cumulative Statistics page.
     ...
     found in src/test/modules/test_custom_stats.
...
# Function Optimization Information
   ...
   for functions
    ...
    function to an SQL-callable function or aggregate (called
...
supportfn(internal) returns internal
    ...
    example, COUNT(x) can be
    replaced by COUNT(*)
    when x is known to not be null.  This can be
    done by a support function that implements
    the SupportRequestSimplifyAggref request type.  The
    ...
    SupportRequestSimplify, it is the support function's
