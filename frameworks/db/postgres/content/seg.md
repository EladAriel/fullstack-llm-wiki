...
source_path: "doc/src/sgml/seg.sgml"
...
The external representation of an interval is formed using one or two floating-point numbers joined by the range operator (`..` or `...`). Alternatively, it can be specified as a center point plus or minus a deviation. Optional certainty indicators (`` or `~`) can be stored as well. (Certainty indicators are ignored by all the built-in operators, however.) `seg-repr-table` gives an overview of allowed representations; `seg-input-examples` shows some examples.

In `seg-repr-table`, `x`, `y`, and `delta` denote floating-point numbers. `x` and `y`, but not `delta`, can be preceded by a certainty indicator.
...
by `seg` operations, but
is preserved as a comment.

`5.0`

Creates a point at 5.0. `>` is ignored but
is preserved as a comment.

`5(+-)0.3`

Creates an interval `4.7 .. 5.3`.
Note that the `(+-)` notation isn't preserved.
...
The `seg` module includes a GiST index operator class for `seg` values. The operators supported by the GiST operator class are shown in `seg-gist-operators`.
...
Is the first `seg` entirely to the left of the second? [a, b] >` `seg` boolean

Is the first `seg` entirely to the right of the second? [a, b] >> [c, d] is true if a > d.

`seg` `&boolean

Does the first `seg` not extend to the right of the second? [a, b] &` `seg` boolean
