...
source_path: "doc/src/sgml/pgtrgm.sgml"
...
The functions provided by the `pg_trgm` module are shown in `pgtrgm-func-table`, the operators in `pgtrgm-op-table`.
...
similarity `similarity` ( `text`, `text` ) real
...
strict_word_similarity `strict_word_similarity` ( `text`, `text` ) real
...
Commutator of the `>` `text` boolean

Commutator of the `` `text` real
...
- Sets the current word similarity threshold that is used by the `` operators. The threshold must be between 0 and 1 (default is 0.6).
- Sets the current strict word similarity threshold that is used by the `>` operators. The threshold must be between 0 and 1 (default is 0.5).
