...
source_path: "doc/src/sgml/ref/create_sequence.sgml"
...
After a sequence is created, you use the functions `nextval`, `currval`, and `setval` to operate on the sequence. These functions are documented in `functions-sequence`.
...
`CREATE SEQUENCE` conforms to the SQL standard, with the following exceptions: - Obtaining the next value is done using the `nextval()` function instead of the standard's `NEXT VALUE FOR` expression. - The `OWNED BY` clause is a PostgreSQL extension.
