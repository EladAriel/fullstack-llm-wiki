...
generated_at: "2026-08-29T09:39:24.611843Z"
---
CREATE VIEW
...
CREATE VIEW [ schema . ] view_name AS WITH RECURSIVE view_name (column_names) AS (SELECT ...) SELECT column_names FROM view_name;
    ...
    WITH [ CASCADED | LOCAL ] CHECK OPTION
      ...
      rejected.  The temporal
      leftovers inserted by an UPDATE or
      DELETE with a FOR PORTION OF clause
      are checked in the same way.  If the CHECK OPTION is
...
```
    ...
    tables.  However, not all views are secure against tampering; see  for details.
...
# Updatable Views
    ...
    FOR PORTION OF clause can do so as well, since the
