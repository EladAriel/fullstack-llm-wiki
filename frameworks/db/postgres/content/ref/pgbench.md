...
generated_at: "2026-08-29T09:39:24.610691Z"
---
pgbench
...
# Per-Transaction Logging
   ...
   serialization, deadlock, or
   other depending on the type of failure (see
    for more information).
...
# Aggregated Logging
   ...
   fields:
     ...
     other_sql_failures
       ...
       (zero unless both --failures-detailed and
       --continue-on-error are specified)
...
# Failures and Serialization/Deadlock Retries
   ...
   of errors:
         ...
         However, if --exit-on-abort is specified, all of the
   ...
   serialization or deadlock errors when --continue-on-error
   is not specified.  With --continue-on-error,
   the client does not abort on such SQL errors and instead proceeds to
   the next transaction.  These cases are reported as
   other failures in the output.  If the error occurs
   ...
   --continue-on-error is used.  Instead, the current
   ...
   when neither --progress, --rate, nor
   --latency-limit is specified, the
   latency average is computed from the total benchmark
   duration divided by both successful and failed transactions, and therefore
   includes failed transactions.  Otherwise, the latency
   average and latency stddev shown in the main
