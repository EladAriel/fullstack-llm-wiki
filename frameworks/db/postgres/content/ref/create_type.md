...
source_path: "doc/src/sgml/ref/create_type.sgml"
...
The second form of `CREATE TYPE` creates an enumerated (enum) type, as described in `datatype-enum`. Enum types take a list of quoted labels, each of which must be less than `NAMEDATALEN` bytes long (64 bytes in a standard PostgreSQL build). (It is possible to create an enumerated type with zero labels, but such a type cannot be used to hold values before at least one label is added using ALTER TYPE.)

## Range Types

The third form of `CREATE TYPE` creates a new range type, as described in `rangetypes`.
...
To indicate that a type is a fixed-length array type, specify the type of the array elements using the `ELEMENT` key word. For example, to define an array of 4-byte integers (`int4`), specify `ELEMENT = int4`. For more details, see `sql-createtype-array` below.
...
- The category code (a single ASCII character) for this type. The default is `'U'` for user-defined type. Other standard category codes can be found in `catalog-typcategory-table`. You may also choose other ASCII characters in order to create custom categories.
...
More examples, including suitable input and output functions, are in `xtypes`.
