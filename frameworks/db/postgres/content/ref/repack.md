...
source_path: "doc/src/sgml/ref/repack.sgml"
...
It is advisable to set `guc-maintenance-work-mem` to a reasonably large value (but not more than the amount of RAM you can dedicate to the `REPACK` operation) before repacking.
...
While `REPACK` is running, the `guc-search-path` is temporarily changed to `pg_catalog, pg_temp`.

Each backend running `REPACK` will report its progress in the `pg_stat_progress_repack` view. See `repack-progress-reporting` for details.
