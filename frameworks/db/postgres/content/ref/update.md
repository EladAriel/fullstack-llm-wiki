...
generated_at: "2026-08-29T09:39:24.540723Z"
---
UPDATE
...
# Description
   ...
   If the FOR PORTION OF clause is used, the update will
   ...
   OF is updated, and no moments outside.  Furthermore, after a row
   is updated, the range or multirange is first shrunk so that its application
   time no longer extends beyond the targeted FOR PORTION
   OF bounds.  Then, new temporal leftovers might
   be inserted: rows whose range or multirange receives the remaining
   application time outside the targeted
   FROM/TO bounds, with the original
...
# Parameters
    ...
    cursor_name
      ...
      specified together with a Boolean condition or FOR PORTION
      OF.  See  for more information
...
```
   ...
   RETURNING clause gives one result for each updated row,
