...
generated_at: "2026-08-29T09:39:24.583291Z"
---
SELECT
...
# FROM Clause
    ...
    elements:
      ...
      GRAPH_TABLE ( graph_name MATCH graph_pattern COLUMNS ( { expression [ AS name ] } [, ...] ) )
      
       
        This clause produces output from matching the specifying graph pattern
        against a property graph.  See 
        and  for more information.
       

       
        graph_name is the name
        (optionally schema-qualified) of an existing property graph (defined
        with ).
       

       
        graph_pattern is a graph
        pattern in a special graph pattern sublanguage.  See .
       

       
        The COLUMNS clause defines the output columns of
        the GRAPH_TABLE clause.  expression is a scalar expression
        using the graph pattern variables defined in the graph_pattern.  The name of the output
        columns are specified using the AS clauses.  If the
        ...
        Like for other FROM clause items, a table alias
        name and column alias names may follow the GRAPH_TABLE
        (...) clause.  (A column alias list would be redundant with
        the COLUMNS clause.)
...
GROUP BY [ ALL | DISTINCT ] grouping_element [, ...]
    ...
    key word removes duplicate grouping sets before processing; it does not
    transform the implied UNION ALL into
    a UNION DISTINCT.
