...
source_path: "doc/src/sgml/ref/create_cast.sgml"
...
Sometimes it is necessary for usability or standards-compliance reasons to provide multiple implicit casts among a set of types, resulting in ambiguity that cannot be avoided as above. The parser has a fallback heuristic based on type categories and preferred types that can help to provide desired behavior in such cases. See `sql-createtype` for more information.
...
`sql-createfunction`, `sql-createtype`, `sql-dropcast`
