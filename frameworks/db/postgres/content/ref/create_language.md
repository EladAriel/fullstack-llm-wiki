...
source_path: "doc/src/sgml/ref/create_language.sgml"
...
`CREATE LANGUAGE` effectively associates the language name with handler function(s) that are responsible for executing functions written in the language. Refer to `plhandler` for more information about language handlers.
...
The system catalog pg_language (see `catalog-pg-language`) records information about the currently installed languages. Also, the `psql` command `\dL` lists the installed languages.
