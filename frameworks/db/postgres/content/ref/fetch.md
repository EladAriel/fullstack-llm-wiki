...
source_path: "doc/src/sgml/ref/fetch.sgml"
...
This page describes usage of cursors at the SQL command level. If you are trying to use cursors inside a `PL/pgSQL` function, the rules are different -- see `plpgsql-cursor-using`.

## Parameters

- `direction` defines the fetch direction and number of rows to fetch. It can be one of the following: `NEXT` Fetch the next row. This is the default if `direction` is omitted.
- Fetch the prior row.
