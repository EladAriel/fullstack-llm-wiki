...
generated_at: "2026-08-29T09:39:24.603346Z"
---
COPY
...
```
    ...
    HEADER
      ...
      lines are discarded.  If set to MATCH, the first line
    ...
    ON_ERROR
      ...
      and set_null means replace the field containing the invalid
      input value with a null value and continue to the next field.
      The default is stop.
     

     
      The ignore and set_null
      options are applicable only for COPY FROM
      when the FORMAT is text or csv.
     

     
      If ON_ERROR is set to ignore or
      set_null, a NOTICE message is emitted at the end of the
      ...
      for set_null option, a NOTICE
      message containing the line of the input file and the column name where
      value was replaced with NULL for each input conversion
      failure.
...
WHERE condition
    ...
    allowed in WHERE expressions, and the evaluation
    does not see any changes made by the COPY itself
...
# Notes
    ...
    For non-partitioned tables, COPY table
...
```
   ...
   When the FORCE_ARRAY option is enabled,
