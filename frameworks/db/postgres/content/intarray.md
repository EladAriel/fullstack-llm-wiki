...
source_path: "doc/src/sgml/intarray.sgml"
...
The functions provided by the `intarray` module are shown in `intarray-func-table`, the operators in `intarray-op-table`.
...
icount `icount` ( `integer[]` ) integer

Returns the number of elements in the array.

`icount('{1,2,3}'::integer[])` 3

sort `sort` ( `integer[]`, `dir` `text` ) integer[]
...
sort_asc `sort_asc` ( `integer[]` ) integer[]

Sorts in ascending order.

`sort(array[11,77,44])` {11,44,77}

sort_desc `sort_desc` ( `integer[]` ) integer[]

Sorts in descending order.

`sort_desc(array[11,77,44])` {77,44,11}

uniq `uniq` ( `integer[]` ) integer[]
...
The operators `&&`, `@>` and ``, and `@@` operators, as well as regular array equality.
