...
source_path: "doc/src/sgml/earthdistance.sgml"
...
The provided functions are shown in `earthdistance-cube-functions`.
...
sec_to_gc `sec_to_gc` ( `float8` ) float8
...
gc_to_sec `gc_to_sec` ( `float8` ) float8
...
ll_to_earth `ll_to_earth` ( `float8`, `float8` ) earth
...
A single operator is provided, shown in `earthdistance-point-operators`.
