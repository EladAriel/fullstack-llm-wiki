...
source_path: "doc/src/sgml/rowtypes.sgml"
...
The `ROW` expression syntax is discussed in more detail in `sql-syntax-row-constructors`.
...
The special field name `*` means all fields, as further explained in `rowtypes-usage`.
...
This equivalence between functional notation and field notation makes it possible to use functions on composite types to implement computed fields. computed field field computed An application using the last query above wouldn't need to be directly aware that `somefunc` isn't a real column of the table.
