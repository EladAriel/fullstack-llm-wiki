...
- Build with support for the ICUICU library, enabling use of ICU collation features (see `collation`). Defaults to auto and requires the ICU4C package to be installed. The minimum required version of ICU4C is currently 55.
...
Both 32-bit and 64-bit builds are possible with the Microsoft Compiler suite. 32-bit PostgreSQL builds are possible with Visual Studio 2022, as well as standalone Windows SDK releases 10 and above. 64-bit PostgreSQL builds are supported with Microsoft Windows SDK version 10 and above or Visual Studio 2022 and above.
