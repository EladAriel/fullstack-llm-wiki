...
source_path: "doc/src/sgml/func/func-formatting.sgml"
...
to_char `to_char` ( `timestamp`, `text` ) text
...
to_date `to_date` ( `text`, `text` ) date

Converts string to date according to the given format.

`to_date('05 Dec 2000', 'DD Mon YYYY')` 2000-12-05

to_number `to_number` ( `text`, `text` ) numeric

Converts string to numeric according to the given format.

`to_number('12,454.8-', '99G999D9S')` -12454.8

to_timestamp `to_timestamp` ( `text`, `text` ) timestamp with time zone

Converts string to time stamp according to the given format. (See also `to_timestamp(double precision)` in `functions-datetime-table`.)
...
`functions-formatting-datetime-table` shows the template patterns available for formatting date and time values.
...
midnight; see `datetime-julian-dates`)
...
Modifiers can be applied to any template pattern to alter its behavior. For example, `FMMonth` is the `Month` pattern with the `FM` modifier. `functions-formatting-datetimemod-table` shows the modifier patterns for date/time formatting.
...
`functions-formatting-numeric-table` shows the template patterns available for formatting numeric values.
...
`functions-formatting-examples-table` shows some examples of the use of the `to_char` function.
