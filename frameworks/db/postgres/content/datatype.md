...
date and time (no time zone conversion)
...
both date and time (no time zone conversion)
...
both date and time, with time zone conversion
...
For `timestamp with time zone` values, an input string that includes an explicit time zone will be converted to UTC (Universal Coordinated Time) using the appropriate offset for that time zone. If no time zone is stated in the input string, then it is assumed to be in the time zone indicated by the system's `guc-timezone` parameter, and is converted to UTC using the offset for the `timezone` zone. In either case, the value is stored internally as UTC. The originally stated or assumed time zone is not retained and cannot be retrieved later.
