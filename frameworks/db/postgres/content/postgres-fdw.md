...
generated_at: "2026-08-29T09:39:24.349410Z"
---
# postgres_fdw —
   ...
   Cost Estimation Options
    ...
    operation behaves:
     ...
     import_stats (boolean)
       ...
       When using this option, it is the user's responsibility
        to ensure that the existing statistics for the foreign
 ...
 loopback3   |           | f     | t            | f      |            1353156
     ...
     postgres_fdw_get_connections Output Columns
        ...
        text
         ...
         connection, or public if a public mapping is used.
         If the user mapping is dropped but the connection remains open
         (i.e., marked as invalid), this will be NULL.
        ...
        boolean
         ...
         NULL is returned if check_conn
         is set to false or if the connection status check
        ...
        int4
         ...
         the connection is closed (with closed set to
         true), this still shows the process ID of
...
-----------------------------
 t
  ...
  Subscription Management
   ...
   postgres_fdw supports subscription connections using
   the same options described in .
  

  
   For example, assuming the remote server foreign-host has
   a publication testpub:
...
CREATE SUBSCRIPTION my_subscription SERVER subscription_server PUBLICATION testpub;
   ...
   To create a subscription, the user must be a member of the  role and have
  ...
  Remote Query Optimization
   ...
   An INNER JOIN between a foreign table and a set-returning
   function appearing as a FROM-list item can likewise be
   pushed down: the function call is absorbed into the remote query so that
   only rows matching the function's output are fetched.  This is attempted
   only when the function is IMMUTABLE (so evaluating it on
   the remote server is guaranteed to produce the same result as evaluating
   it locally), its arguments and any restriction clauses on it are
   shippable, and none of the following disqualifying conditions applies:
   the join type is not INNER; the function is used with
   WITH ORDINALITY; the function returns a
   record type without an accompanying column definition list;
   or the function laterally references another relation at the same query
   level.  A function argument that is a parameter from an enclosing query
   level is allowed and is transmitted to the remote server as a query
   parameter.
