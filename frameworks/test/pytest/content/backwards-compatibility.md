...
source_branch: "main"
source_path: "doc/en/backwards-compatibility.rst"
...
To communicate changes, we issue deprecation warnings using a custom warning hierarchy (see `internal-warnings`). These warnings may be suppressed using the standard means: :option:`-W` command-line flag or :confval:`filterwarnings` configuration option (see `warnings`), but we suggest to use these sparingly and temporarily, and heed the warnings when possible.
...
Features currently deprecated and removed in previous releases can be found in `deprecations`.
...
==============  =================== pytest version  min. Python version ==============  =================== 9.0+            3.10+ 8.4             3.9+ 8.0 - 8.3       3.8+ 7.1 - 7.4       3.7+ 6.2 - 7.0       3.6+ 5.0 - 6.1       3.5+ 3.3 - 4.6       2.7, 3.4+ ==============  ===================
