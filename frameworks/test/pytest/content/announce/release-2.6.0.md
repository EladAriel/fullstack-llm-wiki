...
source_branch: "main"
source_path: "doc/en/announce/release-2.6.0.rst"
...
and the last (failure location) entry are shown, the ones between only in "short" format.  Use `--tb=long` to get back the old behaviour of showing "long" entries everywhere.

- a new warning system which reports oddities during collection
and execution.  For example, ignoring collecting Test* classes with an `__init__` now produces a warning.
