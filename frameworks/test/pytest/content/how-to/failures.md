...
source_branch: "main"
source_path: "doc/en/how-to/failures.rst"
...
## Using `python:library/pdb` with pytest

### Dropping to `pdb <python:library/pdb>` on failures

Python comes with a builtin Python debugger called `pdb <python:library/pdb>`.  `pytest` allows one to drop into the `pdb <python:library/pdb>` prompt via a command line option:
...
### Dropping to `pdb <python:library/pdb>` at the start of a test

`pytest` allows one to drop into the `pdb <python:library/pdb>` prompt immediately at the start of each test via a command line option:
...
Also the :confval:`faulthandler_timeout=X<faulthandler_timeout>` configuration option can be used to dump the traceback of all threads if a test takes longer than `X` seconds to finish.

> **Note:**  This functionality has been integrated from the external
 [pytest-faulthandler](https://github.com/pytest-dev/pytest-faulthandler)_ plugin, with two
 small differences:
...
Unhandled exceptions are exceptions that are raised in a situation in which they cannot propagate to a caller. The most common case is an exception raised in a `__del__ <object.__del__>` implementation.

Unhandled thread exceptions are exceptions raised in a `threading.Thread` but not handled, causing the thread to terminate uncleanly.
...
The warnings may be silenced selectively using the `pytest.mark.filterwarnings ref` mark. The warning categories are `pytest.PytestUnraisableExceptionWarning` and `pytest.PytestUnhandledThreadExceptionWarning`.
