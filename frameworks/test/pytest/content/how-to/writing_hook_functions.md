...
source_branch: "main"
source_path: "doc/en/how-to/writing_hook_functions.rst"
...
For more information, consult the `pluggy documentation about hook wrappers <pluggy:hookwrappers>`.
...
with `trylast=True` (but even without this mark it would come after Plugin1).

4. Plugin3's pytest_collection_modifyitems then executing the code after the yield
point.  The yield receives the result from calling the non-wrappers, or raises an exception if the non-wrappers raised.
...
> **Note:**  pytest only searches for hook implementations whose names start with
...
> **Note:**  This is a quick overview on how to add new hooks and how they work in general, but a more complete
 overview can be found in [the pluggy documentation](https://pluggy.readthedocs.io/en/latest/)_.
...
For a real world example, see newhooks.py from [xdist](https://github.com/pytest-dev/pytest-xdist).
...
> **Note:**  Unlike other hooks, the :hook:`pytest_generate_tests` hook is also discovered when
 defined inside a test module or test class. Other hooks must live in
 `conftest.py plugins <localplugin>` or external plugins.
 See `parametrize-basics` and the `hook-reference`.
...
Another plugin (installed via setuptools entry points, or via the `-p` command-line option) could then define the hook implementation to provide the default value:
...
> **Note:**  **Hook implementations in conftest.py files are not available to other plugins during**
 **their** `pytest_addoption()` **execution**. This is because conftest.py files are
 ...
 However, `initial conftest files <pluginorder>` themselves can implement
 ...
 * plugins specified via the `PYTEST_PLUGINS` environment variable
 See `pluginorder` for the complete plugin discovery order.

## Optionally using hooks from 3rd party plugins

Using new hooks from plugins as explained above might be a little tricky because of the standard `validation mechanism <validation>`: if you depend on a plugin that is not installed, validation will fail and the error message will not make much sense to your users.
...
Plugins often need to store data on `pytest.Item`\s in one hook implementation, and access it in another. One common solution is to just assign some private attribute directly on the item, but type-checkers like mypy frown upon this, and it may also cause conflicts with other plugins. So pytest offers a better way to do this, `item.stash <_pytest.nodes.Node.stash>`.
...
Stashes are available on all node types (like `pytest.Class`, `pytest.Session`) and also on `pytest.Config`, if needed.
