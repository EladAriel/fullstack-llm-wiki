...
source_branch: "main"
source_path: "content/develop/use-cases/recommendation-engine/java-jedis/_index.md"
source_commit: "f8693349287b0efbef3c865b6f6a2aceca88594d"
source_commit_short: "f869334"
source_commit_date: "2026-08-28T10:01:19-05:00"
generated_at: "2026-08-29T09:38:56.130797Z"
---
# _Index
...
([source](https://github.com/redis/docs/blob/main/content/develop/use-cases/recommendation-engine/java-jedis/Recommender.java)):

```java
import redis.clients.jedis.JedisPooled;

// JedisPooled is a pool-backed UnifiedJedis: it exposes both the
// FT.* search commands and a pipelined() API in one client.
JedisPooled jedis = new JedisPooled("localhost", 6379);
Recommender recommender = new Recommender(jedis, "recommend:idx",
        "product:", "user:", 384);
...
Each product is one Redis Hash. The vector field is raw little-endian `float32` bytes &mdash; no JSON wrapping &mdash; because the Redis Search vector encoding expects exactly that. Jedis 6's typed `hset(String, Map<String, String>)` overload would UTF-8-encode the bytes and corrupt the vector, so the helper uses the `byte[]`-typed `hset(byte[], byte[], byte[])` overload for the embedding field and the String-typed overload for everything else, in the same pipelined batch.
...
The Jedis equivalent uses `JedisPooled.ftSearch(String, Query)` with the binary parameter wired through `Query.addParam("vec", floatsToBytes(vec))`:

```java
String knn = filterClause + "=>[KNN " + k + " @embedding $vec AS vector_score]";
Query q = new Query(knn)
        .addParam("vec", Recommender.floatsToBytes(queryVec))
        .returnFields(
        ...
        .limit(0, k)
        .dialect(2);
SearchResult result = jedis.ftSearch("recommend:idx", q);
```
...
([source](https://github.com/redis/docs/blob/main/content/develop/use-cases/recommendation-engine/java-jedis/LocalEmbedder.java)):
...
([source](https://github.com/redis/docs/blob/main/content/develop/use-cases/recommendation-engine/java-jedis/BuildCatalog.java)):
...
The shared `catalog.json` wire format (model name, dim, list of products with base64-encoded `float32` LE bytes for the vector) is identical to what the Python, Node, and Go ports produce, so you can re-use any port's catalog with the Java demo as long as the embedding model matches.
...
The server holds one `LocalEmbedder` and one `Recommender` for the lifetime of the process. Endpoints:
...
* [Maven](https://maven.apache.org/) 3.9 or later for dependency resolution. DJL pulls in a couple of dozen transitive jars, so a manual `javac -cp ...` build is impractical; the `pom.xml` shipped next to the source files lets Maven handle that.
...
1.  Clone the [`redis/docs`](https://github.com/redis/docs) repository and change into the example
    ...
    cd docs/content/develop/use-cases/recommendation-engine/java-jedis
    ```

2.  Build the project. The first build downloads the dependency graph (Jedis, DJL API, the
