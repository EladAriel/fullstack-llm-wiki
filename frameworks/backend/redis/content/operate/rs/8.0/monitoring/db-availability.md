...
source_branch: "main"
source_path: "content/operate/rs/8.0/monitoring/db-availability.md"
source_commit: "f8693349287b0efbef3c865b6f6a2aceca88594d"
source_commit_short: "f869334"
source_commit_date: "2026-08-28T10:01:19-05:00"
generated_at: "2026-08-29T09:38:55.625846Z"
---
# Db Availability
...
If the database is unavailable, returns an error status code and a JSON object that contains [`error_code` and `description` fields]({{<relref "/operate/rs/8.0/references/rest-api/requests/bdbs/availability#get-db-error-codes">}}).
...
If the local database endpoint is unavailable, returns an error status code and a JSON object that contains [`error_code` and `description` fields]({{<relref "/operate/rs/8.0/references/rest-api/requests/bdbs/availability#get-endpoint-error-codes">}}).
...
To change the default threshold for the entire cluster, set `availability_lag_tolerance_ms` with an [update cluster]({{<relref "/operate/rs/8.0/references/rest-api/requests/cluster#put-cluster">}}) request:
...
The following table shows the relationship between a database's status and availability. For more details about the database status values, see [BDB status field]({{<relref "/operate/rs/8.0/references/rest-api/objects/bdb/status">}}).
