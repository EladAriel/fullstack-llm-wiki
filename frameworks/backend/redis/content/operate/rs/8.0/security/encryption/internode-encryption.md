...
source_branch: "main"
source_path: "content/operate/rs/8.0/security/encryption/internode-encryption.md"
source_commit: "f8693349287b0efbef3c865b6f6a2aceca88594d"
source_commit_short: "f869334"
source_commit_date: "2026-08-28T10:01:19-05:00"
generated_at: "2026-08-29T09:38:55.649644Z"
---
...
weight: 15
url: '/operate/rs/8.0/security/encryption/internode-encryption/'
---
As of v6.2.4, Redis Software supports _internode encryption_, which encrypts internal communication between nodes. This improves the security of data as it travels within a cluster.
...
-  Use the `rladmin` command-line utility to set the [data_internode_encryption]({{< relref "/operate/rs/8.0/references/cli-utilities/rladmin/tune#tune-db" >}}) setting for the database:

    ``` shell
    rladmin tune db <database_id> data_internode_encryption enabled
    ``` 

- Use the Redis Software REST API to set the `data_internode_encryption` setting for the database.
...
- [rladmin tune cluster]({{< relref "/operate/rs/8.0/references/cli-utilities/rladmin/tune#tune-cluster" >}}): 
    
    ```sh
    rladmin tune cluster data_internode_encryption enabled
    ```

- [Update cluster policy]({{< relref "/operate/rs/8.0/references/rest-api/requests/cluster/policy#put-cluster-policy" >}}) REST API request:
...
By default, Redis Software uses self-signed certificates for internode encryption. These certificates are automatically generated, managed, and rotated by the system.

### Encryption ciphers and settings

To encrypt internode communications, Redis Software uses TLS 1.2 and the following cipher suites:

- ECDHE-RSA-AES256-GCM-SHA384
- ECDHE-RSA-AES128-GCM-SHA256

As of Redis Software v7.4, internode encryption also supports TLS 1.3 with the following cipher suites:
...
You can use the Redis Software REST API to rotate certificates manually:
...
Instead of using Redis Software's self-signed certificates for internode encryption, you can provide certificates generated by your Certificate Authority (CA).
...
To use the same certificate for data and control plane internode encryption, run the following [`rladmin cluster certificate`]({{< relref "/operate/rs/8.0/references/cli-utilities/rladmin/cluster/certificate#cluster-certificate-set-internal" >}}) command:
...
To use separate certificates for data and control plane internode encryption, run the following [`rladmin cluster certificate`]({{< relref "/operate/rs/8.0/references/cli-utilities/rladmin/cluster/certificate#cluster-certificate-set-internal" >}}) command:
...
To use the same certificate for data and control plane internode encryption, send a [`PUT /v1/cluster/certificates`]({{< relref "/operate/rs/8.0/references/rest-api/requests/cluster/certificates#put-cluster-certificates" >}}) request:
...
To use separate certificates for data and control plane internode encryption, send a [`PUT /v1/cluster/certificates`]({{< relref "/operate/rs/8.0/references/rest-api/requests/cluster/certificates#put-cluster-certificates" >}}) request:
...
- You should monitor the certificates' expiration dates and [renew them]({{<relref "/operate/rs/8.0/security/certificates/updating-certificates#how-to-update-certificates">}}) before they expire.
