...
source_branch: "main"
source_path: "content/operate/rs/8.0/security/certificates/updating-certificates.md"
source_commit: "f8693349287b0efbef3c865b6f6a2aceca88594d"
source_commit_short: "f869334"
source_commit_date: "2026-08-28T10:01:19-05:00"
generated_at: "2026-08-29T09:38:55.650257Z"
---
...
weight: 20
url: '/operate/rs/8.0/security/certificates/updating-certificates/'
---
...
You need to create new certificates before you can update them in the cluster. To create replacement certificates, see [Renew self-signed certificates]({{<relref "/operate/rs/8.0/security/certificates/create-certificates#renew-self-signed-certificates">}}) or [Create CA-signed certificates]({{<relref "/operate/rs/8.0/security/certificates/create-certificates#create-ca-signed-certificates">}}) for detailed instructions.
...
To replace certificates with the `rladmin` CLI, run the [`cluster certificate set`]({{< relref "/operate/rs/8.0/references/cli-utilities/rladmin/cluster/certificate" >}}) command:
...
- `<cert-name>` - The name of the certificate you want to replace. See the [certificates table]({{< relref "/operate/rs/8.0/security/certificates" >}}) for the list of valid certificate names.
...
To replace a certificate using the REST API, use an [update cluster certificates]({{<relref "/operate/rs/8.0/references/rest-api/requests/cluster/certificates">}}) request.
...
- `<cert_name>` - The name of the certificate to replace. See the [certificates table]({{< relref "/operate/rs/8.0/security/certificates" >}}) for the list of valid certificate names.
...
1. From the Cluster Manager UI, update the destination database (_replica_) configuration with the [new certificate]({{< relref "/operate/rs/8.0/databases/import-export/replica-of/create#encrypt-replica-database-traffic" >}}).
...
1. Use the [`crdb-cli`]({{< relref "/operate/rs/8.0/references/cli-utilities/crdb-cli" >}}) utility to update Active-Active database configuration from the command line. Run the following command once for each Active-Active database residing on the modified clusters:
...
1. Update the Active-Active database configuration from the command line with the [`crdb-cli`]({{< relref "/operate/rs/8.0/references/cli-utilities/crdb-cli" >}}) utility. Run this command once for each Active-Active database that resides on the modified clusters:
