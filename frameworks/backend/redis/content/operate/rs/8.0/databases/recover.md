...
source_branch: "main"
source_path: "content/operate/rs/8.0/databases/recover.md"
source_commit: "f8693349287b0efbef3c865b6f6a2aceca88594d"
source_commit_short: "f869334"
source_commit_date: "2026-08-28T10:01:19-05:00"
generated_at: "2026-08-29T09:38:55.619102Z"
---
# Recover
...
linktitle: Recover
weight: 35
url: '/operate/rs/8.0/databases/recover/'
---
When a cluster fails or a database is corrupted, you must:

1. [Restore the cluster configuration]({{< relref "/operate/rs/8.0/clusters/cluster-recovery.md" >}}) from the CCS files
...
These files are stored in the [persistence storage location]({{< relref "/operate/rs/8.0/installing-upgrading/install/plan-deployment/persistent-ephemeral-storage" >}}).

The database recovery process includes:

1. If the cluster failed, [recover the cluster]({{< relref "/operate/rs/8.0/clusters/cluster-recovery.md" >}}).
...
- Before you start database recovery, make sure that the cluster that hosts the database is healthy.
    ...
    you must [recover the cluster]({{< relref "/operate/rs/8.0/clusters/cluster-recovery.md" >}}) before you recover the databases.
...
you can run the recovery process from the [`rladmin`]({{< relref "/operate/rs/8.0/references/cli-utilities/rladmin" >}})
...
1. To see which databases are recoverable, run [`rladmin recover list`]({{<relref "/operate/rs/8.0/references/cli-utilities/rladmin/recover#recover-list">}}):
...
1. Recover the database using one of the following [`rladmin recover`]({{< relref "/operate/rs/8.0/references/cli-utilities/rladmin/recover" >}}) commands:
...
To enable automatic recovery using `rladmin`, run the [`rladmin tune cluster`]({{<relref "/operate/rs/8.0/references/cli-utilities/rladmin/tune#tune-cluster">}}) command:
...
To enable automatic recovery using the REST API, use an [update cluster policy]({{< relref "/operate/rs/8.0/references/rest-api/requests/cluster/policy#put-cluster-policy" >}}) request:
...
You can also set `recovery_wait_time` when you [create a database]({{< relref "/operate/rs/8.0/references/rest-api/requests/bdbs#post-bdbs-v1" >}}) using the REST API.
