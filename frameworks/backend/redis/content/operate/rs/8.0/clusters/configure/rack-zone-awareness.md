...
source_branch: "main"
source_path: "content/operate/rs/8.0/clusters/configure/rack-zone-awareness.md"
source_commit: "f8693349287b0efbef3c865b6f6a2aceca88594d"
source_commit_short: "f869334"
source_commit_date: "2026-08-28T10:01:19-05:00"
generated_at: "2026-08-29T09:38:55.639771Z"
---
# Rack Zone Awareness
...
weight: 70
url: '/operate/rs/8.0/clusters/configure/rack-zone-awareness/'
---
...
You can set up rack-zone awareness for the cluster and its nodes during [cluster creation]({{< relref "/operate/rs/8.0/clusters/new-cluster-setup" >}}):
...
1. Finish [cluster setup]({{< relref "/operate/rs/8.0/clusters/new-cluster-setup" >}}).

1. For every [node you add to the cluster]({{< relref "/operate/rs/8.0/clusters/add-node" >}}), assign a different **Rack-zone ID**.

### Existing cluster

If you did not configure rack-zone awareness during cluster creation, you can configure rack-zone awareness for existing clusters using the [REST API]({{< relref "/operate/rs/8.0/references/rest-api" >}}):

1. For each node in the cluster, assign a different rack-zone ID using the REST API to [update the node]({{< relref "/operate/rs/8.0/references/rest-api/requests/nodes#put-node" >}}):
...
1. [Update the cluster policy]({{< relref "/operate/rs/8.0/references/rest-api/requests/cluster/policy#put-cluster-policy" >}}) to enable rack-zone awareness:
...
To set up two-dimensional rack-zone awareness during cluster creation, assign a `second_rack_id` to each node in the cluster in addition to the `rack_id` using the [REST API]({{<relref "/operate/rs/8.0/references/rest-api">}}) or [rladmin]({{<relref "/operate/rs/8.0/references/cli-utilities/rladmin">}}).

#### REST API method

To create a new cluster with two-dimensional rack-zone awareness, you can use [bootstrap REST API requests]({{<relref "/operate/rs/8.0/references/rest-api/requests/bootstrap#post-bootstrap">}}):
...
1. Run [`rladmin cluster create`]({{<relref "/operate/rs/8.0/references/cli-utilities/rladmin/cluster/create">}}) to create the initial cluster on one node, enable rack-zone awareness, and assign a `rack_id` and `second_rack_id`:
...
1. Run [`rladmin cluster join`]({{<relref "/operate/rs/8.0/references/cli-utilities/rladmin/cluster/join">}}) for each new node you want to add to the cluster and assign a different `rack_id` and `second_rack_id`:
...
You can configure two-dimensional rack-zone awareness for existing clusters using the [REST API]({{< relref "/operate/rs/8.0/references/rest-api" >}}).

For each node in the cluster, assign a different `second_rack_id` using the REST API to [update the node]({{< relref "/operate/rs/8.0/references/rest-api/requests/nodes#put-node" >}}):
...
1. From **databases**, [create a new database]({{< relref "/operate/rs/8.0/databases/create" >}}) or edit an existing database's **configuration**.

1. Expand the **High availability & durability** section.

1. Enable [**Replication**]({{< relref "/operate/rs/8.0/databases/durability-ha/replication" >}}).
...
To enable rack-zone awareness for a database, use a [REST API request]({{< relref "/operate/rs/8.0/references/rest-api/requests/bdbs#put-bdbs" >}}):
...
After you enable rack-zone awareness for an existing database, you should generate an optimized shard placement blueprint using the [REST API]({{< relref "/operate/rs/8.0/references/rest-api" >}}) and use it to rearrange the shards in different racks or zones.

1. [Generate an optimized shard placement blueprint]({{< relref "/operate/rs/8.0/references/rest-api/requests/bdbs/actions/optimize_shards_placement#get-bdbs-actions-optimize-shards-placement" >}}):
...
1. [Rearrange the database shards]({{< relref "/operate/rs/8.0/references/rest-api/requests/bdbs/actions/optimize_shards_placement#put-bdbs-rearrange-shards" >}}) according to the new shard placement blueprint:
