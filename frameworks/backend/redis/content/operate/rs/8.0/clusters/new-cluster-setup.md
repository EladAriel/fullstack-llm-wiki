...
source_branch: "main"
source_path: "content/operate/rs/8.0/clusters/new-cluster-setup.md"
source_commit: "f8693349287b0efbef3c865b6f6a2aceca88594d"
source_commit_short: "f869334"
source_commit_date: "2026-08-28T10:01:19-05:00"
generated_at: "2026-08-29T09:38:55.621583Z"
---
# New Cluster Setup
...
weight: 10
url: '/operate/rs/8.0/clusters/new-cluster-setup/'
---
...
To set up a new cluster, you must first [install the Redis Software package]({{< relref "/operate/rs/8.0/installing-upgrading" >}})
and then set up the cluster as described below.
After the cluster is created you can [add multiple nodes to the cluster]({{< relref "/operate/rs/8.0/clusters/add-node.md" >}}).
...
1. In a browser, go to `https://<name or IP address of the machine with Redis Software installed>:8443`.
    ...
    {{< note >}}
- The management UI uses a [self-signed certificate for TLS encryption]({{< relref "/operate/rs/8.0/security/certificates/updating-certificates" >}}).
...
1. In the **Configuration** section:
    ...
    1. For **FQDN (Fully Qualified Domain Name)**, enter a unique name for the cluster.
        ...
        See the [instructions for DNS setup]({{< relref "/operate/rs/8.0/networking/cluster-dns" >}})
        to make sure your cluster is reachable by name.

    1. Choose whether to [**Enable private & public endpoints support**]({{< relref "/operate/rs/8.0/networking/private-public-endpoints.md" >}}).

    1. Choose whether to [**Enable rack-zone awareness**]({{< relref "/operate/rs/8.0/clusters/configure/rack-zone-awareness.md" >}}).

1. Click **Next**.

1. Configure storage and network settings:

    1. Enter a path for [*Ephemeral storage*]({{< relref "/operate/rs/8.0/installing-upgrading/install/plan-deployment/persistent-ephemeral-storage" >}}), or leave the default path.

    1. Enter a path for [*Persistent storage*]({{< relref "/operate/rs/8.0/installing-upgrading/install/plan-deployment/persistent-ephemeral-storage" >}}),
        or leave the default path.

    1. To enable [*Redis Flex or Auto Tiering*]({{< relref "/operate/rs/8.0/databases/flash/" >}}),
        select **Enable flash storage** and enter the path to the flash storage.

    1. If the cluster is configured to support [rack-zone awareness]({{< relref "/operate/rs/8.0/clusters/configure/rack-zone-awareness.md" >}}), set the **Rack-zone ID** for the new node.
...
- [Creating a new database]({{< relref "/operate/rs/8.0/databases/create.md" >}})
- [Joining a new node to a cluster]({{< relref "/operate/rs/8.0/clusters/add-node.md" >}})
