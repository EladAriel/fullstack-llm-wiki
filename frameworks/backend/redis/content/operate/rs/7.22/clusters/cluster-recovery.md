...
source_branch: "main"
source_path: "content/operate/rs/7.22/clusters/cluster-recovery.md"
source_commit: "f8693349287b0efbef3c865b6f6a2aceca88594d"
source_commit_short: "f869334"
source_commit_date: "2026-08-28T10:01:19-05:00"
generated_at: "2026-08-29T09:38:55.335015Z"
---
# Cluster Recovery
...
weight: 70
url: '/operate/rs/7.22/clusters/cluster-recovery/'
---
...
These files are stored in the [persistent storage location]({{< relref "/operate/rs/7.22/installing-upgrading/install/plan-deployment/persistent-ephemeral-storage" >}}).
...
1. [Recover the databases]({{< relref "/operate/rs/7.22/databases/recover.md" >}}).
...
1. [Install Redis Enterprise Software]({{< relref "/operate/rs/7.22/installing-upgrading/install/install-on-linux" >}}) on the new cluster nodes.
...
1. To recover the original cluster configuration, run [`rladmin cluster recover`]({{< relref "/operate/rs/7.22/references/cli-utilities/rladmin/cluster/recover" >}}) on the first node in the new cluster:
...
1. To join the remaining servers to the new cluster, run [`rladmin cluster join`]({{< relref "/operate/rs/7.22/references/cli-utilities/rladmin/cluster/join" >}}) from each new node:
...
1. Run [`rladmin status`]({{< relref "/operate/rs/7.22/references/cli-utilities/rladmin/status" >}}) to verify the recovered nodes are now active and the databases are pending recovery:
    ...
    {{< note >}}
Make sure that you update your [DNS records]({{< relref "/operate/rs/7.22/networking/cluster-dns" >}})
with the IP addresses of the new nodes.
    {{< /note >}}

After the cluster is recovered, you must [recover the databases]({{< relref "/operate/rs/7.22/databases/recover.md" >}}).
