...
source_branch: "main"
source_path: "content/operate/rs/7.22/installing-upgrading/upgrading/upgrade-database.md"
source_commit: "f8693349287b0efbef3c865b6f6a2aceca88594d"
source_commit_short: "f869334"
source_commit_date: "2026-08-28T10:01:19-05:00"
generated_at: "2026-08-29T09:38:55.357884Z"
---
# Upgrade Database
...
- Send a [`GET /nodes` REST API request]({{< relref "/operate/rs/7.22/references/rest-api/requests/nodes" >}}) and see `supported_database_versions` in the response.
...
- Verify that both the [current database version and the target database version are supported](#db-versions-table) by the cluster's Redis Enterprise Software version.
    ...
    - _(Optional)_ Use the [`rladmin status extra all`]({{< relref "/operate/rs/7.22/references/cli-utilities/rladmin/status" >}}) command to display configuration details:
...
- To avoid data loss during the upgrade, [back up your data]({{< relref "/operate/rs/7.22/databases/import-export/schedule-backups" >}}).  

    You can [export the data]({{< relref "/operate/rs/7.22/databases/import-export/export-data" >}}) to an external location, [enable replication]({{< relref "/operate/rs/7.22/databases/durability-ha/replication" >}}), or [enable persistence]({{< relref "/operate/rs/7.22/databases/configure/database-persistence" >}}).
...
1. Check the Redis database compatibility version for the database to confirm the upgrade.  
    ...
    - Use [`rladmin status databases extra all`]({{< relref "/operate/rs/7.22/references/cli-utilities/rladmin/status#status-databases" >}}) to display a list of the databases in your cluster and their current Redis database compatibility version:
