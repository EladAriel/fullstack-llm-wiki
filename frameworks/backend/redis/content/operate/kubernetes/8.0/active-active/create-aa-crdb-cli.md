...
source_branch: "main"
source_path: "content/operate/kubernetes/8.0/active-active/create-aa-crdb-cli.md"
source_commit: "f8693349287b0efbef3c865b6f6a2aceca88594d"
source_commit_short: "f869334"
source_commit_date: "2026-08-28T10:01:19-05:00"
generated_at: "2026-08-29T09:38:55.193615Z"
---
# Create Aa Crdb Cli
...
Creating an Active-Active database requires routing [network access]({{< relref "/operate/kubernetes/8.0/networking/" >}}) between two Redis Enterprise clusters residing in different Kubernetes clusters. Without the proper access configured for each cluster, syncing between the databases instances will fail.
...
- Routing for external access with an [ingress resources]({{< relref "/operate/kubernetes/8.0/networking/ingress" >}}) (or [route resources]({{< relref "/operate/kubernetes/8.0/networking/routes" >}}) on OpenShift).
- A working [Redis Enterprise cluster (REC)]({{< relref "/operate/kubernetes/8.0/reference/api/redis_enterprise_cluster_api" >}}) with a unique name.
...
- [**REC admin credentials**]({{< relref "/operate/kubernetes/8.0/security/manage-rec-credentials" >}}) `<username> <password>`:
...
1. If your cluster uses an [ingress controller]({{< relref "/operate/kubernetes/8.0/networking/ingress" >}}), add the following to the `spec` section of your REC resource file.
...
No changes are required to the REC spec if you are using [Istio]({{< relref "/operate/kubernetes/8.0/networking/istio-ingress" >}}) in place of an ingress controller. The `activeActive` section added above creates ingress resources. The two custom resources used to configure Istio (Gateway and VirtualService) replace the need for ingress resources.
...
1. If your cluster uses [OpenShift routes]({{< relref "/operate/kubernetes/8.0/networking/routes" >}}), add the following to the `spec` section of your Redis Enterprise cluster (REC) resource file.
...
The `crdb-cli` command can be run from any Redis Enterprise pod hosted on any participating K8s cluster. You'll need the values for the [required parameters]({{< relref "/operate/kubernetes/8.0/active-active/create-aa-crdb-cli#document-required-parameters" >}}) for each Redis Enterprise cluster.
...
You can connect to your databases with the instructions in [Manage databases]({{< relref "/operate/kubernetes/8.0/re-databases/db-controller#connect-to-a-database" >}}). Set a test key with `SET foo bar` in the first database. If your Active-Active deployment is working properly, when connected to your second database, `GET foo` should output `bar`.
