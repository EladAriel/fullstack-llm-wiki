---
type: "Framework Learn Directory Index"
framework: "Arize Phoenix"
generated_at: "2026-08-29T09:39:58.968047Z"
---
# How-To-Evals

This directory contains Arize Phoenix learning pages related to how-to-evals.

## Breadcrumbs

[Wiki Home](../index.md) -> [Content](index.md)

## Navigation

- Wiki Home: [Root Index](../index.md)

## Pages in This Directory

- [Batch Evaluations](batch-evaluations.md)
- [Code Evaluator Output Shapes](code-evaluator-output-shapes.md)
- [Code Evaluators](code-evaluators.md)
- [Configuring The Llm](configuring-the-llm.md)
- [Custom Llm Evaluators](custom-llm-evaluators.md)
- [Prompt Formats](prompt-formats.md)
- [Using Evals With Phoenix](using-evals-with-phoenix.md)

## Subdirectories

- [Configuring The Llm](configuring-the-llm/index.md)
