...
source_branch: "main"
source_path: "docs/phoenix/evaluation/typescript-quickstart.mdx"
source_commit: "c48e50e9906fcc56c1c103ebd93ef3c95ed6b6e7"
source_commit_short: "c48e50e"
source_commit_date: "2026-08-29T01:45:20-06:00"
generated_at: "2026-08-29T09:39:58.798693Z"
---
...
<Card title="@arizeai/phoenix-evals" href="https://www.npmjs.com/package/@arizeai/phoenix-evals" icon="js" description="TypeScript evals package"/>
...
<Steps>
  <Step title={<span className="step-title">Set up and Connect to Phoenix</span>}>
    Before running your experiment and evaluations, make sure Phoenix is running and your environment is set up with the required dependencies.

    <StartPhoenix />

    Then install the dependencies this guide uses into your TypeScript project:
    ...
    Include these in your `.env` file. `PHOENIX_ENDPOINT` is what `@arizeai/phoenix-client` reads for datasets and experiments; `PHOENIX_COLLECTOR_ENDPOINT` is what `@arizeai/phoenix-otel` reads when exporting spans — point both at the same Phoenix instance.

    ```bash
    OPENAI_API_KEY="your-openai-api-key"
    PHOENIX_ENDPOINT="http://localhost:6006"
    PHOENIX_COLLECTOR_ENDPOINT="http://localhost:6006/v1/traces"  # OTLP HTTP traces URL
    # PHOENIX_API_KEY="..."  # Only if your Phoenix instance has auth enabled
    ```
  </Step>
  ...
  <Step title={<span className="step-title">Create an Evaluator</span>}>
    ...
    In this example, we'll use Phoenix's pre-built [Retrieval Relevance evaluator](pre-built-metrics/retrieval-relevance). This evaluator checks whether retrieved context actually contains the information needed to answer the user's question—ensuring your application is grounding its responses in the right documents.

    ```typescript
    import { asExperimentEvaluator } from "@arizeai/phoenix-client/experiments";
    import { createRetrievalRelevanceEvaluator } from "@arizeai/phoenix-evals/llm/createRetrievalRelevanceEvaluator";
    ...
    const retrievalRelevanceEvaluator = createRetrievalRelevanceEvaluator({
      ...
      const retrievalRelevanceCheck = asExperimentEvaluator({
        ...
        evaluate: async ({ input, output }) => {
          // Use the retrieval relevance evaluator from phoenix-evals
  ...
  <Step title={<span className="step-title">View Results in Phoenix</span>}>
