...
source_branch: "main"
source_path: "docs/phoenix/tracing/tutorial/your-first-traces.mdx"
source_commit: "69b3ab92c37ff65812feaa2dbf0b1c0ad5ae55fe"
source_commit_short: "69b3ab9"
source_commit_date: "2026-07-25T11:48:12-06:00"
generated_at: "2026-07-25T19:08:24.915909Z"
---
---
...
<Tabs>
  <Tab title="TypeScript" icon="js">
    ```bash
    npm install ai @ai-sdk/openai @ai-sdk/otel @arizeai/phoenix-otel zod
    ```

    <Note>
    AI SDK v7 requires Node.js 22 or newer. `@arizeai/phoenix-otel` bundles the OpenTelemetry setup and the `@arizeai/openinference-vercel` span processors that translate AI SDK spans for Phoenix; `@ai-sdk/otel` emits those spans.
    </Note>
  </Tab>
...
In order to send traces to Phoenix, you need a Phoenix instance running.

<StartPhoenix />

Now point your application at it:
...
<Tabs>
  ...
  <Tab title="Python" icon="python">
    Setting `auto_instrument=True` in the `register` function looks at the installed [OpenInference](https://github.com/Arize-ai/openinference) packages and automatically instruments them, so you don't need to do manual configuration for each library.
...
<Tabs>
  <Tab title="TypeScript" icon="js">
    ...
    const result = await generateText({
      ...
      instructions: "Classify the query as 'order_status' or 'faq'",
      prompt: userQuery,
    });
    ```
  </Tab>
...
When your LLM calls tools, those executions are automatically traced as child spans. With Phoenix, you can see the complete chain, including the LLM's decision, the exact parameters passed, and the tool's response.
...
<Tabs>
  <Tab title="TypeScript" icon="js">
    ...
    const { text } = await generateText({
      model: openai.chat("gpt-4o-mini"),
      instructions: `Answer using ONLY this context:\n\n${retrievedContext}`,
      prompt: userQuery,
    });
    ```
  </Tab>
...
<Info>
  See the entire agent with grouped tracing [here](https://github.com/Arize-ai/phoenix/tree/main/js/examples/apps/tracing-tutorial/support-agent.ts).
</Info>
...
<Tabs>
  <Tab title="TypeScript" icon="js">
    ```typescript
    // trace and SpanStatusCode are re-exported by @arizeai/phoenix-otel, so
    // no direct dependency on @opentelemetry/api is needed
...
The final SupportBot agent combines the classifier, the order status tool, and the FAQ retrieval into a single agent. 
...
<Columns cols={2}>
  <Card title="Complete TypeScript Tutorial" icon="js" href="https://github.com/Arize-ai/phoenix/tree/main/js/examples/apps/tracing-tutorial/support-agent.ts" description="End-to-end TypeScript tutorial">
  </Card>

  <Card title="Complete Python Tutorial" icon="python" href="https://colab.research.google.com/github/Arize-ai/phoenix/blob/main/tutorials/tracing/python_tracing_quickstart.ipynb" description="End-to-end Python tutorial">
  </Card>
</Columns>
...
Open Phoenix at [http://localhost:6006](http://localhost:6006). You'll see 7 `support-agent` traces - one for each query.
...
- **Trace LLM calls** - Capture inputs, outputs, tokens, and latency automatically once tracing is registered
