...
source_branch: "main"
source_path: "src/langsmith/llm-gateway-fallbacks.mdx"
source_commit: "2aae1dfc98ee953a9a5185fb6fcdd9efb3f4d878"
source_commit_short: "2aae1df"
source_commit_date: "2026-07-25T00:27:23+00:00"
generated_at: "2026-07-25T19:08:33.355335Z"
---
...
The gateway calls each candidate with its own configured model, and each model configuration can point to a different provider and model identifier. The gateway uses the request's `model` only to select a chain; if it doesn't match any chain, the gateway falls back to the first (default) chain. When a fallback triggers, the `model` tag in the Model Configuration overrides the original request's `model`, so you can fall back from `model-x` to `model-y`, for example.
...
A configuration can hold more than one fallback chain, which is useful when different model families need different fallback behavior. The gateway selects a chain by matching the request body's `model` field against each chain's **primary** (first) model configuration's underlying model name:
...
You may also specify an alias for each chain. Your client sets the alias in the request's `model` field (for example, `"model": "heavy"`) to route to that chain.
